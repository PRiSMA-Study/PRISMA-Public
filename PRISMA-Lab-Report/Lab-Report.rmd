...
**Date of included data upload to Synapse: ** 2024-11-01
...
load("derived_data/mnh04_wide.rda")
...
matrix1 <- df_lab %>% 
  ...
  summarise(
    ...
    "Missing MNH06: n (%)     " = paste0(
      ...
      ")"),
    "Missing MNH08: n (%)     " = paste0(
      sum(is.na(M08_TYPE_VISIT_7) & denom_form_7 == 1 & SITE %in% c("Ghana", "India-CMC", "Kenya", "Zambia"), na.rm = TRUE), 
      " (", 
      format(round(sum(is.na(M08_TYPE_VISIT_7) & denom_form_7 == 1 & SITE %in% c("Ghana", "India-CMC", "Kenya", "Zambia"), na.rm = TRUE)/
                     sum(denom_form_7 == 1 & SITE %in% c("Ghana", "India-CMC", "Kenya", "Zambia"), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    #pnc1
    ...
    "Missing MNH06: n (%)         " = paste0(
      ...
      ")"),
    "Missing MNH08: n (%)         " = paste0(
      sum(is.na(M08_TYPE_VISIT_11) & denom_form_11 == 1 & SITE %in% c("Ghana", "India-CMC", "Zambia"), na.rm = TRUE), 
      " (", 
      format(round(sum(is.na(M08_TYPE_VISIT_11) & denom_form_11 == 1 & SITE %in% c("Ghana", "India-CMC", "Zambia"), na.rm = TRUE)/
                     sum(denom_form_11 == 1 & SITE %in% c("Ghana", "India-CMC", "Zambia"), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    #pnc52
...
matrix1[c(18), c("India-SAS", "Pakistan")] = "n/a"
matrix1[c(21,24,33), c("India-CMC", "India-SAS", "Kenya", "Pakistan")] = "n/a"
matrix1[c(30), c("India-SAS", "Kenya", "Pakistan")] = "n/a"
...
#******************table 2.2 *****************
#*!!!!!! temp
if(exists("df_lab$M08_CBC_HB_LBORRES_12") == FALSE) {df_lab <- df_lab %>% mutate(M08_CBC_HB_LBORRES_12 = NA)}
 if (num == 1) {
    matrix2_2 <- df_lab %>%
      group_by(SITE) %>%
      summarise(
 ...
 poc_13 <- mnh06 %>% 
   ...
   mutate(ga_wks = as.numeric(ymd(M06_DIAG_VSDAT) - ymd(EST_CONCEP_DATE))/7) %>% 
   filter(ga_wks >= 26) %>% 
   select(SITE, MOMID, PREGID, M06_BGLUC_POC_MMOLL_LBORRES)
  ...
  pretest_13 <- mnh08 %>% 
   ...
   left_join(MAT_ENROLL %>% select(SITE, MOMID, PREGID, EST_CONCEP_DATE)) %>% 
   mutate(ga_wks = as.numeric(ymd(M08_LBSTDAT) - ymd(EST_CONCEP_DATE))/7) %>% 
   filter(ga_wks >= 26) %>% 
 ...
 df_ogtt <- df_lab %>%
  ...
  h2_glucose_cols <- names(df_ogtt)[grep("^M08_BGLUC_ORAL_2HR_MMOLL_LBORRES", names(df_ogtt))]
    ...
    matrix2_3 <- df_ogtt %>%
      group_by(SITE) %>%
      summarise(
        "Expected POC Blood Glucose Results from MNH06: N" = sum(denom_lab_06_3 == 1 & !(SITE %in% c("India-CMC", "Pakistan")), na.rm = TRUE),
        #Glucose (ANC-28)
        "Missing POC Blood Glucose: n (%)*" = paste0(
          sum(if_all(poc_glucose_cols, is.na) & denom_lab_06_3 == 1 & !(SITE %in% c("India-CMC", "Pakistan")), na.rm = TRUE),
          " (",
          format(round(sum(if_all(poc_glucose_cols, is.na) & denom_lab_06_3 == 1 & !(SITE %in% c("India-CMC", "Pakistan")), na.rm = TRUE)/sum(denom_lab_06_3 == 1 & !(SITE %in% c("India-CMC", "Pakistan")), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        #Glucose (ANC-28)
    ...
    matrix2_3[c(1,2), c("India-CMC", "Pakistan")] = "n/a"
    ...
    tb2_3 <- tb_theme2(matrix2_3) %>%
      ...
      tab_footnote(
        footnote = c("Note: POC blood glucose is not performed at India-CMC and Pakistan. Expected lab results from MNH06 and total number excluded India-CMC and Pakistan")
      ) %>%
      tab_footnote(
        footnote = c("Note: 1 Hour Oral Glucose-tolerance is not performed at India-SAS. Total number excluded India-SAS.")
      ) %>%
      tab_footnote(
        footnote = c("Note: Test completed at ANC32 or ANC36 or unscheduled visit on/after 26 gestational weeks are included.")
      ) %>%
...
tb3_1 <- tb_theme2(matrix3_1) %>%
 ...
 #*******************table 3.2********************
  if(num %in% c(1,2,3,5,10)) {
    matrix3_2 <- df_lab %>%
      ...
      summarise(
        ...
        "Missing Eosinophils (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))) & SITE != "Pakistan", na.rm = TRUE),
          " (",
        ...
        "Missing Platelet Distribution Width (PDW): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_PDW_CT_LBORRES_", num, ")"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_PDW_CT_LBORRES_", num, ")"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE)/sum(!SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
    ...
    matrix3_2[c(5,7,14,15,16,18,19), c("Pakistan")] = "n/a"
  ...
  tab_footnote(
    footnote = c("*Denominator: Participants with complete MNH08 in person or by phone")
      ...
      tab_footnote(
    footnote = c("Note: Neutrophils full cell count, lymphocytes full cell count, monocyte full cell count, eosinophils %, eosinophils full cell count, plateletcrit, platelet distribution width are not performed at Pakistan. Total number excluded Pakistan.")
  ) %>%
    tab_footnote(
...
tb3_7 <- tb_theme2(matrix3_7) %>%
  ...
  tab_footnote(
    footnote = c("*Denominator: ReMAPP participants (after ReMAPP relaunch) with complete MNH08 in person or by phone")
  ) %>%
    tab_footnote(
...
"Missing Gonorrhea: n (%)*" = paste0(
 ...
 tab_stubhead(label = "Part X. Chlamydia and Gonorrhea Test Results") %>%
  ...
  tab_footnote(
    footnote = c("Note: We do not expect CT/NG test results at this time for any site, as the testing plans are to batch test in 2025.")
  ) %>%
...
#******************table 3.11*****************
 if (num %in% c(1,4,10)) {
    ...
    tb3_11 <- tb_theme2(matrix3_11) %>%
      ...
      tab_footnote(
        footnote = c("*Denominator: Participants with testing date (ZCD_LBTSTDAT) on/after following at enrollment and with complete MNH08 in person or by phone: Ghana: 2024-04-09; CMC: 2024-04-23; SAS: 2024-04-29; Kenya: 2024-05-06; Pakistan: 2024-04-05; Zambia: 2024-03-19")
      ) %>%
      tab_footnote(
        footnote = c("**Denominator: Participants with testing date (LEPT_IGM_LBTSTDAT) on/after following at enrollment and with complete MNH08 in person or by phone: Ghana: 2024-04-09; CMC: 2024-03-06; SAS: 2024-03-11; Kenya: 2024-05-06; Pakistan: 2024-04-05; Zambia: 2024-03-19")
      ) %>%
      tab_footnote(
        footnote = c("***Denominator: Participants with testing date (LEPT_IGG_LBTSTDAT) on/after following at enrollment and with complete MNH08 in person or by phone: Ghana: 2024-04-09; CMC: 2024-03-06; SAS: 2024-03-11; Kenya: 2024-05-06; Pakistan: 2024-04-05; Zambia: 2024-03-19")
      ) %>%
      tab_footnote(
        footnote = c("****Denominator: Participants with testing date (HEV_LBTSTDAT) on/after following at enrollment and with complete MNH08 in person or by phone: Ghana: 2024-04-09; CMC: 2024-03-06; SAS: 2024-03-11; Kenya: 2024-05-06; Pakistan: 2024-04-05; Zambia: 2024-03-19")
      ) %>%
      tab_footnote(
        footnote = c("*Denominator: We only expect 1000 participants per site. There will be no miss if the goal is reached")
      ) %>%
...
tb3_12 <- tb_theme2(matrix3_12) %>%
  ...
  tab_footnote(
    footnote = c("*Denominator: ReMAPP participants (after ReMAPP relaunches and before ReMAPP ends) with complete MNH08 in person or by phone")
  ) %>%
    tab_footnote(
    footnote = c("Note: G6PD is for ReMAPP primary cohort exclusively.")
  ) %>%
    tab_footnote(
    footnote = c("Note: Current missingness is showing if missing at all time points")
  ) %>%
  #column width (main table part)
  tab_options(table.width = pct(100)) %>%
...
#ANC32
                          ...
                          fig38_path, fig39_path, fig310_path, fig311_path))}
#ANC36
...
#PNC6
else if (num == 10) {knit<- knitr::include_graphics(c(fig22_path, fig32_path, fig34_path, fig311_path))}
#PNC26
