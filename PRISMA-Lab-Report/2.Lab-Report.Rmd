---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Lab Missingness Report (Issued: 2026-03-12)"
...
**Date of included data upload to Synapse: ** 2026-03-06
...
matrix1 <- df_lab %>% 
  ...
  plyr::summarise(
        #en
    "Expected Form: N" = sum(ENROLL ==1, na.rm= TRUE),
    
    "Missing MNH06: n (%)" = paste0(
      sum(is.na(M06_TYPE_VISIT_1), na.rm = TRUE), 
      " (", 
      format(round(sum(is.na(M06_TYPE_VISIT_1), na.rm = TRUE)/sum(ENROLL ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    "Missing MNH08: n (%)" = paste0(
      sum(is.na(M08_TYPE_VISIT_1), na.rm = TRUE), 
      " (", 
      format(round(sum(is.na(M08_TYPE_VISIT_1), na.rm = TRUE)/sum(ENROLL ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
tb1_2 <- tb_theme2(matrix1 %>% slice(16:33)) %>% 
  tab_header(
    title = paste0("Table 2. Form missingness by site")
  ) %>%
...
## Table 3. Point of care (Malaria, HBV, HCV,  HIV) missingness at enrollment
```{r}
num = 1
visit = "enrollment"
      ...
      "Missing POC Malaria: n (%)*" = paste0(
        ...
        ")"),
          ...
          "Missing POC HBV: n (%)*" = paste0(
            sum(eval(parse(text = paste0("is.na(M06_HBV_POC_LBORRES_", num, ") ")))),
            " (",
            format(round(sum(eval(parse(text = paste0("is.na(M06_HBV_POC_LBORRES_", num, ") "))))/sum(denom_lab_06_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
            ")"),
          #En
          "Missing POC HCV: n (%)*" = paste0(
            sum(eval(parse(text = paste0("is.na(M06_HCV_POC_LBORRES_", num, ") ")))),
            " (",
            format(round(sum(eval(parse(text = paste0("is.na(M06_HCV_POC_LBORRES_", num, ") "))))/sum(denom_lab_06_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
            ")"),
          "Expected HIV Results: N" = (sum(denom_hiv == 1, na.rm = TRUE)),
          "Missing POC HIV: n (%)**" = paste0(
        sum(eval(parse(text = paste0("is.na(M06_HIV_POC_LBORRES_", num, ") "))) & denom_hiv == 1, na.rm = TRUE),
        " (",
        format(round(sum(eval(parse(text = paste0("is.na(M06_HIV_POC_LBORRES_", num, ") ")))& denom_hiv == 1, na.rm = TRUE)/sum(denom_hiv == 1 & denom_lab_06_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
        ")")
      ) %>% 
      t() %>% unlist()
  ) %>% 
...
tb2_1 <- tb_theme2(matrix2_1) %>%
  # table title
 tab_header(
      title = paste0("Table 3. Point of care (Malaria, HBV, HCV,  HIV) missingness at", visit)
    ) %>%
    # stubhead label
  ...
  tab_row_group(
    ...
    tab_footnote(
      footnote = c("*Denominator: Participants with complete MNH06 in person")
    ) %>%
      tab_footnote(
      footnote = c("**Denominator: Same denominator as above except known HIV+ as documented by MOH doesn't expect POC HIV results in Kenya")
    ) %>%
    tab_footnote(
    ...
    gtsave(paste0("results/missing_m06_", visit, "_13.png"), expand = 10)


knitr::include_graphics(paste0("results/missing_m06_", visit, "_13.png"))

```

## Table 4. Hemoglobin point of care and cbc missingness at enrollment
```{r}
    matrix2_2 <- df_lab %>%
      group_by(SITE) %>%
      summarise(
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          plyr::summarise(
            "Expected POC HB from MNH06: N" = sum(denom_lab_06_1==1, na.rm = TRUE),
            #all visits except IPC
            "Missing POC HB: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M06_HB_POC_LBORRES_", num, ") & denom_lab_06_", num, " == 1"))), na.rm = TRUE),
              " (",
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 4. Hemoglobin point of care and cbc missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("**Denominator: Participants with complete MNH08 in person")
      ) %>%
      tab_footnote(
        footnote = c("***Denominator: Participants with both complete MNH06 and MNH08 in person ")
      ) %>%
      tab_source_note(
        source_note = "Note: Grey fonts indicate test not done/required by site."
      ) %>%
      tab_style(
        style = list(
          cell_text(color = "red") 
...
knitr::include_graphics(paste0("results/missing_m06_", visit, "_2.png"))

```

## Table 5. Syphilis point of care and titers missingness at enrollment
```{r}
    matrix2_4 <- df_lab %>%
      group_by(SITE) %>%
      summarise(
    ...
    tb2_4 <- tb_theme2(matrix2_4) %>%
      # table title
      tab_header(
        title = paste0("Table 5. Syphilis point of care and titers missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("**Denominator: Participants with a positive treponemal RDT, for whom the site has initiated a Syphilis titer test, and who have completed MNH08 either in person")
      ) %>%
      tab_footnote(
...
knitr::include_graphics(paste0("results/missing_m06_", visit, "_1.png"))
```

## Table 6. Blood typing missingness at enrollment
```{r}
    matrix3_1 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_1 == 1, na.rm = TRUE),
            "Missing Blood Group: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_BLD_GRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_BLD_GRP_LBORRES_", num, ")"))))/sum(denom_lab_08_1 == 1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing RH Factor: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RH_FACTOR_LBORRES_", num, ")"))) & denom_lab_08_1 == 1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RH_FACTOR_LBORRES_", num, ")"))) & denom_lab_08_1 == 1)/sum(denom_lab_08_1 == 1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      )  %>%
    ...
    tb3_1 <- tb_theme2(matrix3_1) %>%
      # table title
      tab_header(
        title = paste0("Table 6. Blood typing missingness at", visit)
      ) %>%
      # stubhead label
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_1.png"))


```

## Table 7. CBC missingness at enrollment
```{r}

df_lab_cbc <- df_lab %>% 
            mutate(denom = case_when(denom_lab_08_1==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                          (SITE == "Kenya" & denom_remapp == 1) | 
                                                          (SITE == "Pakistan" & denom_remapp==1) |
                                                          (SITE == "Ghana" & denom_remapp==1)|
                                                          (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
          ) ~ 1, TRUE ~ 0)) 



    matrix3_2 <- df_lab_cbc %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      group_by(SITE) %>%
      summarise(
        "Expected Lab Results from MNH08: N" = n(),
        ...
        "Missing Neutrophils (full cell count) : n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/n() *100, 2), nsmall = 0, digits = 2),
          ")"),
        ...
        "Missing Lymphocyte (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/n()*100, 2), nsmall = 0, digits = 2),
          ")"),
        ...
        "Missing Monocyte (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/n()*100, 2), nsmall = 0, digits = 2),
          ")"),
      ...
      add_column(
        ...
        "Missing Hematocrit (HCT): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_HCT_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_HCT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing White Blood Cells (WBC): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_WBC_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_WBC_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Neutrophils (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_PCT_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_PCT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Neutrophils (full cell count) : n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom_lab_08_1 ==1, na.rm =TRUE) *100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Lymphocyte (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_PCT_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_PCT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Lymphocyte (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Erythrocyte Count: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_ERYTH_MM_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_ERYTH_MM_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Mean Cell Volume (MCV): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MCV_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCV_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Mean Cell Hemoglobin (MCH): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MCH_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCH_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Mean Corpuscular Hemoglobin Concentration (MCHC): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MCHC_GDL_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCHC_GDL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Platelets count: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_PLATE_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_PLATE_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Monocyte (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_PCT_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_PCT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Monocyte (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Eosinophils (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        "Missing Eosinophils (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Red Cell Width (%): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_RDW_PCT_LBORRES_", num, ")")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_RDW_PCT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Platelet Distribution Width (PDW): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_PDW_CT_LBORRES_", num, ")","&", "pdw_denom_lab_08_", num, " == 1"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE),
          " (",
    ...
    # matrix3_2[c(5,7,14,16,18,19), c("Pakistan")] = "n/a"
    matrix3_2[c(18,19), c("Pakistan")] = "n/a"
    matrix3_2[c(18,19), c("India-SAS")] = "n/a"
    
    tb3_2 <- tb_theme2(matrix3_2) %>%
      # table title
      tab_header(
        title = paste0("Table 7. CBC missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      # tab_footnote(
      #   footnote = c("Note: Plateletcrit and platelet distribution width are not performed at Pakistan. Total number excluded Pakistan.")
      # ) %>%
      tab_footnote(
        footnote = c("Note: Plateletcrit, platelet distribution width are not performed at India-SAS and Pakistan.")
      ) %>%
      tab_footnote(
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_2.png"))
```

## Table 8. Liver and kidney function test missingness at enrollment
```{r}
    matrix3_3 <- df_lab %>%
      # filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      filter(denom_kft_lft_fxn_enroll==1) %>% 
      group_by(SITE) %>%
      summarise(
        "Expected Lab Results from MNH08: N" = n(),
      ...
      add_column(
        ...
        plyr::summarise(
            ...
            "Missing AST/SGOT: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_AST_UL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_AST_UL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing ALT/SGPT: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_ALT_UL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_ALT_UL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Alkaline Phosphatase (ALP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_ALP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_ALP_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Total Bilirubin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_TBILIRUBIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_TBILIRUBIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Direct Bilirubin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_DBILIRUBIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_DBILIRUBIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Total Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_TPROTEIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_TPROTEIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Albumin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_ALBUMIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_ALBUMIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Gamma GT: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_GAMMAGT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_GAMMAGT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Indirect Bilirubin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_IBILIRUBIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_IBILIRUBIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing BUN: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_BUN_MMOLL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_BUN_MMOLL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Serum Creatinine: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CREAT_UMOLL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CREAT_UMOLL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Sodium: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_SODIUM_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_SODIUM_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Potassium: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_POTASSIUM_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_POTASSIUM_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Chloride: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CHLORIDE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CHLORIDE_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Phosphorus: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_PHOSPHORUS_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_PHOSPHORUS_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Calcium: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CALCIUM_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CALCIUM_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
    ...
    tb3_3 <- tb_theme2(matrix3_3) %>%
      # table title
      tab_header(
        title = paste0("Table 8. Liver and kidney function test missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("Note: Magnesium is optional at India-CMC, India-SAS and Kenya. Pakistan stopped performing on 2023-11-17. Total number only included Ghana and Zambia")
      ) %>%
      tab_footnote(
        footnote = c("Note: Kidney and Liver function testing stopped on 2025-12-31 (CMC); 2024-08-31 (SAS)).")
      ) %>%
      tab_footnote(
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_3.png"))

```

## Table 9. Micronutrients missingness at enrollment
```{r}
    ...
    matrix3_4 <- df_lab %>%
      ...
      summarise(
        ...
        sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_1 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_1 ==1)/sum(denom_folate_serum_1 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        ## NEW FOLATE ROW FOR FOLATE RBC
        #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Folate - RBC: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_1 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_1 ==1)/sum(denom_folate_rbc_1 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        ...
        "Total" = df_lab %>%
          ...
          plyr::summarise(
            ...
            "Missing Serum Vitamin B12 (Total Cobalamin): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Vit B12 (HoloTC): 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Vitamin B12 (Holotranscobalamin II): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            ...
            "Missing Ferritin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Iodine (Thyroglobulin or Tg): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Transferrin Receptor (sTfR): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Retinol Protein Binding 4 (RBP4): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing C-reactive Protein (CRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Alpha 1-acid Glycoprotein (AGP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Histidine-rich Protein 2 (HRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_4 <- tb_theme2(matrix3_4) %>%
      # table title
      tab_header(
        title = paste0("Table 9. Micronutrients missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("Note: holoTC stopped testing: 2024-09-01 (Ghana); 2024-10-15 (CMC); 2024-08-31 (SAS); 2025-01-14 (Kenya); 2024-10-03 (Pakistan); 2024-10-11 (Zambia)")
      ) %>%
      tab_footnote(
        footnote = c("Note: Blood serum folate stopped testing: 2024-02-23 (Ghana); 2024-03-05 (CMC); 2024-03-01 (SAS); 2024-04-11 (Kenya); 2024-04-08 (Pakistan); 2024-04-12 (Zambia)")
      ) %>%
      tab_footnote(
        footnote = c("Note: RBC folate started testing: 2024-02-23 (Ghana); 2024-03-05 (CMC); 2024-03-01 (SAS); 2024-04-11 (Kenya); 2024-04-08 (Pakistan); 2024-04-12 (Zambia)")
      ) %>% 
      # tab_footnote(
 ...
 knitr::include_graphics(paste0("results/missing_m08_", visit, "_4.png"))
```

## Table 10. Thyroid missingness at enrollment
```{r}
    matrix3_5 <- df_lab %>%
      ...
      summarise(
        ...
        "Missing TSH: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)")))),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))))/sum(denom_t3t4_enroll == 1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
      ...
      add_column(
        ...
        "Missing TSH: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)")))  & denom_lab_08_1==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))) & denom_lab_08_1==1)/sum(denom_t3t4_enroll == 1 & denom_lab_08_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        "Missing Free T4: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_THYROID_FREET4_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))) & denom_lab_08_1==1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_THYROID_FREET4_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))) & denom_lab_08_1==1)/sum(denom_t3t4_enroll == 1 & denom_lab_08_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            
            "Missing Free T3: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_THYROID_FREET3_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))) & denom_lab_08_1==1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_THYROID_FREET3_LBORRES_", num, ")", "& (denom_t3t4_enroll == 1)"))) & denom_lab_08_1==1)/sum(denom_t3t4_enroll == 1 & denom_lab_08_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_5 <- tb_theme2(matrix3_5) %>%
      # table title
      tab_header(
        title = paste0("Table 10. Thyroid missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("Note: all thyroid testing stopped on 2024-09-01 (Ghana); 2024-10-17 (CMC); 2024-01-14 (Kenya); 2024-10-03 (Pakistan); 2024-10-11 (Zambia); ongoing (SAS).")
      ) %>%
      gtsave(paste0("results/missing_m08_", visit, "_5.png"), expand = 10)

knitr::include_graphics(paste0("results/missing_m08_", visit, "_5.png"))

```

## Table 11. HbA1c missingness at enrollment
```{r}
   matrix3_6 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      group_by(SITE) %>%
      summarise(
        ...
        "Missing HbA1c: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_HBA1C_LBORRES_", num, ")"))) & SITE %in% c( "Zambia")),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_HBA1C_LBORRES_", num, ")"))) & SITE %in% c("Zambia"))/sum(SITE %in% c("Zambia"), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing HbA1c (%): n (%)*" = paste0(
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          ...
          plyr::summarise(
            ...
            #Hemoglobin A1c (P: En)
            "Missing HbA1c: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_HBA1C_LBORRES_", num, ")"))) & SITE %in% c("Zambia") & denom_lab_08_1==1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_HBA1C_LBORRES_", num, ")"))) & SITE %in% c("Zambia") & denom_lab_08_1==1)/sum(SITE %in% c("Zambia") & denom_lab_08_1==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Hemoglobin A1c (P: En)
            "Missing HbA1c (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_HBA1C_PRCNT_", num, ")"))) & denom_lab_08_1==1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_HBA1C_PRCNT_", num, ")")))& denom_lab_08_1==1)/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    matrix3_6[2, c("Ghana", "India-CMC", "India-SAS", "Kenya", "Pakistan")] = "n/a"
    
    tb3_6 <- tb_theme2(matrix3_6) %>%
      # table title
      tab_header(
        title = paste0("Table 11. HbA1c missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("Note: Ghana, India-CMC, India-SAS, Kenya and Pakistan only report %, no Discrete value expected. Total number excluded India-CMC, India-SAS, Kenya and Pakistan.")
      ) %>%
      gtsave(paste0("results/missing_m08_", visit, "_6.png"), expand = 10)

knitr::include_graphics(paste0("results/missing_m08_", visit, "_6.png"))
```

## Table 12. Red blood cell disorders and hemoglobinopathies missingness at enrollment
```{r}

df_rbc_cmc <- df_lab %>%
  filter(SITE == "India-CMC") %>%  # Fixed the broken quote
  select(SITE, MOMID, PREGID, denom_rbc_disorder,
         starts_with("M08_RBC_SICKLE_LBORRES_"),
         starts_with("M08_RBC_THALA")) %>%
  
  # 1. Use rowwise() for row-based logic
  rowwise() %>%
  mutate(
    # Return first valid 0 or 1 across any THALA LBORRES vars
    M08_RBC_THALA_LBORRES_1 = {
      vals <- c_across(starts_with("M08_RBC_THALA_LBORRES_"))
      match_val <- vals[which(vals %in% c(0, 1))[1]]
      if (!is.na(match_val)) match_val else NA_real_
    },
    
    # Optional: do the same for SICKLE, if needed:
    M08_RBC_SICKLE_LBORRES_1 = {
      vals <- c_across(starts_with("M08_RBC_SICKLE_LBORRES_"))
      if (any(vals %in% c(0, 1))) 1 else NA_real_
    },
    # M08_RBC_SICKLE_LBORRES_1 = {
    #   vals <- c_across(starts_with("M08_RBC_SICKLE_LBORRES_"))
    #   match_val <- vals[which(vals %in% c(0, 1))[1]]
    ...
    # Auto-check any THALA_X variables
    across(
      .cols = matches("^M08_RBC_THALA_[0-9]+_"),
      .fns = ~ ifelse(any(.x %in% c(0, 1)), 1, NA_real_),
      .names = "{.col}1"
    )
  ) %>%
  ungroup() %>%
...
#   mutate(M08_RBC_THALA_LBORRES_1 = if (any(c_across(starts_with("M08_RBC_THALA_LBORRES_")) %in% c(0, 1))) 1 else NA) %>%
# mutate(
...
# ## pull Ghana ids for missing hemoglobinopathy/sickle cell and missing specified results 
# mnh02 <- read.csv(paste0("D:/Users/stacie.loisate/Documents/import/", "2025-11-28_gha", "/mnh02.csv")) %>%
#   rename_with(toupper)
# 
# df_thala <- df_rbc %>%
#   filter(SITE == "Ghana" & denom_rbc_disorder == 1) %>%
#   select(SITE, MOMID, PREGID, M08_RBC_SICKLE_LBORRES_1, M08_RBC_THALA_LBORRES_1) %>%
#   mutate(MISSING_SICKLE_CELL = case_when(is.na(M08_RBC_SICKLE_LBORRES_1) ~ 1, TRUE ~ 0),
#          MISSING_HEMOGLOBINOPATHIES = case_when(is.na(M08_RBC_THALA_LBORRES_1) ~ 1, TRUE ~ 0),
#          ) %>%
#   filter(MISSING_SICKLE_CELL==1 | MISSING_HEMOGLOBINOPATHIES==1) %>%
#   # uncomment the next 2 rows only if pulling for ghana
#   left_join(mnh02 %>% select(PREGID, GHAID), by = c("PREGID")) %>%
#   relocate(GHAID, .after = PREGID) %>%
#   select(-SITE)
# 
# ## pull kenya ids for missing missing specified results
# df_specified_results <- df_rbc %>%
#   filter(SITE == "Ghana" & denom_rbc_disorder == 1 & M08_RBC_THALA_LBORRES_1==1) %>%
#   # select(SITE, MOMID, PREGID, starts_with("M08_RBC_THALA_"), -contains("LBSTDAT"),-contains("LBTSTDAT_"), -contains("LBORRES")) %>% 
#   select(SITE, MOMID, PREGID, paste0("M08_RBC_THALA_", 1:19, "_1")) %>%
#   # mutate(MISSING_SPECIFIED = if_else(any(is.na(c_across(starts_with("M08_RBC_THALA_")))),1L, 0L)) %>%
#   
#   rowwise() %>%
#  mutate(MISSING_SPECIFIED = if_else(any(is.na(c_across(starts_with("M08_RBC_THALA_")))),
#                                      1L,0L)) %>% 
#   ungroup() %>% 
#   # mutate(MISSING_SPECIFIED = if_else(any(is.na(c_across(starts_with("M08_RBC_THALA_3_")))),1L, 0L)) %>%
#   filter(MISSING_SPECIFIED == 1) %>%
#     rename_with(~ str_remove(.x, "_1$"),starts_with("M08_RBC_THALA_")) %>%
#     # uncomment the next 2 rows only if pulling for ghana
#   left_join(mnh02 %>% select(PREGID, GHAID), by = c("PREGID")) %>%
#   relocate(GHAID, .after = PREGID) %>%
#   select(-SITE)
...
# library(writexl)
# export_list <- list()
# export_list[['Missing hemoglobinopathy']] <- df_thala
# export_list[['Missing specified result']] <- df_specified_results
# 
# write_xlsx(export_list, path = paste0("D:/Users/stacie.loisate/Documents/Output/Outcomes-Queries/healthy cohort/", UploadDate, "-Ghana-Missing-Hemoglobinopathies-IDs.xlsx"))
# 
matrix3_7 <- df_rbc %>%
      ...
      add_column(
        ...
        "Total" = df_rbc %>%
          filter(denom_rbc_disorder == 1) %>%
          plyr::summarise(
            "Expected RBC Disorder Test Results from MNH08: N" = sum(denom_rbc_disorder==1, na.rm=TRUE),
            #En
            "Missing Sickle Cell Disease: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RBC_SICKLE_LBORRES_", num, ")"))) & denom_rbc_disorder==1),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RBC_SICKLE_LBORRES_", num, ")"))) & denom_rbc_disorder==1)/sum(denom_rbc_disorder==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #En
            "Missing Hemoglobinopathies & Thalassemias: (%)" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RBC_THALA_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RBC_THALA_LBORRES_", num, ")"))))/sum(denom_rbc_disorder==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Hemoglobinopathies & Thalassemias Results from MNH08: N" = sum(eval(parse(text = paste0("M08_RBC_THALA_LBORRES_", num, " == 1"))), na.rm = TRUE),
            #M08_RBC_THALA_LBORRES_# == 1 for all below
            "Missing SS: n (%)**" = paste0(
    ...
    tb3_7 <- tb_theme2(matrix3_7) %>%
      # table title
      tab_header(
        title = paste0("Table 12. Red blood cell disorders and hemoglobinopathies missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("*Denominator: ReMAPP participants with complete MNH08 in person")
      ) %>%
      tab_footnote(
        footnote = c("**Denominator: ReMAPP participants with positive hemoglobinopathies & thalassemias results")
      ) %>%
      tab_footnote(
        footnote = c("Note: RBC disorder test are for ReMAPP primary cohort exclusively. All sites are using screen date start at/after ReMAPP launch date, except Pakistan is using lab test between REMAPP launch date and test end date.")
      ) %>%
      tab_footnote(
        footnote = c("Zambia note: Thalassemias, sickle cell results delayed due to shipment to KHRC for testing; CMC note: Tests performed at any visit are included to account for the catch-up tests.")
      ) %>%
      tab_source_note(
        source_note = c("Note: Included data with date at/after: 2022-12-28 (Ghana); 2023-06-20(CMC); 2023-12-12(SAS); 2023-04-03(Kenya); 2022-12-15(Zambia); LBSTDAT between: 2022-09-22 (Pakistan).")
      ) %>%
      tab_source_note(
        source_note = c("Note: Testing stopped: 2024-10-29 (Ghana); 2024-01-17 (Pakistan); 2025-08-22(CMC).")
      ) %>%
      tab_style(
        style = list(
          cell_text(color = "red") 
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_7.png"))

```

## Table 13. Urinalysis missingness at enrollment
```{r}
        ...
        "Total" = df_lab %>%
          ...
          plyr::summarise(
            ...
            "Missing Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Leukocytes: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Nitrites: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")"))))/sum(denom_lab_08_1 ==1, na.rm =TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_8 <- tb_theme2(matrix3_8) %>%
      # table title
      tab_header(
        title = paste0("Table 13. Urinalysis missingness at ", visit)
      ) %>%
      # stubhead label
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_8.png"))
```

## Table 14. Tuberculosis (TB) missingness at enrollment
```{r}
...
matrix3_9 <- df_lab %>%
      ...
      summarise(
        ...
        "Completed W4SS Screening Questions: N" = sum(W4SS_SCREEN_PERF_ENROLL ==1, na.rm= TRUE),
        "Reported a GeneXpert Test Result: N" = sum(TB_CULTURE_PERF_ENROLL ==1, na.rm= TRUE),

        ## POSITIVE W4SS SCREENER
         "Tested Positive on W4SS Screening: N" = sum(W4SS_SCREEN_RESULT_ENROLL ==1, na.rm= TRUE),
          
        "GeneXpert Test Result Reported: n(%)*" = paste0(
          sum(W4SS_SCREEN_RESULT_ENROLL ==1 & TB_CULTURE_PERF_ENROLL ==1, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ENROLL ==1 & TB_CULTURE_PERF_ENROLL ==1, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ENROLL ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),

        "Missing GeneXpert Test Result: n(%)*" = paste0(
          sum(W4SS_SCREEN_RESULT_ENROLL ==1 & TB_CULTURE_PERF_ENROLL ==0, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ENROLL ==1 & TB_CULTURE_PERF_ENROLL ==0, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ENROLL ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        ## NEGATIVE W4SS SCREENER
         "Tested Negative on W4SS Screening: N" = sum(W4SS_SCREEN_RESULT_ENROLL ==0, na.rm= TRUE),
          
        "GeneXpert Test Result Reported: n(%)**" = paste0(
          sum(W4SS_SCREEN_RESULT_ENROLL ==0 & TB_CULTURE_PERF_ENROLL ==1, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ENROLL ==0 & TB_CULTURE_PERF_ENROLL ==1, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ENROLL ==0, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),

        # "Missing GeneXpert Test Result: n(%)**" = paste0(
        #   sum(W4SS_SCREEN_RESULT_ENROLL ==0 & TB_CULTURE_PERF_ENROLL ==0, na.rm = TRUE),
        #   " (",
        #   format(round(sum(W4SS_SCREEN_RESULT_ENROLL ==0 & TB_CULTURE_PERF_ENROLL ==0, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ENROLL ==0, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
        #   ")"),
        
        ## REMAPP AIM 3 PARTICIPANTS
        "ReMAPP Aim 3 Participants: N" = sum(REMAPP_AIM3_ENROLL ==1, na.rm= TRUE),
        
        "Sputum specimen collected: n(%)***" = paste0(
          sum(REMAPP_AIM3_ENROLL ==1 & TB_SPUTUM_COLLECT_AIM3==1,  na.rm = TRUE),
          " (",
          format(round(sum(REMAPP_AIM3_ENROLL ==1  & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        "Missing sputum specimen collection: n(%)***" = paste0(
          sum(REMAPP_AIM3_ENROLL ==1 & TB_SPUTUM_COLLECT_AIM3==0,  na.rm = TRUE),
          " (",
          format(round(sum(REMAPP_AIM3_ENROLL ==1  & TB_SPUTUM_COLLECT_AIM3 ==0, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        "GeneXpert Test Result Reported: n(%)****" = paste0(
          sum(REMAPP_AIM3_ENROLL ==1 & TB_CULTURE_PERF_AIM3 ==1 & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm = TRUE),
          " (",
          format(round(sum(REMAPP_AIM3_ENROLL ==1  & TB_CULTURE_PERF_AIM3 ==1 & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL ==1  & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        "Missing GeneXpert Test Result: n(%)****" = paste0(
          sum(REMAPP_AIM3_ENROLL ==1 & TB_CULTURE_PERF_AIM3 ==0 & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm = TRUE),
          " (",
          format(round(sum(REMAPP_AIM3_ENROLL ==1  & TB_CULTURE_PERF_AIM3 ==0 & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL ==1  & TB_SPUTUM_COLLECT_AIM3 ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
  ...
  tb3_9 <- tb_theme2(matrix3_9) %>%
      # table title
      tab_header(
        title = paste0("Table 14. Tuberculosis (TB) missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part IX. Tuberculosis (TB) Test Results") %>%
    tab_row_group(
        label = "Positive W4SS Screening (answered `yes` to any of the symptoms 1-4)",
        rows = 3:5
      ) %>%
    tab_row_group(
        label = "Negative W4SS Screening (answered `no` to all symptoms 1-4)",
        rows = 6:7
      ) %>%
   tab_row_group(
        label = "GeneXpert Testing Among ReMAPP Aim 3 Participants",
        rows = 8:12
      ) %>%
      row_group_order(groups = c(NA,
                                 "Positive W4SS Screening (answered `yes` to any of the symptoms 1-4)",
                                 "Negative W4SS Screening (answered `no` to all symptoms 1-4)",
                                 "GeneXpert Testing Among ReMAPP Aim 3 Participants"
      )) %>%
      tab_footnote(
        footnote = c("*Denominator: Participants testing positive on symptom screen on W4SS at enrollment")
      ) %>%
      tab_footnote(
        footnote = c("**Denominator: Participants testing negative on symptom screen on W4SS at enrollment")
      ) %>%
     tab_footnote(
        footnote = c("***Denominator: Participants enrolled in ReMAPP Aim 3")
      ) %>%
     tab_footnote(
        footnote = c("****Denominator: Participants enrolled in ReMAPP Aim 3 and provided a sputum sample")
      ) %>%
     tab_footnote(
        footnote = c("Note: ReMAPP Aim 3 section includes sputum collection or GeneXpert testing at ANY ANC visit.")
      ) %>%
    gtsave(paste0("results/missing_m08_", visit, "_9.png"), expand = 10)
    
knitr::include_graphics(paste0("results/missing_m08_", visit, "_9.png"))
...
## Table 15. Chlamydia and gonorrhea missingness at enrollment
```{r}

    matrix3_10 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_ctng_", num, " == 1")))) %>%
      group_by(SITE) %>%
      summarise(
        "Expected Lab Results from MNH08: N" = n(),
        #EN, ANC32
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_ctng_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_ctng_1 ==1, na.rm=TRUE),
            #EN, ANC32
            "Missing Chlamydia: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CTNG_CT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CTNG_CT_LBORRES_", num, ")"))))/sum(denom_ctng_1 ==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Gonorrhea: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CTNG_NG_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CTNG_NG_LBORRES_", num, ")"))))/sum(denom_ctng_1 ==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_10 <- tb_theme2(matrix3_10) %>%
      # table title
      tab_header(
        title = paste0("Table 15. Chlamydia and gonorrhea missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("*Denominator: Participants with complete MNH08 in person with a specimen collection date after expansion dates: Ghana: 2024-04-09; CMC: 2024-07-03; SAS: 2024-03-11; Kenya: 2024-03-07; Pakistan: 2024-04-25; Zambia: 2023-11-09")
      ) %>%
      tab_footnote(
        footnote = c("Note: CT/NG testing stopped Ghana: 2025-04-01;CMC: 2025-09-17; SAS: 2025-03-06;Kenya: 2025-02-27; Pakistan: 2025-04-24; Zambia: 2025-03-13")) %>%
      # tab_footnote(
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_10.png"))

```

## Table 16. ZCD/Lepto/Hep E missingness at enrollment
```{r}
    matrix3_11 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      group_by(SITE) %>%
      summarise(
        "Expected Zika-Dengue-Chikungunya (ZCD) IgM/IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000),
        #EN, ANC32, PNC6
        "Missing ZIKV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
              ...
              min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing ZIKV IgG Test Results : n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
        ...
        "Missing DENV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
        ...
        "Missing DENV IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
        ...
        "Missing CHKV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
        ...
        "Missing CHKV IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
              ...
              min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Expected Leptospirosis IgM Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000),
        "Missing Leptospirosis IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
            )/min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Expected Leptospirosis IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000),
        "Missing Leptospirosis IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_LEPT_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
          ...
          ")"),
        "Expected Hepatitis E IgM/IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 1000),
        "Missing HEV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE), 
            sum(eval(parse(text = paste0("is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE), 
        ...
        "Missing HEV IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE), 
            sum(eval(parse(text = paste0("is.na(M08_HEV_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Zika-Dengue-Chikungunya (ZCD) IgM/IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000),
            #EN, ANC32, PNC6
            "Missing ZIKV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
                min(
                  ...
                  min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing ZIKV IgG Test Results : n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(
            ...
            "Missing DENV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
            ...
            "Missing DENV IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
            ...
            "Missing CHKV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(
            ...
            "Missing CHKV IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
                min(
                  ...
                  min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Leptospirosis IgM Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000),
            "Missing Leptospirosis IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                    sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
                )/min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Leptospirosis IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000),
            "Missing Leptospirosis IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_LEPT_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(
              ...
              ")"),
            "Expected Hepatitis E IgM/IgG Test Results from MNH08: N" = min(sum(denom_zcd_lept_hev_1 == 1, na.rm = TRUE), 6000),
            "Missing HEV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE), 
                sum(eval(parse(text = paste0("is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE),
              " (",
              format(round(
                min(
            ...
            "Missing HEV IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1")))), na.rm = TRUE), 
                sum(eval(parse(text = paste0("is.na(M08_HEV_IGG_LBORRES_", num, ") & denom_zcd_lept_hev_1 == 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
    ...
    tb3_11 <- tb_theme2(matrix3_11) %>%
      # table title
      tab_header(
        title = paste0("Table 16. ZCD/Lepto/Hep E missingness at ", visit)
      ) %>%
      # stubhead label
      ...
      tab_footnote(
        footnote = c("*Denominator: Participants with laboratory results on/after test start date at enrollment and with complete MNH08 in person.")
      ) %>%
      # tab_footnote(
      #   footnote = c("**Denominator: Participants with testing date (LEPT_IGM_LBTSTDAT) on/after test start date at enrollment and with complete MNH08 in person.")
      # ) %>%
      # tab_footnote(
      #   footnote = c("***Denominator: Participants with testing date (LEPT_IGG_LBTSTDAT) on/after test start date at enrollment and with complete MNH08 in person.")
      # ) %>%
      # tab_footnote(
      #   footnote = c("****Denominator: Participants with testing date (HEV_LBTSTDAT) on/after test start date at enrollment and with complete MNH08 in person.")
      # ) %>%
      tab_footnote(
        footnote = c("Note: We only expect 1000 participants per site. There will be no miss if the goal is reached")
      ) %>%
      tab_footnote(
        footnote = c("Note: ZCD/HEV/Lepto test start dates are: Ghana: 2024-04-09; CMC: 2024-03-06; SAS: 2024-03-11; Kenya: 2024-03-06; Pakistan: 2024-04-08; Zambia: 2023-11-09")
      ) %>%
      tab_footnote(
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_11.png"))

```

## Table 17. G6PD missingness
```{r}
...
g6pd <- mnh08 %>% 
    ...
    ungroup() %>%
    rename(M08_RBC_G6PD_LBORRES_13 = M08_RBC_G6PD_LBORRES)
    # pivot_wider(names_from = "" , values_from = M08_RBC_G6PD_LBORRES)
  
  df_g6pd_77na <- df_lab %>%
    #Include data for ReMAPP only, may use RBC_G6PD_LBTSTDAT instead
    filter(denom_remapp == 1) %>%
    left_join(g6pd) %>%
    select(SITE, MOMID, PREGID, denom_remapp, num_range("M08_RBC_G6PD_LBORRES_", 1:14)) %>% 
    left_join(df_lab %>% select(MOMID, PREGID, starts_with("M08_RBC_G6PD_LBORRES_INTERPRET_")), by = c("MOMID", "PREGID")) %>%
  ...
  df_g6pd_77na <- df_g6pd_77na %>% 
        mutate(g6pd_missing = sum(if_all(var_columns, is.na)))
...
matrix3_12 <- df_g6pd_77na %>%
    ...
    summarise(
      ...
      "Missing Glucose-6-Phosphate Dehydrogenase (G6PD): n (%)*" = paste0(sum(g6pd_missing==1 & denom_remapp == 1, na.rm=TRUE), " (",
          format(round(sum(g6pd_missing==1 & denom_remapp == 1, na.rm=TRUE)/sum(denom_remapp == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2), ")") 
    ) %>%
    t() %>% as.data.frame() %>%
    `colnames<-`(c(.[1,])) %>%
    slice(-1) %>%
    add_column(
      ...
      "Total" = df_g6pd_77na %>%
      plyr::summarise(
          "Expected G6PD Test Results from MNH08: N" = sum(denom_remapp==1, na.rm=TRUE),
          
      "Missing Glucose-6-Phosphate Dehydrogenase (G6PD): n (%)*2" = paste0(sum(g6pd_missing==1 & denom_remapp == 1, na.rm=TRUE), " (",
                   format(round(sum(g6pd_missing==1 & denom_remapp == 1, na.rm=TRUE)/sum(denom_remapp == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2), ")")         #   "Missing Glucose-6-Phosphate Dehydrogenase (G6PD): n (%)*" = paste0(sum(if_all(var_columns, is.na)), " (",
        #           format(round(sum(if_all(var_columns,is.na))/sum(denom_remapp==1,na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
        # ")")
        ) %>%
        t() %>% unlist()
    ) %>%
  ...
  tb3_12 <- tb_theme2(matrix3_12) %>%
    # table title
    tab_header(
      title = paste0("Table 17. G6PD missingness")
    ) %>%
    # stubhead label
    tab_stubhead(label = "Part XII. G6PD Test Results") %>%
    tab_footnote(
      footnote = c("*Denominator: ReMAPP participants (after ReMAPP start date and before ReMAPP end date) with complete MNH08 in person")
    ) %>%
    tab_footnote(
    ...
    tab_source_note(
      source_note = c("Note: Included data with SCRN_OBSSTDAT at/after: 2022-12-28 (Ghana); 2023-06-20(CMC); 2023-12-12(SAS); 2023-04-03(Kenya); 2022-12-15(Zambia); LBSTDAT between: 2022-09-22 (Pakistan). Kenya data includes categorical interpretation variable.")
    ) %>%
    tab_source_note(
...
knitr::include_graphics(paste0("results/missing_m08_", visit, "_12.png"))

```

# Missingness at ANC-20
## Table 18. Hemoglobin missingness at ANC20
```{r}
num = 2 
visit = "ANC20"
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 18. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 19. CBC missingness at ANC20
```{r}

df_lab_cbc <- df_lab %>% 
            mutate(denom = case_when(denom_lab_08_2==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                          (SITE == "Kenya" & denom_remapp_cbc_2 == 1) | 
                                                          (SITE == "Pakistan" & denom_remapp_cbc_2==1) |
                                                          (SITE == "Ghana" & denom_remapp_cbc_2==1)|
                                                          (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
          ) ~ 1, TRUE ~ 0)) 

    matrix3_2 <- df_lab_cbc %>%
      # filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & SITE != "Pakistan") %>%
      mutate(M08_LBSTDAT_1 = ymd(M08_LBSTDAT_1),
             M08_LBSTDAT_2 = ymd(M08_LBSTDAT_2),
             M08_LBSTDAT_3 = ymd(M08_LBSTDAT_3),
             M08_LBSTDAT_5 = ymd(M08_LBSTDAT_5),
             M08_LBSTDAT_10 = ymd(M08_LBSTDAT_10)
      ...
      summarise(
        ...
        "Missing Monocyte (%): n (%)*" = paste0(
          ...
          ")"),
        "Missing Monocyte (full cell count): n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/n()*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing Eosinophils (%): n (%)*" = paste0(
      ...
      add_column(
        ...
        "Total" = df_lab_cbc %>%
          ...
          # filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & SITE != "Pakistan") %>%
          filter(denom_lab_08_2 ==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                                                (SITE == "Kenya" & denom_remapp_cbc_2 == 1) | 
                                                                                (SITE == "Pakistan" & denom_remapp_cbc_2==1) |
                                                                                (SITE == "Ghana" & denom_remapp_cbc_2==1)|
                                                                                (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
        ...
        plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom==1, na.rm = TRUE),
            "Missing Hematocrit (HCT): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_HCT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_HCT_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing White Blood Cells (WBC): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_WBC_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_WBC_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Neutrophils (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_PCT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_PCT_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Neutrophils (full cell count) : n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_NEU_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Lymphocyte (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_PCT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_PCT_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Lymphocyte (full cell count): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_LYMPH_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Erythrocyte Count: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_ERYTH_MM_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_ERYTH_MM_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Mean Cell Volume (MCV): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MCV_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCV_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Mean Cell Hemoglobin (MCH): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MCH_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCH_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Mean Corpuscular Hemoglobin Concentration (MCHC): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MCHC_GDL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MCHC_GDL_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Platelets count: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_PLATE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_PLATE_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Monocyte (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_PCT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_PCT_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Monocyte (full cell count): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))) & SITE != "Pakistan", na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))) & SITE != "Pakistan", na.rm = TRUE)/sum(SITE != "Pakistan" & denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Eosinophils (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))), na.rm = TRUE)/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            
            "Missing Eosinophils (full cell count): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))) , na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Red Cell Width (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_RDW_PCT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_RDW_PCT_LBORRES_", num, ")"))))/ sum(denom==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Platelet Distribution Width (PDW): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_PDW_CT_LBORRES_", num, ")","&", "pdw_denom_lab_08_", num, " == 1"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE),
              " (",
            ...
            "Missing Plateletcrit (PCT) (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_PCT_PCT_LBORRES_", num, ")"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_PCT_PCT_LBORRES_", num, ")"))) & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE)/sum(denom==1 & !SITE %in% c("Pakistan", "India-SAS"), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    # matrix3_2[c(5,7,14,16,18,19), c("Pakistan")] = "n/a" ## 15/16 removed
    matrix3_2[c(18,19), c("Pakistan")] = "n/a" ## 15/16 removed
    ...
    tb3_2 <- tb_theme2(matrix3_2) %>%
      # table title
      tab_header(
        title = paste0("Table 19. CBC missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. CBC Results") %>%
      tab_row_group(
      ...
      tab_footnote(
        footnote = c("Pakistan note: Plateletcrit and platelet distribution width are not performed. Remaining CBC testing stopped from 2024-04-22")
      ) %>%
      #    tab_footnote(
...
## Table 20. Urinalysis missingness at ANC20
```{r}
    ...
    matrix3_8 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_2==1, na.rm = TRUE),
            "Missing Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")"))))/sum(denom_lab_08_2==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Leukocytes: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")"))))/sum(denom_lab_08_2==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Nitrites: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")"))))/sum(denom_lab_08_2==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_8 <- tb_theme2(matrix3_8) %>%
      # table title
      tab_header(
        title = paste0("Table 20. Urinalysis missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part VIII. Urinalysis Test Results") %>%
      tab_row_group(
...
# Missingness at ANC-28

## Table 21. Hemoglobin missingness at ANC28
```{r}
num = 3
visit = "ANC28"
    ...
    matrix2_2 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          plyr::summarise(
            "Expected POC HB from MNH06: N" = sum(denom_lab_06_3 ==1, na.rm = TRUE),
            #all visits except IPC
            "Missing POC HB: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M06_HB_POC_LBORRES_", num, ") & denom_lab_06_", num, " == 1"))), na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M06_HB_POC_LBORRES_", num, ") & denom_lab_06_", num, " == 1"))), na.rm = TRUE)/sum(denom_lab_06_3 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected CBC HB from MNH08: N" = sum(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & (SITE != "Kenya" | (SITE == "Kenya" & denom_remapp == 1)), na.rm = TRUE),
            "Missing CBC HB: n (%)**" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_HB_LBORRES_", num, ") & denom_lab_08_", num, " == 1"))) & (SITE != "Kenya" | (SITE == "Kenya" & denom_remapp == 1)), na.rm = TRUE),
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 21. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 22. Blood glucose (point of care and OGTT) missingness at ANC28
```{r}
  ...
  tb2_3 <- tb_theme2(matrix2_3) %>%
    # table title
    tab_header(
      title = paste0("Table 22. Blood glucose (point of care and OGTT) missingness at ", visit)
    ) %>%
    # stubhead label
    ...
    tab_footnote(
      footnote = c("Ghana note: Poc glucose test not done between 2023-10-20 and 2024-04-08. Denominator removed this period.")
    ) %>%
    gtsave(paste0("results/missing_m06_", visit, "_3.png"), expand = 10)

  knitr::include_graphics(paste0("results/missing_m06_", visit, "_3.png"))
```

## Table 23. CBC missingness at ANC28
```{r}

df_lab_cbc <- df_lab %>% 
            mutate(denom = case_when(denom_lab_08_3==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                          (SITE == "Kenya" & denom_remapp_cbc_3 == 1) | 
                                                          (SITE == "Pakistan" & denom_remapp_cbc_3==1) |
                                                          (SITE == "Ghana" & denom_remapp_cbc_3==1) |
                                                          # (SITE == "India-SAS" & !!sym(paste0("M08_LBSTDAT_", num)) >= as.Date("2024-07-22"))) ~ 1,
                                                          (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")) ~ 1,
                                     TRUE ~ 0)) 
...
matrix3_2 <- df_lab_cbc %>%
      ...
      add_column(
        ...
        "Total" = df_lab_cbc %>%
          ...
          plyr::summarise(
            ...
            "Missing Monocyte (full cell count): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_MONO_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Eosinophils (%): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_PCT_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
              " (",
        ...
        "Missing Eosinophils (full cell count): n (%)*" = paste0(
            sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE), #  & SITE != "Pakistan"
              " (",
          # sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE),
          # " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_EOS_FCC_LBORRES_", num, ")"))), na.rm = TRUE)/sum(denom==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    ...
    tb3_2 <- tb_theme2(matrix3_2) %>%
      # table title
      tab_header(
        title = paste0("Table 23. CBC missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. CBC Results") %>%
      tab_row_group(
      ...
      tab_footnote(
        footnote = c("Pakistan note: Plateletcrit, platelet distribution width are not performed. Remaining CBC testing stopped from 2024-04-22")
      ) %>%
...
## Table 24. Urinalysis missingness at ANC28
```{r}
    ...
    matrix3_8 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_3==1, na.rm=TRUE),
            "Missing Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")"))))/sum(denom_lab_08_3==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Leukocytes: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")"))))/sum(denom_lab_08_3==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Nitrites: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")"))))/sum(denom_lab_08_3==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_8 <- tb_theme2(matrix3_8) %>%
      # table title
      tab_header(
        title = paste0("Table 24. Urinalysis missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part VIII. Urinalysis Test Results") %>%
      tab_row_group(
...
# Missingness at ANC-32

## Table 25. Hemoglobin missingness at ANC32
```{r}
num = 4 
visit = "ANC32"
  ...
  # if (num == 4) {
    matrix2_2 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          plyr::summarise(
            "Expected POC HB from MNH06: N" = sum(denom_lab_06_4==1, na.rm = TRUE),
            #all visits except IPC
            "Missing POC HB: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M06_HB_POC_LBORRES_", num, ") & denom_lab_06_", num, " == 1"))), na.rm = TRUE),
              " (",
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 25. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 26. Syphilis point of care and titers missingness at ANC32
```{r}
  # if (num == 4) {

    matrix2_4 <- df_lab %>%
    ## Pakistan: recode syphilis point of care to accomodate tests that happened at ANC36 
      mutate(M06_SYPH_POC_LBORRES_4 = case_when(SITE == "Pakistan" & is.na(M06_SYPH_POC_LBORRES_4) & !is.na(M06_SYPH_POC_LBORRES_5) ~ M06_SYPH_POC_LBORRES_5, 
                                                TRUE ~ M06_SYPH_POC_LBORRES_4)) %>% 
      group_by(SITE) %>%
      summarise(
        "Expected Lab Results from MNH06: N" = sum(M06_SYPH_POC_LBORRES_1 == 0 & denom_syph_poc_anc32 == 1, na.rm = TRUE),

        "Missing POC Syphilis: n (%)*" = paste0(
          sum(((is.na(M06_SYPH_POC_LBORRES_4) & SITE != "Zambia" & denom_syph_poc_anc32 == 1) |
             (is.na(M06_SYPH_POC_LBORRES_3) & SITE == "Zambia" & denom_syph_poc_anc32== 1)) &  M06_SYPH_POC_LBORRES_1 == 0, na.rm = TRUE),
      " (", format(round(sum(((is.na(M06_SYPH_POC_LBORRES_4) & SITE != "Zambia" & denom_syph_poc_anc32 == 1) |
          (is.na(M06_SYPH_POC_LBORRES_3) & SITE == "Zambia" & denom_syph_poc_anc32== 1)) & M06_SYPH_POC_LBORRES_1 == 0, na.rm = TRUE)/ sum(M06_SYPH_POC_LBORRES_1 == 0 & ((denom_syph_poc_anc32 == 1 & SITE != "Zambia") | (denom_syph_poc_anc32 == 1 & SITE == "Zambia")), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2), ")"),

      #  "Missing POC Syphilis: n (%)*" = paste0(
      #     sum(((is.na(M06_SYPH_POC_LBORRES_4) & SITE != "Zambia" & denom_lab_06_4 == 1) | 
      #        (is.na(M06_SYPH_POC_LBORRES_3) & SITE == "Zambia" & denom_lab_06_3== 1)) &  M06_SYPH_POC_LBORRES_1 == 0, na.rm = TRUE),
      # " (", format(round(sum(((is.na(M06_SYPH_POC_LBORRES_4) & SITE != "Zambia" & denom_lab_06_4 == 1) | 
      #     (is.na(M06_SYPH_POC_LBORRES_3) & SITE == "Zambia" & denom_lab_06_3== 1)) & M06_SYPH_POC_LBORRES_1 == 0, na.rm = TRUE)/ sum(M06_SYPH_POC_LBORRES_1 == 0 & ((denom_lab_06_4 == 1 & SITE != "Zambia") | (denom_lab_06_3 == 1 & SITE == "Zambia")), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2), ")"),
    ...
    tb2_4 <- tb_theme2(matrix2_4) %>%
      # table title
      tab_header(
        title = paste0("Table 26. Syphilis point of care and titers missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part IV. Syphilis Test Results") %>%
      tab_footnote(
        footnote = c("*Denominator: Participants with negative treponemal RDT at enrollment and complete MNH06 in person at ANC32 visit")
      ) %>%
      tab_footnote(
        footnote = c("**Denominator: Participants with a positive treponemal RDT at enrollment or at ANC32 visit, for whom the site has initiated a Syphilis titer test for ANC32 visit, and who have completed MNH08 either in person.")
      ) %>%
      tab_footnote(
        footnote = c("***Denominator: Participants with positive non-treponemal antibody titer. This denominator will likely differ from the denominator used for “Expected Syphilis Titers Lab Test” as participants may have a positive RDT and then a negative non-treponemal antibody titer lab test")
      ) %>%
      tab_footnote(
        footnote = c("Note: Syphilis titer test start date for ANC32 are: 
                     Ghana: 2024-04-09; CMC: 2023-07-18; SAS: 2024-07-29; Kenya: 2024-03-06; Pakistan: 2024-07-19; Zambia: 2023-11-09")
      ) %>%
     tab_footnote(
        footnote = c("Note: Syphilis POC test start date for ANC32 are: 
                     Ghana: 2024-04-09; SAS: 2023-12-12; Kenya: 2024-03-06; Pakistan: 2024-04-24; Zambia: 2023-11-09")
      ) %>%
      tab_footnote(
        footnote = c("Note: Zambia is performing syphilis poc at ANC28 instead of ANC32. Pakistan performed syphilis poc at ANC32 or ANC36.")
      ) %>%
      gtsave(paste0("results/missing_m06_", visit, "_4.png"), expand = 10)

    knitr::include_graphics(paste0("results/missing_m06_", visit, "_4.png"))
```

## Table 27. Liver and kidney function test missingness at ANC32
```{r}
  ...
  # if(num %in% c(4)) {
    matrix3_3 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          ...
          filter(denom_kft_lft_fxn_anc32==1) %>% 
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_kft_lft_fxn_anc32==1, na.rm=TRUE),
    ...
    tb3_3 <- tb_theme2(matrix3_3) %>%
      # table title
      tab_header(
        title = paste0("Table 27. Liver and kidney function test missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part III. Liver and Kidney Function Test Results") %>%
      tab_row_group(
      ...
      # ) %>%
      tab_footnote(
        footnote = c("Note: Kidney and Liver function testing stopped on 2024-10-01 (Ghana); 2024-10-16 (CMC); 2024-08-31 (SAS); 2025-01-15 (Kenya); 2024-10-03 (Pakistan); 2024-10-11 (Zambia).")
      ) %>%
      tab_footnote(
...
## Table 28. Micronutrients missingness at ANC32
```{r}
  # if(num %in% c(1,4)) {
    matrix3_4 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      ...
      summarise(
        ...
        #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Folate - Blood Serum: n (%)*" = paste0(
        sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_4 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_4 ==1)/sum(denom_folate_serum_4 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        ## NEW FOLATE ROW FOR FOLATE RBC
        #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Folate - RBC: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_4 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_4 ==1)/sum(denom_folate_rbc_4 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
                #Quansys 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Ferritin: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")")))),
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_4==1, na.rm=TRUE),
            #Vit B12 (TC): 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Vitamin B12 (Total Cobalamin): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Vit B12 (HoloTC): 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Vitamin B12 (Holotranscobalamin II): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
            "Missing Folate - Blood Serum: n (%)*" = paste0(
            sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_4 ==1),
          ...
          #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Ferritin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Iodine (Thyroglobulin or Tg): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Transferrin Receptor (sTfR): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Retinol Protein Binding 4 (RBP4): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing C-reactive Protein (CRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Alpha 1-acid Glycoprotein (AGP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Histidine-rich Protein 2 (HRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_4 <- tb_theme2(matrix3_4) %>%
      # table title
      tab_header(
        title = paste0("Table 28. Micronutrients missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part IV. Micronutrient Test Results") %>%
      tab_row_group(
...
## Table 29. Thyroid missingness at ANC32
```{r}
  # if (num %in% c(4)) {
    matrix3_5 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected TSH Results from MNH08: N" = sum(denom_lab_08_4==1, na.rm=TRUE),
            "Missing TSH: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_THYROID_TSH_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Free T4, Free T3 Results from MNH08: N" = sum(denom_t3t4_anc32 == 1, na.rm = TRUE),
            "Missing Free T4: n (%)*" = paste0(
              sum(is.na(M08_THYROID_FREET4_LBORRES_4) & (denom_t3t4_anc32 == 1), na.rm = TRUE),
    ...
    tb3_5 <- tb_theme2(matrix3_5) %>%
      # table title
      tab_header(
        title = paste0("Table 29. Thyroid missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part V. Thyroid Panel Results") %>%
      tab_row_group(
      ...
      tab_footnote(
        footnote = c("**Denominator: Participants who completed MNH08 either in person initially, but later only participants with abnormal TSH levels (< 0.3 mIU/L or > 4.0 mIU/L) at ANC32 after a specific date: 2024-09-01 (Ghana); 2024-10-14 (CMC); 2024-01-14 (Kenya); 2024-10-03 (Pakistan); 2024-10-11 (Zambia); ongoing (SAS).")
      ) %>%
      gtsave(paste0("results/missing_m08_", visit, "_5.png"), expand = 10)
...
## Table 30. Urinalysis missingness at ANC32
```{r}
    matrix3_8 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      group_by(SITE) %>%
        ...
        "Total" = df_lab %>%
          ...
          plyr::summarise(
            ...
            "Missing Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Leukocytes: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Nitrites: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")"))))/sum(denom_lab_08_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_8 <- tb_theme2(matrix3_8) %>%
      # table title
      tab_header(
        title = paste0("Table 30. Urinalysis missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part VIII. Urinalysis Test Results") %>%
      tab_row_group(
...
<!-- ## Table 31. Tuberculosis (TB) missingness at ANC32 -->
<!-- ```{r} -->
<!--   # if (num %in% c(1,4)) { -->
<!--     matrix3_9 <- df_lab %>% -->
...
<!--       # table title -->
<!--       tab_header( -->
<!--         title = paste0("Table 31. Tuberculosis (TB) missingness at ", visit) -->
<!--       ) %>% -->
<!--       # stubhead label -->
<!--       tab_stubhead(label = "Part IX. Tuberculosis (TB) Test Results") %>% -->
<!--       tab_footnote( -->
<!--         footnote = c("*Denominator: Participants with positive symptom screen on W4SS at visit and with complete MNH08 in person") -->
<!--       ) %>% -->
<!--       tab_footnote( -->
...
## Table 31. Chlamydia and gonorrhea missingness at ANC32
```{r}
...
# if (num %in% c(1,4)) {
    matrix3_10 <- df_lab %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_ctng_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_ctng_4==1, na.rm=TRUE),
            #EN, ANC32
            "Missing Chlamydia: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CTNG_CT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CTNG_CT_LBORRES_", num, ")"))))/sum(denom_ctng_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Gonorrhea: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CTNG_NG_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CTNG_NG_LBORRES_", num, ")"))))/sum(denom_ctng_4==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_10 <- tb_theme2(matrix3_10) %>%
      # table title
      tab_header(
        title = paste0("Table 31. Chlamydia and gonorrhea missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part X. Chlamydia and Gonorrhea Test Results") %>%
      tab_row_group(
...
## Table 32. ZCD/Lepto/Hep E missingness at ANC32
```{r}
  ...
  # if (num %in% c(4,10)) {
    matrix3_13 <- df_lab %>%
      ...
      group_by(SITE) %>%
      summarise(
        "Expected Zika-Dengue-Chikungunya (ZCD) IgM/IgG Test Results from MNH08: N" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 1000),
        #EN, ANC32, PNC6
        "Missing ZIKV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
              ...
              min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Missing ZIKV IgG Test Results : n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
        ...
        "Missing DENV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
        ...
        "Missing DENV IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
        ...
        "Missing CHKV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
        ...
        "Missing CHKV IgG Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
          ),
          " (",
          format(round(
            min(
              ...
              min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Expected Leptospirosis IgM Test Results from MNH08: N*" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 1000),
        "Missing Leptospirosis IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
            sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
          " (",
          format(round(
            min(max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
            )/min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 1000)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Expected Hepatitis E IgM/IgG Test Results from MNH08: N*" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000),
        "Missing HEV IgM Test Results: n (%)*" = paste0(
          min(
            max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1")))), na.rm = TRUE), 
            sum(eval(parse(text = paste0("is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1")))), na.rm = TRUE),
          " (",
          format(round(
            min(
              max(0, 1000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1")))), na.rm = TRUE), 
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Zika-Dengue-Chikungunya (ZCD) IgM/IgG Test Results from MNH08: N" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000),
            #EN, ANC32, PNC6
            "Missing ZIKV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
                min(
                  ...
                  min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing ZIKV IgG Test Results : n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_ZIKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(
            ...
            "Missing DENV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
            ...
            "Missing DENV IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_DENIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
            ...
            "Missing CHKV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(
            ...
            "Missing CHKV IgG Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_ZCD_CHKIGG_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
              ),
              " (",
              format(round(
                min(
                  ...
                  min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Leptospirosis IgM Test Results from MNH08: N*" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000),
            "Missing Leptospirosis IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)),
              " (",
              format(round(
                min(max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)), 
                    sum(eval(parse(text = paste0("is.na(M08_LEPT_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1"))), na.rm = TRUE)
                )/min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Expected Hepatitis E IgM/IgG Test Results from MNH08: N*" = min(sum(eval(parse(text = paste0("denom_zcd_lept_hev_", num, " == 1"))), na.rm = TRUE), 6000),
            "Missing HEV IgM Test Results: n (%)*" = paste0(
              min(
                max(0, 6000-sum(eval(parse(text = paste0("!is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1")))), na.rm = TRUE), 
                sum(eval(parse(text = paste0("is.na(M08_HEV_IGM_LBORRES_", num, ") & denom_zcd_lept_hev_" , num,  "== 1")))), na.rm = TRUE),
              " (",
              format(round(
                min(
    ...
    tb3_13 <- tb_theme2(matrix3_13) %>%
      # table title
      tab_header(
        title = paste0("Table 32. ZCD/Lepto/Hep E missingness at ", visit)
      ) %>%
      # stubhead label
 ...
 knitr::include_graphics(paste0("results/missing_m08_", visit, "_13.png"))

```

# Missingness at ANC-36

## Table 33. Hemoglobin missingness at ANC36
```{r}
num = 5
visit = "ANC36"
  ...
  # if (num %in% c(2,3,5)) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 33. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 34. CBC missingness at ANC36
```{r}
  ...
  # if(num %in% c(5)) {
    matrix3_2 <- df_lab %>%
      ...
      # filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & SITE != "Pakistan") %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                                            (SITE == "Kenya" & denom_remapp_cbc_5 == 1) | 
                                                                            (SITE == "Pakistan" & denom_remapp_cbc_5==1) |
                                                                            (SITE == "Ghana" & denom_remapp_cbc_5==1)|
                                                                            (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
      )) %>%
      group_by(SITE) %>%
      summarise(
      ...
      add_column(
        ...
        "Total" = df_lab %>%
         ...
         mutate(denom = ifelse(denom_lab_08_5==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                                                (SITE == "Kenya" & denom_remapp_cbc_5 == 1) | 
                                                                                (SITE == "Pakistan" & denom_remapp_cbc_5==1) |
                                                                                (SITE == "Ghana" & denom_remapp_cbc_5==1)|
                                                                                (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
    ...
    tb3_2 <- tb_theme2(matrix3_2) %>%
      # table title
      tab_header(
        title = paste0("Table 34. CBC missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. CBC Results") %>%
      tab_row_group(
      ...
      tab_footnote(
        footnote = c("Pakistan note: Neutrophils full cell count, lymphocytes full cell count, monocyte full cell count, plateletcrit, platelet distribution width are not performed. Remaing CBC testing stopped from 2024-04-22.")
      ) %>%
      tab_footnote(
        footnote = c("SAS Note: Plateletcrit, platelet distribution width are not performed at India-SAS. Post-enrollment CBC testing started on 2024-07-22.")
      ) %>%
...
## Table 35. Urinalysis missingness at ANC36
```{r}
    matrix3_8 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
      group_by(SITE) %>%
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_5==1, na.rm=TRUE),
            "Missing Protein: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_PROT_LBORRES_", num, ")"))))/sum(denom_lab_08_5==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Leukocytes: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_LEUK_LBORRES_", num, ")"))))/sum(denom_lab_08_5==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            "Missing Nitrites: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_UA_NITRITE_LBORRES_", num, ")"))))/sum(denom_lab_08_5==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_8 <- tb_theme2(matrix3_8) %>%
      # table title
      tab_header(
        title = paste0("Table 35. Urinalysis missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part VIII. Urinalysis Test Results") %>%
      tab_row_group(
...
## Table 36. Tuberculosis (TB) missingness at ANC36
```{r}
...
matrix3_9 <- df_lab %>%
      ...
      summarise(
        ...
        "Completed W4SS Screening Questions: N" = sum(W4SS_SCREEN_PERF_ANC36_COMBINED ==1, na.rm= TRUE),
        "Reported a GeneXpert Test Result: N" = sum(TB_CULTURE_PERF_ANC36_COMBINED ==1, na.rm= TRUE),

        ## POSITIVE W4SS SCREENER
         "Tested Positive on W4SS Screening: N" = sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1, na.rm= TRUE),
          
        "GeneXpert Test Result Reported: n(%)*" = paste0(
          sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1 & TB_CULTURE_PERF_ANC36_COMBINED ==1, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1 & TB_CULTURE_PERF_ANC36_COMBINED ==1, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),

        "Missing GeneXpert Test Result: n(%)*" = paste0(
          sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1 & TB_CULTURE_PERF_ANC36_COMBINED ==0, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1 & TB_CULTURE_PERF_ANC36_COMBINED ==0, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==1, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        ## NEGATIVE W4SS SCREENER
         "Tested Negative on W4SS Screening: N" = sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0, na.rm= TRUE),
          
        "GeneXpert Test Result Reported: n(%)**" = paste0(
          sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0 & TB_CULTURE_PERF_ANC36_COMBINED ==1, na.rm = TRUE),
          " (",
          format(round(sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0 & TB_CULTURE_PERF_ANC36_COMBINED ==1, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")

        # "Missing GeneXpert Test Result: n(%)**" = paste0(
        #   sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0 & TB_CULTURE_PERF_ANC36_COMBINED ==0, na.rm = TRUE),
        #   " (",
        #   format(round(sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0 & TB_CULTURE_PERF_ANC36_COMBINED ==0, na.rm = TRUE)/sum(W4SS_SCREEN_RESULT_ANC36_COMBINED ==0, na.rm= TRUE)*100, 2), nsmall = 0, digits = 2),
        #   ")")
  ...
  tb3_9 <- tb_theme2(matrix3_9) %>%
      # table title
      tab_header(
        title = paste0("Table 36 Tuberculosis (TB) missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part IX. Tuberculosis (TB) Test Results") %>%
    tab_row_group(
    ...
    tab_row_group(
      ...
      tab_footnote(
        footnote = c("*Denominator: Participants testing positive on symptom screen on W4SS at ANC32 OR ANC36")
      ) %>%
      tab_footnote(
        footnote = c("*Denominator: Participants testing negative on symptom screen on W4SS at ANC32 OR ANC36")
      ) %>% 
    gtsave(paste0("results/missing_m08_", visit, "_9.png"), expand = 10)
...
* No labs required at delivery

# Missingness at PNC-0 visit

## Table 37. Hemoglobin missingness at PNC0
```{r}
num = 7
visit = "PNC0"
  ...
  # if (num == 7) {
    matrix2_2 <- df_lab %>%
      ...
      summarise(
        ...
        "Missing CBC HB: n (%)**" = paste0(
          ...
          format(round(sum(eval(parse(text = paste0("is.na(M08_CBC_HB_LBORRES_", num, ") & denom_lab_08_", num, " == 1"))), na.rm = TRUE)/sum(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))), na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        "Expected HB from MNH06 or MNH08: N" = sum(eval(parse(text = paste0("denom_lab_06_", num, " == 1 | denom_lab_08_", num, "== 1"))), na.rm = TRUE),
        #all visits except IPC
        "Missing HB: n (%)***" = paste0(
          sum(eval(parse(text = paste0("is.na(M06_HB_POC_LBORRES_", num, ") & is.na(M08_CBC_HB_LBORRES_", num, ") & (denom_lab_06_", num, " == 1 | denom_lab_08_", num, " == 1)"))), na.rm = TRUE),
          " (",
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 37. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
      ...
      tab_row_group(
        label = "Missing HB Test Results in MNH06 or MNH08",
        rows = 5:6
      ) %>%
      row_group_order(groups = c("Missing POC HB Test Results in MNH06",
                                 "Missing CBC HB Test Results in MNH08"
      ...
      tab_footnote(
        footnote = c("***Denominator: Participants with both complete MNH06 OR MNH08 in person")
      ) %>% 
      tab_footnote(
...
## Table 38. Hemoglobin missingness at PNC1
```{r}
num = 8
visit = "PNC1"
  ...
  # if (num %in% c(8,9,12)) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 38. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 39. Hemoglobin missingness at PNC4
```{r}
num = 9
visit = "PNC4"
  ...
  # if (num %in% c(8,9,12)) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 39. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 40. Hemoglobin missingness at PNC6
```{r}
...
visit = "PNC6"
  ...
  # if (num == 10) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 40. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 41. CBC missingness at PNC6
```{r}

df_lab_cbc <- df_lab %>% 
            mutate(denom = case_when(denom_lab_08_10==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                          (SITE == "Kenya" & denom_remapp_cbc_10 == 1) | 
                                                          (SITE == "Pakistan" & denom_remapp_cbc_10==1) |
                                                          (SITE == "Ghana" & denom_remapp_cbc_10==1)|
                                                          (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
          ) ~ 1, TRUE ~ 0)) 
...
matrix3_2 <- df_lab_cbc %>%
      ...
      # filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & SITE != "Pakistan") %>%
      filter(denom_lab_08_10==1 & (!SITE %in% c("Pakistan", "Kenya", "Ghana", "India-SAS") | 
                                                                            (SITE == "Kenya" & denom_remapp_cbc_10 == 1) | 
                                                                            (SITE == "Pakistan" & denom_remapp_cbc_10==1) |
                                                                            (SITE == "Ghana" & denom_remapp_cbc_10==1)|
                                                                            (SITE == "India-SAS" & eval(parse(text = paste0("M08_LBSTDAT_", num))) >= "2024-07-22")
...
#neutrophils full cell count, lymphocytes full cell count, Monocyte (full cell count), eosinophils full cell count, plateletcrit, platelet distribution width are not performed at Pakistan
    ...
    tb3_2 <- tb_theme2(matrix3_2) %>%
      # table title
      tab_header(
        title = paste0("Table 41. CBC missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. CBC Results") %>%
      tab_row_group(
      ...
      tab_footnote(
        footnote = c("Pakistan note: Neutrophils full cell count, lymphocytes full cell count, monocyte full cell count, plateletcrit, platelet distribution width are not performed. Remaining CBC testing stopped from 2024-04-22.")
      ) %>%
...
## Table 42. Micronutrients missingness at PNC6
```{r}
  ...
  # if(num %in% c(10)) {
    matrix3_4 <- df_lab %>%
      ...
      summarise(
        ...
        #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Folate - Blood Serum: n (%)*" = paste0(
        sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_10 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_10 ==1)/sum(denom_folate_serum_10 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
        
        ## NEW FOLATE ROW FOR FOLATE RBC
        #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
        "Missing Folate - RBC: n (%)*" = paste0(
          sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_10 ==1),
          " (",
          format(round(sum(eval(parse(text = paste0("is.na(M08_FOLATE_RBC_NMOLL_LBORRES_", num, ")"))) & denom_folate_rbc_10 ==1)/sum(denom_folate_rbc_10 ==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
      ...
      add_column(
        ...
        "Total" = df_lab %>%
          filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1")))) %>%
          plyr::summarise(
            "Expected Lab Results from MNH08: N" = sum(denom_lab_08_10==1, na.rm=TRUE),
            #Vit B12 (TC): 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Vitamin B12 (Total Cobalamin): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_COB_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Vit B12 (HoloTC): 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Vitamin B12 (Holotranscobalamin II): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_VITB12_HOL_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Folate (serum) 7-plex (P: En, ANC-32, PNC-6; R3: ANC-20)
            "Missing Folate - Blood Serum: n (%)*" = paste0(
            sum(eval(parse(text = paste0("is.na(M08_FOLATE_PLASMA_NMOLL_LBORRES_", num, ")"))) & denom_folate_serum_10 ==1),
            ...
            "Missing Ferritin: n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_FERRITIN_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Iodine (Thyroglobulin or Tg): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_IODINE_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Serum Transferrin Receptor (sTfR): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_TRANSFERRIN_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Retinol Protein Binding 4 (RBP4): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_RBP4_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing C-reactive Protein (CRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_CRP_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Alpha 1-acid Glycoprotein (AGP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_AGP_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")"),
            #Quansys 7-plex (P: En, ANC-32, PNC-6; R: ANC-20)
            "Missing Histidine-rich Protein 2 (HRP): n (%)*" = paste0(
              sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")")))),
              " (",
              format(round(sum(eval(parse(text = paste0("is.na(M08_HRP_LBORRES_", num, ")"))))/sum(denom_lab_08_10==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
              ")")
          ) %>%
          t() %>% unlist()
      ) %>%
    ...
    tb3_4 <- tb_theme2(matrix3_4) %>%
      # table title
      tab_header(
        title = paste0("Table 42. Micronutrients missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part IV. Micronutrient Test Results") %>%
      tab_row_group(
...
## Table 43. ZCD/Lepto/Hep E missingness at PNC6
```{r}
  ...
  # if (num %in% c(4,10)) {
    ...
    tb3_13 <- tb_theme2(matrix3_13) %>%
      # table title
      tab_header(
        title = paste0("Table 43. ZCD/Lepto/Hep E missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part XI. ZCD, and Leptospirosis IgM/IgG Test Results") %>%
      tab_row_group(
...
## Table 44. Hemoglobin missingness at PNC26
```{r}
num = 11
visit = "PNC26"
  ...
  # if (num == 11) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 44. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
...
## Table 45. Hemoglobin missingness at PNC52
```{r}
num = 12
visit = "PNC52"
  ...
  # if (num %in% c(8,9,12)) {
    ...
    tb2_2 <- tb_theme2(matrix2_2) %>%
      # table title
      tab_header(
        title = paste0("Table 45. Hemoglobin missingness at ", visit)
      ) %>%
      # stubhead label
      tab_stubhead(label = "Part II. Hemoglobin results (POC & CBC)") %>%
      tab_row_group(
