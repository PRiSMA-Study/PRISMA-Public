...
df_ogtt <- df_ogtt %>% 
  ...
  mutate(denom_poc_glucose_gh = case_when(
    ...
    if_any(poc_glucose_date_cols, ~.x >= "2024-04-05") ~ 1,
    TRUE ~ 0
  )) %>% 
  ungroup()
    ...
    tb2_3 <- tb_theme2(matrix2_3) %>%
      ...
      tab_footnote(
        footnote = c("Ghana note: Poc glucose test between 2023-10-20 and 2024-05-05. Denominator removed this period.")
      ) %>%
 ...
 if(num %in% c(2)) {
  ...
  tab_footnote(
    footnote = c("*Denominator: Participants with complete MNH08 in person or by phone")
      ...
      tab_footnote(
    footnote = c("Pakistan note: Neutrophils full cell count, lymphocytes full cell count, monocyte full cell count, eosinophils %, eosinophils full cell count, plateletcrit, platelet distribution width are not performed. Rest CBC stopped testing from April 2024. Total number excluded Pakistan.")
  ) %>%
    tab_footnote(
    footnote = c("SAS note: Plateletcrit, platelet distribution width are not performed. Total number excluded India-SAS.")
  ) %>%
  tab_options(table.width = pct(100)) %>%
  #revise column width
  cols_width(1 ~ px(225)) %>%
  ...
  if(num %in% c(3,10)) {
    matrix3_2 <- df_lab %>%
      filter(eval(parse(text = paste0("denom_lab_08_", num, " == 1"))) & (SITE != "Kenya" | (SITE == "Kenya" & denom_remapp == 1))) %>%
      group_by(SITE) %>%
      summarise(
        "Expected Lab Results from MNH08: N" = n(),
        #En, ANC20, ANC28, ANC36, PNC6 for all below
...
#Magnesium is optional. Not perfomred at India-CMC, India-SAS and Kenya. Pakistan stopped performing on 2023-11-17.
matrix3_3[19, c("India-CMC", "India-SAS", "Kenya", "Pakistan")] = "n/a"
matrix3_3[, c("India-CMC")] = "n/a"
matrix3_3 <- matrix3_3 %>% relocate(`India-CMC`, .after = Ghana)
...
tb3_3 <- tb_theme2(matrix3_3) %>%
  ...
  tab_footnote(
    footnote = c("CMC Note: Liver and kidney function tests stopped on 2024-10-14 for ANC32 visit. Total number excluded CMC")
  ) %>%
  gtsave(paste0("results/missing_m08_", visit, "_3.png"), expand = 10)
 }
...
tb3_4 <- tb_theme2(matrix3_4) %>%
  ...
  #******************table 3.5*****************
   if (num %in% c(1)) {
    ...
    tb3_5 <- tb_theme2(matrix3_5) %>%
      ...
      tab_footnote(
        footnote = c("CMC note: thyroid tests stoppend on 2024-10-14")
      ) %>%
  ...
  if (num %in% c(4)) {
    ...
    tb3_5 <- tb_theme2(matrix3_5) %>%
      ...
      tab_footnote(
        footnote = c("**Denominator: Participants who completed MNH08 either in person or by phone initially, but later only participants with abnormal TSH levels (< 0.3 mIU/L or > 4.0 mIU/L) at ANC32 after a specific date: CMC:2024-10-14. Other sites please provide the date for change.")
      ) %>%
      gtsave(paste0("results/missing_m08_", visit, "_5.png"), expand = 10)
  }
 #******************table 3.6*****************
...
tb3_7 <- tb_theme2(matrix3_7) %>%
  ...
  tab_source_note(
    source_note = c("Note: Included data with SCRN_OBSSTDAT at/after: 2022-12-28(Ghana); 2023-06-20(CMC); 2023-12-12(SAS); 2023-04-03(Kenya); 2022-12-15(Zambia); LBSTDAT between: 2022-09-22 and 2024-01-17(Pakistan). Each site please double check dates variable used and the exact dates.")
  ) %>%
  tab_style(
        style = list(
          cell_text(color = "red") 
...
#******************table 3.11*****************
 if (num %in% c(1,4,10)) {
    ...
    tb3_11 <- tb_theme2(matrix3_11) %>%
      ...
      tab_footnote(
        footnote = c("Note: Test start dates are: Ghana: 2024-04-09; CMC: 2024-03-06; SAS: 2024-03-12; Kenya: 2024-03-06; Pakistan: 2024-04-08; Zambia: 2023-11-09")
      ) %>%
...
tb3_12 <- tb_theme2(matrix3_12) %>%
  ...
  tab_source_note(
    source_note = c("Note: Included data with SCRN_OBSSTDAT at/after: 2022-12-28(Ghana); 2023-06-20(CMC); 2023-12-12(SAS); 2023-04-03(Kenya); 2022-12-15(Zambia); SCRN_OBSSTDAT between: 2022-09-22 and 2024-01-17(Pakistan). Each site please double check dates variable I used and the exact dates.")
  ) %>%
  tab_style(
        style = list(
          cell_text(color = "red") 
