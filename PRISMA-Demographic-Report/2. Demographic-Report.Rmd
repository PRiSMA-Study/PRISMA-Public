---
title: "<span style='font-size: 18px'> <span style='text-align: center'>Demographics Report (Issued: 2025-04-24)"
# date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: no
...
matrix_demo_b <- df_demo_b %>%
  ...
  summarise(
    ...
    "Age: median (IQR)" =
      paste0(median(MAT_AGE, na.rm = TRUE),
             " (", IQR(MAT_AGE, na.rm = TRUE),")"),
    "Age: < 20 n (%*)" =
      paste0(sum(MAT_AGE > 0 & MAT_AGE < 20, na.rm = TRUE),
             " (", format(sum(MAT_AGE > 0 & MAT_AGE < 20, na.rm = TRUE)/sum(!is.na(MAT_AGE))*100, digits = 2, nsmall = 2), ")"),
    "Age: 20-34 n (%*)" =
      paste0(sum(MAT_AGE >= 20 & MAT_AGE <= 34, na.rm = TRUE),
             " (", format(sum(MAT_AGE >= 20 & MAT_AGE <= 34, na.rm = TRUE)/sum(!is.na(MAT_AGE))*100, digits = 2, nsmall = 2), ")"),
    "Age: >= 35 n (%*)" =
      paste0(sum(MAT_AGE >= 35, na.rm = TRUE),
             " (", format(sum(MAT_AGE >= 35, na.rm = TRUE)/sum(!is.na(MAT_AGE))*100, digits = 2, nsmall = 2), ")"),
    "Age: N* (%)" =
      paste0(sum(!is.na(MAT_AGE)),
             " (", format(sum(!is.na(MAT_AGE))/n()*100, digits = 1, nsmall = 1),")"),
    "Height: <145 n (%*)" =
    ...
    "BMI: <18.5 n (%*)" =
      paste0(sum(BMI_LEVEL_ENROLL == 1, na.rm = TRUE),
             " (", format(sum(BMI_LEVEL_ENROLL == 1, na.rm = TRUE)/sum(!is.na(BMI_LEVEL_ENROLL))*100, digits = 2, nsmall = 2), ")"),
    "BMI: 18.5 to <25 n (%*)" =
      paste0(sum(BMI_LEVEL_ENROLL == 2, na.rm = TRUE),
             " (", format(sum(BMI_LEVEL_ENROLL == 2, na.rm = TRUE)/sum(!is.na(BMI_LEVEL_ENROLL))*100, digits = 2, nsmall = 2), ")"),
    "BMI: 25 to <30 n (%*)" =
      paste0(sum(BMI_LEVEL_ENROLL == 3, na.rm = TRUE),
             " (", format(sum(BMI_LEVEL_ENROLL == 3, na.rm = TRUE)/sum(!is.na(BMI_LEVEL_ENROLL))*100, digits = 2, nsmall = 2), ")"),
    "BMI: 30+ n (%*)" =
      paste0(sum(BMI_LEVEL_ENROLL == 4, na.rm = TRUE),
             " (", format(sum(BMI_LEVEL_ENROLL == 4, na.rm = TRUE)/sum(!is.na(BMI_LEVEL_ENROLL))*100, digits = 2, nsmall = 2), ")"),
    "BMI: N* (%)" =
      paste0(sum(!is.na(BMI_LEVEL_ENROLL)),
             " (", format(sum(!is.na(BMI_LEVEL_ENROLL))/n()*100, digits = 2, nsmall = 2), ")"),
    ...
    "Attended primary, secondary, or higher, n (%*)" =
  ...
  add_column(
    ...
    "Age N* (%)" =
      paste0(sum(!is.na(MAT_AGE)),
             " (", format(sum(!is.na(MAT_AGE))/n()*100, digits = 2, nsmall = 2),")"),
    "Height <145 n (%*)" =
      paste0(sum(HEIGHT_GROUP == 1, na.rm = TRUE),
             " (", format(sum(HEIGHT_GROUP == 1, na.rm = TRUE)/sum(!is.na(HEIGHT_GROUP))*100, digits = 2, nsmall = 2), ")"),
    "Height 145 to <150 n (%*)" =
  ...
  summarise(
    ...
    "Receives folic supplements: n (%*)" =
      paste0(sum(FOLIC_SUPPL_ENROLL == 1, na.rm = TRUE),
             " (", format(sum(FOLIC_SUPPL_ENROLL == 1, na.rm = TRUE)/sum(!is.na(FOLIC_SUPPL_ENROLL))*100, digits = 2, nsmall = 2), ")"),
    "Receives folic supplements: N* (%)" =
      paste0(sum(!is.na(FOLIC_SUPPL_ENROLL)),
             " (", format(sum(!is.na(FOLIC_SUPPL_ENROLL))/n()*100, digits = 2, nsmall = 2), ")"),
    "Sleeps under net: n (%*)" =
      paste0(sum(SLEEP_UNDER_NET == 1, na.rm = TRUE),
             " (", format(sum(SLEEP_UNDER_NET == 1, na.rm = TRUE)/sum(!is.na(SLEEP_UNDER_NET))*100, digits = 2, nsmall = 2), ")"),
    "Sleeps under net: N* (%)" =
      paste0(sum(!is.na(SLEEP_UNDER_NET)),
             " (", format(sum(!is.na(SLEEP_UNDER_NET))/n()*100, digits = 2, nsmall = 2), ")"),
    "Facility birth: n (%*)" =
  ...
  summarise(
    ...
    "MUAC_ENROLL: median (IQR)" =
          paste0(round(median(MUAC_ENROLL, na.rm = TRUE)),
                 " (", format(IQR(MUAC_ENROLL, na.rm=TRUE), digits=1, nsmall=1),")"),
    "MUAC_ENROLL: N* (%)" = 
          paste0(sum(!is.na(MUAC_ENROLL)),
                 " (", format(sum(!is.na(MUAC_ENROLL))/n()*100, digits = 2, nsmall = 2),")"),
  ) %>%
  t() %>% as.data.frame() %>%
  `colnames<-`(c(.[1,])) %>%
  slice(-1) %>%
