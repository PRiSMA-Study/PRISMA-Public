...
## MNH04 ## 
# mnh04_enroll <- mnh04 %>% filter(M04_TYPE_VISIT==1 & PREGID %in% enrolled_ids_vec) 
mnh04_constructed_completeness <- mnh04 %>% 
  filter(M04_TYPE_VISIT==1 & PREGID %in% enrolled_ids_vec)  %>% 
  # MNH04 form missing 
...
mnh06_all_visits <- mnh06 %>% filter(PREGID %in% enrolled_ids_vec) 

## MNH08 ## 
mnh08_constructed_completeness <- mnh08 %>% 
  filter(M08_TYPE_VISIT==1 & PREGID %in% enrolled_ids_vec) %>% 
  # MNH08 form missing 
  mutate(M08_MISSING = ifelse(is.na(M08_MAT_VISIT_MNH08), 1, 0)) %>% 
  # MNH08 denominator if form is completed 
  mutate(M08_FORM_COMPLETE = ifelse(!is.na(M08_MAT_VISIT_MNH08), 1, 0)) 

mnh08_all_visits <- mnh08 %>% filter(PREGID %in% enrolled_ids_vec) 
...
mat_infection_diagnosed <- mnh04_constructed_completeness %>% 
  ...
  mutate(SYPH_DIAG_MISSING = case_when(SYPH_DIAG_RESULT == 0 & M04_FORM_COMPLETE==1~ 1, TRUE ~ 0),
...
mat_infection_measured<- mnh06_constructed_completeness %>%
  ...
  mutate(SYPH_MEAS_RESULT = case_when(M06_SYPH_POC_LBORRES %in% c(1,0) ~ 1, TRUE ~ 0),
         HIV_MEAS_RESULT = case_when(M06_HIV_POC_LBORRES %in% c(1,0) ~ 1, TRUE ~ 0)
  ) %>% 
  ## Is the test result missing among those with a completed form? (form completed == yes AND test result = yes or no)
  mutate(SYPH_MEAS_MISSING = case_when(SYPH_MEAS_RESULT == 0 & M06_FORM_COMPLETE==1 ~ 1, TRUE ~ 0),
...
mat_other_infection_mnh04  <- mnh04_constructed_completeness %>% 
  ...
  mutate(MAL_DIAG_RESULT = ifelse(M04_MALARIA_EVER_MHOCCUR %in% c(1,0), 1, 0),
         TB_DIAG_RESULT = ifelse(M04_TB_MHOCCUR %in% c(1,0), 1, 0),
         COVID_DIAG_RESULT = ifelse(M04_COVID_LBORRES %in% c(1,0), 1, 0)
  ) %>% 
  ## Is the test result missing among those with a completed form?  
  mutate(MAL_DIAG_MISSING = ifelse(MAL_DIAG_RESULT == 0 & M04_FORM_COMPLETE==1,1,0),
         TB_DIAG_MISSING = ifelse(TB_DIAG_RESULT == 0 & M04_FORM_COMPLETE==1,1,0),
         COVID_DIAG_MISSING = ifelse(COVID_DIAG_RESULT == 0 & M04_FORM_COMPLETE==1,1,0)) %>% 
  ## TB variables
...
mat_other_infection_mnh06 <- mnh06_constructed_completeness %>% 
  ...
  mutate(MAL_MEAS_RESULT = ifelse(M06_MALARIA_POC_LBORRES %in% c(1,0), 1, 0),
         HBV_MEAS_RESULT = ifelse(M06_HBV_POC_LBORRES %in% c(1,0), 1, 0),
         HCV_MEAS_RESULT = ifelse(M06_HCV_POC_LBORRES %in% c(1,0), 1, 0),
         COVID_MEAS_RESULT = ifelse(M06_COVID_POC_LBORRES %in% c(1,0), 1, 0)
         
  ) %>% 
  ## Is the test result missing among those with a completed form? 
  mutate(MAL_MEAS_MISSING = ifelse(MAL_MEAS_RESULT == 0 & M06_FORM_COMPLETE==1,1,0),
         HBV_MEAS_MISSING = ifelse(HBV_MEAS_RESULT == 0 & M06_FORM_COMPLETE==1,1,0),
         HCV_MEAS_MISSING = ifelse(HCV_MEAS_RESULT == 0 & M06_FORM_COMPLETE==1,1,0),
         COVID_MEAS_MISSING = ifelse(COVID_MEAS_RESULT == 0 & M06_FORM_COMPLETE==1,1,0)
  ) %>% 
  ## generate summary any infection variables
...
mat_other_infection_mnh08 <- mnh08_constructed_completeness %>% 
  ...
  # Is there av valid result reported?
  # Malaria, HBV, HCV, COVID
  mutate(TB_LAB_RESULT = ifelse(M08_TB_CNFRM_LBORRES %in% c(1,2,0), 1, 0)
  ) %>% 
  ## Is the test result missing among those who had at least with one symptom (in MNH04)? 
  mutate(TB_LAB_MISSING = ifelse(TB_LAB_RESULT == 0 & W4SS_SYMPTOMS_ANY==1,1,0)
  )  %>% 
  ## generate summary any infection variables
...
mat_infections_combined <- full_join(mat_infection_sti, mat_infection_other, by = c("SITE", "MOMID", "PREGID", "M04_TYPE_VISIT",
  ...
  mutate(INFECTION_ANY_METHOD_DENOM = ifelse(M04_TYPE_VISIT==1 | M06_TYPE_VISIT==1, 1, 0),
         INFECTION_ENROLL_DENOM = case_when(M04_TYPE_VISIT==1 | M06_TYPE_VISIT==1 ~ 1, TRUE ~ 0)) %>% 
  select(SITE, MOMID, PREGID, INFECTION_ENROLL_DENOM, HIV_POSITIVE_ENROLL, HIV_MISSING, 
         SYPH_POSITIVE_ENROLL, SYPH_MISSING, SYPH_POSITIVE_ANY_VISIT, SYPH_MISSING,
         M04_GONORRHEA_MHOCCUR, GON_DIAG_MISSING, M04_CHLAMYDIA_MHOCCUR, CHL_DIAG_MISSING, 
         M04_GENULCER_MHOCCUR, GENU_DIAG_MISSING, M04_OTHR_STI_MHOCCUR, OTHR_DIAG_MISSING, 
         ANY_MEAS_STI, ANY_DIAG_STI, STI_ANY_METHOD,MAL_POSITIVE_ENROLL, MAL_MISSING, 
         M06_HBV_POC_LBORRES, HBV_MEAS_MISSING,M06_HCV_POC_LBORRES, HCV_MEAS_MISSING,
         W4SS_SYMPTOMS_ANY, W4SS_RESPONSE, W4SS_MISSING_SYMP, 
         M08_TB_CNFRM_LBORRES, TB_LAB_RESULT, 
