...
#* Last updated: January 29 2024, Erin Oakley, Savannah O'Malley & Stacie Loisate
#*****************************************************************************
library(knitr)
library(tidyverse)
library(reshape2)
...
**1.** Reported test results (diagnosed or measured) of 1 or 0 in MNH04 Maternal Clinical Status or MNH06 Point of Care for enrollment visit
...
sti_enroll_tab <- MAT_INFECTION %>% 
  ...
  ## VAGINAL SWAB TESTING
       ...
       format(sum(CTNG_TEST_PERF_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2)),

      ## Chlamydia prevalence at enrollment
      "Chlamydia prevalence at enrollment^e^" = paste0(
        format(sum(CT_POSITIVE_ENROLL == 1 & CTNG_TEST_PERF_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
        format(round(sum(CT_POSITIVE_ENROLL == 1 & CTNG_TEST_PERF_ENROLL == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
        ")"),
      ...
      "Gonorrhea prevalence at enrollment^e^" = paste0(
        format(sum(NG_POSITIVE_ENROLL == 1 & CTNG_TEST_PERF_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
        format(round(sum(NG_POSITIVE_ENROLL == 1 & CTNG_TEST_PERF_ENROLL == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
        ")") ,
      ...
      ## Chlamydia prevalence at enrollment
      "Chlamydia incidence following enrollment^e^" = paste0(
      ...
      ## Gonorrhea prevalence at enrollment
      "Gonorrhea incidence following enrollment^e^" = paste0(
...
## the table is too long -- split into 2 
sti_enroll_tab1 <- sti_enroll_tab[1:14,]
# sti_enroll_tab2 <- sti_enroll_tab[9:14,]
sti_enroll_tab2 <- sti_enroll_tab[15:29,]
...
sti_enroll_output1 <- tb_theme1(sti_enroll_tab1) %>% 
  ...
  row_group_order(groups = c(NA,
                            ...
                            "<span style='font-size: 18px'>CT/NG Vaginal Swabs, n (%)</span>"
                            )) %>% 
  # bold denominator rows
  tab_style(
      style = list(
  ...
  tab_style(
      ...
      locations = cells_body(rows = c(
                 "N with CT/NG test performed at enrollment",
                 "N with CT/NG test performed at any ANC (excluding enrollment)"))) %>% 
tab_style(
      style = list(
        cell_text(weight = "bold")
        ),
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> Denominator is total participants with a CT/NG swab and test performed.</span>")
 ) 
...
gdm_vd <- subset(gdm, gdm$DIAB_GEST_DENOM==1 & (DIAB_GEST_FASTING!=55 | DIAB_GEST_1HR!=55 | DIAB_GEST_1HR!=55) & gdm$SITE!="India-SAS")
