---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2024 June 18)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
#* Last updated: June 18 2024, Erin Oakley
#*****************************************************************************
library(knitr)
library(tidyverse)
library(reshape2)
...
# Erin's path: 
path_to_data <- "Z:/Outcome Data/2024-06-14/" 
...
hemorrhage <- read_csv(paste0(path_to_data, "hemorrhage.csv")) %>% rename_all(toupper)
...
near_miss <- read_dta(paste0(path_to_data, "near-miss-maternal-outcomes.dta")) %>% rename_all(toupper)%>% filter(SITE != "")
...
maternal_infection <- read_csv(paste0(path_to_data, "MAT_INFECTION.csv"))  %>% rename_all(toupper) %>% filter(SITE != "")
...
anemia_pph6_tab <- anemia %>% 
  ...
  ## pnc6
    ...
    "Expected denominator (PNC6 window completed, >12 weeks postpartum)" = paste0(
...
anemia_pph26_tab <- anemia %>% 
  ...
  summarise(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(ANEMIA_PNC26_MISS ==0 & ANEMIA_PNC26_DENOM==1, na.rm = TRUE)/sum(ANEMIA_PNC26_DENOM==1 & ANEMIA_PNC26_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
anemia_pph6_output <- tb_theme1(anemia_pph6_tab) %>% 
  tab_header(
    title = md("**Table 2c**")) %>% 
  tab_spanner(
    label = html("<span style='font-size: 18px'>6-12 weeks postpartum</span>"),
    columns = 1:7
  ) %>%
  tab_row_group(
...
tab_style(
  ...
  tab_style(
      ...
      locations = cells_body(rows = c(
                 "Expected denominator (PNC6 window completed, >12 weeks postpartum)",
                 "Expected denominator (PNC6 window completed, >12 weeks postpartum)"))) %>% 
tab_style(
      style = list(
        cell_text(weight = "bold")
...
prov_preterm_tab <- preterm_indication %>% 
  ...
  summarise(
    ...
    "Denominator (provider-initiated preterm)" = paste0(
...
prov_preterm_tab_output1 <- tb_theme1(prov_preterm_tab1) %>% 
 ...
 # bold denominator rows 2
  tab_style(
      ...
      locations = cells_body(rows = c(
                 "Denominator (provider-initiated preterm)",
                 "Denominator (provider-initiated preterm)"))) %>% 
tab_style(
      style = list(
        cell_text(weight = "bold")
...
-   MNH19 form filled with information on date of hospitalization and clinical diagnosis of PROM (varnames [form]: `OHOSTDAT`, `MAT_EST_OHOSTDAT`, `LD_COMPL_MHTERM_1`)
...
d.  date and time of membrane rupture (if occurred) (varname [form]: `MEMBRANE_RUPT_MHSTDAT`, `MEMBRANE_RUPT_MHSTTIM` [MH09])
...
pprom_tab <- preterm_indication %>% 
  ...
  summarise(
    ...
    "Total completed pregnancies" = paste0(
      format(sum(PREG_END== 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
   ...
   "Excluded: pregnancy loss at <20 weeks with no information on PPROM" = paste0(
     ...
     "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(PREG_END==1 & PPROM_OCCUR_MISS==0, na.rm = TRUE)/sum(PREG_END==1 & PPROM_OCCUR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
pprom_tab_output <- tb_theme1(pprom_tab) %>% 
  tab_header(
    title = md("**Table 4**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for data completeness</span>"),
        rows = 1:5
    ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Data completeness</span>"),
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for PPROM prevalence</span>"),
    rows = 6:8
  ) %>%
  tab_row_group(
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Denominator for data completeness</span>",
                            "<span style='font-size: 18px'>Data completeness</span>",
                            "<span style='font-size: 18px'>Denominator for PPROM prevalence</span>",
...
depression_complete_tab <- depression %>%
  ...
  summarise(
    ...
    ## ANC32
    "Expected denominator^a^ " = paste0(
      format(sum([PC_ANCOVER31_DENOM==1==1, na.rm = TRUE), nsmall = 0, digits = 2)),
...
**Definition:** Any infection identified during ANC.
...
### Table 7e. All infections combined at enrollment
```{r}
...
overt_diabetes <- gdm %>% 
  ...
  summarise(
    ...
    "Denominator (pregnancies continued to 28 weeks gestation)" = paste0(
      format(sum(COMPLETE_ANC28 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(COMPLETE_ANC28 == 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(DIAB_OVERT_MISS==0 & COMPLETE_ANC28 == 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
overt_diabetes_output <- tb_theme1(overt_diabetes) %>% 
  tab_header(
    title = md("**Table 8a**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for data completeness<sup>a</sup></span>"),
    rows = 1:3
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Data completeness</span>"),
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for overt diabetes prevalence</span>"),
    rows = 4:6
  ) %>%
  tab_row_group(
  ...
  row_group_order(groups = c("<span style='font-size: 18px'>Denominator for data completeness<sup>a</sup></span>",
                             "<span style='font-size: 18px'>Data completeness</span>",
                             "<span style='font-size: 18px'>Denominator for overt diabetes prevalence</span>",
                             "<span style='font-size: 18px'>Overt diabetes prevalence</span>"
                             ))  %>%
   tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants who had continued pregnancy to 28 weeks gestation (excluding earlier pregnancy losses and withdrawals from the study) AND who have progressed through the ANC-28 visit window.</span>")
  ) 
```
...
pretest_fast_diabetes <- gdm %>% 
  ...
  summarise(
    ...
    "Excluded: Overt diabetes (>=6.5% HbA1c)" = paste0(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(DIAB_GEST_FASTING_MISS==0 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1 & DIAB_GEST_FASTING_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
pretest_fast_diabetes_output <- tb_theme1(pretest_fast_diabetes) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for Gestational Diabetes prevalence</span>"),
    rows = 5:7
  ) %>%
  tab_row_group(
  ...
  row_group_order(groups = c("<span style='font-size: 18px'>Denominator for data completeness<sup>a</sup></span>",
                             "<span style='font-size: 18px'>Data completeness</span>",
                             "<span style='font-size: 18px'>Denominator for Gestational Diabetes prevalence</span>",
...
ogtt_diabetes <- gdm %>% 
  ...
  summarise(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(DIAB_GEST_1HR_MISS==0 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1 & DIAB_GEST_1HR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
ogtt_diabetes_output <- tb_theme1(ogtt_diabetes) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Gestational Diabetes by 1-hr OGTT</span>"),
    rows = 6:7
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Denominator for data completeness<sup>a</sup></span>",
                             "<span style='font-size: 18px'>Data completeness</span>",
                             "<span style='font-size: 18px'>Denominator for Gestational Diabetes prevalence</span>",
                             "<span style='font-size: 18px'>Gestational Diabetes by 1-hr OGTT</span>"
                             ))   %>%
   tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants who had continued pregnancy to 28 weeks gestation (excluding earlier pregnancy losses and withdrawals from the study) AND who have progressed through the ANC-28 visit window.</span>")
  )
  ...
  summarise(
   ...
   "Denominator (pregnancies continued to 28 weeks gestation)" = paste0(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(DIAB_GEST_2HR_MISS==0 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1 & DIAB_GEST_2HR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  summarise(
   ...
   "Denominator (pregnancies continued to 28 weeks gestation)" = paste0(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(DIAB_GEST_ANY_MISS==0 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1 & DIAB_GEST_ANY_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
any_test_diabetes_output <- tb_theme1(any_test_diabetes) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Gestational Diabetes by any test</span>"),
    rows = 6:7
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Denominator for data completeness<sup>a</sup></span>",
                             "<span style='font-size: 18px'>Data completeness</span>",
                             "<span style='font-size: 18px'>Denominator for Gestational Diabetes prevalence</span>",
                             "<span style='font-size: 18px'>Gestational Diabetes by any test</span>"
                             ))   %>%
   tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants who had continued pregnancy to 28 weeks gestation (excluding earlier pregnancy losses and withdrawals from the study) AND who have progressed through the ANC-28 visit window.</span>")
  )  %>%
...
GDM_pre <- gdm %>% filter(DIAB_GEST_FASTING==1) %>% 
  select(MOMID) %>% unlist()

GDM_1 <- gdm %>% filter(DIAB_GEST_1HR==1) %>% 
  select(MOMID) %>% unlist()

GDM_2 <- gdm %>% filter(DIAB_GEST_2HR==1) %>% 
  select(MOMID) %>% unlist()
...
-   A woman who is pregnant, in labor, or within 42 days of the pregnancy end, *AND*

-   Experienced the PRISMA-near-miss criteria (organ dysfunction, hysterectomy, blood transfusion of any volume, eclampsia with severe features, uterine rupture, sepsis, or laparotomy other than C-section), *AND*

-   Experiences a potentially life-threatening condition (PLTC; postpartum hemorrhage, preeclampsia, eclampsia, sepsis or severe infection, ruptured uterus, admission to ICU, blood transfusion, laparotomy other than C-section, abortion complications, severe hypertension, obstructed labor, prolonged labor, pulmonary edema, placenta accrete, placenta abruption, endometritis, admitted for in-patient care [M19_VISIT_FAORRES == 3] or referred to another institution [M19_VISIT_FAORRES == 4 or M19_ADMIT_DSTERM == 2]), *AND*

-   Survives the experience.

**Denominator:** 

-   Expected denominator: woman has passed 42 days postpartum

-   Near-miss denominator: Passed 42 days postpartum AND did not closeout during the window AND we have an MNH09/MNH12/MNH19 for that participant AND we have seen them on or after 42 days postpartum OR they have experienced PLTC 
...
near_miss_taba <- near_miss %>% 
  ...
  summarise(
    ...
    "Closed out during pregnancy or within 42 days of pregnancy end without experiencing a PLTC" = paste0(
     ...
     "Missing because no MNH09, MNH12, or MNH19 forms" = paste0(
     ...
     "Missing because not seen at least 42 days postpartum and did not experience a PLTC" = paste0(
    ...
    "Near miss denominator <sup>a</sup>" = paste0(
...
near_miss_tabb <- near_miss %>% 
  ...
  summarise(      
    ...
    "Laparotomy other than C-section" = paste0(
    ...
    "Eclampsia with severe features *" = paste0(
      format(sum(NEAR_MISS_DENOM ==0 & NEAR_MISS_DENOM ==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(NEAR_MISS_DENOM ==0 & NEAR_MISS_DENOM ==1, na.rm = TRUE)/sum(NEAR_MISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"), 
   ...
   "Uterine rupture" = paste0(
    ...
    "Experienced organ dysfunction" = paste0(
    ...
    "Met at least one near miss criteria" = paste0(
    ...
    "Met near miss criteria and PLTC and survived" = paste0(
    ...
    "Adjudication required: Near miss but no PLTC <sup>b</sup>" = paste0(
...
## the table is too long -- split into 2 
near_miss_tab1 <- near_miss_taba
near_miss_tab2 <- near_miss_tabb


# part 1
near_miss_tab_output1 <- tb_theme1(near_miss_tab1) %>% 
  tab_header(
    title = md("**Table 9**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Data completeness</span>"),
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Experienced potentially life-threatening condition (PLTC) </span>"),
    rows = 5:19
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Data completeness</span>",
                             "<span style='font-size: 18px'>Experienced potentially life-threatening condition (PLTC) </span>"
                             ))  %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Any woman who has passed 42 days postpartum without closeout AND has completed a visit 42+ days postpartum OR she experienced PLTC.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>*</sup> These items will be added to the PLTC criteria or near-miss criteria once available.</span>")
 )

# part 2
near_miss_tab_output2 <- tb_theme1(near_miss_tab2) %>% 
  tab_header(
    title = md("**Table 9 (continued)**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Underwent critical intervention</span>"),
    rows = 1:4
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Maternal near-miss (PRISMA definition)</span>"),
    rows = 5:12
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Underwent critical intervention</span>",
                             "<span style='font-size: 18px'>Maternal near-miss (PRISMA definition)</span>"
                             ))  %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Observations that qualify as a near miss but not as a PLTC require further adjudication.</span>" )  ) %>% 
...
gtsave(near_miss_tab_output1, file = "near_miss_tab1.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "near_miss_tab1.png"))
...
gtsave(near_miss_tab_output2, file = "near_miss_tab2.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "near_miss_tab2.png"))
...
uter_rup_tab <- rupture %>% 
  ...
  summarise(
    ...
    "Missing: information for a maternal death prior to delivery" = paste0(
      format(sum(PREG_END== 1 & MAT_UTER_RUP_MISS==4, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & MAT_UTER_RUP_MISS==4, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Denominator (non-missing)" = paste0(
      format(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm = TRUE)/sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
uter_rup_tab_output <- tb_theme1(uter_rup_tab) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for uterine rupture prevalence</span>"),
    rows = 6:8
  ) %>%
  tab_row_group(
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Denominator for data completeness</span>",
                            "<span style='font-size: 18px'>Data completeness</span>",
                            "<span style='font-size: 18px'>Denominator for uterine rupture prevalence</span>",
...
**Definition:** Labor lasting ≥20 hours for primipara women or ≥14 hours for multipara women
...
prolong_tab <- labor %>% 
  ...
  summarise(
    ...
    "Missing: unknown parity & labor lasted 14-20 hrs" = paste0(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(PREG_END_DELIV==1 & PRO_LABOR_MISS==0, na.rm = TRUE)/sum(PREG_END_DELIV==1 & PRO_LABOR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
prolong_tab_output <- tb_theme1(prolong_tab) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for prolonged labor prevalence</span>"),
    rows = 8:10
  ) %>%
  tab_row_group(
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Denominator for data completeness</span>",
                            "<span style='font-size: 18px'>Data completeness</span>",
                            "<span style='font-size: 18px'>Denominator for prolonged labor prevalence</span>",
...
-   ***Chronic hypertension*** includes any participant with a history of chronic hypertension reported at enrollment, those currently taking medication to treat hypertension upon enrollment, those with a new diagnosis of hypertension at an ANC visit at <20 weeks gestation, those with a new medication to treat hypertension at an ANC visit at <20 weeks gestation, and/or those with one or more high blood pressure measures recorded at <20 weeks.

-   ***Gestational hypertension*** includes any participant with one or more of the following occuring at or after 20 weeks gestation, excluding those with preexisting chronic hypertension: 2 or more high blood pressure measures recorded on separate visits; 1 or more severe high blood pressure measures; 1 or more high blood pressure readings AND the participant starts a medication to treat hypertension; a clinical diagnosis of gestational hypertension at an ANC visit, a hospitalization, or labor and delivery. 
...
hdp_tab <- hdp %>% 
  ...
  summarise(
    ...
    "Denominator (non-missing)" = paste0(
      ...
      " (",
      format(round(sum(PREG_END==1 & HDP_GROUP >= 0 & HDP_GROUP <=5, na.rm = TRUE)/sum(PREG_END==1 & HDP_GROUP >= 0 & HDP_GROUP <=5, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
hdp_tab_output <- tb_theme1(hdp_tab) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for HDPs</span>"),
    rows = 5:11
  ) %>%
  tab_row_group(
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Denominator for data completeness</span>",
                            "<span style='font-size: 18px'>Data completeness</span>",
                            "<span style='font-size: 18px'>Denominator for HDPs</span>",
...
hdp_tab2 <- hdp %>% 
  ...
  summarise(
    ...
    "Total cases of preeclampsia with severe features" = paste0(
      format(sum(PREG_END==1 & HDP_GROUP ==5, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & HDP_GROUP ==5, na.rm = TRUE)/sum(PREG_END==1 & HDP_GROUP ==5, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
hdp_tab2_output <- tb_theme1(hdp_tab2) %>% 
  tab_header(
    title = md("**Table 13b**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Preeclampsia with severe features by symptoms</span>"),
        rows = 1:10
    ) %>%
 row_group_order(groups = c( "<span style='font-size: 18px'>Preeclampsia with severe features by symptoms</span>"))
