...
Hemorrhage_tab <- hemorrhage %>%
  ...
  summarise(
    ...
    "Est blood loss >= 500mL" = paste0(
      format(sum(RSN_EST_BLOOD_LOSS_IPC >= 500 | RSN_EST_BLOOD_LOSS_HOSP_ANC >= 500, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(RSN_EST_BLOOD_LOSS_IPC >= 500 | RSN_EST_BLOOD_LOSS_HOSP_ANC >= 500, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    
    "Procedure/blood transfusion" = paste0(
      format(sum(RSN_PPH_PROC_IPC_BALLOON==1 |RSN_PPH_PROC_IPC_SURG==1 |RSN_PPH_PROC_IPC_BRACE==1|
    ...
    "Est blood loss >= 1000mL" = paste0(
      format(sum(RSN_EST_BLOOD_LOSS_IPC >= 1000 | RSN_EST_BLOOD_LOSS_PNC >= 1000 | RSN_EST_BLOOD_LOSS_HOSP_PNC >= 1000, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(RSN_EST_BLOOD_LOSS_IPC >= 1000 | RSN_EST_BLOOD_LOSS_PNC >= 1000 | RSN_EST_BLOOD_LOSS_HOSP_PNC >= 1000, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
hemorrhage_output1 <- tb_theme1(Hemorrhage_tab1) %>%
  ...
  tab_style(
    style = list(
      cell_text(indent = px(30), size = 24, whitespace =  "nowrap")
    ),
    locations = cells_stub(rows = c(
      "Est blood loss >= 500mL",
      "Procedure/blood transfusion",
      
      "Est blood loss >= 1000mL",
      "Procedure/blood transfusion "
    ))
  ) %>% 
...
# Leptospirosis
    ...
    "Leptospirosis ever during pregnancy (IgM+ or incident IgG+) ^b^" = paste0(
      format(sum(LEPT_IGM_POSITIVE_EVER_PREG ==1 | (LEPT_IGG_POSITIVE_EVER_PREG == 1 & LEPT_IGM_POSITIVE_ENROLL==0 & LEPT_IGG_POSITIVE_ENROLL == 0 & LEPT_IGG_PERF_EVER_PREG==1), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(LEPT_IGM_POSITIVE_EVER_PREG ==1 | (LEPT_IGG_POSITIVE_EVER_PREG == 1 & LEPT_IGM_POSITIVE_ENROLL==0 & LEPT_IGG_POSITIVE_ENROLL == 0 & LEPT_IGG_PERF_EVER_PREG==1), na.rm = TRUE)/sum(LEPT_IGM_PERF_EVER_PREG ==1| LEPT_IGG_PERF_EVER_PREG ==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
