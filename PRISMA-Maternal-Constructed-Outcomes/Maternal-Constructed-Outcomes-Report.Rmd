---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2024 June 17)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
#* Last updated: June 14 2024, Erin Oakley
#*****************************************************************************
library(knitr)
library(tidyverse)
library(reshape2)
...
# Erin's path: 
path_to_data <- "Z:/Outcome Data/2024-06-17/"
...
preterm_tab <- preterm_indication %>% 
  ...
  summarise(
     ...
     "Excluded: pregnancy losses <20 weeks GA" = paste0(
...
-   Develops severe complications (severe postpartum hemorrhage, severe pre-eclampsia, eclampsia, sepsis or severe infection, uterine rupture, severe abortion complications) OR undergoes a critical intervention (use of blood products, laparotomy [not a C-section], admission to ICU), *AND*

-   Experiences organ dysfunction [MNH09, MNH12, MNH19], *AND*

-   Survives the experience.

**Denominator:** Passed the PNC6 window AND did not closeout during the window AND we have an MNH09/MNH12/MNH19 for that participant AND (we have seen them after the PNC6 window OR they have experienced severe complication or underwent a critical intervention).

**Numerator:** 
-   Women who have experienced a severe complication, critical intervention, or have experienced organ dysfunction *AND*

-   Passed the PNC6 window AND did not closeout during the window AND we have an MNH09/MNH12/MNH19 for that participant AND (we have seen them after the PNC6 window OR they have experienced severe complication, underwent a critical intervention, or organ failure). 
...
near_miss_taba <- near_miss %>% 
  ...
  summarise(
    ...
    "Closed out during pregnancy or within 42 days of pregnancy end" = paste0(
    ...
    #   ")"),
    # 
    # "Missing because not seen at least 42 days postpartum" = paste0(
    #   format(sum(DIAB_GEST_ANY==1 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(DIAB_GEST_ANY==1 & COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    
    "Women who have passed 42 days after pregnancy end without closing out AND visit after six weeks OR have the event <sup>a</sup>" = paste0(
...
near_miss_tabb <- near_miss %>% 
  ...
  summarise(      
   ...
   "Uterine rupture" = paste0(
    ...
    "Near miss but no PLTC <sup>b</sup>" = paste0(
...
near_miss_tab_output1 <- tb_theme1(near_miss_tab1) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Passed the PNC6 window AND did not closeout during the window AND we have an MNH09/MNH12/MNH19 for that participant AND (we have seen them after the PNC6 window OR they have experienced severe complication, underwent a critical intervention, or organ failure).</span>")
 )
...
near_miss_tab_output2 <- tb_theme1(near_miss_tab2) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Observations that qualify as a near miss but not as a PLTC require further adjudication.</span>" )
 )
...
prolong_tab <- preterm_indication %>% 
  ...
  summarise(
    ...
    "Denominator (non-missing)" = paste0(
      format(sum(PREG_END==1 & PRO_LABOR_MISS==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & PRO_LABOR_MISS==0, na.rm = TRUE)/sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Prolonged labor" = paste0(
      format(sum(PREG_END==1 & PRO_LABOR_MISS==0 & PRO_LABOR==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & PRO_LABOR_MISS==0 & PRO_LABOR==1, na.rm = TRUE)/sum(PREG_END==1 & PRO_LABOR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
     "No prolonged labor" = paste0(
      format(sum(PREG_END==1 & PRO_LABOR_MISS==0 & PRO_LABOR==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & PRO_LABOR_MISS==0 & PRO_LABOR==0, na.rm = TRUE)/sum(PREG_END==1 & PRO_LABOR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
prolong_tab_output <- tb_theme1(uter_rup_tab) %>% 
  tab_header(
    title = md("**Table 10**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Denominator for data completeness</span>"),
