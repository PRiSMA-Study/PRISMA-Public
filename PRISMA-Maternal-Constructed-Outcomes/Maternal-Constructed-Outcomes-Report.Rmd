---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2024 October 30)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
**Includes data from synapse last updated:** 2024 September 20
...
**-** Cause of death is unknown: analysis of verbal autopsy data [MNH27] is forthcoming.

**-** Missing date of maternal death (cannot classify as early or late death)
...
# replace NA with 0s (this likely means there is not yet data)
 ...
 row_group_order(groups = c("<span style='font-size: 18px'>Data completeness</span>",
  ...
  tab_style(
      ...
      locations = cells_stub(rows = c("Maternal mortality ^d^, n",
                                      "Death not due to pregnancy, n",
                                      "Cause of death unknown, n",
                                      "Cause of death missing, n",
                                      
                                      "Late maternal mortality^f^, n",
                                      "Death not due to pregnancy, n  ",
                                      "Cause of death unknown, n  ",
                                      "Cause of death missing, n  "
                                      )) 
  ...
  tab_style(
      ...
      locations = cells_body(rows = c(
                ...
                "Late pregnancy-related death^e^, n"))) %>% 
tab_style(
      style = list(
        cell_text(weight = "bold")
        ),
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>e</sup>Late pregnancy-related deaths are those occuring 42-365 days postpartum, for any reason.</span>")
  )  %>% 

  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>f</sup>Late maternal mortality: deaths occurring 42-365 days postpartum, due to causes related to pregnancy or its management.</span>")
  )
    ...
    "Postpartum hemorrhage" = paste0(
      ...
      ")"),   
        ...
        "PPH reported in MNH09/12/19" = paste0(
            format(sum(M09_PPH_CEOCCUR_6 == 1 | (HEM_HOSP_ANY==1 & TIMING_HOSP==2) |
            ...
            " (",
            format(round(sum(M09_PPH_CEOCCUR_6 == 1 | (HEM_HOSP_ANY==1 & TIMING_HOSP==2) |
                         M12_BIRTH_COMPL_MHTERM_1_7 == 1 | M12_BIRTH_COMPL_MHTERM_1_8 == 1 | M12_BIRTH_COMPL_MHTERM_1_9 == 1|
...
sti_enroll_tab <- MAT_INFECTION %>% 
  ...
  # Syphilis
    ...
    "Syphilis incidence following pregnancy (positive RDT or diagnosed)" = paste0(
...
zcd_tab <- MAT_INFECTION %>% 
  ...
  # Zika
    ...
    #   " (",
    #   format(round(sum(ZIK_IGM_POSITIVE_ANY_ANC == 1|ZIK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(ENROLL_EXPANSION == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    #   " (",
    #   format(round(sum(DEN_IGM_POSITIVE_ANY_ANC == 1|DEN_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(ENROLL_EXPANSION == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    #   " (",
    #   format(round(sum(CHK_IGM_POSITIVE_ANY_ANC == 1|CHK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(ENROLL_EXPANSION == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
  ...
  # Leptospirosis 
    ...
    #   " (",
    #   format(round(sum(LEP_IGM_POSITIVE_ANY_ANC == 1|LEP_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(ENROLL_EXPANSION == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
...
placenta_tab <- placenta %>% 
  ...
  summarise(
     ...
     "Placenta accreta indicated as reason for C-section^b^" = paste0(
...
tab_style(
 ...
 row_group_order(groups = c("<span style='font-size: 18px'>Data completeness <sup>a</sup></span>",
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> This denominator is restricted to pregnancies ending in a delivery at/after 20 weeks gestation AND included labor or attempted labor induction. Pregnancies ending in a c-section prior to labor initiation and withou any attempted induction are excluded. </span>"))
