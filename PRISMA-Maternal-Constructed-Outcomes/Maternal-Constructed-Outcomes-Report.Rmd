---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2024 November 18)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
#* Last updated: October 29 2024, Erin Oakley, Savannah O'Malley & Stacie Loisate
#*****************************************************************************
library(knitr)
library(tidyverse)
library(reshape2)
...
# Erin's path: 
# path_to_data <- "Z:/Outcome Data/2024-09-20/" 
...
**Includes data from synapse last updated:** 2024 November 18
...
summary_tab1 <- MAT_INFECTION %>% 
  ...
  ## Ever depression
    "Depressive symptoms during pregnancy (ever) ^d^" = paste0(
...
#Postpartum outcomes: 
  ...
  ## Depression
     "Depressive symptoms, postpartum (6-14 weeks) ^f^" = paste0(
...
*NOTE: cause of death data is currently based on csvs provided by sites, in the future this will be updated to reflect causes provided in MNH37*
...
**5.** *For maternal and late maternal mortality:*  Cause of death is not due to accidental or incidental causes but is due to pregnancy or management of pregnancy [MNH27].

**Common causes for a participant to be marked as "Missing":**

**-** Cause of death is missing: analysis of verbal autopsy data [MNH27] is forthcoming.
...
# ## without percents
 mortality_tab <- mortality %>%
   ...
   summarise(
    ...
    "Any death^a^,n (per 100,000)" = paste0(
    ...
    "Missing date of death^b^, n" = paste0(
       format(sum(MAT_DEATH_MISSDATE == 1, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Pregnancy-related death^c^, n" = paste0(
       format(sum(MAT_DEATH_42 == 1, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Maternal mortality ^d^, n" = paste0(
       format(sum(MAT_MORTALITY == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
      
    "Death not due to pregnancy, n" = paste0(
       format(sum(MAT_MORTALITY == 2, na.rm = TRUE), nsmall = 0, digits = 2)),
    
    "Cause of death unknown^e^, n" = paste0(
       format(sum(MAT_MORTALITY == 3, na.rm = TRUE), nsmall = 0, digits = 2)),
    
    "Cause of death missing^f^, n" = paste0(
       format(sum(MAT_MORTALITY == 55, na.rm = TRUE), nsmall = 0, digits = 2)),
    
    "Late pregnancy-related death^g^, n" = paste0(
       format(sum(MAT_DEATH_42 == 2, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Late maternal mortality^h^, n" = paste0(
       format(sum(MAT_LATE_MORTALITY == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
    
      "Death not due to pregnancy, n  " = paste0(
       format(sum(MAT_LATE_MORTALITY == 2, na.rm = TRUE), nsmall = 0, digits = 2)),
    
      "Cause of death unknown^e^, n  " = paste0(
       format(sum(MAT_LATE_MORTALITY == 3, na.rm = TRUE), nsmall = 0, digits = 2)),
    
      "Cause of death missing^f^, n  " = paste0(
       format(sum(MAT_LATE_MORTALITY == 55, na.rm = TRUE), nsmall = 0, digits = 2))
...
# replace NA with 0s (this likely means there is not yet data)
 ...
 mortality_output <- tb_theme1(mortality_tab) %>%
   tab_header(
     title = md("**Table 1**")) %>%
   tab_row_group(
     label = html("<span style='font-size: 18px'>Deaths occurring at any point during pregnancy</span>"),
     rows = 1:3
   ) %>%
    tab_row_group(
     ...
     rows = 4:8
      ...
      tab_row_group(
     label = html("<span style='font-size: 18px'>Deaths occurring 42-365 days postpartum</span>"),
     rows = 9:13
   ) %>%
 row_group_order(groups = c("<span style='font-size: 18px'>Deaths occurring at any point during pregnancy</span>",
                   "<span style='font-size: 18px'>Deaths occurring during pregnancy up to 42 days postpartum</span>",
                   "<span style='font-size: 18px'>Deaths occurring 42-365 days postpartum</span>"
                  )) %>%
  ...
  tab_style(
      ...
      locations = cells_body(rows = c(
                "Any death^a^,n (per 100,000)",
                "Pregnancy-related death^c^, n",
                "Late pregnancy-related death^g^, n"))) %>% 
tab_style(
      style = list(
        cell_text(weight = "bold")
        ),
      locations = cells_stub(rows = c(
                 ...
                 "Late pregnancy-related death^g^, n"))) %>% 
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Any death includes deaths occurring at any time and for any reason, including accidental/incidental causes. Denominator: number of pregnancy end points.</span>")
  ) %>% 
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>b</sup>Date of death missing;  date of death has been estimated as the midpoint between when the woman was last seen alive and when her death was reported.</span>")
  ) %>% 
  tab_footnote(
 ...
 tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>e</sup>Cause of death unknown: InterVA has been conducted but reported cause was inconclusive, ie 'Indeterminate' or 'Unknown'</span>")
  )  %>% 
    tab_footnote(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>h</sup>Late maternal mortality: deaths occurring 42-365 days postpartum, due to causes related to pregnancy or its management.</span>")
  )
...
preterm_tab <- preterm_indication %>% 
  ...
  summarise(
    ...
    "Missing: information on labor onset or rupture of membranes [MNH09]" = paste0(
      format(sum(PRETERM_PROV_MISS== 4 & PREG_END== 1 & PREG_END_DELIV==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PRETERM_PROV_MISS== 4 & PREG_END== 1 & PREG_END_DELIV==1, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
### 5b. Suspected maternal depression, using standard cutoff score (>= 11)

**Numerator:** Valid depression score $\geq$ 11 (calculated from variables `EPDS0101` – `EPDS0110` [MNH25]) AND visit type = i (`TYPE_VISIT` [MNH25]).
...
### 5c. Maternal depressive symptoms, using site-specific cutoff score
...
**Numerator:** Valid depression score $\geq$ site-specific cutoff (calculated from variables `EPDS0101` – `EPDS0110` [MNH25]) AND visit type = i (`TYPE_VISIT` [MNH25]) AND expected for that visit (window has passed without closeout)
    ...
    "Postpartum hemorrhage" = paste0(
      ...
      ")"),   
        ...
        "PPH reported in MNH09/12/19" = paste0(
            format(sum(PPH_REPORT_IPC == 1 | 
                         (HEM_HOSP_ANY==1 & TIMING_HOSP==2) |
                         BIRTH_COMPL_PPH_PNC0 == 1 | 
                         BIRTH_COMPL_PPH_PNC1 == 1 | BIRTH_COMPL_PPH_PNC4 == 1|
                         BIRTH_COMPL_PPH_PNC6==1 | BIRTH_COMPL_PPH_PNC26==1 |
                         BIRTH_COMPL_PPH_PNC52==1, na.rm = TRUE), nsmall = 0, digits = 2),
            " (",
            format(round(sum(PPH_REPORT_IPC == 1 | (HEM_HOSP_ANY==1 & TIMING_HOSP==2) |
                         BIRTH_COMPL_PPH_PNC0 == 1 | 
                           BIRTH_COMPL_PPH_PNC1 == 1 |
                           BIRTH_COMPL_PPH_PNC4 == 1|
                         BIRTH_COMPL_PPH_PNC6==1 | 
                           BIRTH_COMPL_PPH_PNC26==1 |
                           BIRTH_COMPL_PPH_PNC52==1, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
            ")"),


          ## hemorrhage subcategories:
         "Estimated blood loss >= 500mL" = paste0(
          format(sum(EST_BLOOD_LOSS_IPC >= 500, na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(EST_BLOOD_LOSS_IPC >= 500, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
    

         "Procedure or blood transfusion" = paste0(
          format(sum(PPH_IPC_BALLOON==1 |PPH_IPC_SURG==1 |PPH_IPC_BRACE==1|
                       PPH_IPC_VESSEL==1|PPH_IPC_HYSTER==1|PPH_IPC_OTHER==1 | BLOOD_TRANS_IPC==1, na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(PPH_IPC_BALLOON==1 |PPH_IPC_SURG==1 |PPH_IPC_BRACE==1|
                             PPH_IPC_VESSEL==1|PPH_IPC_HYSTER==1|PPH_IPC_OTHER==1 | BLOOD_TRANS_IPC==1, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
    ...
    "Severe postpartum hemorrhage" = paste0(
      ...
      ## severe subcategories:
         "Estimated blood loss >= 1000mL" = paste0(
          format(sum(EST_BLOOD_LOSS_IPC >= 1000, na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(EST_BLOOD_LOSS_IPC >= 1000, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
    
    
         "Procedure or blood transfusion " = paste0(
          format(sum(HEM_PPH_SEV==1 & (PPH_IPC_BALLOON==1 |PPH_IPC_SURG==1 |PPH_IPC_BRACE==1|
                       PPH_IPC_VESSEL==1|PPH_IPC_HYSTER==1|PPH_IPC_OTHER==1 | BLOOD_TRANS_IPC==1), na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(HEM_PPH_SEV==1 & (PPH_IPC_BALLOON==1 |PPH_IPC_SURG==1 |PPH_IPC_BRACE==1|
                             PPH_IPC_VESSEL==1|PPH_IPC_HYSTER==1|PPH_IPC_OTHER==1 | BLOOD_TRANS_IPC==1), na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
          ")"),
...
## Methods of blood loss measurements 
  ...
  "Method unknown" = paste0(
      format(sum(BLOOD_LOSS_METHOD_IPC %in% c(99, 55), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(BLOOD_LOSS_METHOD_IPC %in% c(99, 55) , na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  "Denominator  " = paste0(
    ...
    "Balloon/condom tamponade" = paste0(
      format(sum(PPH_IPC_BALLOON == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_IPC_BALLOON == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Surgical interventions" = paste0(
      format(sum(PPH_IPC_SURG == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_IPC_SURG == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Brace sutures" = paste0(
      format(sum(PPH_IPC_BRACE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_IPC_BRACE == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Vessel ligation" = paste0(
      format(sum(PPH_IPC_VESSEL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_IPC_VESSEL == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Hysterectomy" = paste0(
      format(sum(PPH_IPC_HYSTER == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_IPC_HYSTER == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Blood transfusion" = paste0(
      format(sum(BLOOD_TRANS_IPC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(BLOOD_TRANS_IPC == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  "Denominator   " = paste0(
    ...
    "Oxytocin" = paste0(
      format(sum(PPH_MED_OCY == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_OCY == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Misoprostol" = paste0(
      format(sum(PPH_MED_MISO == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_MISO == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Tranexaminic acid" = paste0(
      format(sum(PPH_MED_TRANEX == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_TRANEX == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Carbetocin" = paste0(
      format(sum(PPH_MED_CARBET == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_CARBET == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Methylergonovine" = paste0(
      format(sum(PPH_MED_METHYL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_METHYL == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Carboprost (PGF2-alpha)" = paste0(
      format(sum(PPH_MED_CARBOP == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_CARBOP == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Other " = paste0(
      format(sum(PPH_MED_OTHER == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_OTHER == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "No medications given" = paste0(
      format(sum(PPH_MED_NONE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_MED_NONE == 1, na.rm = TRUE)/sum(HEM_PPH==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
hemorrhage_figs <- hemorrhage %>% 
  select(SITE, MOMID, PREGID, EST_BLOOD_LOSS_IPC,HEM_PPH, HEM_PPH_SEV) %>% 
  filter(EST_BLOOD_LOSS_IPC > 0) %>% 
  mutate(CUTOFF = case_when(EST_BLOOD_LOSS_IPC >= 1000~ "Severe Hemorrhage(>=1000mL)",
                            EST_BLOOD_LOSS_IPC >= 500 & EST_BLOOD_LOSS_IPC < 1000~ "Hemorrhage (>=500mL)",
...
ggplot() + 
                    geom_histogram(data = hemorrhage_figs, aes(x = EST_BLOOD_LOSS_IPC, fill = CUTOFF),  color = "gray", binwidth = 50) + 
...
sti_enroll_tab <- MAT_INFECTION %>% 
  ...
  ## HIV
    ...
    "HIV incidence following enrollment (positive RDT or diagnosed) ^b^" = paste0(
      format(sum(HIV_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HIV_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  # Syphilis
    ...
    "Syphilis incidence following enrollment (positive RDT or diagnosed) ^b^" = paste0(
      format(sum(SYPH_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(SYPH_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Syphilis incidence following pregnancy (positive RDT or diagnosed) ^c^" = paste0(
      format(sum(SYPH_POSITIVE_ANY_PNC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(SYPH_POSITIVE_ANY_PNC == 1, na.rm = TRUE)/sum(PREG_END == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Syphilis diagnosis and RDT missing at enrollment ^d^" = paste0(
  ...
  #       "N with CT/NG test performed at enrollment" = paste0(
  #      format(sum(CTNG_TEST_PERF_1 == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
  #            
  #     ## Chlamydia prevalence at enrollment 
  #     "Chlamydia prevalence at enrollment" = paste0(
  #       format(sum(CT_POSITIVE_1 == 1 & CTNG_TEST_PERF_1 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
  #       " (",
  #       format(round(sum(CT_POSITIVE_1 == 1 & CTNG_TEST_PERF_1 == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_1 == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
  #       ")"),
  #     
  #     # "N with NG test performed at enrollment" = paste0(
  #     #   format(sum(CT_TEST_PERF_1 == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
  #     
  #     ## Gonorrhea prevalence at enrollment 
  #     "Gonorrhea prevalence at enrollment (<20wks)" = paste0(
  #       format(sum(NG_POSITIVE_1 == 1 & CTNG_TEST_PERF_1 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
  #       " (",
  #       format(round(sum(NG_POSITIVE_1 == 1 & CTNG_TEST_PERF_1 == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_1 == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
  #       ")") ,
  #     
  #     "N with CT/NG test performed at any ANC (excluding enrollment)" = paste0(
  #       format(sum(CTNG_TEST_PERF_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
  #     
  #     ## Chlamydia prevalence at enrollment 
  #     "Chlamydia incidence following enrollment (>=20wks)" = paste0(
  #       format(sum(CT_POSITIVE_ANY_ANC == 1 & CTNG_TEST_PERF_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
  #       " (",
  #       format(round(sum(CT_POSITIVE_ANY_ANC == 1 & CTNG_TEST_PERF_ANY_ANC == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_ANY_ANC == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
  #       ")"),
  #     
  #     # "N with NG test performed at enrollment" = paste0(
  #     #   format(sum(CT_TEST_PERF_1 == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
  #     
  #     ## Gonorrhea prevalence at enrollment 
  #     "Gonorrhea incidence following enrollment (>=20wks)" = paste0(
  #       format(sum(NG_POSITIVE_ANY_ANC == 1 & CTNG_TEST_PERF_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
  #       " (",
  #       format(round(sum(NG_POSITIVE_ANY_ANC == 1 & CTNG_TEST_PERF_ANY_ANC == 1, na.rm = TRUE)/sum(CTNG_TEST_PERF_ANY_ANC == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
  #       ")"),
  # 
  # Gonorrhea
    "Gonorrhea diagnosed positive at enrollment ^a^" = paste0(
    ...
    "Gonorrhea incidence following enrollment (diagnosed) ^b^" = paste0(
      format(sum(GON_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(GON_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  # Chlamydia
    "Chlamydia diagnosed positive at enrollment ^a^" = paste0(
    ...
    "Chlamydia incidence following enrollment (diagnosed) ^b^" = paste0(
      format(sum(CHL_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(CHL_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    # Genital ulcers
      "Genital ulcers diagnosed positive at enrollment ^a^" = paste0(
      ...
      format(sum(GENU_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(GENU_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  # Other STIs
    "Other STIs diagnosed positive at enrollment ^a^" = paste0(
    ...
    "Other STIs incidence following enrollment (diagnosed) ^b^" = paste0(
      format(sum(OTHR_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(OTHR_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
## the table is too long -- split into 2 
sti_enroll_tab1 <- sti_enroll_tab[1:8,]
sti_enroll_tab2 <- sti_enroll_tab[9:23,]
...
sti_enroll_output1 <- tb_theme1(sti_enroll_tab1) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is total participants with a pregnancy outcome.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is total participants with an enrollment visit.</span>")
 ) 
...
sti_enroll_output2 <- tb_theme1(sti_enroll_tab2) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>STIs by method at enrollment <sup>d</sup>, n (%)</span>"),
    rows = 13:15
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Gonorrhea, n (%)</span>",
                             ...
                             "<span style='font-size: 18px'>STIs by method at enrollment <sup>d</sup>, n (%)</span>"
                             )) %>% 
 #  tab_footnote(
 #    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants with an enrollment visit excluding missing (missing defined as default values reported for test results).</span>")
 # )  %>% 
 ...
 # ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants who reported having an 'other' STI (`OTHR_STI_MHOCCUR=1`) AND a valid test (`TEST_VAR=1 or 0`) OR who reported not having an 'other' STI (`OTHR_STI_MHOCCUR=0`).</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is total participants with at least one MNH04 OR MNH06 during ANC. Excludes cases reported at enrollment.</span>")
 ) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is total participants with an enrollment visit excluding missing (missing defined as default values reported for test results).</span>")
 )  
...
malaria_tab <- MAT_INFECTION %>% 
  ...
  ## Malaria
    ...
    "Malaria incidence following enrollment (positive RDT or diagnosed) ^b^" = paste0(
      format(sum(MAL_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAL_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
hepatitis_tab <- MAT_INFECTION %>% 
  ...
  # Hepatitis B
    ...
    "Hep B incidence following enrollment (positive RDT) ^b^" = paste0(
      format(sum(HBV_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBV_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  # Hepatitis C
    ...
    "Hep C incidence following enrollment (positive RDT) ^b^" = paste0(
      format(sum(HCV_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HCV_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  "N with at least one expansion test performed" = paste0(
      format(sum(EXPANSION_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Hep E IgM positive at enrollment ^d^" = paste0(
      format(sum(HEV_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HEV_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & HEV_IGM_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Hep E IgG positive at enrollment ^d^" = paste0(
      format(sum(HEV_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HEV_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & HEV_IGG_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    # "Hep E incidence following enrollment (IgM or IgG) ^b^" = paste0(
    #   format(sum(HEV_IGM_POSITIVE_ANY_ANC == 1|HEV_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(HEV_IGM_POSITIVE_ANY_ANC == 1|HEV_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(ENROLL_EXPANSION == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    "Hep E missing at enrollment (IgG or IgM) ^e^" = paste0(
      ...
      " (",
      format(round(sum(HEV_IGM_MISSING_ENROLL == 1 | HEV_IGG_MISSING_ENROLL==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
tb_tab <- MAT_INFECTION %>% 
  ...
  summarise(
    ...
    "TB symptoms incidence following enrollment ^b^" = paste0(
      format(sum(TB_SYMP_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(TB_SYMP_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "TB sputum incidence following enrollment ^b^" = paste0(
      format(sum(TB_SPUTUM_POSITIVE_ANY_ANC == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(TB_SPUTUM_POSITIVE_ANY_ANC == 1, na.rm = TRUE)/sum(INFECTION_ENROLL_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
zcd_tab <- MAT_INFECTION %>% 
  ...
  # Zika
      ...
      " (",
      format(round(sum(ZIK_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & ZIK_IGM_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Zika IgG positive at enrollment ^a^" = paste0(
      format(sum(ZIK_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ZIK_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & ZIK_IGG_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    # "Zika incidence following enrollment (IgM or IgG) ^b^" = paste0(
    #   format(sum(ZIK_IGM_POSITIVE_ANY_ANC == 1|ZIK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(ZIK_IGM_POSITIVE_ANY_ANC == 1|ZIK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    "Zika missing at enrollment (IgG or IgM) ^b^" = paste0(
      ...
      " (",
      format(round(sum(ZIK_IGM_MISSING_ENROLL == 1 | ZIK_IGG_MISSING_ENROLL==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    # Dengue
      ...
      " (",
      format(round(sum(DEN_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & DEN_IGM_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Dengue IgG positive at enrollment ^a^" = paste0(
      format(sum(DEN_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEN_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & DEN_IGG_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    # "Dengue incidence following enrollment (IgM or IgG) ^b^" = paste0(
    #   format(sum(DEN_IGM_POSITIVE_ANY_ANC == 1|DEN_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(DEN_IGM_POSITIVE_ANY_ANC == 1|DEN_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    "Dengue missing at enrollment (IgG or IgM) ^b^" = paste0(
      ...
      " (",
      format(round(sum(DEN_IGM_MISSING_ENROLL == 1 | DEN_IGG_MISSING_ENROLL==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    # Chikungunya
      ...
      " (",
      format(round(sum(CHK_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & CHK_IGM_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Chikungunya IgG positive at enrollment ^a^" = paste0(
      format(sum(CHK_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(CHK_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & CHK_IGG_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    
    # "Chikungunya incidence following enrollment (IgM or IgG) ^b^" = paste0(
    #   format(sum(CHK_IGM_POSITIVE_ANY_ANC == 1|CHK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(CHK_IGM_POSITIVE_ANY_ANC == 1|CHK_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    "Chikungunya missing at enrollment (IgG or IgM) ^b^" = paste0(
      ...
      " (",
      format(round(sum(CHK_IGM_MISSING_ENROLL == 1 | CHK_IGG_MISSING_ENROLL==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  # Leptospirosis 
      ...
      " (",
      format(round(sum(LEP_IGM_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & LEP_IGM_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    "Leptospirosis  IgG positive at enrollment ^a^" = paste0(
      format(sum(LEP_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(LEP_IGG_POSITIVE_ENROLL == 1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1 & LEP_IGG_MISSING_ENROLL == 0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
    # "Leptospirosis incidence following enrollment (IgM or IgG) ^b^" = paste0(
    #   format(sum(LEP_IGM_POSITIVE_ANY_ANC == 1|LEP_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(LEP_IGM_POSITIVE_ANY_ANC == 1|LEP_IGG_POSITIVE_ANY_ANC==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    ...
    "Leptospirosis missing at enrollment (IgG or IgM) ^b^" = paste0(
      ...
      " (",
      format(round(sum(LEP_IGM_MISSING_ENROLL == 1 | LEP_IGG_MISSING_ENROLL==1, na.rm = TRUE)/sum(EXPANSION_ENROLL == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
all_infection_output <- tb_theme1(all_infection_tab) %>% 
  ...
  tab_style(
      ...
      locations = cells_stub(rows = c(
                  "N with enrollment visit"))) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants with an enrollment visit.</span>")
 ) 
...
near_miss_tabb <- near_miss %>% 
 ...
 "Any potentially life-threatening condition" = paste0(
    ...
    "Pre-eclampsia without severe features " = paste0(
      format(sum(PREECLAMPSIA==1 & NEAR_MISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREECLAMPSIA==1 & NEAR_MISS_DENOM == 1, na.rm = TRUE)/sum(NEAR_MISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Pre-clampsia with severe features/Eclampsia " = paste0(
      format(sum(PREECLAMPSIA_SEV == 1 & NEAR_MISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREECLAMPSIA_SEV ==1 & NEAR_MISS_DENOM == 1, na.rm = TRUE)/sum(NEAR_MISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
thy_tab <- thy %>% 
  ...
  summarise(
     ...
     "Expected number at Enrollment^a^" = paste0(
...
thy_tab_output <- tb_theme1(thy_tab) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Thyroid function at Enrollment</span>"),
        rows = 3:9
    ) %>%
 row_group_order(groups = c( "<span style='font-size: 18px'>Data completeness</span>",
                            "<span style='font-size: 18px'>Thyroid function at Enrollment</span>"))  %>% 
  tab_footnote(
...
hdp_tab2 <- hdp %>% 
  ...
  summarise(
    ...
    "Postpartum seizures (within 7 days of delivery)" = paste0(
      format(sum(SEVERE_FEAT_POSTP_SEIZURES==1 & HDP_GROUP ==5, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(SEVERE_FEAT_POSTP_SEIZURES==1 & HDP_GROUP ==5, na.rm = TRUE)/sum(PREG_END==1 & HDP_GROUP ==5, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
placenta_tab <- placenta %>%
  ...
  summarise(
     ...
     "Denominator^a^" = paste0(
      ...
      format(sum(CES_ANY == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
     ...
     "Suspected placental abruption" = paste0(
      format(sum(PLACENTA_ABRUPTION_M09 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PLACENTA_ABRUPTION_M09 == 1, na.rm = TRUE)/sum(SB_LB == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),



     "Suspected placenta accreta indicated as reason for C-section^b^" = paste0(
      format(sum(PLACENTA_ACCRETE_M09 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PLACENTA_ACCRETE_M09 == 1, na.rm = TRUE)/sum(CES_ANY == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
induced_tab <- induced %>% 
  ...
  summarise(
     ...
     "Excluded: Pregnancy losses <20 weeks GA or induced abortion" = paste0(
      format(sum(PREG_END== 1 & MAT_INDUCED_MISS==3 & PREG_LOSS_DEATH==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & MAT_INDUCED_MISS==3 & PREG_LOSS_DEATH==0, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),  
     
    "Excluded: Maternal deaths at <20 weeks GA" = paste0(
      format(sum(PREG_END== 1 & MAT_INDUCED_MISS==3 & PREG_LOSS_DEATH==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & MAT_INDUCED_MISS==3 & PREG_LOSS_DEATH==1, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Excluded: Pregnancies >=20 weeks that ended without labor AND without attempted induction" = paste0(
      format(sum(PREG_END== 1 & MAT_INDUCED_MISS==4, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & MAT_INDUCED_MISS==4, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
     
     "Missing: unknown if labor occurred" = paste0(
      format(sum(PREG_END== 1 & MAT_INDUCED_MISS==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & MAT_INDUCED_MISS==1, na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),  
...
### Table 16b. Induced Labor Scenarios
```{r}
...
induced_tab2 <- labor %>% 
  ...
  summarise(
    ...
    "Total cases of induced labor" = paste0(
      format(sum(MAT_INDUCED==1, na.rm = TRUE), nsmall = 0, digits = 2)),
    
    "Induced by artificial rupture of membranes" = paste0(
      format(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==1, na.rm = TRUE)/sum(MAT_INDUCED==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Induced by another method, prior to rupture of membranes" = paste0(
      format(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==2, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==2, na.rm = TRUE)/sum(MAT_INDUCED==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Induced labor following PROM or PPROM" = paste0(
      format(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==3, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==3, na.rm = TRUE)/sum(MAT_INDUCED==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Unknown induction method" = paste0(
      format(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==4, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_INDUCED==1 & MAT_INDUCED_TYPE==4, na.rm = TRUE)/sum(MAT_INDUCED==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    
    "Unknown timing of induction" = paste0(
      format(sum(MAT_INDUCED==1 & (MAT_INDUCED_TYPE==5 | MAT_INDUCED_TYPE==6), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_INDUCED==1 & (MAT_INDUCED_TYPE==5 | MAT_INDUCED_TYPE==6), na.rm = TRUE)/sum(MAT_INDUCED==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
induced_tab2[is.na(induced_tab2)] <- paste0("0 (0)")

induced_tab2_output <- tb_theme1(induced_tab2) %>% 
  tab_header(
    title = md("**Table 16b**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Induction of Labor Scenarios <sup>a</sup> </span>"),
        rows = 1:6
    ) %>%
  # bold denominator rows 2
  tab_style(
...
tab_style(
      ...
      locations = cells_stub(rows = c(
                 "Total cases of induced labor",
                 "Total cases of induced labor"))) %>% 
 row_group_order(groups = c( "<span style='font-size: 18px'>Induction of Labor Scenarios <sup>a</sup> </span>"))  %>%
  
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Note that participants induced prior to rupture of membranes are considered to be provider-initiated deliveries; participants with induced labor after spontaneous rupture of membranes (PROM or PPROM) are considered to be non-provider-initiated deliveries. </span>"))

```

```{r, out.width = '100%'}

gtsave(induced_tab2_output, file = "induced_tab2_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "induced_tab2_output.png"))
