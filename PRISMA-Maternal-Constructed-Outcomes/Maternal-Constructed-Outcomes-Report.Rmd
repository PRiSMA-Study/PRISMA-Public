...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is pregnancies ending in a livebirth or stillbirth (>=20 weeks GA) where the woman experienced labor (i.e., excluding cesarean deliveries without any labor).Note that this outcome is not available for Ghana and India-CMC sites due to data collection differences.</span>")
 ) %>%
...
hemorrhage_output1 <- tb_theme1(Hemorrhage_tab1) %>%
  ...
  tab_style(
    ...
    locations = cells_stub(rows = c(
      "Est blood loss >= 500mL^b^",
      "Procedure/blood transfusion^b^",
      
      "Est blood loss >= 1000mL^b^",
      "Procedure/blood transfusion^b^ "
    ))
  ) %>% 
...
hemorrhage_output2 <- tb_theme1(Hemorrhage_tab2) %>%
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Procedures for postpartum hemorrhage among women with PPH <sup>c</sup></span>"),
    rows = 1:7
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Medications given to prevent/treat postpartum hemorrhage <sup>c</sup></span>"),
    rows = 8:16
  ) %>%
  
  row_group_order(groups = c("<span style='font-size: 18px'>Procedures for postpartum hemorrhage among women with PPH <sup>c</sup></span>",
                             "<span style='font-size: 18px'>Medications given to prevent/treat postpartum hemorrhage <sup>c</sup></span>")) %>%
  # bold denominator rows
  tab_style(
    style = list(
      cell_text(weight = "bold")
