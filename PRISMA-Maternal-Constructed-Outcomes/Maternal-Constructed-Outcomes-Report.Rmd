---
title: "<span style='font-size: 18px'> <span style='text-align: center'> ReMAPP-Maternal-Outcomes [Issued: 2025 May 07]"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
knit: (function(inputFile, encoding) { 
      rmarkdown::render(inputFile,
                    encoding=encoding, 
                    output_file= paste( './Output/' , Sys.Date(), '-Maternal-Outcomes-Report-ReMAPP.pdf', sep='')) })
editor_options: 
  markdown: 
    wrap: 72
---
...
thy<- read_dta(paste0(path_to_data, "MAT_THYR.dta"))  %>% rename_all(toupper) %>% filter(SITE != "") %>% 
  #filter(PREGID %in% remind.moms$PREGID)
end <- read_dta(paste0(path_to_data, "MAT_ENDPOINTS.dta"))  %>% rename_all(toupper) %>% filter(SITE != "") %>% #filter(PREGID %in% remind.moms$PREGID)
