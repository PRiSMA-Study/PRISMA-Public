---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2025 May 05)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
tab_style(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>b</sup>Date of death missing. Date of death has been estimated as the midpoint between when the woman was last seen alive and when her death was reported. This estimated death date is used to categorize the death as either pregnancy-related death (during pregnancy up to 42 days postpartum) or late pregnancy related death (42-365 days postpartum).</span>")
  ) %>%
  tab_footnote(
...
knitr::include_graphics(paste0(path_to_data, "anemia-fig.pdf"))
...
## Methods of blood loss measurements
  ...
  "Method unknown" = paste0(
      ...
      " (",
      format(round(sum(BLOOD_LOSS_METHOD_IPC ==9, na.rm = TRUE)/sum(HEM_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
all_infection_output <- tb_theme1(all_infection_tab) %>%
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'>Note: Infections included: HIV, syphilis, chlamydia, gonorrhea, genital ulcers, other STIs (Table 7a); malaria (Table 7b); Hep B, Hep C, Hep E (Table 7c); TB (Table 7d); zika, dengue, chikungunya, and leptospirosis (Table 7e).</span>"))
