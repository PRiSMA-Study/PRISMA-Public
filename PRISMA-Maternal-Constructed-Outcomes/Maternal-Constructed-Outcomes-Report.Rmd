...
summary_tab1 <- MAT_INFECTION %>% 
  ...
  summary_tab5 <- preterm_indication1 %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  
  ## Preterm delivery
    "Preterm delivery (20 to <37 weeks) ^e^" = paste0(
  ...
  summary_tab6 <- hemorrhage %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  
  ## Antepartum hemorrhage
    "Antepartum hemorrhage ^c^" = paste0(
    ...
    "Postpartum hemorrhage ^c^" = paste0(
  ...
  summary_tab7 <- rupture1 %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  
  ## Uterine rupture
     "Uterine rupture  ^c^" = paste0(
  ...
  ## Gestational hypertension
     "Gestational hypertension ^e^" = paste0(
  ...
  ## Preeclampsia  
    "Preeclampsia (all categories) ^e^ ^f^" = paste0(
...
#Postpartum outcomes: 
  summary_tab9 <- near_miss %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  
  ## Near-Miss
     "Maternal Near-Miss ^a^" = paste0(
  ...
  summary_tab10 <- anemia %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
    
  ## Anemia
     "Any postpartum anemia (6-14 weeks) ^b^" = paste0(
     ...
     "Moderate/severe postpartum anemia (6-14 weeks) ^b^" = paste0(
  ...
  summary_tab11 <- depression %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  
  ## Depression
     "Postpartum Depression (6-14 weeks) ^b^" = paste0(
 ...
 summary_tab_2 <- rbind(summary_tab9,summary_tab10,summary_tab11)
...
summary_tab_output <- tb_theme1(summary_tab) %>% 
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Enrollment outcomes</span>",
                            ...
                            "<span style='font-size: 18px'>Pregnancy endpoint outcomes</span>")) %>% 
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is all participants who have at least one EPDS assessment at enrollment, ANC-20, ANC-32, or ANC-36); scoring is based on the standard cut-off.  See Section 5: Perinatal Depression for more details.</span>")
 ) %>%
  
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> Denominator is total pregnancies (including all pregnancies where an endpoint has been recorded in MNH09 (labor and delivery) OR MNH04 (pregnancy loss reported during ANC visit) OR MNH19 (pregnancy loss reported during hospitalization) OR if a maternal death is reported prior to delivery), excluding pregnancy losses prior to 20 weeks gestation and those missing data for the outcome.</span>")
 ) %>% 
    
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>f</sup> This summary preeclampsia outcome includes all sub-categories of the condition, including preeclampsia without severe features, preeclampsia superimposed on chronic hypertension, and preeclampsia with severe features.</span>")
 )
...
summary_tab_output2 <- tb_theme1(summary_tab_2) %>% 
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Postpartum outcomes</span>")) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is participants who passed 42 days postpartum without closeout AND have completed a visit 42+ days postpartum OR experienced potentially life-threatening condition (PLTC).</span>")
 ) %>% 
  
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is participants who have progressed in the study through the PNC-6 window, excluding those with missing data for the outcome.</span>")
 )
