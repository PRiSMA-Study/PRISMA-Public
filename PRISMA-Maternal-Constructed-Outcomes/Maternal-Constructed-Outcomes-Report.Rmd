---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Maternal-Outcomes (Issued: 2024 April 10)"
output:
  pdf_document:
    toc: no
    toc_depth: 6
...
mortality <- read_dta(paste0(path_to_data, "mortality_MaternalOutcomes.dta")) %>% rename_all(toupper) 
hemorrhage <- read.csv(paste0(path_to_data, "hemorrhage.csv")) %>% rename_all(toupper) 
depression <- read_dta(paste0(path_to_data, "mnh25_MaternalOutcomes.dta"))
...
**Includes data from synapse last updated:** 2024 April 5
...
**Definition:** Low hemoglobin levels in pregnancy, as based on results from complete blood count (CBC) measures or point-of-care hemoglobin tests. Levels of anemia in pregnancy are defined as: no anemia (>=11 g/dL); mild anemia (>=10 & <11 g/dL); moderate anemia (>=7 & <10 g/dL); severe anemia (< 7 g/dL)
...
**Definition:** Low hemoglobin levels in the postpartum period, as based on results from complete blood count (CBC) measures or point-of-care hemoglobin tests. Levels of anemia in the postpartum period are defined as: no anemia (>=12 g/dL); mild anemia (>=11 & <12 g/dL); moderate anemia (>=8 & <11 g/dL); severe anemia (< 8 g/dL).
...
anemia_pph_output <- tb_theme1(anemia_pph_tab) %>% 
 ...
 row_group_order(groups = c( "<span style='font-size: 18px'>Data completeness at PNC-6 <sup>a</sup></span>",
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Prevalence is based on tests during the PNC-6 late visit window (6-12 weeks postpartum).</span>")) %>% 
  
    tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Prevalence is based on tests during the PNC-26 late visit window (26-39 weeks postpartum).</span>")) %>% 
...
a.  ***NOTE:* we are not considering missed visit windows or if non-schedule routine visit is indicated*
...
depression_complete_tab <- depression %>%
  ...
  summarise(
    ...
    "Visit not completed^a^, n (%)" = paste0(
      ...
      " (",
      format(round(sum(DEPR_ANC20_MISS== 1, na.rm = TRUE)/sum(DEPR_ANC20_D == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Visit completed but no summary score, n (%)" = paste0(
      format(sum(DEPR_ANC20_MISS==-1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_ANC20_MISS==-2, na.rm = TRUE)/sum(DEPR_ANC20_D == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Visit not completed^a^, n (%) " = paste0(
      ...
      " (",
      format(round(sum(DEPR_ANC32_MISS== -2, na.rm = TRUE)/sum(DEPR_ANC32_D== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Visit completed but no summary score, n (%) " = paste0(
      format(sum(DEPR_ANC32_MISS==-1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_ANC32_MISS==-1, na.rm = TRUE)/sum(DEPR_ANC32_D== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Visit not completed^a^, n (%)  " = paste0(
      ...
      " (",
      format(round(sum(DEPR_PNC6_MISS== -2, na.rm = TRUE)/sum(DEPR_PNC6_D== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Visit completed but no summary score, n (%)  " = paste0(
      format(sum(DEPR_PNC6_MISS==-1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_PNC6_MISS==-1, na.rm = TRUE)/sum(DEPR_PNC6_D== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
*For Antepartum hemorrhage: All participants with at least one ANC visit (at least one MNH04 filled).*

*For Postpartum hemorrhage: All participants with an IPC visit (MNH09 filled).* 
...
Hemorrhage_tab <- hemorrhage %>% 
  ...
  ## hemorrhage outcome 
    ...
    "Any hemorrhage at any time point^b^" = paste0(
      ...
      " (",
      format(round(sum(HEM_ANY == 1, na.rm = TRUE)/sum(HEM_PPH_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
Hemorrhage_tab1 <- Hemorrhage_tab[1:12,]
Hemorrhage_tab2 <- Hemorrhage_tab[13:25,]

hemorrhage_output1 <- tb_theme1(Hemorrhage_tab1) %>% 
  tab_header(
    title = md("**Table 6**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Hemorrhage</span>"),
    rows = 1:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Procedures for postpartum hemorrhage <sup>c</sup></span>"),
    rows = 6:11
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Hemorrhage</span>",
                           "<span style='font-size: 18px'>Procedures for postpartum hemorrhage <sup>c</sup></span>")) %>% 
  tab_style(
      style = list(
        cell_text(indent = px(40), size = 24)
        ),
      locations = cells_stub(rows = c('Estimated blood loss >= 1000mL^b^')) ## use on time windows
) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is all participants with at least one ANC visit (at least one MNH04 filled).</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is all participants with an IPC visit (MNH09 filled).</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is all participants with postpartum hemorrhage.</span>")
 )
...
hemorrhage_output2 <- tb_theme1(Hemorrhage_tab2) %>% 
      ...
      tab_row_group(
        label = html("<span style='font-size: 18px'>Methods of blood loss measurements<sup>d</sup></span>"),
        rows = 9:13
      ) %>%
      row_group_order(groups = c("<span style='font-size: 18px'>Medications given to prevent/treat postpartum hemorrhage<sup>b</sup></span>",
                                 "<span style='font-size: 18px'>Methods of blood loss measurements<sup>d</sup></span>")) %>% 
      tab_footnote(
        footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is all participants with an IPC visit (MNH09 filled).</span>")
     ) %>% 
      tab_footnote(
        footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is all participants with a valid blood loss at IPC measurement (varname [form]: `PPH_ESTIMATE_FAORRES` [MNH09]). </span>")
     )
```
...
# knitr::include_graphics(paste0(path_to_data, "hemorrhage_fig.pdf"))
...
ggplot() + 
  geom_histogram(data = hemorrhage_figs, aes(x = M09_PPH_ESTIMATE_FAORRES_6, fill = CUTOFF),  color = "gray", binwidth = 50) +   scale_fill_manual(values = c("darkgreen", "darkorange","darkred"), name = "") + 
  facet_grid(rows = vars(SITE), scales = "free_y") + 
...
**-** Missing diagnosed or measured data (see list of variables in footnotes in Tables 7a and 7b below). 

### Table 7a. STIs
```{r}

## reorder to be output vars and then missing vars 
sti_enroll_tab <- mat_infection_sti %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
    
  ## HIV
    "HIV RDT Positive^a^ [MNH06]" = paste0(
    ...
    "HIV Diagnosed Positive^b^ [MNH04]" = paste0(
    ...
    "HIV Prevalence at Enrollment (Positive RDT or Diagnosed)^c^" = paste0(
    ...
    "HIV RDT Missing^d^ [MNH06]" = paste0(
    ...
    "HIV Diagnosis Missing^e^ [MNH04]" = paste0(
  ...
  # Syphilis
    "Syphilis RDT Positive^a^ [MNH06]" = paste0(
    ...
    "Syphilis Diagnosed Positive^b^ [MNH04]" = paste0(
    ...
    "Syphilis Prevalence, Enrollment (Positive RDT or Diagnosed)^c^" = paste0(
    ...
    "Syphilis Prevalence, Any Visit (Positive RDT or Diagnosed)^e^" = paste0(
    ...
    "Syphilis RDT Missing^d^ [MNH06]" = paste0(
    ...
    "Syphilis Diagnosis Missing^e^ [MNH04]" = paste0(
  ...
  # Gonorrhea
    "Gonorrhea Diagnosed Positive^f^ [MNH04]" = paste0(
    ...
    "Gonorrhea Diagnosis Missing^e^ [MNH04]" = paste0(
  ...
  # Chlamydia
    "Chlamydia Diagnosed Positive^f^ [MNH04]" = paste0(
    ...
    "Chlamydia Diagnosis Missing^e^ [MNH04]" = paste0(
    ...
    # Genital Ulcers
      "Genital Ulcers Diagnosed Positive^f^ [MNH04]" = paste0(
    ...
    "Genital Ulcers Diagnosis Missing^e^ [MNH04]" = paste0(
  ...
  # Other STIs
    "Other STIs Diagnosed Positive^f^ [MNH04]" = paste0(
    ...
    "Other STIs Diagnosis Missing^e^ [MNH04]" = paste0(
    ...
    # Any stis 
      "Any Measured STI Positive^d^ [MNH06]" = paste0(
    ...
    "Any Diagnosed STI Positive^e^ [MNH06]" = paste0(
    ...
    "Any STI (either diagnosed or measured)^c^ [MNH06]" = paste0(
...
## the table is too long -- split into 2 
sti_enroll_tab1 <- sti_enroll_tab[1:11,]
sti_enroll_tab2 <- sti_enroll_tab[12:22,]
...
sti_enroll_output1 <- tb_theme1(sti_enroll_tab1) %>% 
  ...
  row_group_order(groups = c("<span style='font-size: 18px'>HIV, n (%)</span>",
                           "<span style='font-size: 18px'>Syphilis, n (%)</span>")) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants with a valid test result `(TEST_VAR=1 or 0)`. RDT measured variables used in this table from MNH06: `HIV_POC_LBORRES` & `SYPH_POC_LBORRES`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is total participants with a valid diagnosis `(DIAGNOSIS_VAR=1 or 0)`. Diagnosed variables used in this table from MNH04: `HIV_EVER_MHOCCUR`, `SYPH_MHOCCUR`, `GONORRHEA_MHOCCUR`, `CHLAMYDIA_MHOCCUR`, `GENULCER_MHOCCUR`, `STI_OTHR_MHOCCUR`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is total participants with a MNH04 OR MNH06 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is total participants with a MNH06 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> Denominator is total participants with a MNH04 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 )
...
sti_enroll_output2 <- tb_theme1(sti_enroll_tab2) %>% 
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>STIs by Method, n (%)</span>"),
    rows = 9:11
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Gonorrhea, n (%)</span>",
                             ...
                             "<span style='font-size: 18px'>STIs by Method, n (%)</span>"
                             )) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is total participants with a MNH04 OR MNH06 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>f</sup> Denominator is total participants who reported having an 'other' STI (`OTHR_STI_MHOCCUR=1)` AND a valid test `(TEST_VAR=1 or 0)` OR who reported not having an 'other' STI (`OTHR_STI_MHOCCUR=0)`.</span>")
 )
...
### Table 7b. Other infections
```{r}
## reorder to be output vars and then missing vars 
infection_other_tab <- mat_infection_other %>% 
  rowwise() %>% 
  group_by(SITE) %>% 
  summarise(
  ## Malaria
    "Malaria RDT Positive^1^ [MNH06]" = paste0(
    ...
    "Malaria Diagnosed Positive^2^ [MNH04]" = paste0(
    ...
    "Malaria RDT Missing^a^ [MNH06]" = paste0(
    ...
    "Malaria Diagnosis Missing^b^ [MNH04]" = paste0(
  ...
  # Hepatitis B
    "Hep B RDT Positive^1^ [MNH06]" = paste0(
    ...
    "Hep B RDT Missing^a^ [MNH06]" = paste0(
  ...
  # Hepatitis C
    "Hep C RDT Positive^1^ [MNH06]" = paste0(
    ...
    "Hep C RDT Missing^a^ [MNH06]" = paste0(
  ...
  # COVID
    "Covid RDT Positive^1^ [MNH06]" = paste0(
    ...
    "Covid Diagnosed Positive^2^ [MNH04]" = paste0(
    ...
    "Covid RDT Missing^a^ [MNH06]" = paste0(
    ...
    "Covid Diagnosis Missing^b^ [MNH04]" = paste0(
    ...
    # TB 
    "TB Symptoms Positive^c^ [MNH04]" = paste0(
    ...
    "Missing All TB Symptoms^b^ [MNH04]" = paste0(
    ...
    "TB Sputum Test Positive^d^ [MNH08]" = paste0(
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
infection_other_tab[is.na(infection_other_tab)] <- paste0("0 (0)")

## the table is too long -- split into 2 
# sti_enroll_tab1 <- infection_other_tab[1:11,]
# sti_enroll_tab2 <- infection_other_tab[12:22,]

infection_other_output <- tb_theme1(infection_other_tab) %>% 
  tab_header(
    title = md("**Table 7b**")) %>% 
  tab_row_group(
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Covid, n (%)</span>"),
    rows = 9:12
  ) %>%
  tab_row_group(
  ...
  row_group_order(groups = c("<span style='font-size: 18px'>Malaria, n (%)</span>",
                           ...
                           "<span style='font-size: 18px'>Covid, n (%)</span>",
                           "<span style='font-size: 18px'>TB, n (%)</span>"
                           )) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is total participants with a valid test result `(TEST_VAR=1 or 0)`. RDT measured variables used in this table from MNH06: `HIV_POC_LBPERF`, `HBV_POC_LBORRES`, `HCV_POC_LBORRES`, `COVID_POC_LBORRES`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is total participants with a valid diagnosis `(DIAGNOSIS_VAR=1 or 0)`. Diagnosed variables used in this table from MNH04: `MALARIA_EVER_MHOCCUR`, `TB_CETERM_1-4`, `COVID_LBORRES`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is total participants with a MNH06 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is total participants with a MNH04 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> Denominator is total participants with valid response to the TB symptom screen in MNH04 `(VAR==1 OR VAR==0)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>f</sup> Denominator is total participants with who reported having at least one TB symptom in MNH04 `(VAR==1 OR VAR==0)`.</span>")
 )
...
gtsave(infection_other_output, file = "infection_other_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "infection_other_output.png"))
...
### Table 7c. All infections combined 
```{r}
...
all_infection_output <- tb_theme1(all_infection_tab) %>% 
  tab_header(
    title = md("**Table 7c**")) %>% 
  tab_row_group(
    label = html("<span style='font-size: 18px'>Any infection at enrollment, n (%)</span>"),
    rows = 1:3
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>Any infection at enrollment, n (%)</span>")) %>% 
  tab_footnote(
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is total participants with a MNH04 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is total participants with MNH04 OR MNH06 at enrollment filled out `(TYPE_VISIT=1)`.</span>")
 )
```
...
pretest_fast_diabetes <- gdm %>% 
  ...
  summarise(
    ...
    "Missing ANC-28  lab form (MNH08) and no blood glucose test at other visits" = paste0(
      format(sum(DIAB_GEST_FASTING_MISS==2 & COMPLETE_ANC28 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DIAB_GEST_FASTING_MISS==2 & COMPLETE_ANC28 == 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
ogtt_diabetes <- gdm %>% 
  ...
  summarise(
    ...
    "Missing ANC-28  lab form (MNH08) and no blood glucose test at other visits" = paste0(
      format(sum(DIAB_GEST_1HR_MISS==2 & COMPLETE_ANC28 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DIAB_GEST_1HR_MISS==2 & COMPLETE_ANC28 == 1, na.rm = TRUE)/sum(COMPLETE_ANC28 == 1 & DIAB_OVERT != 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
### Table 8d. Gestational diabetes by any test
```{r}
