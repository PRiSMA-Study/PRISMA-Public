---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA-Infant-Outcomes (Issued: 2024 June 18)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
### Table 11a. Infant trajectory outcome at birth

**Denominator:** All living infants with at least completed visits at birth (0 – 5 days) 
(have MNH11 & MNH09)  AND have all anthropometric measures AND have valid z-scores.
...
inf_traj_birth_tab <- infant_trajectory %>% 
  ...
  summarise(
    ...
    "Invalid Z score measure ^b^" = paste0(
...
### Table 11b. Infant trajectory outcome at one month

**Denominator:** All living infants with at least completed visits at birth (21 - 42 days) 
 AND have all anthropometric measures AND have valid z-scores. 
...
### Table 11c. Infant trajectory outcome at six months

**Denominator:** All living infants with at least completed visits at six months (171 - 192 days)
 AND have all anthropometric measures AND have valid z-scores.
...
inf_traj_sixm_tab <- infant_trajectory %>% 
  ...
  summarise(
    ...
    "Missing information on infant length" = paste0(
      ...
      " (",
      format(round(sum(LENGTH_MRSRE_M6 == 0 & VITAL_M6 == 1, na.rm= TRUE)/sum(VITAL_M6==6, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Missing information on infant weight" = paste0(
      format(sum(WEIGHT_MRSRE_M6 == 0 & VITAL_M6 == 1, na.rm=TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(WEIGHT_MRSRE_M6 == 0 & VITAL_M6 == 1, na.rm=TRUE)/sum(VITAL_M6==6, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Missing information on head circumference " = paste0(
      format(sum(HC_MRSRE_M6 == 0 & VITAL_M6 == 1, na.rm=TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HC_MRSRE_M6 == 0 & VITAL_M6 == 1, na.rm=TRUE)/sum(VITAL_M6==6, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
     "Missing at least one anthropometry measurement" = paste0(
      format(sum(MSRE_COMPLT_M6 == 0 & VITAL_M6 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MSRE_COMPLT_M6 == 0 & VITAL_M6 == 1, na.rm = TRUE)/sum(VITAL_M6==6, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Invalid Z Score Measure ^b^" = paste0(
      format(sum(ONE_MSRE_FLAG_M6 == 1 & VITAL_M6 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ONE_MSRE_FLAG_M6 == 1 & VITAL_M6 == 1, na.rm = TRUE)/sum(VITAL_M6==6, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
