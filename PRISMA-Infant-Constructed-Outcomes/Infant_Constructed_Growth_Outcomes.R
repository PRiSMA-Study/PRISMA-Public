...
mat_end_point <- read_dta("~/Analysis/Merged_data/2024-06-28/MAT_ENDPOINTS.dta")

mat_enroll <- read_csv ("~/Analysis/Merged_data/2024-06-28/MAT_ENROLL.csv")

inf_outcomes <- read_csv ("~/Analysis/Merged_data/2024-06-28/INF_OUTCOMES.csv")
...
inf_outcomes_f <- inf_outcomes %>% 
  ...
  mutate (PRETERM = case_when( PRETERMBIRTH_CAT %in% c (12,13,14,15) ~ 1,
                               PRETERMBIRTH_CAT == 11 ~ 2, 
                               PRETERMBIRTH_CAT == 10 ~ 3,
                               TRUE ~ 55))
