...
global date "251125" // today's date
...
////////////////////////////////////////////////////////////////////
	...
	* having preeclampsia: 
		* Relevant dates include: 
		...
		gen SYMPTOM_PROTEIN_LD_DATE = PREG_END_DATE if SYMPTOM_PROTEIN_LD==1 & ///
			(UA_PROT_PRIOR20_LBORRES == 0 | UA_PROT_PRIOR20_LBORRES == 1 | ///
			UA_PROT_PRIOR20_LBORRES == 55 | UA_PROT_PRIOR20_LBORRES == .) & ///
			PREECLAMPSIA == 1 & ///
			(PREG_END_GA >=140)
		...
		gen ORG_FAIL_DATE = min(MAT_DYS_M09_DATE, ORG_FAIL_DATE_M12) if ///
			(MAT_DYS_M09==1 | MAT_DYS_M12==1) & PREECLAMPSIA==1 & SEVERE_FEAT_ORGAN == 1 
		format ORG_FAIL_DATE %td 
			
		tab ORG_FAIL_DATE if PREECLAMPSIA==1 & ///
			(MAT_DYS_M09==1 | MAT_DYS_M12==1) & SEVERE_FEAT_ORGAN == 1 
			
		tab ORG_FAIL_DATE SEVERE_FEAT_ORGAN, m 
			
				// Pulmonary edema if PE only
		tab HDP_PE_DATE_ANC HDP_PE_ANC, m 	
			* drop dates if conditions not met:'
		replace HDP_PE_DATE_ANC = . if SEVERE_FEAT_PULMED != 1
		
		gen SEVERE_PE_LD_DATE = PREG_END_DATE if SEVERE_PE == 1 & ///
			SEVERE_FEAT_PULMED == 1 & PREG_END_GA >=140
		format SEVERE_PE_LD_DATE %td 
				
				// Visual symptoms if PE only 
		tab HDP_VIS_DATE_ANC HDP_VIS_ANC, m 
			* drop dates if conditions not met:'
		replace HDP_VIS_DATE_ANC = . if SEVERE_FEAT_VISUAL != 1			
				
		gen SEVERE_VISUAL_LD_DATE = PREG_END_DATE if ///
			PREECLAMPSIA==1 & SEVERE_FEAT_VISUAL==1 & PREG_END_GA >=140 & ///
		...
		tab SYMPTOM_EPIPAIN_DATE_HOSP SEVERE_FEAT_EPIPAIN, m // DON'T USE THIS DATE
		
		gen SEVERE_FEAT_EPIPAIN_DATE = SYMPTOM_EPIPAIN_DATE_HOSP if ///
			SEVERE_FEAT_EPIPAIN==1 
		format SEVERE_FEAT_EPIPAIN_DATE %td 
		
		tab SEVERE_FEAT_EPIPAIN_DATE SEVERE_FEAT_EPIPAIN, m 
				
				// Severe headache if PE only 
		gen SYMPTOM_HEADACHE_LD_DATE = PREG_END_DATE if PREECLAMPSIA == 1 & ///
			SEVERE_FEAT_HEAD == 1 & SYMPTOM_HEADACHE_LD == 1
		format SYMPTOM_HEADACHE_LD_DATE %td 
		
		tab SYMPTOM_HEADACHE_LD_DATE SEVERE_FEAT_HEAD, m 
	
		* drop dates if conditions not met:'
		replace SYMPTOM_HEADACHE_DATE_HOSP = . if SEVERE_FEAT_HEAD !=1 
		
		tab SYMPTOM_HEADACHE_DATE_HOSP SEVERE_FEAT_HEAD, m 
		
		list SYMPTOM_HEADACHE_LD_DATE SYMPTOM_HEADACHE_DATE_HOSP if ///
			SEVERE_FEAT_HEAD==1
	...
	* Select from dates: 
	...
	replace PREECLAMPSIA_DATE = min(BP_PROT_DATE, SYMPTOM_PROTEIN_LD_DATE, ///
		...
		SEVERE_PE_LD_DATE, HDP_VIS_DATE_ANC, SEVERE_VISUAL_LD_DATE) if PREECLAMPSIA==1 
		
		*NOTE: HERE, WE REMOVE SEVERE HEADACHE & EPIGASTRIC PAIN AS POSSIBLE 
		*DATES OF ONSET; INSTEAD THESE SYMTPOMS MUST OCCUR AFTER PE ONSET AND 
		*SO CANNOT BE THE FIRST DATE. 
		* THE REMOVED VARIABLES INCLUDE: SEVERE_FEAT_EPIPAIN_DATE, 
	...
	* of these cases here: 
	...
	gen PREECLAMPSIA_SEV_DATE = min(sev_symp_temp_date, FIRST_SEV_HIGH_BP_DATE, ///
		...
		PREECLAMPSIA_SEV==1 
	label var PREECLAMPSIA_SEV "Date of severe preeclampsia onset (first date of severe feature)"
