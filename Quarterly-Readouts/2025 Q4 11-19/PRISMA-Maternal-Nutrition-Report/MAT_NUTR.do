...
global datadate "2025-10-31"
	*data upload date
global latest_report_date "2025-10-31"
...
//DIFFERENTIATES MICROCYTIC, NORMOCYTIC, MACROCYTIC ANEMIA
	...
	/*
	could try this code (more human readable)
	
recode CBC_MCV_LBORRES (0/85=1)   (85/97.8=2)    (nonmiss=3) if TRIMESTER == 1, gen(MCV)
recode CBC_MCV_LBORRES (0/85.8=1) (85.8/99.4=2)  (nonmiss=3) if TRIMESTER == 2, gen(MCV_tri2)
recode CBC_MCV_LBORRES (0/82.4=1) (82.4/100.4=2) (nonmiss=3) if TRIMESTER == 3, gen(MCV_tri3)
	
	replace MCV= MCV_tri2 if MCV_tri2!=.
	replace MCV= MCV_tri3 if MCV_tri3!=.
	drop MCV_tri*
	*/
...
**#MICRONUTRIENTS (not quansys)
	...
	*Trimester 1: 56 (ug/dL) 
	*Trimester 2: 50 (ug/dL) 
	*Trimester 3: 50 (ug/dL) 
	*common threshold
	
	gen 	ZINC = 1 if ///
				TRIMESTER == 1 & ZINC_LBORRES < 56 | ///
				TRIMESTER == 2 & ZINC_LBORRES < 50 | ///
				TRIMESTER == 3 & ZINC_LBORRES < 50
	replace ZINC = . if ZINC_LBORRES < 0 
	//missing if default value
	replace ZINC = 2 if ///
				TRIMESTER == 1 & ZINC_LBORRES>=56 & ZINC_LBORRES<. | ///
				TRIMESTER == 2 & ZINC_LBORRES>=50 & ZINC_LBORRES<. | ///
				TRIMESTER == 3 & ZINC_LBORRES>=50 & ZINC_LBORRES<.
	...
	**#8. Folate: red blood cell (RBC) (nmol/L)
	...
	FOL_RBC_CALC>=226.5 
	replace FOL_RBC = 3 if ///
	inrange(FOL_RBC_CALC,906,.)
	...
	**#Folate: any (serum/RBC)
	...
	label define 	FOL_ANY ///
						...
						3"Normal/optimal" ///
						4"Elevated", replace
	label val 		FOL_ANY FOL_ANY
	label var FOL_ANY "Folate status (serum/RBC; RBC prioritized)"
	
	gen FOL_ANY_ANC20 = FOL_ANY if TYPE_VISIT <= 2
	gen FOL_ANY_ANC32 = FOL_ANY if TYPE_VISIT >=4 & TYPE_VISIT <= 5
	gen FOL_ANY_PNC6 = FOL_ANY if TYPE_VISIT == 10
	label val FOL_ANY* FOL_ANY
	
	foreach time in "ANC20" "ANC32" "PNC6" {
		gen FOL_ANY_`time'_D = 1 if inlist(FOL_ANY_`time' ,1,2,3,4)
	}
...
**#Blood lead **ReMAPP Aim 3**
	...
	*/
	...
	label var FERRITIN_LBORRES "Ferritin (ug/L)"
	label var FERRITIN_70 "Low ferritin; <70 if inflammation; <15 if no inflammation"
	label var RBP4_LBORRES "RBP4 (umol/L)"
	label var MCV "Micro/normal/macrocytic"
	
	
	order SITE MOMID PREGID  LBSTDAT TYPE_VISIT  GA_DAYS TRIMESTER PP_DAYS  VITB12_COB_LBORRES VITB12_COB CRP_LBORRES AGP_LBORRES  CRP AGP INFLAMMATION FERRITIN_LBORRES FERRITIN_70 IODINE_LBORRES HIGH_TG_44  TRANSFERRIN_LBORRES STFR RBP4_LBORRES RBP4 CBC_MCV_LBORRES MCV  HB_LBORRES 
...
FERRITIN_CONT_ANC20 FERRITIN_CONT_ANC32 FERRITIN_CONT_PNC6 CRP_CONT_ANC20 CRP_CONT_ANC32 CRP_CONT_PNC6 AGP_CONT_ANC20 AGP_CONT_ANC32 AGP_CONT_PNC6 STFR_CONT_* RBP4_CONT_* VITB12_CONT_* FOL_ANY_* , ///
...
**#Calculate denominators
	...
	foreach var in MCV_ANC32 MCV_ANC32_DENOM AGP_ANC32 AGP_ANC32_DENOM CRP_ANC32 CRP_ANC32_DENOM  FERRITIN70_ANC32 FERRITIN_ANC32_DENOM  HIGH_TG_ANC32 HIGH_TG_ANC32_DENOM   RBP4_ANC32 RBP4_ANC32_DENOM STFR_ANC32 STFR_ANC32_DENOM  VITB12_COB_ANC32 VITB12_COB_ANC32_DENOM  INFLAMMATION_ANC32 INFLAMMATION_ANC32_DENOM QUANSYS_TEST_ANC32 FOL_ANY_ANC32 {
		*replace `var'= . if ANC32_EXP!=1
		replace `var' = 55 if ANC32_EXP==1 & `var' == .
	}

	foreach var in VITB12_COB_PNC6 VITB12_COB_PNC6_DENOM  FERRITIN70_PNC6  FERRITIN_PNC6_DENOM HIGH_TG_PNC6 HIGH_TG_PNC6_DENOM  STFR_PNC6 STFR_PNC6_DENOM RBP4_PNC6 RBP4_PNC6_DENOM CRP_PNC6 CRP_PNC6_DENOM AGP_PNC6 AGP_PNC6_DENOM MCV_PNC6 MCV_PNC6_DENOM INFLAMMATION_PNC6 INFLAMMATION_PNC6_DENOM QUANSYS_TEST_PNC6 FOL_ANY_PNC6 {
		*replace `var' = . if PNC6_EXP!=1
		replace `var' = 55 if PNC6_EXP==1 & `var' == . 
	}
  
  
keep SITE MOMID PREGID ENROLL REMAPP_ENROLL MCV_* VITB12_* FERRITIN* HIGH_TG* STFR* RBP4* CRP* AGP* INFLAMMATION* QUANSYS_TEST* FOL_ANY_* ANC20_EXP ANC32_EXP PNC6_EXP
order SITE MOMID PREGID ENROLL REMAPP MCV_* VITB12_* FERRITIN* HIGH_TG* STFR* RBP4* CRP* AGP* INFLAMMATION* QUANSYS_TEST* FOL_ANY_* ANC20_EXP ANC32_EXP PNC6_EXP 
