*Original: Savannah Omalley
*Updated May 2026 by Emma Cook (emmacook@gwu.edu)

global datadate "2026-05-01"
use "Z:\Outcome Data/$datadate\MAT_NUTR.dta", clear
global output "D:\Users\emmacook\Documents\Maternal Nutrition Report\output/$datadate"
cap mkdir "$output"
*set graph style
...
*Stfr
graph bar stfr_*, by(SITE,legend(pos(6)) title("Prevalence of high sTfR") note("Data from: $datadate")) bar(1, color("0 59 92"%90)) bar(2, color("168 153 104")) bar(3, color("0 156 222")) ylabel(0.2"20%" 0.4"40%" 0.6"60%" 0.8"80%" 1.0"100%")  legend(label(1 "T1") label(2 "T2") label(3 "T3") label(4 "PNC6") ring(1)  col(3) bmargin(small)) legend( col(4) )
