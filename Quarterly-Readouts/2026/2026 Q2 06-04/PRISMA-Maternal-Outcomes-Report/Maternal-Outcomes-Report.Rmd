...
knit: (function(inputFile, encoding) { 
      rmarkdown::render(inputFile,
                    encoding=encoding, 
                    output_file= paste('D:/Users/emmacook/Documents/Maternal_Outcomes_Report/output/2026-05-01/', Sys.Date(),'-Maternal-Outcomes-Report', '.pdf', sep='')) })
...
#* Last updated: May 2026, Emma Cook & Erin Oakley
  #* updated to resolve error caused by gt package version change
  #* this code was run for Q2 2026 readout using version 1.3.0 
  #* 
  #* reduced out.width on tables 7e and 9b to get them to fit on page
#*****************************************************************************
...
#* NOTE: This code requires version 3.2.1 of tibble
...
# Erin's path: 
 # path_to_data <- "Z:/Outcome Data/2026-05-01/" 
...
# Emma's path: 
  ...
  path_to_save <- paste0("D:/Users/emmacook/Documents/Maternal_Outcomes_Report/output/", datadate, "/")

# Precious's path: 
# path_to_data <- "~/Analysis/Outcome Data/2026-05-01/" 
# path_to_save <- paste0("D:/Users/williams_pj/Documents//Analysis/PP_OUTCOMES/", datadate, "/output/")
##path for the output: output_file= paste( 'D:/Users/williams_pj/Documents/Analysis/PP_OUTCOMES/2026-05-01/' , Sys.Date(), '-Maternal-Outcomes-Report.pdf', sep='')
...
placenta<- read_dta(paste0(path_to_data, "MAT_PLACENTA_PREVIA.dta"))  %>% rename_all(toupper) %>% filter(SITE != "")
  #filter(PREGID %in% remind.moms$PREGID)
highhb<- read_dta(paste0(path_to_data, "MAT_HIGHHB.dta"))  %>% rename_all(toupper) %>% filter(SITE != "")
  #filter(PREGID %in% remind.moms$PREGID)
...
#Postpartum outcomes:
 ...
 summary_tab13 <- mortality %>%
  rowwise() %>%
  group_by(SITE) %>%
  summarise(

  ## Maternal deaths
      "Any death,n (per 100,000)" = paste0(
 ...
 summary_tab_2 <- rbind(summary_tab2,summary_tab3,summary_tab4,
  ...
  summary_tab_3 <- rbind(
                      summary_tab10,summary_tab11,summary_tab12, summary_tab13)
...
summary_tab_output <- tb_theme1(summary_tab) %>%
 ...
 row_group_order(groups = c("infection_ever")) %>%
...
summary_tab_output2 <- tb_theme1(summary_tab_2) %>%
 ...
 row_group_order(groups = c("preg_out")) %>%
...
summary_tab_output3 <- tb_theme1(summary_tab_3) %>%
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>Postpartum outcomes</span>"),
        rows = 1:5,
    id = "postpartum_outcomes"
  ) %>%
 row_group_order(groups = c("postpartum_outcomes")) %>%
...
```{r warning=TRUE, out.width='100%'}

gt::gtsave(summary_tab_output, file = "summary_tab_output.png", path = path_to_save, expand=10)
...
```{r}
 ...
 "Death occurred 365+ days postpartum, n" = paste0(
       format(sum(MAT_DEATH_42 == 3, na.rm = TRUE), nsmall = 0, digits = 2)),

    
  "Denominator, n^c^" = paste0(
       format(sum(MAT_MORTALITY_D == 1, na.rm = TRUE), nsmall = 0, digits = 2)),


     "Any death <= 42 days postpartum, n" = paste0(
      format(sum(MAT_DEATH_42 == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_DEATH_42 == 1 , na.rm = TRUE)/sum(MAT_MORTALITY_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
     "Obstetric cause, n^d^" = paste0(
      format(sum(MAT_MORTALITY == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_MORTALITY == 1 , na.rm = TRUE)/sum(MAT_MORTALITY_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2), ")"
      ),
     ...
     "Non-obstetric, n" = paste0(
      ...
      " (",
      format(round(sum(MAT_MORTALITY == 2 , na.rm = TRUE)/sum(MAT_MORTALITY_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
 
     "Unknown/unclear, n^e^" = paste0(
      format(sum(MAT_MORTALITY == 3, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_MORTALITY == 3 , na.rm = TRUE)/sum(MAT_MORTALITY_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
     ...
     "Cause of death missing, n^f^" = paste0(
      ...
      " (",
      format(round(sum(MAT_MORTALITY == 55 , na.rm = TRUE)/sum(MAT_MORTALITY_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
     
##Late maternal mortality
"Denominator, n^g^" = paste0(
       format(sum(MAT_MORTALITY_LATE_D == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
     
    "Any death 42-365 days postpartum, n" = paste0(
      format(sum(MAT_DEATH_42 == 2, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_DEATH_42 == 2 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  
     "Obstetric cause, n^h^ " = paste0(
      format(sum(MAT_MORTALITY_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_MORTALITY_LATE == 1 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
     ...
     "Non-obstetric, n " = paste0(
      ...
      " (",
      format(round(sum(MAT_MORTALITY_LATE == 2 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
 
     "Unknown/unclear, n^e^ " = paste0(
      format(sum(MAT_MORTALITY_LATE == 3, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_MORTALITY_LATE == 3 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
     ...
     "Cause of death missing, n^f^ " = paste0(
      ...
      " (",
      format(round(sum(MAT_MORTALITY_LATE == 55 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
# replace NA with 0s (this likely means there is not yet data)
 ...
 row_group_order(groups = c("death_prisma", "death_preg", "death_pp")) %>%
    # bold rows any death, pregnancy related deaths
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Any death,n (per 100,000)^a^"),
    cells_stub(rows = "Any death,n (per 100,000)^a^")
  )
) %>%
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Any death includes deaths occurring at any time before closeout and for any reason, including accidental/incidental causes. Per 100,000 pregnancy endpoints.</span>")
  ) %>%
  tab_footnote(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>c</sup>Denominator: those who have passed 42 days postpartum without withdrawing or loss to follow up, OR who have finished the follow-up period OR who have died.</span>")
  ) %>%
   tab_footnote(
 ...
 tab_footnote(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>g</sup>Denominator: those who have passed 364 days postpartum without closing out. Note: excludes deaths occurring during pregnancy and < 364 days postpartum.</span>")
  )  %>%
...
row_group_order(
  groups = c("data_complete", "anemia_prev", "test_type")
) %>%
  tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Denominator (non-missing)"),
    cells_stub(rows = "Denominator (non-missing)")
  )) %>%
  # bold denominator rows 2
   tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (completed pregnancies)"),
    cells_stub(rows = "Expected denominator (completed pregnancies)")
  )) %>%
  tab_footnote(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (completed trimester 1 & recruited at <14 weeks GA)"),
    cells_stub(rows = "Expected denominator (completed trimester 1 & recruited at <14 weeks GA)")
  )
)
# TRIMESTER 2
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (continued pregnancy & completed trimester 2)"),
    cells_stub(rows = "Expected denominator (continued pregnancy & completed trimester 2)")
  )
)
...
anemia_tab_tri3_output <- tb_theme1(anemia_tab_tri3) %>%
  ...
  tab_spanner(
    label = html("<span style='font-size: 18px'>Trimester 3 (&ge;28 weeks)</span>"),
    columns = 1:7
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Data completeness</span>"),
  ...
  tab_style(
    style = list(cell_text(weight = "bold")),
    locations = list(
      cells_body(rows = "Expected denominator (continued pregnancy into trimester 3)"),
      cells_stub(rows = "Expected denominator (continued pregnancy into trimester 3)")
    )
  ) %>%
  tab_footnote(
...
knitr::include_graphics(paste0(path_to_data, "anemia-fig.png"))
...
anemia_pph52_tab <- anemia %>%
  ...
  summarise(
    ...
    "Expected denominator (PNC52 window completed, >64 weeks postpartum)" = paste0(
      format(sum(ANEMIA_PNC52_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),

    #Data Completeness:

    "No valid test results in PNC52 window" = paste0(
      format(sum(ANEMIA_PNC52_MISS == 1 & ANEMIA_PNC52_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52_MISS == 1 & ANEMIA_PNC52_DENOM==1, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "No lab/POC forms (MNH06 or MNH08) in PNC52 window" = paste0(
      format(sum(ANEMIA_PNC52_MISS ==2 & ANEMIA_PNC52_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52_MISS ==2 & ANEMIA_PNC52_DENOM==1, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    #Denominator (for non-missing)

    "Denominator (non-missing)" = paste0(
      format(sum(ANEMIA_PNC52_MISS ==0 & ANEMIA_PNC52_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),

    #Values:

    "No anemia (>=12 g/dL)" = paste0(
      format(sum(ANEMIA_PNC52 == 0 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52 == 0 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Mild anemia (>=11 & <12 g/dL)" = paste0(
      format(sum(ANEMIA_PNC52 == 1 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52 == 1 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "Moderate anemia (>=8 & <11 g/dL)" = paste0(
      format(sum(ANEMIA_PNC52 == 2 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52 == 2 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Severe anemia (<8 g/dL)" = paste0(
      format(sum(ANEMIA_PNC52 == 3 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(ANEMIA_PNC52 == 3 & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    #Test type for lowest HB test:

    "CBC Test" = paste0(
      format(sum(HB_LOW_PNC52_TEST=="CBC" & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HB_LOW_PNC52_TEST=="CBC" & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "POC Test" = paste0(
      format(sum(HB_LOW_PNC52_TEST=="POC" & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HB_LOW_PNC52_TEST=="POC" & ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm = TRUE)/sum(ANEMIA_PNC52_DENOM==1 & ANEMIA_PNC52_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
anemia_pph52_tab[is.na(anemia_pph52_tab)] <- paste0("0 (0)")
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (PNC6 window completed, >14 weeks postpartum)"),
    cells_stub(rows = "Expected denominator (PNC6 window completed, >14 weeks postpartum)")
  )
) %>%
  tab_footnote(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (PNC26 window completed, >39 weeks postpartum)"),
    cells_stub(rows = "Expected denominator (PNC26 window completed, >39 weeks postpartum)")
  )
) %>%  tab_footnote(
...
# PNC52 (making a separate table for more space, given longer lines)

anemia_pph52_output <- tb_theme1(anemia_pph52_tab) %>%
  tab_header(
    title = md("**Table 2c continued**")) %>%
  tab_spanner(
    label = html("<span style='font-size: 18px'>52-64 weeks postpartum</span>"),
    columns = 1:7
  ) %>%
 tab_row_group(
  label = html("<span style='font-size: 18px'>Data completeness at PNC52<sup>a</sup></span>"),
  rows = 1:3,
  id = "data_complete"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Anemia prevalence at PNC52 (lowest Hb measure)</span>"),
  rows = 4:8,
  id = "anemia_prev"
) %>%
tab_row_group(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (PNC52 window completed, >64 weeks postpartum)"),
    cells_stub(rows = "Expected denominator (PNC52 window completed, >64 weeks postpartum)")
  )
) %>%  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> This outcome is based on tests collected during the PNC-52 visit window (52-64 weeks postpartum); the data completeness denominator includes any participant with a valid lab test during this window, as well as participants without lab data who have progressed in the study through the PNC-52 window. A small number of participants completed the PNC-52 week visit late (after 64 weeks); these participants are included. Note that participants who had an early pregnancy loss (<20 weeks GA) are excluded from this PNC-52 measure per study follow-up protocol. </span>")) %>%
...
gtsave(anemia_pph52_output, file = "anemia_pph52_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "anemia_pph52_output.png"))
...
row_group_order(
  groups = c("data_complete", "preterm_delivery", "preterm_class")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (total completed pregnancies)"),
    cells_stub(rows = "Expected denominator (total completed pregnancies)")
  )
) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> The expected denominator for this outcome is all completed pregnancies; this includes all pregnancies where an endpoint has been recorded in MNH09 (labor and delivery) OR MNH04 (pregnancy loss reported during ANC visit) OR MNH19 (pregnancy loss reported during hospitalization) OR if a maternal death is reported prior to delivery. </span>"))
...
tab_row_group(
  ...
  rows = 10:19,
  id = "preg_fetal_conditions"
) %>%
row_group_order(
  groups = c("provider_preterm", "maternal_conditions", "preg_fetal_conditions")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Denominator"),
    cells_stub(rows = "Denominator")
  )
) %>%
tab_source_note(
  source_note = html("<span style='font-size: 18px'>* Outcomes indicated with an asterisk (*) are inconsistent with provider-initiated delivery and require further review.</span>")
)
...
row_group_order(
  groups = c("elective", "other", "missing_info")
) %>%
  tab_footnote(
...
tab_style(
  ...
  locations = list(
    cells_body(
      rows = "Denominator (spontaneous preterm)"
    ),
    cells_stub(
      rows = "Denominator (spontaneous preterm)"
    )
...
row_group_order(
  groups = c("data_complete", "pprom_prev")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
depression_complete_tab <- depression %>%
  ...
  summarise(
    ...
    "Expected denominator^a^" = paste0(
      format(sum(DEPR_MISS_TRI_12_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),


    "Missing, n (%)^b^" = paste0(
      format(sum(DEPR_TRI_12_MISS==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_TRI_12_MISS==1, na.rm = TRUE)/sum(DEPR_MISS_TRI_12_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Observed denominator^c^" = paste0(
      format(sum(DEPR_TRI_12_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),

    ## Trimester 3
    "Expected denominator^a^ " = paste0(
      format(sum(DEPR_MISS_TRI_3_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Missing, n (%)^b^ " = paste0(
      format(sum(DEPR_TRI_3_MISS==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_TRI_3_MISS==1, na.rm = TRUE)/sum(DEPR_MISS_TRI_3_DENOM== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Observed denominator^c^ " = paste0(
      format(sum(DEPR_TRI_3_DENOM==1, na.rm = TRUE), nsmall = 0, digits = 2)),
    ...
    "Missing, n (%)^b^  " = paste0(
      format(sum(DEPR_MISS_PNC6<0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_MISS_PNC6<0, na.rm = TRUE)/sum(DEPR_MISS_PNC6_DENOM== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
depression_complete_output <- tb_theme1(depression_complete_tab) %>%
  tab_header(
    title = md("**Table 5a**")) %>%
 tab_row_group(
  label = html("<span style='font-size: 18px'>Data Completeness, Trimester 1–2</span>"),
  rows = 1:3,
  id = "tri12_complete"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Data Completeness, Trimester 3</span>"),
  rows = 4:6,
  id = "tri3_complete"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Data Completeness, 6 weeks postpartum</span>"),
  rows = 7:9,
  id = "pnc6_complete"
) %>%
row_group_order(
  groups = c("tri12_complete", "tri3_complete", "pnc6_complete")
) %>%
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Missing indicates that either the MNH25 form was filled out for that visit type, but the visit was not completed (MAT_VISIT >=3) or the observation was expected and there is no form for that participant.</span>")
 )  %>%
tab_footnote(
...
depression_standard_tab <- depression %>%
  ...
  summarise(
    ...
    "Depressive symptoms, Trimester 1-2, n (%)^c^" = paste0(
      format(sum(DEPR_STND_TRI_12==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_STND_TRI_12==1, na.rm = TRUE)/sum(DEPR_TRI_12_DENOM== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
  ...
  #Trimester 3
    ...
    "Depressive symptoms, Trimester 3, n (%)" = paste0(
      format(sum(DEPR_STND_TRI_3==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_STND_TRI_3==1, na.rm = TRUE)/sum(DEPR_TRI_3_DENOM== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Depressive symptoms, 6-<15 weeks postpartum, n (%)" = paste0(
      format(sum(DEPR_STND_PNC6_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(DEPR_STND_PNC6_NUM==1, na.rm = TRUE)/sum(DEPR_PNC6_DENOM==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
tab_row_group(
  label = html("<span style='font-size: 18px'>Depressive symptoms, 1st & 2nd trimester</span>"),
  rows = 5:6,
  id = "dep_tri12"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Depressive symptoms, 3rd trimester</span>"),
  rows = 7:8,
  id = "dep_tri3"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Depressive symptoms, postpartum</span>"),
  rows = 9:10,
  id = "dep_postpartum"
) %>%
row_group_order(
  groups = c("dep_ever", "dep_anc_ever", "dep_tri12", "dep_tri3", "dep_postpartum")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> ANC, ever (among any woman who has at least one EPDS assessment in pregnancy).</span>")
 )  %>%
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Only Zambia reports depressive symptoms in 1st trimester, so 1st & 2nd trimester have been combined.</span>")
 ) 

```

```{r 5b-export, out.width = '100%'}
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> ANC, ever (among any woman who has at least one assessment in pregnancy).</span>")
 )  %>%
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Only Zambia reports depressive symptoms in 1st trimester, so 1st & 2nd trimester have been combined.</span>")
 ) 
...
tab_row_group(
  ...
  rows = 10:15,
  id = "blood_loss_methods"
) %>%
row_group_order(
  groups = c("hemorrhage", "blood_loss_methods")
) %>%
tab_style(
  style = list(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = c("Denominator", "Denominator ")),
    cells_stub(rows = c("Denominator", "Denominator "))
  )
) %>%
tab_source_note(
  source_note = html("<span style='font-size: 18px'><sup>a</sup> Denominator is all participants with a reported birth outcome.</span>")
) %>%
  tab_footnote(
...
row_group_order(
  groups = c("pph_procedures", "pph_medications")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c(NA, "hiv_group", "syphilis_group")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "N with enrollment visit"),
    cells_stub(rows = "N with enrollment visit")
  )
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
...
row_group_order(
  groups = c("hepb_group", "hepc_group")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c(NA, "tb_group")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "N with GeneXpert performed ever during pregnancy"),
    cells_stub(rows = "N with GeneXpert performed ever during pregnancy")
  )
) %>%
  tab_footnote(
...
row_group_order(
  groups = c("zika_group", "dengue_group")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
zcd_tab_output2 <- tb_theme1(zcd_tab_tab2) %>%
 ...
 tab_row_group(
  ...
  rows = 1:7,
  id = "chikungunya_group"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Leptospirosis, n (%)</span>"),
  rows = 8:14,
  id = "leptospirosis_group"
) %>%
row_group_order(
  groups = c("chikungunya_group", "leptospirosis_group")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
```{r}
## reorder to be output vars and then missing vars
if(!"PREG_END" %in% names(mat_infection)) {

  # PJW ADD: add PREG_END from end dataset if missing in infections
  mat_infection_end <- mat_infection %>%
    left_join(
      end %>%
 ...
 all_infection_tab <- mat_infection_end %>%
  rowwise() %>%
  group_by(SITE) %>%
  summarise(
...
row_group_order(
  groups = c("infection_enroll", "infection_preg")
) %>%
...
row_group_order(
  groups = c("data_complete", "diabetes_prev")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c("data_complete", "gdm_prev")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator (pregnancies continued to 28 weeks gestation)"),
    cells_stub(rows = "Expected denominator (pregnancies continued to 28 weeks gestation)")
  )
) %>%
   tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>a</sup> Expected denominator is total participants who had pregnancy to 28 weeks gestation (excluding pregnancies ended and withdrawals prior to 28 weeks) AND who have progressed through the ANC-28 visit window (>30 weeks gestation).</span>")
...
row_group_order(
  groups = c("data_complete", "gdm_ogtt")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c("data_complete", "gdm_2hr_ogtt")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c("data_complete", "gdm_any_test")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
near_miss_tabb <- near_miss %>%
 ...
 "Any potentially life-threatening condition" = paste0(
    ...
    "Post partum hemorrhage" = paste0(
      format(sum(HEM_PPH_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HEM_PPH_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Pre-eclampsia without severe features " = paste0(
      format(sum(PREECLAMPSIA_NONSEV_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREECLAMPSIA_NONSEV_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Pre-clampsia with severe features/Eclampsia " = paste0(
      format(sum(PREECLAMPSIA_SEV_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREECLAMPSIA_SEV_NEARMISS ==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Severe hypertension " = paste0(
      format(sum(HIGH_BP_SEVERE_ANY_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HIGH_BP_SEVERE_ANY_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    ...
    "Suspected placenta accrete" = paste0(
      format(sum(PLACENTA_ACCRETE_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PLACENTA_ACCRETE_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

   "Suspected placental abruption" = paste0(
      format(sum(PLACENTA_ABRUPTION_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PLACENTA_ABRUPTION_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
   ...
   "Obstructed labor" = paste0(
      format(sum(OBS_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(OBS_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

   "Prolonged labor^a^" = paste0(
      format(sum(PRO_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PRO_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
   ...
   "Severe abortion complications" = paste0(
    ...
    "Pulmonary edema" = paste0(
     ...
     "Severe anemia (<7 g/dL for ANC & IPC; < 8 g/dL for PNC)" = paste0(
       format(sum(ANEMIA_SEV==1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
       " (",
       format(round(sum(ANEMIA_SEV==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
 ...
 "Any potentially life-threatening condition" = paste0(
   ...
   "Prolonged labor^a^" = paste0(
      format(sum(PRO_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1  & SITE %in% c("India-SAS", "Kenya", "Pakistan","Zambia"), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PRO_LABOR_NEARMISS == 1 & NEARMISS_DENOM == 1 & SITE %in% c("India-SAS", "Kenya", "Pakistan","Zambia"), na.rm = TRUE)/sum(NEARMISS_DENOM == 1 & SITE %in% c("India-SAS", "Kenya", "Pakistan","Zambia"), na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Near-miss denominator, n"),
    cells_stub(rows = "Near-miss denominator, n")
  )
) %>%
tab_style(
  style = list(cell_text(color = "grey")),
  locations = cells_body(
    columns = c("Ghana", "India-CMC", "India-SAS", "Kenya", "Pakistan", "Zambia", "Total"),
    rows = 7
  )
) %>%  tab_footnote(
...
near_miss_tabc <- near_miss %>%
  ...
  # Near-miss list:
    ...
    "Hysterectomy" = paste0(
      ...
      "Severe PPH" = paste0(
      format(sum(HEM_PPH_SEV_NEARMISS == 1 & NEARMISS_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HEM_PPH_SEV_NEARMISS==1 & NEARMISS_DENOM == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
tab_style(
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup>Mother needed and/or experienced transfusion AND had one of the following: severe anemia 0-7 days before the transfusion, severe postpartum hemorrhage 0-30 days before transfusion, antepartum hemorrhage 0-30 days before transfusion, or placental abruption 0-7 days before transfusion.</span>")
 ) %>%
  tab_footnote(
...
uter_rup_tab <- rupture %>%
  ...
  summarise(
    ...
    "Uterine rupture" = paste0(
     ...
     "Recorded at L&D^b^" = paste0(
      format(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_IPC==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_IPC==1, na.rm = TRUE)/sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
     "Dx at hospitalization^b^" = paste0(
      format(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_HOSP==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_HOSP==1, na.rm = TRUE)/sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
     "Dx recalled at PNC^b^" = paste0(
      format(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_PNC==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END==1 & MAT_UTER_RUP_MISS==0 & MAT_UTER_RUP_PNC==1, na.rm = TRUE)/sum(PREG_END==1 & MAT_UTER_RUP_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
row_group_order(
  groups = c("data_complete", "uterine_rupture")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  style = list(
    cell_text(indent = px(15), style = "italic")
  ),
  locations = cells_stub(
    rows = c(
      "Recorded at L&D^b^",
      "Dx at hospitalization^b^",
      "Dx recalled at PNC^b^"
    )
  )
) %>%
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> A woman may fall into more than one category (e.g., at L&D and PNC); numbers may add up to >100% of events. </span>"))
...
## 11. Prolonged and obstructed labor
...
row_group_order(
  groups = c("data_complete", "prolonged_labor")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
**Definition:** Clinical diagnosis of obstructed labor as reported by each site. 

**Denominator:** Pregnancies ending in a livebirth or stillbirth (>=20 weeks GA) where the woman experienced labor (i.e., excluding cesarean deliveries without any labor).

**Note:** There may be some differences in definition of obstructed labor by site due to national/local guidelines and site-level interpretation. In general, obstructed labor is understood as arrest of dilation or arrest of descent. India-CMC site specifically has a stricter definition than other sites, requiring that intra-operative diagnosis of signs and symptoms including distended bladder and Bandls ring. This distinction leads to a smaller number of cases at the India-CMC site compared to others. 


### Table 11b. Obstructed labor
```{r}
...
obstr_tab <- labor %>%
  ...
  summarise(
     ...
     "Excluded: Pregnancy losses <20 weeks GA or induced abortion" = paste0(
      format(sum(PREG_END== 1 & (OBS_LABOR_MISS==2), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & (OBS_LABOR_MISS==2), na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Excluded: Maternal deaths at <20 weeks GA" = paste0(
      format(sum(PREG_END== 1 & (OBS_LABOR_MISS==3), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & (OBS_LABOR_MISS==3), na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Excluded: Pregnancies >=20 weeks that ended in cesarean delivery without labor" = paste0(
      format(sum(PREG_END== 1 & (OBS_LABOR_MISS==4), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & (OBS_LABOR_MISS==4), na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "Missing: Unknown if labor occurred or conflicting delivery information" = paste0(
      format(sum(PREG_END== 1 & (OBS_LABOR_MISS==5), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & (OBS_LABOR_MISS==5), na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Missing: Obstructed labor status is missing/unknown" = paste0(
      format(sum(PREG_END== 1 & (OBS_LABOR_MISS==1), na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END== 1 & (OBS_LABOR_MISS==1), na.rm = TRUE)/sum(PREG_END== 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Denominator (non-missing)" = paste0(
      format(sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0, na.rm = TRUE), nsmall = 0, digits = 2)),

    "Obstructed labor" = paste0(
      format(sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0 & OBS_LABOR==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0 & OBS_LABOR==1, na.rm = TRUE)/sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "No obstructed labor" = paste0(
      format(sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0 & OBS_LABOR==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0 & OBS_LABOR==0, na.rm = TRUE)/sum(PREG_END_DELIV==1 & OBS_LABOR_MISS==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
obstr_tab[is.na(obstr_tab)] <- paste0("0 (0)")

obstr_tab_output <- tb_theme1(obstr_tab) %>%
  tab_header(
    title = md("**Table 11b**")) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Data completeness <sup>a</sup></span>"),
...
tab_row_group(
  label = html("<span style='font-size: 18px'>Obstructed labor prevalence</span>"),
  rows = 7:9,
  id = "obstructed_labor"
) %>%
row_group_order(
  groups = c("data_complete", "obstructed_labor")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
gtsave(obstr_tab_output, file = "obstr_tab_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "obstr_tab_output.png"))
...
thy_tab_hyper <- thy %>%
 ...
 ##Trimester 3   
     "Expected, T3^b^" = paste0(
      format(sum(THYR_ANC32_EXP == 1, na.rm = TRUE), nsmall = 0, digits = 2)),

     "Missing  " = paste0(
      format(sum(THYR_T3 == 55 & THYR_ANC32_EXP == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(THYR_T3 == 55 & THYR_ANC32_EXP == 1, na.rm = TRUE)/sum(THYR_ANC32_EXP == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Observed, T3" = paste0(
      format(sum(THYR_T3 < 55, na.rm = TRUE), nsmall = 0, digits = 2)),

       "Overt hyperthyroidism  " = paste0(
      format(sum(THYR_T3 == 3, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(THYR_T3 == 3, na.rm = TRUE)/sum(THYR_T3 < 55, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "Subclinical hyperthyroidism  " = paste0(
      format(sum(THYR_T3 == 2, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(THYR_T3 == 2, na.rm = TRUE)/sum(THYR_T3 < 55, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"), 
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
thy_tab_hyper[is.na(thy_tab_hyper)] <- paste0("0 (0)")

thy_tab_hyper_output <- tb_theme1(thy_tab_hyper) %>%
  tab_header(
    title = md("**Table 12a**")) %>%
tab_row_group(
...
row_group_order(
  groups = c("enrollment", NA, "tri3")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = c("Observed, T1", "Observed, T2", "Observed, T3")),
    cells_stub(rows = c("Observed, T1", "Observed, T2", "Observed, T3"))
  )
) %>%
tab_style(
  style = list(
  ...
  tab_footnote(
    ...
    tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup>Trimester 3 expected: per protocol, thyroid labs are expected at ANC32. Expected participants are those who delivered after 34 weeks (to allow for full ANC32 window) and have passed the late window for ANC36 without closing out. </span>" ))
...
gtsave(thy_tab_hyper_output, file = "thy_tab_hyper_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "thy_tab_hyper_output.png"))

```
\newpage

### Table 12b. Hypothyroidism
```{r}
 ...
 ##Trimester 3   
    ...
    "Observed, T3" = paste0(
      format(sum(THYR_T3 < 55, na.rm = TRUE), nsmall = 0, digits = 2)),
       ...
       "Overt hypothyroidism  " = paste0(
      format(sum(THYR_T3 == 5, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(THYR_T3 == 5, na.rm = TRUE)/sum(THYR_T3 < 55, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "Subclinical hypothyroidism  " = paste0(
      format(sum(THYR_T3 == 4, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(THYR_T3 == 4, na.rm = TRUE)/sum(THYR_T3 < 55, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"), 
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
thy_tab_hypo[is.na(thy_tab_hypo)] <- paste0("0 (0)")

thy_tab_hypo_output <- tb_theme1(thy_tab_hypo) %>%
  tab_header(
    title = md("**Table 12b**")) %>%
 tab_row_group(
  label = html("<span style='font-size: 18px'>Enrollment</span>"),
...
gtsave(thy_tab_hypo_output, file = "thy_tab_hypo_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "thy_tab_hypo_output.png"))
...
row_group_order(
  groups = c("data_complete", "hdp_prev")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  ...
  locations = cells_stub(
    rows = c(
      "Chronic hypertension (<20 weeks GA)",
...
hdp_tab2_output <- tb_theme1(hdp_tab2) %>%
  ...
  rows = 1:12,
  id = "pe_severe_symptoms"
) %>%
row_group_order(
  groups = c("pe_severe_symptoms")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Total cases of preeclampsia with severe features"),
    cells_stub(rows = "Total cases of preeclampsia with severe features")
  )
) %>%
  tab_footnote(
...
row_group_order(
  groups = c("data_complete", "high_bp_pnc6")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
    cells_body(rows = "Expected denominator ^a^"),
    cells_stub(rows = "Expected denominator ^a^")
  )
) %>%
  tab_footnote(
...
row_group_order(
  groups = c("data_complete", "high_bp_pnc26")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
row_group_order(
  groups = c("data_complete", "high_bp_pnc52")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
**Placenta previa:** Placenta partially or fully covers the cervix. Placenta previa is a risk factor for maternal hemorrhage. Identified by ultrasound [`M01_PREVIA_PERES_FTS#`==3] and at labor and delivery [as an indication for C-section, `M09_CES_PRINDC_INF#_18` == 1; as a contributor to antepartum hemorrhage, `M09_APH_FAORRES_2` == 1]. Most placenta previa cases are recorded during ultrasounds, of which nearly 25% are identified in the first ultrasound.
...
placenta1 <- left_join(placenta,end)

placenta_tab <- placenta1 %>%
  rowwise() %>%
  group_by(SITE) %>%
  summarise(
...
"Women with a pregnancy endpoint after 20 weeks" = paste0(

      format(sum(PREG_END == 1 & PREG_END_GA>=140, na.rm = TRUE), nsmall = 0, digits = 2)),
     
     "Suspected placental abruption^b^" = paste0(
      format(sum(PLACENTA_ABRUPTION == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PLACENTA_ABRUPTION == 1, na.rm = TRUE)/sum(PREG_END == 1 & PREG_END_GA>=140, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
#      format(sum(MAT_CES_ANY == 1, na.rm = TRUE), nsmall = 0, digits = 2)),
  ...
  "Suspected placenta accreta indicated as reason for C-section^b^" = paste0(
      ...
      " (",
      format(round(sum(PLACENTA_ACCRETE == 1, na.rm = TRUE)/sum(PREG_END == 1 & PREG_END_GA>=140, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
placenta_tab_output <- tb_theme1(placenta_tab) %>%
  tab_header(
    title = md("**Table 15**")) %>%
 tab_row_group(
  label = html("<span style='font-size: 18px'>Placenta previa</span>"),
  rows = 1:2,
  id = "placenta_previa"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Placental abruption & accreta</span>"),
  rows = 3:5,
  id = "placental_conditions"
) %>%
row_group_order(
  groups = c("placenta_previa", "placental_conditions")
) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup>Denominator is all enrolled women</span>" )) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup>This information can only be captured at L&D; denominator is women who delivered after 20 weeks</span>" )) 
...
row_group_order(
  groups = c("data_complete", "induced_labor")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
## 17. Unplanned surgery

Includes any unplanned surgery that occurs during pregnancy and within 42 days postpartum.

Most events occur at labor and delivery (recorded on MNH09 & MNH10) in the context of interventions for management of postpartum hemorrhage.

Other events may be recorded at labor and delivery or on the hospitalization form (MNH19). These include:


-   Laparotomy (includes "laparotomy" checkbox on MNH19, hysterectomy, and bowel resection)
-   Other unplanned surgery, recorded from free text response variable `M19_TX_PRTRT`  (includes D&C, D&E, hysterotomy)

Additionally, we report the following events:
...
See section 9 for more details on the denominator. 


### Table 17. Surgeries (Planned + Unplanned)
```{r}
nearmiss2 <- left_join(near_miss,labor, join_by(SITE, MOMID,PREGID)) %>% filter(NEARMISS_DENOM==1)


surgery_tab <- nearmiss2 %>%
  rowwise() %>%
  group_by(SITE) %>%
  summarise(
...
"Denominator^a^" = paste0(
  ...
  "Total surgeries, n(%)" = paste0(
      format(sum(UNPLANNED_SURGERY == 1 | EPIS_PROCCUR == 1 | MAT_CES_ANY==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum( UNPLANNED_SURGERY == 1 | EPIS_PROCCUR == 1 | MAT_CES_ANY==1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

 "Planned cesarean, n(%)" = paste0(
      format(sum(MAT_CES_PLAN==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_CES_PLAN==1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),


 "Cesarean, unknown, n(%)" = paste0(
      format(sum(MAT_CES_PLAN==55, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_CES_PLAN==55, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

 "Episiotomy, n(%)" = paste0(
      format(sum(EPIS_PROCCUR==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(EPIS_PROCCUR==1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

 "Total unplanned surgeries, n(%)" = paste0(
      format(sum(UNPLANNED_SURGERY == 1 | MAT_CES_EMERGENT==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(UNPLANNED_SURGERY == 1 | MAT_CES_EMERGENT==1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

 "Emergent cesarean, n(%)" = paste0(
      format(sum(MAT_CES_EMERGENT==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_CES_EMERGENT==1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

# "Other unplanned surgeries, n(%)" = paste0(
#      format(sum(UNPLANNED_SURGERY == 1, na.rm = TRUE), nsmall = 0, digits = 2),
#     " (",
#      format(round(sum(UNPLANNED_SURGERY == 1  , na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
#      ")"),

 "Intervention for PPH, n(%)^b^" = paste0(
      format(sum(PPH_PROC_IPC_SURG == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PPH_PROC_IPC_SURG == 1, na.rm = TRUE)/sum(NEARMISS_DENOM == 1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
"Laparotomy, n(%)^c^" = paste0(
      format(sum(LAPAROTOMY == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(LAPAROTOMY == 1, na.rm = TRUE)/sum(NEARMISS_DENOM, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

"Hysterectomy, n(%)" = paste0(
      format(sum(HYSTERECTOMY == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HYSTERECTOMY == 1, na.rm = TRUE)/sum(NEARMISS_DENOM, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

"Miscellaneous unplanned surgery, n(%)^d^" = paste0(
      format(sum(RSN_PPH_PROC_IPC_VESSEL == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(RSN_PPH_PROC_IPC_VESSEL == 1, na.rm = TRUE)/sum(NEARMISS_DENOM, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
surgery_tab[is.na(surgery_tab)] <- paste0("0 (0)")

surgery_tab_output <- tb_theme1(surgery_tab) %>%
  tab_header(
    title = md("**Table 17**")) %>%
   ...
   tab_style(
      style = list(
        cell_text(indent = px(10), style = "italic")
        ),
      locations = cells_stub(rows = c(
        'Emergent cesarean, n(%)',
        'Intervention for PPH, n(%)^b^',
        'Laparotomy, n(%)^c^',
        'Miscellaneous unplanned surgery, n(%)^d^'))
      ) %>%
  
  tab_style(
      style = list(
        cell_text(indent = px(25))
        ),
      locations = cells_stub(rows = c(
        'Hysterectomy, n(%)'))
) %>%
...
tab_style(
   ...
   tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup>Denominator is women who have passed 42 days, minus those who are healthy and have closed out early, are missing MNH09/MNH12/MNH19 form, or who have not been seen 42+ days postpartum. See section 9 for more details.</span>" )) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup>Surgical interventions for PPH include vessel ligation, brace sutures, and other unspecified surgical intervention.</span>" ))  %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup>Laparotomy includes laparotomy indicated on MNH19, hysterectomy, and bowel resection.</span>" ))  %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup>Miscellaneous unplanned surgery includes D&C and D&E.</span>" )) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>Note: </sup>Participants may fall into more than one category, e.g., hysterectomy (recorded in M19) + surgical intervention for PPH (recorded in M09).</span>" ))
...
gtsave(surgery_tab_output, file = "surgery_tab_output.png", path = path_to_save, expand = 10)
knitr::include_graphics(paste0(path_to_save, "surgery_tab_output.png"))

```

\newpage

## 18. Maternal High Hemoglobin in Pregnancy

**Definition:** High hemoglobin levels in pregnancy, as based on results from complete blood count (CBC) measures or point-of-care (POC) hemoglobin tests. Hemoglobin level is based on the highest recorded hemoglobin measure in pregnancy, adjusted for smoking status and elevation as applicable. Categorical hemoglobin levels in pregnancy are defined as:
...
-   Normal HB (>=11 & <13 g/dL);

-   Low HB (mild anemia) (>=10 & <11 g/dL)

-   Low HB (moderate anemia) (>=7 & <10 g/dL)

-   Low HB (severe anemia) (<7 g/dL)
...
-   Normal HB (>=10.5 & <13 g/dL);

-   Low HB (mild anemia) (>=9.5 & <10.5 g/dL)

-   Low HB (moderate anemia) (>=7 & <9.5 g/dL)
...
### Table 18. High hemoglobin in pregnancy
```{r}

highhb<-left_join(highhb,end)
...
highhb_tab <- highhb %>%
  ...
  summarise(
    ...
    "No valid test results" = paste0(
      format(sum(HBCAT_ANC_MISS == 1 & PREG_END==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC_MISS == 1 & PREG_END==1, na.rm = TRUE)/sum(PREG_END==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "No lab/POC forms (MNH06 or MNH08)" = paste0(
      format(sum(HBCAT_ANC_MISS ==2 & PREG_END==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC_MISS ==2 & PREG_END==1, na.rm = TRUE)/sum(PREG_END==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    #Denominator (for non-missing)

    "Denominator (non-missing)" = paste0(
      format(sum(HBCAT_ANC_MISS ==0 & PREG_END==1, na.rm = TRUE), nsmall = 0, digits = 2)),

    #Values:

    "Very High HB (>=15 g/dL)" = paste0(
      format(sum(HBCAT_ANC == 1 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 1 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "High HB (13 to <15 g/dL)" = paste0(
      format(sum(HBCAT_ANC == 2 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 2 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

     "Normal HB" = paste0(
      format(sum(HBCAT_ANC == 3 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 3 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "Low HB (mild anemia)" = paste0(
      format(sum(HBCAT_ANC == 4 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 4 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Low HB (moderate anemia)" = paste0(
      format(sum(HBCAT_ANC == 5 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 5 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "Low HB (severe anemia)" = paste0(
      format(sum(HBCAT_ANC == 6 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HBCAT_ANC == 6 & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    #Test type for lowest HB test:

    "CBC Test" = paste0(
      format(sum(HB_HIGH_ANC_TEST=="CBC" & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HB_HIGH_ANC_TEST=="CBC" & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),

    "POC Test" = paste0(
      format(sum(HB_HIGH_ANC_TEST=="POC" & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(HB_HIGH_ANC_TEST=="POC" & PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm = TRUE)/sum(PREG_END==1 & HBCAT_ANC_MISS ==0, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
...
```{r}
# replace NA with 0s (this likely means there is not yet data)
highhb_tab[is.na(highhb_tab)] <- paste0("0 (0)")

highhb_output <- tb_theme1(highhb_tab) %>%
  tab_header(
    title = md("**Table 18**")) %>%
  tab_row_group(
  label = html("<span style='font-size: 18px'>Data completeness <sup>a</sup></span>"),
...
tab_row_group(
  label = html("<span style='font-size: 18px'>High hemoglobin prevalence (highest Hb measure)</span>"),
  rows = 4:10,
  id = "high_hb_prev"
) %>%
tab_row_group(
  label = html("<span style='font-size: 18px'>Test type (for highest Hb measure)</span>"),
  rows = 11:12,
  id = "test_type"
) %>%
row_group_order(
  groups = c("data_complete", "high_hb_prev", "test_type")
) %>%
tab_style(
  style = list(cell_text(weight = "bold")),
  locations = list(
...
tab_style(
  ...
  tab_footnote(
   ...
   tab_footnote(
   footnote = html("<span style='font-size: 18px'><sup>Note:</sup> For each observation of high Hb category, we rely on CBC measures of hemoglobin or POC measures of hemoglobin that are not within 7 days of a CBC test. For each participant, this table presents the highest Hb status observed during the window of interest.</span>")
 )
```

```{r, out.width = '100%'}

gtsave(highhb_output, file = "highhb_output.png", path = path_to_save, expand=10)
knitr::include_graphics(paste0(path_to_save, "highhb_output.png"))
