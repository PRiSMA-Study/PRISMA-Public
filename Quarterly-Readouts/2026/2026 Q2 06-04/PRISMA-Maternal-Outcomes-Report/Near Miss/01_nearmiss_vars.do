...
**March 24, 2026: Transferred to E Oakley (emoakley@gwu.edu)
clear
set more off
cap log close
...
log using "$log/construct_near-miss_$date", replace
	
	global data "D:\Users\emoakley\Documents\Outcome Data/$dadate"
	global wrk "D:\Users\emoakley\Documents\Outcome Data/$dadate/nearmiss"
		cap mkdir "$wrk"
		cd "$wrk"
...
**# MNH04 - ANC clinical status
	...
	*generate a long and wide file of those who reported hospitalization
	...
	import excel "$da/mnh04_merged", first case(preserve)
	
	save "$data/mnh04", replace
	*/
...
**# MNH09 - Labor and Delivery		
	...
	*/
	...
	import excel "$da/mnh09_merged", first case(preserve)
	
	save "$data/mnh09", replace
	*/
	...
	**#!! Note to Erin: suggest to review and incorporate missing/NA/don't know  (55/77/99)
	...
	**#PPH Surgical intervention
	replace PPH_SPFY_FAORRES = strlower(PPH_SPFY_FAORRES)
	gen PPH_OTHR_SURG = 1 if ///
	regexm(PPH_SPFY_FAORRES,"sutur") | ///
	regexm(PPH_SPFY_FAORRES,"mva") 
	...
	**#Indicator woman completed MNH09
	...
	keep SITE MOMID PREGID MAT_VITAL_MNH09 MAT_VISIT_MNH09 MAT_VISIT_OTHR_MNH09 MAT_LD_OHOSTDAT DELIV_DSSTDAT_INF1 MAT_DEATH_DTHDAT ORG_FAIL_HRT ORG_FAIL_RESP ORG_FAIL_RENAL ORG_FAIL_LIVER ORG_FAIL_NEUR ORG_FAIL_UTER ORG_FAIL_HEM ORG_FAIL_MHOCCUR_88 ORG_FAIL_SPFY_MHTERM ORG_FAIL_M09 CHK_ORG_FAIL_SPFY MISS_SOURCE PPH_TRNSFSN_PROCCUR PLACENTA_ACCRETE PLACENTA_ABRUPTION M09 HYSTERECTOMY_M09 PULMONARY_EDEMA_M09 EPIS_PROCCUR PPH_OTHR_SURG
	order SITE MOMID PREGID MAT_VITAL_MNH09 MAT_VISIT_MNH09 MAT_VISIT_OTHR_MNH09 MAT_LD_OHOSTDAT DELIV_DSSTDAT_INF1 MAT_DEATH_DTHDAT ORG_FAIL_HRT ORG_FAIL_RESP ORG_FAIL_RENAL ORG_FAIL_LIVER ORG_FAIL_NEUR ORG_FAIL_UTER ORG_FAIL_HEM ORG_FAIL_MHOCCUR_88 ORG_FAIL_SPFY_MHTERM ORG_FAIL_M09 CHK_ORG_FAIL_SPFY MISS_SOURCE PLACENTA_ACCRETE PLACENTA_ABRUPTION HYSTERECTOMY_M09 PULMONARY_EDEMA_M09 EPIS_PROCCUR PPH_OTHR_SURG
...
**# MNH10 - Postdelivery maternal outcome
	...
	*/
	...
	import excel "$da/mnh10_merged", first case(preserve)
	
	save "$data/mnh10", replace
	*/
...
**# MNH19 Hospitalization
	...
	*/
	...
	import excel "$da/mnh19_merged", first case(preserve)
	
	save "$data/mnh19", replace
	*/
	...
	**#Other surgery
	replace TX_PRTRT = strlower(TX_PRTRT)
	replace TX_PRTRT = "" if TX_PRTRT=="n/a"
	gen SURGERY_OTHER_M19 = 1 if regexm( TX_PRTRT, "d&c")
	replace SURGERY_OTHER_M19 = 1 if ///
	regexm( TX_PRTRT, "dilatation") | regexm( TX_PRTRT, "curettage")
	*replace SURGERY_OTHER_M19 = 1 if regexm( TX_PRTRT, "hysterotomy")
		//Blair said hysterotomy is not specific enough
	replace SURGERY_OTHER_M19 = 1 if regexm( TX_PRTRT, "de&e")
		*appears to be dilation and expulsion (2nd tri abortion)
	
	*these need to be confirmed with clinicians:
	*replace SURGERY_OTHER_M19 = 1 if regexm( TX_PRTRT, "mac donald stitch")
		//Blair said should not be included
	*replace SURGERY_OTHER_M19 = 1 if regexm( TX_PRTRT, "encerclage")
		//Blair said should not be included
	gen SURGERY_OTHER_M19_DT = HOSP_DATE if SURGERY_OTHER_M19 == 1
...
///Merge MNH19 to MNH09 to create near-miss.dta
	...
	*We only want to keep these outcomes if they occured within 42 days PP
	...
	assert SURGERY_OTHER_M19_DT !=. if SURGERY_OTHER_M19==1
	 rename *_DT *_dt
	 
	 *in case of multiple near-miss events, pick the first:	 
	 foreach var in M19 ORG_FAIL_M19 TRANSFUSION_M19 LAPAROTOMY_M19 SURGERY_OTHER_M19 PULM_EDEMA_M19 HYSTERECTOMY_M19 ABORT_COMPL_M19 ENDOMETRITIS_M19 {
	 	bysort PREGID : egen `var'_DT = min(`var'_dt)
	 }
	 
	 drop *_dt
	///COLLAPSE
	collapse (max) ORG_FAIL_M19 ORG_FAIL_HRT_M19 ORG_FAIL_RESP_M19 ORG_FAIL_RENAL_M19 ORG_FAIL_LIVER_M19 ORG_FAIL_NEUR_M19 ORG_FAIL_UTER_M19 ORG_FAIL_HEM_M19 ORG_FAIL_OTHR_M19 TRANSFUSION_M19 LAPAROTOMY_M19 SURGERY_OTHER_M19 WOUNDCARE_M19 VENTILATION_M19 CHK_INTERVENTION_ORGFAIL_M19 ADMIT_INPATIENT_M19 REFERRED_M19 M19 within42 after42 PULM_EDEMA_M19 ENDOMETRITIS_M19 ABORT_COMPL_M19 HYSTERECTOMY_M19 (firstnm) ORG_FAIL_SPFY_M19 TX_PRTRT (min) M19_DT ORG_FAIL_M19_DT TRANSFUSION_M19_DT LAPAROTOMY_M19_DT SURGERY_OTHER_M19_DT PULM_EDEMA_M19_DT HYSTERECTOMY_M19_DT ABORT_COMPL_M19_DT ENDOMETRITIS_M19_DT , by(SITE MOMID PREGID)
...
**# MNH12 Maternal PNC Clinical Status
	...
	*/
	...
	import excel "$da/mnh12_merged", first case(preserve)
	
	save "$data/mnh12", replace
	*/
	...
	**#Determine date of organ failure, hysterectomy, and hospitalization	
		...
		*update 9-Dec-2025: we will query ALL organ failures, with or without MNH19
		/*
...
**#!! Note to Erin: these queries were never sent to sites (since MNH19 not always filled/able to be filled). Potentially a place to review 
	*THIS QUERY IS MNH19 MATCHED TO MNH12 (postnatal visit)
	*Query will pull MNH12 reports with no matched hospital form (if she had an organ failure, hysterectomy, hospitalization with no matched form)
		...
		**Query # 3: MNH19 exists but outside min/max range
		gen merge_note = "MNH19 outside range" if _merge == 3 & query_hosprange == 1
...
use "$wrk/mnh12_wide.dta",clear
	...
	save "$wrk/near-miss-prelim.dta",replace
	disp as result "Code #1 completed - proceed to code #2"	
