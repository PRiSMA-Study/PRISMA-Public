...
*Update: June 18, 2025 by S O'Malley (Fixed error handling length of momid; this was previously causing some momids to be truncated and eventually dropped from the dataset due to non-merging)
*Update: March 4, 2026 by E Cook  (updated directories)
...
global dadate "2026-05-01" // UPLOAD DATE 
global runquery = 0

*Stacked Data Folders (TNT)
global da "D:/Users/emmacook/Documents/Raw stacked data/$dadate" // update date as needed
global OUT "Z:\Emma_working_files\Endpoints/$dadate"

*Emma's working files (TNT)
global wrk "Z:\Emma_working_files\Endpoints/$dadate" // set pathway here for where you want to save output data files (i.e., constructed analysis variables)
// must manually create Endpoints folder if it doesn't exist
	
cap mkdir "$wrk" //automatically creates date folder if it doesn't exist
cd "$wrk"	
global queries "$wrk/queries"
cap mkdir "$queries" //automatically creates queries folder if it doesn't exist
...
*Step 0: check for duplicates in MAT_ENROLL:
	...
	save "Z:/Outcome Data/$dadate/MAT_ENROLL", replace 
	keep if ENROLL==1
...
//////////////////////////////////////////
	...
	*A few situations with multiple pregnancy end dates, take the first:
	bysort momid pregid ( PREG_END_DATE_MNH04 ): gen dupnum = _n
	keep if dupnum == 1
	drop dup*
	isid momid pregid
	...
	*indicator for early pregnancy loss < 13 weeks: 
	...
	rename m19_dthdat m19_dthdat_str
	gen m19_dthdat = date(m19_dthdat_str, "YMD")
	gen MAT_DEATH_M19 = 1 if m19_mat_arrival_dsdecod ==2 | m19_visit_faorres ==5 | m19_admit_dsterm==3 | inrange( m19_dthdat,0,.)
	gen MAT_DEATH_DATE_M19 = m19_dthdat if MAT_DEATH_M19 == 1
	...
	*clean up dataset: 
	...
	global recastlength = strlen(momid) + 1
	*format momid %38s
	recast str$recastlength momid
		*Added June 18, 2025 by SFO
...
*** INCORPORATE CLOSEOUT FORM:
	...
	*check for duplicates: 
	...
	list site momid CLOSEOUT_TYPE CLOSEOUT_DT if duplicate >=1
	*In May 30 data set, this is a single momid with two closeouts both for maternal deat
	*in this case, take the first instance
	bysort pregid (CLOSEOUT_DT) : gen closeout_order = _n
	list site momid CLOSEOUT_TYPE CLOSEOUT_DT closeout_order if duplicate >=1
	drop if momid == "KEARC02163" & closeout_order > 1
	drop closeout_order 
	...
	*Generate a GA at closeout: 
	...
	gen CLOSEOUT_GA = CLOSEOUT_DT - PREG_START_DATE if ///
		...
		replace CLOSEOUT_GA = CLOSEOUT_DT - PREG_START_DATE if PREG_END==0
			//Added Dec-18-2025 by SF OMalley, we were losing these GAs if woman closed out before pregnancy end
...
**Bring in MNH12
	...
	**COLLAPSE
	...
	/*
	*Skip the MNH37 code for now, because the data is spotty 
...
if $runquery == 1 {
		...
		*check when last seen alive and date when death was recorded
		format VISIT_LAST_ALIVE_MNH09 %td
...
//step 3: code if within 42 days
	...
	MAT_DEATH_DATE <= PREG_END_PP42_DT & MAT_DEATH == 1
	replace MAT_DEATH_42 =2 if /// occured >42 days pp
	MAT_DEATH_DATE > PREG_END_PP42_DT & MAT_DEATH_INFAGE<=365 & MAT_DEATH == 1
	replace MAT_DEATH_42 = 3 if ///
	MAT_DEATH_DATE > PREG_END_PP42_DT & inrange(MAT_DEATH_INFAGE,365,.)
	replace MAT_DEATH_42 = 55 if /// missing
	MAT_DEATH_DATE<0 & MAT_DEATH == 1
	...
	1"Preg-related death" 2"Late preg-related death" ///
	3 "Death > 365 days PP" 55"Missing DoD", replace
	...
	*Closeout_type == "1-Follow-up complete (1 yr)" but closeout days postparum < 300 days
	cap bigtab SITE CLOSEOUT_TYPE if CLOSEOUT_TYPE==1 & CLOSEOUT_DAYS_PP<300
	...
	*missing site variables
	...
	list SITE PREGID PREG_END_GA PREG_END_DATE  if  (PREG_END_GA ==. | PREG_END_DATE ==.)  & PREG_END == 1
	...
	*which sites have this error?
	...
	if $runquery == 1 {
		...
		gen month_pregend= month(PREG_END_DATE)
		gen year_pregend = year(PREG_END_DATE)
		
	levelsof(SITE) if PREG_END_GA < 0 & PREG_END == 1, local(sitelev) clean 
	*which sites have this error?
		
		foreach site of local sitelev {
			*for each site with the error, export the following excel:
					
		export excel SITE MOMID PREGID PREG_END_GA PREG_END_SOURCE PREG_START_DATE  month_pregend year_pregend  using ///
		"$queries/`site'-endpoints-queries-$dadate.xlsx" if ///
		SITE=="`site'" & PREG_END_GA < 0 & PREG_END == 1, ///
		sheet("Negative-PregEnd-GA",modify)  firstrow(variables) 
			
		}
		
		}
	
	*If PREG_END_GA < 0 or PREG_END_DATE is missing, set to "missing"
	replace PREG_END = 55 if (PREG_END_DATE == . | PREG_END_GA < 0) & PREG_END == 1
	replace PREG_END_DATE = date("1905-05-05", "YMD") if PREG_END == 55
	replace PREG_END_GA = -5 if PREG_END == 55
	
	keep if inlist(PREG_END,0,1)
	...
	*Save to outcomes folder once reviewed:
	...
	if $runquery == 1 {
		...
		use "Z:\Emma_working_files\Endpoints/$dadate\REVIEW_EMPTY_MNH09_FORMS.dta", clear
...
//////////////////////////////////////////////
	...
	*/
	...
	use "$wrk/MAT_ENDPOINTS", clear
	
	tab PREG_END SITE,m
	tab MAT_DEATH SITE,m //confirm # deaths look plausible
