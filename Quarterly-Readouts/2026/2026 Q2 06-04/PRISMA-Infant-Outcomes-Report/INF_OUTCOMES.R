...
#* Last updated: 12 Februry 2025 (updated bilirubin unit conversions for sites who updated)
...
hyperbili <- inf_baseline %>% 
  ...
  # units conversion 
  mutate(M14_TCB_UMOLL_LBORRES = ifelse(SITE %in% c("Kenya", "Zambia", "India-SAS", "Pakistan") & !M14_TCB_UMOLL_LBORRES %in% c(-5,-7), M14_TCB_UMOLL_LBORRES/17.1, M14_TCB_UMOLL_LBORRES),
         M11_TBILIRUBIN_UMOLL_LBORRES = ifelse(SITE %in% c("Kenya", "Zambia", "India-SAS", "Pakistan") & !M11_TBILIRUBIN_UMOLL_LBORRES %in% c(-5,-7), M11_TBILIRUBIN_UMOLL_LBORRES/17.1, M11_TBILIRUBIN_UMOLL_LBORRES),
  )
...
# test_pak <- hyperbili %>% filter(SITE == "Pakistan")  ## umol/L --> mg/dl
...
## note: going to to try and harmonize this in future data uploads (will share email with sites)
