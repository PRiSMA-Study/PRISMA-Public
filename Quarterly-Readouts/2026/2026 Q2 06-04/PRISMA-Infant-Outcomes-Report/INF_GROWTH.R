...
clean_date <- function(x, min_year = 2018) {
  
  if (is.numeric(x)) {
    d <- as.Date(x, origin = "1899-12-30")
    d[lubridate::year(d) < min_year] <- as.Date(NA)
    return(d)
  }
  ...
  yr <- lubridate::year(d)
  d[!is.na(d) & yr < min_year] <- as.Date(NA)
...
make_datetime <- function(date, time) {
  as.POSIXct(
    paste(as.character(date), clean_time(time)),
    format = "%Y-%m-%d %H:%M",
    tz = "UTC"
  )
}
...
clean_time <- function(x) {
  bad_times <- c("77:77", "99:99", "55:55", "55:55:00", "")
  
  x <- as.character(x)
  
  x[x %in% bad_times | is.na(x)] <- "23:59"
  
  x
}

#*****************************************************************************
#1: Set my directories, load data and create enrolment based datasets ----
#*****************************************************************************
#Store all outcome data locally (reduces rerunning this all the time
...
# Set path to save all datasets being created 
path_to_save <- paste0("~/Analysis/Infant-Constructed-Variables/data/", UploadDate, "/")
print (path_to_save)
#Create a path to queries
base_queries_dir <- "~/Analysis/Infant-Constructed-Variables/queries"
path_to_queries <- file.path(base_queries_dir, UploadDate)
dir.create(path_to_queries, recursive = TRUE, showWarnings = FALSE)

#Set path to tnt drive 
path_to_tnt <- paste0("Z:/Outcome Data/", UploadDate, "/")
path_to_stacked <- paste0("Z:/Stacked Data/", UploadDate)

## Import stacked data, outcome data and save locally----
###INF_OUTCOMES ----
...
### MNH09 ----
if (file.exists(file.path(path_to_data, "mnh09.rda"))) {
  load(file.path(path_to_data, "mnh09.rda"))           # loads: mnh09
} else {
  mnh09 <- read.csv(file.path(path_to_stacked, "mnh09_merged.csv"))
  ...
  save(mnh09, file = file.path(path_to_data, "mnh09.rda"))
}

### MNH11 ----
if (file.exists(file.path(path_to_data, "mnh11.rda"))) {
  load(file.path(path_to_data, "mnh11.rda"))           # loads: mnh11
} else {
  mnh11 <- read.csv(file.path(path_to_stacked, "mnh11_merged.csv"))
  df_names <- ls(pattern = "^mnh", envir = .GlobalEnv)
  ...
  save(mnh11, file = file.path(path_to_data, "mnh11.rda"))
}
...
#*****************************************************************************
# 2: Create Date of Birth dataset ----
#*****************************************************************************
mnh09_sub <- mnh09 %>%
  ...
  # Clean delivery dates using your standardized function
  mutate(
    DELIV_DSSTDAT_INF1 = clean_date(DELIV_DSSTDAT_INF1),
    DELIV_DSSTDAT_INF2 = clean_date(DELIV_DSSTDAT_INF2),
    DELIV_DSSTDAT_INF3 = clean_date(DELIV_DSSTDAT_INF3),
    DELIV_DSSTDAT_INF4 = clean_date(DELIV_DSSTDAT_INF4),
    
    SEX_INF1 = as.numeric(SEX_INF1),
    SEX_INF2 = as.numeric(SEX_INF2),
    SEX_INF3 = as.numeric(SEX_INF3),
    SEX_INF4 = as.numeric(SEX_INF4)
  ) %>%
  
  # Replace default value time with NA 
  mutate(
    DELIV_DSSTTIM_INF1 = na_if(DELIV_DSSTTIM_INF1, "77:77"),
    DELIV_DSSTTIM_INF2 = na_if(DELIV_DSSTTIM_INF2, "77:77"),
    DELIV_DSSTTIM_INF3 = na_if(DELIV_DSSTTIM_INF3, "77:77"),
    DELIV_DSSTTIM_INF4 = na_if(DELIV_DSSTTIM_INF4, "77:77")
  ) %>%
  
  # Convert time to ITime (robust to NA)
  mutate(
    # replace default value time with NA
    DELIV_DSSTTIM_INF1 = replace(as.character(DELIV_DSSTTIM_INF1), DELIV_DSSTTIM_INF1 == "77:77", NA),
    DELIV_DSSTTIM_INF2 = replace(as.character(DELIV_DSSTTIM_INF2), DELIV_DSSTTIM_INF2 == "77:77", NA),
    DELIV_DSSTTIM_INF3 = replace(as.character(DELIV_DSSTTIM_INF3), DELIV_DSSTTIM_INF3 == "77:77", NA),
    DELIV_DSSTTIM_INF4 = replace(as.character(DELIV_DSSTTIM_INF4), DELIV_DSSTTIM_INF4 == "77:77", NA)
  ) %>%
  mutate(
    # concatenate date + time
    DELIVERY_DATETIME_INF1 = paste(DELIV_DSSTDAT_INF1, DELIV_DSSTTIM_INF1),
    ...
    # parse datetime (only works when both date + time exist)
    DELIVERY_DATETIME_INF1 = as.POSIXct(DELIVERY_DATETIME_INF1, format = "%Y-%m-%d %H:%M", tz = "UTC"),
    DELIVERY_DATETIME_INF2 = as.POSIXct(DELIVERY_DATETIME_INF2, format = "%Y-%m-%d %H:%M", tz = "UTC"),
    DELIVERY_DATETIME_INF3 = as.POSIXct(DELIVERY_DATETIME_INF3, format = "%Y-%m-%d %H:%M", tz = "UTC"),
    DELIVERY_DATETIME_INF4 = as.POSIXct(DELIVERY_DATETIME_INF4, format = "%Y-%m-%d %H:%M", tz = "UTC"),
    
    # fallback: if time missing (or parse fails), keep date-only as POSIXct
    DELIVERY_DATETIME_INF1 = if_else(
      is.na(DELIVERY_DATETIME_INF1) & !is.na(DELIV_DSSTDAT_INF1),
      as.POSIXct(DELIV_DSSTDAT_INF1, tz = "UTC"),
      DELIVERY_DATETIME_INF1
    ),
    DELIVERY_DATETIME_INF2 = if_else(
      is.na(DELIVERY_DATETIME_INF2) & !is.na(DELIV_DSSTDAT_INF2),
      as.POSIXct(DELIV_DSSTDAT_INF2, tz = "UTC"),
      DELIVERY_DATETIME_INF2
    ),
    DELIVERY_DATETIME_INF3 = if_else(
      is.na(DELIVERY_DATETIME_INF3) & !is.na(DELIV_DSSTDAT_INF3),
      as.POSIXct(DELIV_DSSTDAT_INF3, tz = "UTC"),
      DELIVERY_DATETIME_INF3
    ),
    DELIVERY_DATETIME_INF4 = if_else(
      is.na(DELIVERY_DATETIME_INF4) & !is.na(DELIV_DSSTDAT_INF4),
      as.POSIXct(DELIV_DSSTDAT_INF4, tz = "UTC"),
      DELIVERY_DATETIME_INF4
    )) %>% 
  filter(!is.na(INFANTID_INF1)) %>% 
  distinct(SITE, MOMID, PREGID, .keep_all = TRUE)


## Get Date of Birth, Sex and Birth Outcome for Each ID ----
...
mnh11_constructed <- mnh11 %>% 
  mutate(
    ...
    WEIGHT_SOURCE = case_when(WEIGHT_PRISMA > 0 ~ "PRISMA",
                                   WEIGHT_ANY > 0 & WEIGHT_PRISMA < 0 ~ "FACILITY",
                                   TRUE ~ "MISSING_WEIGHT"), 
    
    M11_VISIT_OBSSTDAT = clean_date (VISIT_OBSSTDAT),
    
    M11_VISIT_OBSSTTIM = clean_time (VISIT_OBSSTTIM),
    
    M11_DATETIME = make_datetime(M11_VISIT_OBSSTDAT, M11_VISIT_OBSSTTIM),
  ...
  MNH11_HAVE = 1, 
  #Basically visit is complete if they have any one of the measurement or if visit complete is marked
  VISIT_COMPLETE = if_else ((HC_MRSRE == 1 | 
                             LENGTH_MRSRE == 1 |  
                             WEIGHT_MRSRE == 1 | 
                             INF_VISIT_MNH11 %in% c(1,2,3)), 1, 0),
  TYPE_VISIT = 6 ) %>% 
  mutate(
  ...
  select(
    SITE, MOMID, PREGID, INFANTID, M11_VISIT_OBSSTDAT, MNH11_HAVE, INF_DSTERM,
    M11_DATETIME, WEIGHT_PRISMA, WEIGHT_ANY, WEIGHT_SOURCE, WEIGHT_MRSRE, 
    BW_EST_FAORRES, BW_FAORRES_REPORT, BW_FAORRES,
    ...
    SEX_INF, INF_VITAL_MNH11, M11_DATE_MRSRE, TYPE_VISIT, VISIT_COMPLETE) 

## Create a dataset which joins MNH09 & MNH11 together ----
allbirth_anthro <- living_infants %>% 
  ...
  mutate( 
    # AGE_CALC = case_when( WEIGHT_SOURCE == "PRISMA" & !is.na(BW_EST_FAORRES) &
    #                       BW_EST_FAORRES >= 0 & BW_EST_FAORRES <= 72 ~ round(BW_EST_FAORRES / 24, 0),
    #   
    #   TRUE ~ round(as.numeric(difftime(M11_VISIT_OBSSTDAT, DOB_Date, units = "days")), 0)
    # ),
    # AGE_CALC = round(as.numeric (difftime (M11_VISIT_OBSSTDAT, DOB_Date, units = "days")), 0),
    # AGE_IN_WKS = round(as.numeric (difftime  (M11_VISIT_OBSSTDAT, DOB_Date, units = "weeks")), 0),
    
    M11_VISIT_OBSSTDAT = as.Date(M11_VISIT_OBSSTDAT),
    DOB_Date = as.Date(trunc(DOB, 'days')),
    ANTHRO_DATETIME = case_when( WEIGHT_SOURCE == "PRISMA" & !is.na(BW_EST_FAORRES) &
                                   BW_EST_FAORRES >= 0 & BW_EST_FAORRES <= 72 ~ 
                                   DOB + lubridate::hours(BW_EST_FAORRES),
                                 TRUE ~ M11_DATETIME ),
    AGE_HOURS_TOTAL = as.numeric( difftime(ANTHRO_DATETIME, DOB, units = "hours")),
    AGE_HOURS = AGE_HOURS_TOTAL %% 24,
    AGE_CALC = AGE_HOURS_TOTAL %/% 24,
    AGE_IN_WKS = AGE_HOURS_TOTAL %/% (24 * 7),
    WEIGHT = case_when(!is.na(WEIGHT_ANY) & WEIGHT_ANY > 0 ~ round(WEIGHT_ANY / 1000, 2),
                    TRUE ~ NA),
    AGE_IN_DAYS = ifelse ((AGE_CALC > 5 | AGE_CALC < 0 ), NA, AGE_CALC),
  ...
  select(SITE, MOMID, PREGID, INFANTID, AGE_IN_DAYS, AGE_CALC, AGE_HOURS_TOTAL, M11_VISIT_OBSSTDAT,
         INF_VITAL_MNH11, INF_DSTERM, M11_DATETIME, DOB, DOB_Date, EDD_BOE, PREG_GA_CALC, everything()) %>% 
  distinct(SITE, PREGID, INFANTID, TYPE_VISIT, .keep_all = TRUE)

test <- allbirth_anthro %>% filter (SITE == "India-SAS") %>%
  filter (is.na (AGE_IN_DAYS))
  #filter (AGE_CALC > 5 | AGE_CALC < 0 | is.na(AGE_CALC))

# test3 <- test2 %>% anti_join(test, by = c("SITE", "MOMID", "PREGID", "INFANTID"))

## Birth Previous Queries replies from sites ----
# Read a specific sheet
site_query_response <- read_xlsx(
  path = file.path( paste0("Z:/Outcome Data/", PrevDate, "/Infant Outcome Query Responses.xlsx")),
  sheet = "ANTHRO"
)

birth_query_response <- site_query_response %>% filter (TYPE_VISIT == 6) %>%
  mutate(VISIT_OBSSTDAT = clean_date(VISIT_DATE))

##Queries Dataset MNH11----
###Age ----
q1_age <- allbirth_anthro %>% filter (is.na(AGE_IN_DAYS)) %>% 
  select (SITE, MOMID, PREGID, INFANTID, AGE_CALC, DOB_Date, M11_DATETIME) %>%
  left_join(infant_dob, by = c("SITE", "MOMID", "PREGID", "INFANTID")) %>%
  left_join(mnh11 %>% select (SITE, MOMID, PREGID, INFANTID, VISIT_OBSSTDAT, VISIT_OBSSTTIM), 
            by = c("SITE", "MOMID", "PREGID", "INFANTID"))  %>%
   mutate (
    QUERY_GROUP = "QUERY 1: AGE",
    QUERY = case_when(
      is.na (VISIT_OBSSTDAT) | VISIT_OBSSTDAT == "" ~ "MNH11 visit date missing (cannot compute age)",
      !is.na(AGE_CALC) & AGE_CALC < 0 ~ paste0("Age <0 days"),
      !is.na(AGE_CALC) & AGE_CALC > 5 ~ paste0("Age >5 days"),
      TRUE ~ "Other reasons")) %>% 
  anti_join(birth_query_response, by = c("SITE", "MOMID", "PREGID", "INFANTID", "QUERY")) %>% 
  transmute(SITE, MOMID, PREGID, INFANTID, 
            AGE = AGE_CALC, VISIT_OBSSTTIM,
            VISIT_OBSSTDAT = as.character (VISIT_OBSSTDAT),
            INFANT_DOB = as.character (DOB),
            M11_DATETIME = as.character (M11_DATETIME),
            DOB_Date,
            QUERY) 

q1_age 

###Birthweight Missingness----
birthweight_missingness <- mnh11_constructed %>% 
  filter (PREGID %in% enrolled_ids_vec & INFANTID %in% living_infants$INFANTID)  %>%
  filter (is.na(WEIGHT_ANY)| !(INF_DSTERM %in% 1) | WEIGHT_ANY < 0 )  %>% 
  mutate ( 
    # helpers (explicitly encode validity)
  prisma_bw_valid   = !is.na(BW_FAORRES) & BW_FAORRES > 0,
  facility_bw_valid = !is.na(BW_FAORRES_REPORT) & BW_FAORRES_REPORT > 0,

  time_missing_or_invalid = is.na(BW_EST_FAORRES) | BW_EST_FAORRES %in% c(-5, -7) | BW_EST_FAORRES < 0,
  time_ge_72              = !is.na(BW_EST_FAORRES) & BW_EST_FAORRES >= 72,
  time_lt_72              = !is.na(BW_EST_FAORRES) & BW_EST_FAORRES >= 0 & BW_EST_FAORRES < 72,
  
  # main reason variable (ONLY meaningful when weight is missing)
  bw_missing_reason = case_when(
    
    is.na (MNH11_HAVE) & !(INFANTID %in% c(mnh11$INFANTID)) ~ "MNH11 form is missing",
    
    # 1) MNH11 date missing (often indicates incomplete birth visit record)
    M11_DATE_MRSRE == 0 ~ "MNH11 visit date missing",
    
    # 1.5) MNH11 infant living status mismatch with mnh09 (mnh09 livebirth vs mnh11 death/unknown status)
    !(INF_DSTERM %in% 1) ~ "MNH11 birth outcome conflicts with MNH09 livebirth",
    
    # 2) No usable BW anywhere
    (!prisma_bw_valid & !facility_bw_valid & INF_DSTERM == 1) ~ "No valid BW in PRISMA or facility fields (BW<0)",
    
    # 3) PRISMA BW exists but time >=72 AND facility not usable
    (prisma_bw_valid & time_ge_72 & !facility_bw_valid) ~ "PRISMA BW recorded but measured >=72h; facility BW missing",
    
    # 4) PRISMA BW exists but time missing/invalid (still usable for WEIGHT_PRISMA per your rule),
    # but WEIGHT_ANY ended up missing => usually means PRISMA got dropped elsewhere or BW got overwritten
    (prisma_bw_valid & time_missing_or_invalid & !facility_bw_valid) ~ "PRISMA BW recorded but time missing/invalid; facility BW missing",
    
    # 5) Facility BW exists but WEIGHT_ANY still missing => logic/typing issue
    (facility_bw_valid & (is.na(WEIGHT_ANY) | WEIGHT_ANY <= 0)) ~ "Facility BW valid but WEIGHT_ANY not populated (logic/type issue)",
    
    TRUE ~ "Other / needs review"))

table (birthweight_missingness$SITE, birthweight_missingness$bw_missing_reason)

tb_bw_reason_by_site <- birthweight_missingness %>%
  count(SITE, bw_missing_reason, name = "n") %>%
  group_by(SITE) %>%
  ungroup() %>%
  arrange(SITE, desc(n))

tb_bw_reason_by_site

tb_bw_reason_comp <- birthweight_missingness %>%
  count(SITE, bw_missing_reason, name = "n") %>%
  group_by(SITE) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup()

ggplot(tb_bw_reason_comp, aes(x = SITE, y = pct, fill = bw_missing_reason)) +
  geom_col() +
  geom_text(
    aes(label = n),
  ...
  labs(
    x = "Site",
    y = "Composition of birthweight missingness",
    title = "Reasons for missing birthweight, by site",
    fill = "Missingness reason"
  )



#BW Query dataset
bw_missing_queries <- birthweight_missingness %>%
  transmute(SITE, MOMID, PREGID, INFANTID, QUERY = bw_missing_reason) %>% # keep relevant columns
  anti_join(birth_query_response, by = c("SITE", "MOMID", "PREGID", "INFANTID", "QUERY")) 
  
# Keep MNH11 variable names needed
mnh11_query_df <- mnh11 %>%
  select(SITE, MOMID, PREGID, INFANTID,
         INF_DSTERM, VISIT_OBSSTDAT,
         BW_FAORRES, BW_EST_FAORRES, BW_FAORRES_REPORT)

q2_bw_missing  <- bw_missing_queries %>%
  left_join(mnh11_query_df, by = c("SITE", "MOMID", "PREGID", "INFANTID")) 


### HC missingness ----
q3_hc_missing <- allbirth_anthro %>%
  filter(is.na(MNH11_HAVE) | HC_MRSRE == 0 | is.na(HC_FAORRES) | HC_FAORRES <= 0) %>%
  mutate(
    QUERY_GROUP = "QUERY 3: HEAD CIRCUMFERENCE",
    QUERY = case_when(
      is.na(MNH11_HAVE) & !(INFANTID %in% (mnh11$INFANTID)) ~ "MNH11 form is missing",
      M11_DATE_MRSRE == 0 ~ "MNH11 visit date missing (HC cannot be verified)",
      is.na(HC_FAORRES) |HC_FAORRES <= 0 ~ "HC missing (<=0)",
      TRUE ~ "Other reason")) %>% 
  select(SITE, MOMID, PREGID, INFANTID, QUERY, starts_with("HC_F")) %>% # keep relevant columns
  anti_join(birth_query_response, by = c("SITE", "MOMID", "PREGID", "INFANTID", "QUERY")) 
  
  

###Length missingness ----
q4_length_missing <- allbirth_anthro %>%
  filter(is.na(MNH11_HAVE) | LENGTH_MRSRE == 0 | is.na(LENGTH) | LENGTH <= 0) %>%
  mutate (
    QUERY_GROUP = "QUERY 4: LENGTH",
    QUERY = case_when(
      is.na(MNH11_HAVE) & !(INFANTID %in% (mnh11$INFANTID)) ~ "MNH11 form is missing",
      M11_DATE_MRSRE == 0 ~ "MNH11 visit date missing (length cannot be verified)",
      is.na(LENGTH) | LENGTH <= 0 ~ "Length is missing (<=0)",
      TRUE ~ "Other reason")) %>% 
  select(SITE, MOMID, PREGID, INFANTID, QUERY, LENGTH, starts_with("LENGTH_F")) %>% # keep relevant columns
  anti_join(birth_query_response, by = c("SITE", "MOMID", "PREGID", "INFANTID", "QUERY")) 

### Queries export file a  ----
#list of query tables already built
q2_bw_export <- q2_bw_missing %>%
  transmute(
    SITE, MOMID, PREGID, INFANTID,
    QUERY_BIRTHWEIGHT = QUERY,
    INF_DSTERM, VISIT_OBSSTDAT = as.character(VISIT_OBSSTDAT),
    BW_FAORRES, BW_EST_FAORRES, BW_FAORRES_REPORT
  )

q3_hc_export <- q3_hc_missing %>%
  select(
    SITE, MOMID, PREGID, INFANTID,
    QUERY_HC = QUERY,
    starts_with("HC_F")
...
q4_length_export <- q4_length_missing %>%
  select(
    SITE, MOMID, PREGID, INFANTID,
    QUERY_LENGTH = QUERY,
    LENGTH, starts_with("LENGTH_F")
  )

q_anthro <- q2_bw_export %>%
  full_join(q3_hc_export, 
            by = c("SITE", "MOMID", "PREGID", "INFANTID")) %>%
  full_join(q4_length_export, 
            by = c("SITE", "MOMID", "PREGID", "INFANTID")) %>%
...
#*****************************************************************************
#4: Postnatal Visit Anthropometry Dataset ---- 
#*****************************************************************************
## Create mnh13_sub dataframe ----
mnh13_constructed <- mnh13 %>% 
  ...
  mutate(
    DOB_Date = as.Date(trunc(DOB, 'days')), 
    M13_VISIT_OBSSTDAT = clean_date(VISIT_OBSSTDAT),  # ensures consistent Date format
  ...
  mutate(
   #change weight to kg from grams
    WEIGHT = case_when(!is.na(WEIGHT_PERES) & WEIGHT_PERES > 0 ~ round(WEIGHT_PERES / 1000, 2),
                       TRUE ~ NA),
    WEIGHT_SOURCE = ifelse(WEIGHT >= 0.01 & !is.na (WEIGHT), "PP_VISIT", "MISSING_WEIGHT"),
    ...
    VISIT_COMPLETE = if_else ((HC_MRSRE == 1 | 
                              LENGTH_MRSRE == 1 |  
                              WEIGHT_MRSRE == 1 | 
                              INF_VISIT_MNH13 %in% c(1,2,3)), 1, 0),
  ...
  ungroup() %>%
  select (SITE, MOMID, PREGID, INFANTID, TYPE_VISIT, AGE_IN_DAYS, M13_VISIT_OBSSTDAT, WEIGHT,WEIGHT_SOURCE, WEIGHT_PERES, 
          LENGTH, LENGTH_PERES_1, LENGTH_PERES_2, LENGTH_PERES_3, HC_FAORRES, HC_PERES_1, HC_PERES_2, HC_PERES_3,
          INF_VISIT_MNH13, INF_VITAL_MNH13, HC_MRSRE, WEIGHT_MRSRE, LENGTH_MRSRE, VISIT_COMPLETE) %>% 
  distinct(SITE, PREGID, INFANTID, AGE_IN_DAYS, .keep_all = TRUE) 

## Postnatal previous queries replies from sites ----
visit_query_response <- site_query_response %>% filter (TYPE_VISIT %in% c(7:14)) %>%
  mutate(M13_VISIT_OBSSTDAT = clean_date(VISIT_DATE))

## MNH13 missingness queries ----
###Weight missingness (MNH13) ----
q13_weight_missing <- mnh13_constructed %>%
  filter(VISIT_COMPLETE == 1 & WEIGHT_MRSRE == 0 & INF_VISIT_MNH13 == 1 ) %>%
  filter ((AGE_IN_DAYS >= 21 & AGE_IN_DAYS <= 42) | (AGE_IN_DAYS >= 171 & AGE_IN_DAYS <= 222) ) %>%
  select(SITE, MOMID, PREGID, INFANTID, AGE_IN_DAYS, WEIGHT_MRSRE, WEIGHT, M13_VISIT_OBSSTDAT) %>% 
  left_join(
    mnh13 %>% 
      transmute(
        SITE, MOMID, PREGID, INFANTID, 
        TYPE_VISIT, INF_VISIT_MNH13, INF_VITAL_MNH13,
        WEIGHT_PERES,  
        M13_VISIT_OBSSTDAT = clean_date(VISIT_OBSSTDAT)
      ),
    by = c("SITE", "MOMID", "PREGID", "INFANTID", "M13_VISIT_OBSSTDAT")
  ) %>%
  mutate(
    QUERY = case_when(
      # NEW: completed visit but infant not alive
      INF_VISIT_MNH13 %in% c(1:3) & !is.na(INF_VITAL_MNH13) & INF_VITAL_MNH13 != 1 ~
        "Visit marked completed but infant is not alive",
      is.na(M13_VISIT_OBSSTDAT) ~ "MNH13 visit date missing",
      is.na(WEIGHT_PERES) | WEIGHT_PERES <= 0 ~ "Weight missing",
      !is.na(WEIGHT) & WEIGHT < 0.01 ~ "Weight is invalid",
      TRUE ~ "Other / needs review"
    )
  ) %>%
  anti_join(visit_query_response, 
            by = c("SITE", "MOMID","PREGID","INFANTID",
                   "TYPE_VISIT","M13_VISIT_OBSSTDAT","QUERY")) %>% 
  transmute(
    SITE, MOMID, PREGID, INFANTID,
    VISIT_OBSSTDAT = as.character (M13_VISIT_OBSSTDAT),
    TYPE_VISIT, INF_VISIT_MNH13, INF_VITAL_MNH13,
    WEIGHT, WEIGHT_PERES,
    QUERY_WEIGHT = QUERY
  )

### Length/Height missingness (MNH13) ----
q13_length_missing <- mnh13_constructed %>%
  filter(VISIT_COMPLETE == 1 & LENGTH_MRSRE == 0 & INF_VISIT_MNH13 == 1) %>%
  filter ((AGE_IN_DAYS >= 21 & AGE_IN_DAYS <= 42) | (AGE_IN_DAYS >= 171 & AGE_IN_DAYS <= 222) ) %>%
  select(SITE, MOMID, PREGID, INFANTID, AGE_IN_DAYS, LENGTH_MRSRE, LENGTH, M13_VISIT_OBSSTDAT, TYPE_VISIT) %>% 
  left_join(
    mnh13 %>%
      mutate(M13_VISIT_OBSSTDAT = clean_date(VISIT_OBSSTDAT)) %>%
      select(
        SITE, MOMID, PREGID, INFANTID,
        TYPE_VISIT, INF_VISIT_MNH13, INF_VITAL_MNH13,
        starts_with("LENGTH_"),
    ...
    by = c("SITE", "MOMID", "PREGID", "INFANTID", "M13_VISIT_OBSSTDAT","TYPE_VISIT")
  ) %>%
  mutate(
    QUERY = case_when(
      # NEW: completed visit but infant not alive
      INF_VISIT_MNH13 %in% c(1:3) & !is.na(INF_VITAL_MNH13) & INF_VITAL_MNH13 != 1 ~
        "Visit marked completed but infant is not alive",
      is.na(M13_VISIT_OBSSTDAT) ~ "MNH13 visit date missing",
      is.na(LENGTH) | LENGTH <= 0 ~ "Length missing",
      !is.na(LENGTH) & LENGTH < 0.01 ~ "Length is invalid",
      TRUE ~ "Other / needs review"),
    VISIT_OBSSTDAT = as.character (M13_VISIT_OBSSTDAT),
  ) %>%
  anti_join(visit_query_response, 
  ...
  select(
    SITE, MOMID, PREGID, INFANTID,
    VISIT_OBSSTDAT,TYPE_VISIT, 
    INF_VISIT_MNH13, INF_VITAL_MNH13,
    LENGTH, starts_with("LENGTH_"),
    QUERY_LENGTH = QUERY
  )

### HC missingness (MNH13)---
q13_hc_missing <- mnh13_constructed %>%
  filter(VISIT_COMPLETE == 1 & HC_MRSRE == 0 & INF_VISIT_MNH13 == 1) %>%
  filter ((AGE_IN_DAYS >= 21 & AGE_IN_DAYS <= 42) | (AGE_IN_DAYS >= 171 & AGE_IN_DAYS <= 222) ) %>%
  select(SITE, MOMID, PREGID, INFANTID, AGE_IN_DAYS, HC_MRSRE, HC_FAORRES, M13_VISIT_OBSSTDAT) %>% 
  left_join(
    mnh13 %>%
      mutate(M13_VISIT_OBSSTDAT = clean_date(VISIT_OBSSTDAT)) %>%
      select(
  ...
  mutate(
    QUERY = case_when(
      ...
      INF_VISIT_MNH13 %in% c(1:3) & !is.na(INF_VITAL_MNH13) & INF_VITAL_MNH13 != 1 ~
        "Visit marked completed but infant is not alive",
      is.na(M13_VISIT_OBSSTDAT) ~ "MNH13 visit date missing",
      is.na(HC_FAORRES) | HC_FAORRES <= 0 ~ "Head circumference is missing",
      TRUE ~ "Other / needs review"),
    VISIT_OBSSTDAT = as.character (M13_VISIT_OBSSTDAT)
  ) %>%
  anti_join(visit_query_response, 
...
q_mnh13_anthro <- q13_weight_missing %>%
  full_join(q13_length_missing,
            by = c("SITE","MOMID","PREGID","INFANTID", "TYPE_VISIT", 
                   "INF_VISIT_MNH13", "INF_VITAL_MNH13","VISIT_OBSSTDAT")) %>%
  full_join(q13_hc_missing,
            by = c("SITE","MOMID","PREGID","INFANTID", "TYPE_VISIT", 
                   "INF_VISIT_MNH13", "INF_VITAL_MNH13", "VISIT_OBSSTDAT")) %>%
  arrange(SITE, MOMID, PREGID, INFANTID)

### Queries export file b ----
query_list <- list(
  QUERY_AGE    = q1_age,
  QUERY_ANTHRO = q_anthro,
  QUERY_MNH13  = q_mnh13_anthro
)

# styles (same as yours)
red_style <- createStyle(fontColour = "#000000", bgFill = "#FFC7CE", textDecoration = "bold")
...
all_sites <- sort(unique(bind_rows(query_list)$SITE))

for (s in all_sites) {
  
  wb <- createWorkbook()
  s_safe <- str_replace_all(as.character(s), "[^A-Za-z0-9_\\-]+", "_")
  
  for (nm in names(query_list)) {
    
    df <- query_list[[nm]] %>%
      filter(SITE == s) %>%
      arrange(MOMID, PREGID, INFANTID)
    
    addWorksheet(wb, nm)
    
    # ✅ Use writeData (NOT writeDataTable) to avoid table theme weirdness
    writeData(wb, nm, df, headerStyle = header_style)
    
    if (ncol(df) > 0) setColWidths(wb, nm, cols = 1:ncol(df), widths = "auto")
    freezePane(wb, nm, firstRow = TRUE)
    
    # 🔴 Highlight query cells (only) using conditionalFormatting
    if (nm == "QUERY_ANTHRO" && nrow(df) > 0) {
      
      qcols <- which(names(df) %in% c("QUERY_BIRTHWEIGHT","QUERY_HC", "QUERY_LENGTH"))
      
      for (qc in qcols) {
        conditionalFormatting(
          wb, nm,
          cols  = qc,                      # one column at a time
          rows  = 2:(nrow(df) + 1),
          type  = 'notBlanks',
          style = red_style
        )
    ...
    # 🔴 Highlight query cells (only) using conditionalFormatting
    if (nm == "QUERY_MNH13" && nrow(df) > 0) {
      
      qcols <- which(names(df) %in% c("QUERY_WEIGHT", "QUERY_HC", "QUERY_LENGTH"))
  ...
  out_path <- file.path(
    path_to_queries,
    paste0(s_safe, "_Inf_Anthropometry_Queries_", UploadDate, ".xlsx")
  )
...
# test2 <- wide_infant_trajectories %>% 
#   filter (TYPE_VISIT == 6 & SITE == "Kenya") %>% 
#   filter (is.na(AGE_IN_DAYS))

## Create allvisit_anthro dataframes ----
allvisit_anthro <- mnh13_constructed %>%
  #filter only enrolled moms
  filter (PREGID %in% enrolled_ids_vec) %>%
  #filter where visit is complete
  filter (VISIT_COMPLETE == 1)  %>%
...
#*****************************************************************************
#5: Merge all Birth + Postnatal Visit Datasets ---- 
#*****************************************************************************

##Prepare birth and visit dataframes with Z scores for merging ----
for_merge_mnh11_raw <- tbirth_measure %>% 
...
for_merge_mnh13_raw <- tvisit_measure %>% 
  select(SITE, MOMID, PREGID, INFANTID, TYPE_VISIT, VISIT_DATE, AGE_IN_DAYS, PREG_GA_CALC, AGE_IN_WKS, WEIGHT, LENGTH, HC_FAORRES, 
         HC_MRSRE, WEIGHT_MRSRE, LENGTH_MRSRE,
         VITAL = INF_VITAL_MNH13, FLEN = flen, FWFL = fwfl, FWEI = fwei, ZLEN = zlen, FHC = fhc, ZWFL = zwfl, ZWEI = zwei, ZHC = zhc,
         ZLEN_IG, ZWEI_IG, ZWFL_IG, ZHC_IG, FLEN_IG, FWEI_IG, FWFL_IG, FHC_IG )

## Merge completed births and postnatal visits with at least one Z scores ----
merged_anthro <- bind_rows(for_merge_mnh11_raw, for_merge_mnh13_raw)
names (merged_anthro)
gc()
...
## Add completeness score using existing indicators ----
#Identify best (most complete) row per infant-age group
merged_anthro_scored <- merged_anthro %>%
  mutate(
    row_id = row_number(),
    completeness_score = HC_MRSRE + WEIGHT_MRSRE + LENGTH_MRSRE,
    prefer_birth_visit = if_else(TYPE_VISIT == 6, 1L, 0L)
  )
# If completeness_score ties, prefer TYPE_VISIT == 6 (Visit 6)
chosen_dups <- merged_anthro_scored %>%
  group_by(SITE, MOMID, PREGID, INFANTID, AGE_IN_DAYS) %>%
  arrange(desc(completeness_score), 
          desc(prefer_birth_visit), 
          TYPE_VISIT,
          .by_group = TRUE) %>%  # lower TYPE_VISIT comes first (6 < 7)
  slice(1) %>%
  ungroup() %>%
...
## Final de-duplicated dataset ----
merged_anthro_dedup <- chosen_dups %>% select(-HC_MRSRE, -WEIGHT_MRSRE, -LENGTH_MRSRE, -AGE_IN_DAYS)

### Other Age Query Issues ----
...
all_data_birth <-  allbirth_anthro %>%
  transmute(SITE, MOMID, PREGID, INFANTID, INF_VISIT_COMP = 1, VISIT_DATE = M11_VISIT_OBSSTDAT, AGE_IN_DAYS, WEIGHT_SOURCE,
         WEIGHT_MRSRE, LENGTH_MRSRE, HC_MRSRE, VISIT_COMPLETE, DATE_MRSRE = M11_DATE_MRSRE, TYPE_VISIT, MNH09_HAVE, DOB, SEX, 
...
all_data_visit <-  allvisit_anthro %>%
  ...
  transmute(SITE, MOMID, PREGID, INFANTID, INF_VISIT_COMP = 1, WEIGHT_MRSRE, VISIT_COMPLETE, AGE_IN_DAYS, WEIGHT_SOURCE,
...
#*****************************************************************************
#6: Long Infant Anthro Outcome Dataset ---- 
#*****************************************************************************
...
### Combined (ANY) Standards ----
  ...
  select(
    ...
    INF_SGA_PRETERM,INF_AGA_PRETERM, INF_SGA_TERM,INF_AGA_TERM, TERM_GA, WEIGHT_SOURCE,
    WEIGHT_MRSRE, LENGTH_MRSRE, HC_MRSRE, WEIGHT, LENGTH, HC_FAORRES, VISIT_COMPLETE, SEX,
...
write.csv(outcome_all, paste0(path_to_tnt, "INF_GROWTH_LONG" ,".csv"), row.names=FALSE)
...
#*****************************************************************************
#7: Wide Infant Anthro Outcome Dataset ---- 
#*****************************************************************************
...
birth_filtered <- outcome_all %>%  filter(TYPE_VISIT == 6) %>% 
  #We created new indicator var for WEIGHT SOURCE for all visits 
  ##but only kept the var in the birthweight, so in the wide data, we keep it for only birthweight
  #This is to differentiate facility vs prisma staff measured birthweight
  mutate(BIRTHWEIGHT_SOURCE =  WEIGHT_SOURCE )%>%
  select(-WEIGHT_SOURCE)
...
one_month_filtered <- outcome_all %>%
  ...
  transmute(
    SITE, MOMID, PREGID, INFANTID,
    INF_VISIT_COMP_M1   = INF_VISIT_COMP,
    TYPE_VISIT_M1       = TYPE_VISIT,
    VISIT_DATE_M1       = VISIT_DATE,
    AGE_IN_DAYS_M1      = AGE_IN_DAYS,
    AGE_IN_WKS_M1       = AGE_IN_WKS,
    AGE_IN_MTH_M1       = AGE_IN_MTH,
    
    WEIGHT_MRSRE_M1     = WEIGHT_MRSRE,
    LENGTH_MRSRE_M1     = LENGTH_MRSRE,
    HC_MRSRE_M1         = HC_MRSRE,
    WEIGHT_M1           = WEIGHT,
    LENGTH_M1           = LENGTH,
    HC_FAORRES_M1       = HC_FAORRES,
    
    VITAL_M1            = VITAL,
    FLEN_M1             = FLEN,
    FWFL_M1             = FWFL,
    FWEI_M1             = FWEI,
    MSRE_COMPLT_M1      = MSRE_COMPLT,
    NON_TRAJ_FLAG_M1    = NON_TRAJ_FLAG,
    ONE_MSRE_FLAG_M1    = ONE_MSRE_FLAG,
    ...
    UNDER_WEIGHT_M1            = UNDER_WEIGHT,
    UNDERWEIGHT_SEVERE_M1     = UNDERWEIGHT_SEVERE,
    STUNTING_M1               = STUNTING,
    STUNTING_SEVERE_M1        = STUNTING_SEVERE,
    WASTING_M1                = WASTING,
    WASTING_SEVERE_M1         = WASTING_SEVERE,
    OVERWEIGHT_M1             = OVERWEIGHT,
    MICROCEPHALY_M1           = MICROCEPHALY,
    
    UNDER_WEIGHT_IG_M1        = UNDER_WEIGHT_IG,
    UNDERWEIGHT_SEVERE_IG_M1  = UNDERWEIGHT_SEVERE_IG,
    STUNTING_IG_M1            = STUNTING_IG,
    STUNTING_SEVERE_IG_M1     = STUNTING_SEVERE_IG,
    WASTING_IG_M1             = WASTING_IG,
    WASTING_SEVERE_IG_M1      = WASTING_SEVERE_IG,
    OVERWEIGHT_IG_M1          = OVERWEIGHT_IG,
    MICROCEPHALY_IG_M1        = MICROCEPHALY_IG,
    
    UNDER_WEIGHT_BOTH_M1            = UNDER_WEIGHT_BOTH,
    UNDERWEIGHT_SEVERE_BOTH_M1     = UNDERWEIGHT_SEVERE_BOTH,
    STUNTING_BOTH_M1               = STUNTING_BOTH,
    STUNTING_SEVERE_BOTH_M1        = STUNTING_SEVERE_BOTH,
    WASTING_BOTH_M1                = WASTING_BOTH,
    WASTING_SEVERE_BOTH_M1         = WASTING_SEVERE_BOTH,
    OVERWEIGHT_BOTH_M1             = OVERWEIGHT_BOTH,
    MICROCEPHALY_BOTH_M1           = MICROCEPHALY_BOTH
  )
...
six_months_filtered <- outcome_all %>%
  ...
  transmute(
    SITE, MOMID, PREGID, INFANTID,
    INF_VISIT_COMP_M6   = INF_VISIT_COMP,
    TYPE_VISIT_M6       = TYPE_VISIT,
    VISIT_DATE_M6       = VISIT_DATE,
    AGE_IN_DAYS_M6      = AGE_IN_DAYS,
    AGE_IN_WKS_M6       = AGE_IN_WKS,
    AGE_IN_MTH_M6       = AGE_IN_MTH,
    
    WEIGHT_MRSRE_M6     = WEIGHT_MRSRE,
    LENGTH_MRSRE_M6     = LENGTH_MRSRE,
    HC_MRSRE_M6         = HC_MRSRE,
    WEIGHT_M6           = WEIGHT,
    LENGTH_M6           = LENGTH,
    HC_FAORRES_M6       = HC_FAORRES,
    
    VITAL_M6            = VITAL,
    FLEN_M6             = FLEN,
    FWFL_M6             = FWFL,
    FWEI_M6             = FWEI,
    MSRE_COMPLT_M6      = MSRE_COMPLT,
    NON_TRAJ_FLAG_M6    = NON_TRAJ_FLAG,
    ONE_MSRE_FLAG_M6    = ONE_MSRE_FLAG,
    ...
    UNDER_WEIGHT_M6            = UNDER_WEIGHT,
    UNDERWEIGHT_SEVERE_M6     = UNDERWEIGHT_SEVERE,
    STUNTING_M6               = STUNTING,
    STUNTING_SEVERE_M6        = STUNTING_SEVERE,
    WASTING_M6                = WASTING,
    WASTING_SEVERE_M6         = WASTING_SEVERE,
    OVERWEIGHT_M6             = OVERWEIGHT,
    MICROCEPHALY_M6           = MICROCEPHALY,
    
    UNDER_WEIGHT_IG_M6        = UNDER_WEIGHT_IG,
    UNDERWEIGHT_SEVERE_IG_M6  = UNDERWEIGHT_SEVERE_IG,
    STUNTING_IG_M6            = STUNTING_IG,
    STUNTING_SEVERE_IG_M6     = STUNTING_SEVERE_IG,
    WASTING_IG_M6             = WASTING_IG,
    WASTING_SEVERE_IG_M6      = WASTING_SEVERE_IG,
    OVERWEIGHT_IG_M6          = OVERWEIGHT_IG,
    MICROCEPHALY_IG_M6        = MICROCEPHALY_IG,
    
    UNDER_WEIGHT_BOTH_M6            = UNDER_WEIGHT_BOTH,
    UNDERWEIGHT_SEVERE_BOTH_M6     = UNDERWEIGHT_SEVERE_BOTH,
    STUNTING_BOTH_M6               = STUNTING_BOTH,
    STUNTING_SEVERE_BOTH_M6        = STUNTING_SEVERE_BOTH,
    WASTING_BOTH_M6                = WASTING_BOTH,
    WASTING_SEVERE_BOTH_M6         = WASTING_SEVERE_BOTH,
    OVERWEIGHT_BOTH_M6             = OVERWEIGHT_BOTH,
    MICROCEPHALY_BOTH_M6           = MICROCEPHALY_BOTH
  )
...
table (wide_infant_trajectories$BIRTHWEIGHT_SOURCE, wide_infant_trajectories$SITE)
