---
title: "<span style='font-size: 18px'> <span style='text-align: center'>Demographics Report (Issued: 2026-02-02)"
# date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: no
...
*Demographic table is showing baseline household and individual characteristics.*
...
# MAT_INFECTION <- read.xlsx(paste0(path_to_tnt, "MAT_INFECTION", ".xlsx"))
# df_demo <- read.xlsx(paste0("D:/Users/stacie.loisate/Documents/import/outcomes/", "MAT_DEMOGRAPHIC" ,".xlsx"))
df_demo <- read.xlsx(paste0("Z:/Outcome Data/", UploadDate, "/", "MAT_DEMOGRAPHIC" ,".xlsx"))
...
df_demo_b <- df_demo %>%
  select(SITE, MAT_AGE, MAT_AGE_UNDER18,  BMI_LEVEL_ENROLL, HEIGHT_GROUP, 
         ...
         PAID_WORK) %>%
  mutate(across(everything(), ~ ifelse(. < 0, NA, .))) %>% 
  mutate(SCHOOL_YRS = case_when(SCHOOL_YRS == -5 ~ NA, TRUE ~ SCHOOL_YRS),
         EDUCATED = case_when(EDUCATED == 55 ~ NA, TRUE ~ EDUCATED),
         )
