...
# table(syph_export_labels$SYPH_POSITIVE_EVER_PREG_CAT_LABEL, syph_export_labels$SITE)
...
# ## stop here to check if there are any reported PNC cases 
# if (dim(ct_titer %>% filter(TYPE_VISIT %in% c(13,14) & CT_TEST_POSITIVE==1))[1]>=1){ 
...
ct_titer_export <- ct_titer %>% 
  ...
  mutate(CT_TEST_POSITIVE_ENROLL = CT_TEST_POSITIVE_1,
         ...
         CT_TEST_POSITIVE_UNKNOWN_BASELINE = case_when(any(c_across(starts_with("CT_TEST_POSITIVE_")) == 1) & CT_TEST_POSITIVE_ENROLL %in% c(55,77,99,NA,2) ~ 1,
                                                       TRUE ~ 0),
...
# table(ct_export_labels$CT_TEST_POSITIVE_UNKNOWN_BASELINE, ct_export_labels$SITE)
#*****************************************************************************
# Gonorrhea titers ----
# Vaginal swab testing
#*****************************************************************************
...
# ## stop here to check if there are any reported PNC cases 
# if (dim(ng_titer %>% filter(TYPE_VISIT %in% c(13,14) & NG_TEST_POSITIVE==1))[1]>=1){ 
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
...
ng_titer_export <- ng_titer %>% 
  ...
  mutate(NG_TEST_POSITIVE_ENROLL = NG_TEST_POSITIVE_1,
         ...
         NG_TEST_POSITIVE_UNKNOWN_BASELINE = case_when(any(c_across(starts_with("NG_TEST_POSITIVE_")) == 1) & NG_TEST_POSITIVE_ENROLL %in% c(2, 55,77,99,NA) ~ 1,
                                                       TRUE ~ 0),
...
# ## stop here to check if there are any reported PNC cases 
# if (dim(zik %>% filter(TYPE_VISIT %in% c(13,14) & ZIK_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
...
# test <- zik_export_labels %>%
#   mutate(
#     BOTH_INCIDENT = case_when(ZIK_IGM_INCIDENT==1 & ZIK_IGG_INCIDENT==1 ~ 1, TRUE ~ 0),
#     SUM = rowSums(across(c(ZIK_IGM_POSITIVE_ENROLL, 
#                            ZIK_IGM_UNKNOWN_BASELINE, 
#                            ZIK_IGM_INCIDENT, 
#                            ZIK_IGG_INCIDENT)) == 1, na.rm = TRUE),
#     DISCREP = case_when(ZIK_POSITIVE_EVER_PREG != SUM ~ 1, TRUE ~ 0),
#     DISCREP_VALUE = abs(ZIK_POSITIVE_EVER_PREG - SUM)
#   ) %>%
#   group_by(SITE) %>%
#   summarise(
#     BOTH_INCIDENT = sum(BOTH_INCIDENT),
#     DISCREP_VALUE = sum(DISCREP, na.rm = TRUE),
#     ZIK_DISCREP_FINAL = abs(DISCREP_VALUE-BOTH_INCIDENT),
#     .groups = "drop"
#   )
# 
# 
# ## stop here to check if there are any reported PNC cases
# if (dim(test %>% filter(ZIK_DISCREP_FINAL !=0))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("totals don't add up")
# } else {
#   print("totals add up! no action")
# }
...
## stop here to check if there are any reported PNC cases 
# if (dim(chk %>% filter(TYPE_VISIT %in% c(13,14) & CHK_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
...
## stop here to check if there are any reported PNC cases 
# if (dim(chk %>% filter(TYPE_VISIT %in% c(6,7,8,9,10,11,12,14) & CHK_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
...
## stop here to check if there are any reported PNC cases 
# if (dim(den %>% filter(TYPE_VISIT %in% c(13,14) & DEN_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
# ## stop here to check if there are any reported PNC cases 
# if (dim(den %>% filter(TYPE_VISIT %in% c(6,7,8,9,10,11,12,14) & DEN_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("PNC cases exist")
# } else {
#   print("no PNC cases")
# }
...
# ## stop here to check if there are any reported PNC cases 
# if (dim(lept %>% filter(TYPE_VISIT %in% c(13,14) & LEPT_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
# ## stop here to check if there are any reported PNC cases 
# if (dim(lept %>% filter(TYPE_VISIT %in% c(6,7,8,9,10,11,12,14) & LEPT_IGM_POSITIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("PNC cases exist")
# } else {
#   print("no PNC cases")
# }
...
# if (dim(tb %>% filter(TYPE_VISIT %in% c(13,14) & TB_POSTIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("Unscheduled cases exist")
# } else {
#   print("no Unscheduled cases")
# }
# 
# ## stop here to check if there are any reported PNC cases 
# if (dim(tb %>% filter(TYPE_VISIT %in% c(6,7,8,9,10,11,12,14) & TB_POSTIVE==1))[1]>=1){ # if any missing signs of life are flagged, loop will run and extract query output
#   stop("PNC cases exist")
# } else {
#   print("no PNC cases")
# }
...
mat_infection <- mat_infection %>% 
  ...
  rowwise() %>%
  mutate(
    ANY_INFECTION_ENROLL_LIST = {
      vals <- c_across(all_of(enroll_infections))
      clean_vals <- vals[!is.na(vals)]
      keep_names <- enroll_infections[!is.na(vals) & vals == 1]
      if (length(keep_names) == 0) NA_character_ else paste(keep_names, collapse = ", ")
    },
    ANY_INFECTION_INCIDENT_LIST = {
      vals <- c_across(all_of(incident_infections))
      clean_vals <- vals[!is.na(vals)]
      keep_names <- incident_infections[!is.na(vals) & vals == 1]
      if (length(keep_names) == 0) NA_character_ else paste(keep_names, collapse = ", ")
    },
    ANY_INFECTION_EVER_PREG_LIST = {
      vals <- c_across(all_of(ever_preg_infections))
      clean_vals <- vals[!is.na(vals)]
      keep_names <- ever_preg_infections[!is.na(vals) & vals == 1]
      if (length(keep_names) == 0) NA_character_ else paste(keep_names, collapse = ", ")
    }
  ) %>%
  ungroup()
...
mat_infection <- mat_infection %>% 
  ...
  mutate(PREG_END = case_when(is.na(PREG_END) ~ 0,
                              TRUE ~ PREG_END)) 


table(mat_infection$PREG_END)
dim(mat_infection)
dim(mat_enroll)
