...
*Update: February 5, 2026 by E Oakley (add PNC52 visit window outcomes)
...
global dadate "2026-01-30" // this should be considered the date the data was most recently updated & will be used to calculate whether different gestational windows are completed
...
global wrk "D:\Users\emoakley\Documents\Outcome Data\2026-01-30"
...
/* Maternal Anemia - Measured & Clinical at multiple time points 
	...
	MAT_ANEMIA_M_PNC26 Low hemoglobin levels in the postpartum period, classified as mild (11-11.9 g/dL), moderate (8-10.9 g/dL), or severe (<8 g/dL).
		...
		*See also by measure type: MAT_ANEMIA_CBC_PNC52 MAT_ANEMIA_POC_PNC52 
...
/////////////////////////////////////////////////////////////////////////
	...
	*CONSTRUCT: PNC-52 visit window anemia     
	*Window includes  52-64 weeks postpartum: Day 364 - Day 448 postpartum
		*BUT: Window also asks to "or until confirmed lost to follow-up". With 
		*this in mind, we will look at the stop date vs. these late visits: 
	
		*Check for late PNC-52 visits: 
		tab TEST_PP, m 
		
		list TEST_PP TEST_DATE HB_LBORRES STOP_DATE if TEST_PP >448 & TEST_PP!=.
		
		*drop if test date is more than a week after closeout date: 
		drop if TEST_DATE > (STOP_DATE+7) & TEST_PP!=. & TEST_PP >448 & TEST_PP!=. 
	
	
	*First, restrict to window: must be a test after pregnancy, 
	*pregnancy must be completed (PREG_END) AND AFTER 364 PP: 
	keep if TEST_TIMING == 2 & TEST_PP >= 364 & TEST_PP !=. & PREG_END == 1 
	...
	*Create indicator for lowest CBC test: 	
	...
	gen HB_LOW_CBC_PNC52 = .
	gen HB_LOW_CBC_PNC52_DT = .
		format HB_LOW_CBC_PNC52_DT %td
	gen HB_LOW_CBC_PNC52_PP = . 
	
	foreach num of numlist 1/$i {
			
		replace HB_LOW_CBC_PNC52_DT = TEST_DATE`num' if TEST_TYPE`num' == "CBC" & ///
			(HB_LOW_CBC_PNC52 == . | (HB_LOW_CBC_PNC52 > HB_LBORRES`num'))
		
		replace HB_LOW_CBC_PNC52_PP = TEST_PP`num' if TEST_TYPE`num' == "CBC" & ///
			(HB_LOW_CBC_PNC52 == . | (HB_LOW_CBC_PNC52 > HB_LBORRES`num'))
		
		replace HB_LOW_CBC_PNC52 = HB_LBORRES`num' if TEST_TYPE`num' == "CBC" & ///
			(HB_LOW_CBC_PNC52 == . | (HB_LOW_CBC_PNC52 > HB_LBORRES`num'))
	
	}
	
	tab HB_LOW_CBC_PNC52, m 
	...
	*Create indicator for lowest test of any type: 	
	...
	gen HB_LOW_PNC52 = .
	gen HB_LOW_PNC52_DT = .
		format HB_LOW_PNC52_DT %td
	gen HB_LOW_PNC52_PP = . 
	
	gen HB_LOW_PNC52_TEST = ""
	
	foreach num of numlist 1/$i {
			
		* overall loop for all tests: 
		replace HB_LOW_PNC52_DT = TEST_DATE`num' if  ///
			(HB_LOW_PNC52 == . | (HB_LOW_PNC52 > HB_LBORRES`num'))
		
		replace HB_LOW_PNC52_PP = TEST_PP`num' if  ///
			(HB_LOW_PNC52 == . | (HB_LOW_PNC52 > HB_LBORRES`num'))
			
		replace HB_LOW_PNC52_TEST = TEST_TYPE`num' if  ///
			(HB_LOW_PNC52 == . | (HB_LOW_PNC52 > HB_LBORRES`num'))
		
		replace HB_LOW_PNC52 = HB_LBORRES`num' if  ///
			(HB_LOW_PNC52 == . | (HB_LOW_PNC52 > HB_LBORRES`num'))
			
	}
	
	tab HB_LOW_PNC52, m 
	tab HB_LOW_PNC52_TEST, m 
	...
	foreach num of numlist 1/$i {
		...
		*ensure we default to CBC if the two tests are equal: 
		replace HB_LOW_PNC52_DT = TEST_DATE`num' if ///
			TEST_TYPE`num' == "CBC" & HB_LOW_PNC52_TEST == "POC" & ///
			HB_LBORRES`num' == HB_LOW_PNC52 
	...
	*use max number of POC tests as max number of times to repeat: 
	foreach num of numlist 1/$z {
		...
		* * * Step 1: Set date boundaries for any low test that is POC 
		gen POC_UPBOUND = HB_LOW_PNC52_DT + 7 if HB_LOW_PNC52_TEST == "POC"
		gen POC_LOWBOUND = HB_LOW_PNC52_DT - 7 if HB_LOW_PNC52_TEST == "POC"
		format POC_UPBOUND POC_LOWBOUND %td
		
		* * * Step 2: Run loop to identify any CBC test within the bounds 
		
		gen CBC_CONCURRENT = 0 if HB_LOW_PNC52_TEST == "POC"
		
		foreach num of numlist 1/$i {
		
		replace CBC_CONCURRENT = 1 if HB_LOW_PNC52_TEST == "POC" & ///
			TEST_TYPE`num' == "CBC" & ///
			TEST_DATE`num' >= POC_LOWBOUND & TEST_DATE`num' <= POC_UPBOUND
		...
		/*Check the loop:
		list HB_LOW_PNC52 HB_LOW_PNC52_DT TEST_TYPE1 TEST_DATE1 HB_LBORRES1 ///
			TEST_TYPE2 TEST_DATE2 HB_LBORRES2 if CBC_CONCURRENT == 1 
		*/
		...
		* * * Step 3: Drop the low POC test if concurrent CBC test:
		...
		replace HB_LBORRES`num' = . if HB_LBORRES`num' == HB_LOW_PNC52 & ///
			TEST_TYPE`num' == "POC" & CBC_CONCURRENT == 1 & ///
			TEST_DATE`num' == HB_LOW_PNC52_DT
		
		}
		
		* * * Step 4: Update the loop for those with concurrent CBC
		replace HB_LOW_PNC52 = . if CBC_CONCURRENT == 1 
		...
		*Review those with a POC test only: 
		list HB_LOW_PNC52 HB_LOW_PNC52_DT TEST_TYPE1 TEST_DATE1 HB_LBORRES1 ///
			TEST_TYPE2 TEST_DATE2 HB_LBORRES2 if HB_LOW_PNC52_TEST == "POC"
		*/	
		drop CBC_CONCURRENT POC_UPBOUND POC_LOWBOUND	
		
	}
	
	list if HB_LOW_PNC52 < 7 & HB_LOW_PNC52 != . & HB_LOW_PNC52_TEST == "POC"
		
	* * * * PNC52 period clean up: restrict dataset:
	
	keep MOMID PREGID ENTRY_TOTAL POC_COUNT HB_LOW_CBC* HB_LOW_P*
	
	order POC_COUNT, after(ENTRY_TOTAL) 
	
	label var HB_LOW_CBC_PNC52 "Lowest HB test by CBC - PNC-52 Visit Window"
	label var HB_LOW_CBC_PNC52_DT "Date of lowest HB test by CBC - PNC-52 Visit Window"
	label var HB_LOW_CBC_PNC52_PP "Days postpartum at lowest HB test by CBC - PNC-52 Visit Window"
	
	label var HB_LOW_PNC52 "Lowest HB test - PNC-52 Visit Window"
	label var HB_LOW_PNC52_DT "Date of lowest HB test - PNC-52 Visit Window"
	label var HB_LOW_PNC52_PP "Days postpartum at lowest HB test - PNC-52 Visit Window"
	label var HB_LOW_PNC52_TEST "Test type for lowest HB measure - PNC-52 Visit Window"
	...
	* CONSTRUCT ANEMIA MEASURES HERE: 
	...
	gen ANEMIA_CBC_PNC52 = . 
	
	//// miminum HB postpartum anemia status criteria: 
	replace ANEMIA_CBC_PNC52 = 0 if HB_LOW_CBC_PNC52 >= 12.0 & HB_LOW_CBC_PNC52 != . 
	replace ANEMIA_CBC_PNC52 = 1 if HB_LOW_CBC_PNC52 >= 11.0 & HB_LOW_CBC_PNC52 <12.0
	replace ANEMIA_CBC_PNC52 = 2 if HB_LOW_CBC_PNC52 >= 8.0 & HB_LOW_CBC_PNC52 <11.0
	replace ANEMIA_CBC_PNC52 = 3 if HB_LOW_CBC_PNC52 <8.0 & HB_LOW_CBC_PNC52 != .
		
	label var ANEMIA_CBC_PNC52 "Most severe anemia status at PNC-52 visit window (CBC)"
		
	*Missing indicator: 
	gen ANEMIA_CBC_PNC52_MISS = 0 if ANEMIA_CBC_PNC52 != . 
	replace ANEMIA_CBC_PNC52_MISS = 1 if ANEMIA_CBC_PNC52 == .  
	
	label define anem_miss 0 "0-Non-missing" 1 "1-No valid test result" ///
		2 "2-No labs in window" 3 "3-Days postpartum information missing"	
	
	label values ANEMIA_CBC_PNC52_MISS anem_miss 
	label var ANEMIA_CBC_PNC52_MISS "Missing reason - Anemia PNC-52 Visit Window (CBC)"
	
	*gen ANEMIA_CBC_PNC52_H // consult with Xiaoyan before constructing 
		
	gen ANEMIA_PNC52 = .
	
	//// miminum HB postpartum
	replace ANEMIA_PNC52 = 0 if HB_LOW_PNC52 >= 12.0 & HB_LOW_PNC52 != . 
	replace ANEMIA_PNC52 = 1 if HB_LOW_PNC52 >= 11.0 & HB_LOW_PNC52 <12.0 
	replace ANEMIA_PNC52 = 2 if HB_LOW_PNC52 >= 8.0 & HB_LOW_PNC52 <11.0 
	replace ANEMIA_PNC52 = 3 if HB_LOW_PNC52 <8.0 & HB_LOW_PNC52 != . 


	label var ANEMIA_PNC52 "Most severe anemia status - PNC-52 Visit Window"
	
	gen ANEMIA_PNC52_MISS = 0 if ANEMIA_PNC52 != . 
	replace ANEMIA_PNC52_MISS = 1 if ANEMIA_PNC52 == . 
		
	label values ANEMIA_PNC52_MISS anem_miss 
	label var ANEMIA_PNC52_MISS "Missing reason - Anemia PNC-52 Visit Window"
	
	tab ANEMIA_PNC52, m 
	tab HB_LOW_PNC52_TEST, m 
	
	*gen ANEMIA_PNC52_H // consult with Xiaoyan before constructing 
	
	
	save "$wrk/ANEMIA_PNC52_window", replace 		
 ...
 *Denominator: PNC 52
	*Completed PNC-52 visit window (>=448 days PP)
	*Exclude: 
		*Closeouts BEFORE the PNC-52 visit window STARTS < 364 days pp with no labs
		*Deaths at < 364 days pp with no labs 
		*Pregnancy losses at <20 weeks (no longer followed)
		
		// indicator for PNC52 window completed  
		gen COMPLETE_PNC52 = 0 
		replace COMPLETE_PNC52 = 1 if date("$dadate", "YMD") >= PREG_END_DATE + 448 & ///
			PREG_START_DATE != . 
		label var COMPLETE_PNC52 "Denominator - completed PNC 52 window"
		
		tab COMPLETE_PNC52, m  
	
		
		// indicator for proceeded through study until the end of the window: 
		// this code is different from previous because we EXPECT the closeout to be 
		// >=364 days postpartum 
		gen PROCEED_PNC52 = 0 
		replace PROCEED_PNC52 = 1 if CLOSEOUT == 0 & MAT_DEATH == 0 & PREG_END == 1 
		
		replace PROCEED_PNC52 = 1 if CLOSEOUT == 1 & PREG_END ==1 & ///
			CLOSEOUT_DT >=  PREG_END_DATE + 364 & CLOSEOUT_DT != . & ///
			PREG_END_DATE != . 
		
		replace PROCEED_PNC52 = 1 if MAT_DEATH == 1 & PREG_END==1 & ///
			MAT_DEATH_DATE >=  PREG_END_DATE + 364 & MAT_DEATH_DATE != . & ///
			PREG_END_DATE != .
		
		replace PROCEED_PNC52 = 99 if PREG_END_DATE == . 
		
			*Check remaining 0s: 
		list PROCEED_PNC52 CLOSEOUT CLOSEOUT_GA PREG_END_DATE CLOSEOUT_DT ///
			MAT_DEATH MAT_DEATH_GA if ///
			PROCEED_PNC52 == 0
			
		label var PROCEED_PNC52 "Women proceeded in study through the end of window"
		
		*Set denominator: 
		gen ANEMIA_PNC52_DENOM = 0 
		replace ANEMIA_PNC52_DENOM = 1 if COMPLETE_PNC52 == 1 & ///
			PROCEED_PNC52 == 1 & PREG_LOSS == 0 
			
			// NOTE: We will keep in denominator if the person has any labs:
		replace ANEMIA_PNC52_DENOM = 1 if HB_LOW_PNC52 != . 
		
			// Those who are in the denominator, but missing, we will add to missing & missing indicator 
		replace ANEMIA_PNC52 = 55 if ANEMIA_PNC52_DENOM == 1 & ANEMIA_PNC52 == . 
		
			// Those who are in the denom but had no labs at all = 2 
		replace ANEMIA_PNC52_MISS = 2 if ANEMIA_PNC52_DENOM == 1 & ANEMIA_PNC52 == 55 & ///
			ANEMIA_PNC52_MISS != 1 
			
			// Those who are unknown days PP (i.e., preg end date missing)
		replace ANEMIA_PNC52_MISS = 3 if PREG_END==1 & PREG_END_DATE == . & ///
			ANEMIA_PNC52_MISS != 1 
			
		*CHECKS: 
		tab ANEMIA_PNC52 ANEMIA_PNC52_DENOM, m 
		tab ANEMIA_PNC52_MISS ANEMIA_PNC52_DENOM,m  
			
		label var ANEMIA_PNC52_DENOM "Denominator for PNC52 - Anemia"	
...
* Fix missing indicator for ANEMIA_ANC overall (denom = PREG_END): 
	...
	*drop functional variables from MAT_ENROLL: 
	drop M01_US_OHOSTDAT STR_PREG_START_DATE EST_CONCEP_DATE_US ///
	EST_CONCEP_DATE_LMP GA_DIFF_DAYS US_GA_WKS_ENROLL US_GA_DAYS_ENROLL ///
	LMP_GA_WKS_ENROLL LMP_GA_DAYS_ENROLL INVALID_SCRN_DATE PREG_LOSS_DEATH /// 
	MISS_PREG_START_DATE	
	
	tab ANEMIA_PNC52 if ANEMIA_PNC52_DENOM==1, m 
