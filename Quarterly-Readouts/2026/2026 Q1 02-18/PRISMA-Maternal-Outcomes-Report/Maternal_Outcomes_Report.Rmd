...
#* Last updated: February 2026, Erin Oakley, Savannah O'Malley & Stacie Loisate
#*****************************************************************************
library(knitr)
library(tidyverse)
library(reshape2)
...
# Erin's path: 
# path_to_data <- "Z:/Outcome Data/2026-01-30/" 
...
"Denominator, n^g^" = paste0(
    ...
    "Any death 43-365 days postpartum, n (%)" = paste0(
      format(sum(MAT_DEATH_42 == 2, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(MAT_DEATH_42 == 2 , na.rm = TRUE)/sum(MAT_MORTALITY_LATE_D==1, na.rm=TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
...
tab_style(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>c</sup>Denominator: those who have passed 42 days postpartum without closing out, OR who have died.</span>")
  ) %>%
   tab_footnote(
 ...
 tab_footnote(
  ...
  tab_footnote(
     footnote = html("<span style='font-size: 18px'><sup>g</sup>Denominator: those who have passed 364 days postpartum without closing out. Note: excludes deaths occurring during pregnancy and < 42 days postpartum.</span>")
  )  %>%
...
tab_style(
   ...
   tab_style(
      style = list(
        cell_text(indent = px(20), style = "italic")
        ),
      locations = cells_stub(rows = c(
                 "Hysterectomy"))
      ) %>%
