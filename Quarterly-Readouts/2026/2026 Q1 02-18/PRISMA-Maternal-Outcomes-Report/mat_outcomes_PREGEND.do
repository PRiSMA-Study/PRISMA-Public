...
*Update: August 4, 2025 by E Oakley (add location of birth)
...
global dadate "2026-01-30" // SYNAPSE/UPLOAD DATE 
...
global wrk "D:\Users\emoakley\Documents\Outcome Data/$dadate" // set pathway here for where you want to save output data files (i.e., constructed analysis variables)

global date "260203" // today's date
...
*Birth classification - spontaneous (DELIVERY_SPON)
	...
	**** to be reviewed again by sites 
	...
	// missing information - Unknown rupture of membranes AND unknown if labor induced
	replace DELIVERY_SPON = 55 if DELIVERY_ANY == 1 & ///
		MEM_SPON == 55  & LABOR_INDUCED == 55 & LABOR_ANY==0 & DELIVERY_CLASS == 0  
...
*Preterm premature rupture of membranes - PPROM (PPROM_OCCUR)
	...
	*check on PPROM observations NOT marked as spontaneous preterm:
	list site PRETERM_ANY PRETERM_SPON PRETERM_SPON_IND ///
		...
		*Update for 10-31-25 dataset (processed 11/10):
			*We find 9 observations:
				*7 are observations that experienced PPROM but not Preterm Delivery (GA >=37)
				*The remaining 3 are PPROM with pregnancy loss at <20 weeks GA (i.e., not a preterm birth)
...
* * * PROM - added on 3-11-2025; as requested by IHME
	...
	*data checks for re-running the code on new data:
	...
	list PREG_LOSS_INDUCED MAT_DEATH MAT_DEATH_GA PREG_LOSS PREG_END_GA ///	
		if PREG_END_GA<140 & PREG_END_GA>=0 & PREG_LOSS==0
	...
	*Obstructed labor 
	...
	tab m09_obstructed_labor_ceoccur SITE, m 
...
*** INCORPORATE ADDITIONAL DETAILS ON UTERINE RUPTURE REPORTED AT PNC: 
	...
	* Add location of the birth: 
	tab m09_mat_ld_oholoc, m 
	
	tab m09_mat_ld_oholoc site, m  
	
	gen specify = lower(m09_mat_ld_othr_spfy_oholoc)
	
	*review OE coding ("other" is 7%):
	list site specify if m09_mat_ld_oholoc==88
	tab specify if m09_mat_ld_oholoc==88
	
	///////////////////////////////////////////////////////
	//Ghana: 
	tab specify if m09_mat_ld_oholoc==88 & site == "Ghana"
	
	*1=health facility (hospital, clinic, health center, maternity home, dispensary)
	gen other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
	(specify == "health facility outside study area-->specify")
	
	*2=other facility (non-clinical)
	replace other_facility_loc = 2 if m09_mat_ld_oholoc == 88 & ///
	(specify == "non-health facility outside study area-->specify")
	
	///////////////////////////////////////////////////////
	//Kenya: 
	tab specify if m09_mat_ld_oholoc==88 & site == "Kenya"	///
	& other_facility_loc==.
	
	*1=health facility (hospital, clinic, health center, maternity home, dispensary)
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "hospital") > 0) & site == "Kenya"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "health centre") > 0) & site == "Kenya"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "dispensary") > 0) & site == "Kenya"
		
	*55=cannot distinguish: 
	replace other_facility_loc = 55 if m09_mat_ld_oholoc == 88 & ///
		site == "Kenya" & ///
		(specify == "uradi" | specify == "nyakach" | specify == "kombewa " | ///
		 specify == "st antony tororo uganda " | specify == "st helier " | ///
		 specify == "st lukes " | specify == "st monica ")
		 
		* shortened facility names: 
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		site == "Kenya" & (specify =="inuka " | specify =="kadenge ratuoro " ///
		| specify == "st elizabeth chiga " | specify == "st jairus ")
		
	*77=in transit: 
	replace other_facility_loc = 77 if m09_mat_ld_oholoc == 88 & ///
		(specify =="on the way to hospital " | ///
		 specify =="along the way to hospital ") 
		
	///////////////////////////////////////////////////////
	//Zambia: 
	tab specify if m09_mat_ld_oholoc==88 & site == "Zambia"
	
	*2=other facility (non-clinical)
	replace other_facility_loc = 2 if m09_mat_ld_oholoc == 88 & ///	
	site == "Zambia" & (specify =="other non-clinical l")
	
	*55=cannot distinguish: 
	replace other_facility_loc = 55 if m09_mat_ld_oholoc == 88 & ///
		specify == "dk" & site == "Zambia"	
	
	///////////////////////////////////////////////////////
	//India-CMC: 
	tab specify if m09_mat_ld_oholoc==88 & site == "India-CMC"	
	
	*77=in transit
	replace other_facility_loc = 77 if m09_mat_ld_oholoc == 88 & ///
		(specify == "during travel" | specify == "auto delivered ") ///
		& site == "India-CMC"		
	
	*55=cannot distinguish: 
	replace other_facility_loc = 55 if m09_mat_ld_oholoc == 88 & ///
		specify == "chengalpattu gh " & site == "India-CMC"		
	
	///////////////////////////////////////////////////////
	//India-SAS: 
	tab specify if m09_mat_ld_oholoc==88 & site == "India-SAS" & ///
		other_facility_loc == .
	
	*1=health facility (hospital, clinic, health center, maternity home, dispensary)
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "hospital") > 0) & site == "India-SAS"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "clinic") > 0) & site == "India-SAS"
	
	*77=in transit
	replace other_facility_loc = 77 if m09_mat_ld_oholoc == 88 & ///
		(specify == "ambulance delivery " | specify == "in transit " | ///
		 specify == "car delivery ") ///
		 & site == "India-SAS"		
	
	*55=cannot distinguish: 
	replace other_facility_loc = 55 if m09_mat_ld_oholoc == 88 & ///
		(specify == "anita dai ke ghr" | specify == "brajesh dai palwal" | ///
		 specify == "gh palwal " | specify == "sushma dai" | ///
		 specify == "roshni nurse pingor " | specify == "vandna nurse ke ghr") ///
		& site == "India-SAS"	
	
	///////////////////////////////////////////////////////
	//Pakistan: 
	tab specify if m09_mat_ld_oholoc==88 & site == "Pakistan" ///
	& other_facility_loc == .
	
	*1=health facility (hospital, clinic, medical center/re, maternity home)
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "hospital") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "clinic") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "centre") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "center") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "maternity home") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "medical complex") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "sgh") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "diagnostics") > 0) & site == "Pakistan"
		
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		(strpos(specify, "pphi") > 0) & site == "Pakistan"
		
		* shortened or unique facility names: 
	replace other_facility_loc = 1 if m09_mat_ld_oholoc == 88 & ///
		site == "Pakistan" & (specify =="chiniot" | ///
		specify == "international medical educational trust" | ///
		specify == "k.m.c" | specify == "khairpur medical college" | ///
		specify == "pns shifa" | specify == "ehsas metnral home" | ///
		specify == "private" | specify == "razia madical" | ///
		specify == "razia madical complex" | specify == "s,g.h i h" | ///
		specify == "sgh ih" | specify == "sindh gorment ih" | ///
		specify == "sindh government rg" | specify == "sindh govt.garo" | ///
		specify == "social" | specify == "syed medical" | ///
		specify == "sindh govt.rg" | specify == "sind govt rg" | ///
		specify == "international medical" | specify == "razia medical" | ///
		specify == "jiq" | specify == "new khan")	
	
	*55=cannot distinguish: 
	replace other_facility_loc = 55 if m09_mat_ld_oholoc == 88 & ///
		(specify == "al-shafi" | specify == "darul shifa" | ///
		 specify == "faisal base") ///
		& site == "Pakistan"	
	
	///////////////////////////////////////////////////////////////////////////
	*facility birth: 
	gen BIRTH_LOC = 1 if m09_mat_ld_oholoc==1 
	*"other" health facility birth: 
	replace BIRTH_LOC = 2 if m09_mat_ld_oholoc==88 & other_facility_loc == 1
	*"other" non-health facility birth:
	replace BIRTH_LOC = 3 if m09_mat_ld_oholoc==88 & other_facility_loc == 2
	*home birth: 
	replace BIRTH_LOC = 4 if m09_mat_ld_oholoc==2
	*transit:
	replace BIRTH_LOC = 5 if m09_mat_ld_oholoc==88 & other_facility_loc == 77
	*"other" unknown location: 
	replace BIRTH_LOC = 6 if m09_mat_ld_oholoc==88 &  /// 
		(other_facility_loc ==55 | other_facility_loc ==.)
		
	*missing/unknown: 
	replace BIRTH_LOC = 55 if m09_mat_ld_oholoc==. | m09_mat_ld_oholoc==55 | ///
		m09_mat_ld_oholoc==77 
	
	tab BIRTH_LOC, m 
	
	label var BIRTH_LOC "Location of birth"
	
	label define loc 1 "1-Facility (PRISMA)" 2 "2-Facility-Other" ///
		3 "3-Non-medical Facility" 4 "4-Home birth" 5 "5-Birth in transit" ///
		6 "6-Other location-not classified" 55 "55-Unknown"
		
	label values BIRTH_LOC loc 
	
	sort site
	by site: tab BIRTH_LOC, m 

	rename site SITE 
	rename momid MOMID 
	rename pregid PREGID 
	
	gen HOME_BIRTH = 0 if BIRTH_LOC == 1 | BIRTH_LOC == 2 | BIRTH_LOC == 3 | ///
		BIRTH_LOC == 5 
	replace HOME_BIRTH = 1 if BIRTH_LOC == 4
	replace HOME_BIRTH = 55 if BIRTH_LOC == 6 | BIRTH_LOC == 55 
	
	label var HOME_BIRTH "Home birth indicator"
	
	keep SITE MOMID PREGID BIRTH_LOC HOME_BIRTH 
	save "$wrk/DELIVERY_LOCATION", replace 
	
	restore 
	
	
	merge 1:1 MOMID PREGID using "$wrk/DELIVERY_LOCATION"
	
	drop if _merge == 2 
	drop _merge 
	
	replace BIRTH_LOC = 55 if BIRTH_LOC == . 
	replace HOME_BIRTH = 55 if BIRTH_LOC == . 
	
	tab BIRTH_LOC, m 
	
	sort SITE 
	by SITE: tab BIRTH_LOC if PREG_END==1, m 
	...
	preserve 
		...
		order SITE MOMID PREGID BIRTH_LOC PREG_END_DELIV PRO_LABOR_DENOM PRO_LABOR ///
		...
		keep SITE MOMID PREGID BIRTH_LOC PREG_END_DELIV PRO_LABOR_DENOM PRO_LABOR ///
			PRO_LABOR_MISS OBS_LABOR_DENOM OBS_LABOR OBS_LABOR_MISS LABOR_ANY ///
			LABOR_HOURS MEM_SPON MEM_ART MEM_CES PROM MEM_HOURS ///
			MAT_DELIVERY_CLASS MAT_DELIVERY_PROV MAT_DELIVERY_SPON MAT_DELIVERY_PROV_MISS ///
			MAT_CES_ANY MAT_CES_EMERGENT MAT_CES_PLAN ///
	...
	foreach var of varlist BIRTH_LOC PREG_END_DELIV PRO_LABOR_DENOM PRO_LABOR ///
			PRO_LABOR_MISS OBS_LABOR_DENOM OBS_LABOR OBS_LABOR_MISS LABOR_ANY ///
			MAT_DELIVERY_CLASS MAT_DELIVERY_PROV MAT_DELIVERY_SPON MAT_DELIVERY_PROV_MISS ///
			MEM_SPON MEM_ART MEM_CES PROM ///
			MAT_CES_ANY MAT_CES_EMERGENT MAT_CES_PLAN ///
			MAT_INDUCED MAT_INDUCED_TYPE INDUCED_IND_* MAT_INDUCED_MISS {
