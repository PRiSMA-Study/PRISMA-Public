...
#*****************************************************************************
table(mnh01_filtered$M01_TYPE_VISIT, mnh01_filtered$SITE)
...
## update for 2/07 data uploads 
# filter(SCRNID != "SCR-40317")
...
MAT_ENROLL_FULL <- MAT_ENROLL_FULL %>%
  ...
  mutate(REMAPP_ENROLL = case_when(
    ((SITE == "Ghana" & ENROLL_SCRN_DATE >= "2022-12-28" & ENROLL_SCRN_DATE <= "2024-10-29") | ## end date confirmed added 
       (SITE == "Kenya" & ENROLL_SCRN_DATE >= "2023-04-03" & ENROLL_SCRN_DATE <= "2025-03-11") | ## end date confirmed added 
       (SITE == "Zambia" & ENROLL_SCRN_DATE >= "2022-12-15") |
       (SITE == "Pakistan" & ENROLL_SCRN_DATE >= "2022-09-22" & ENROLL_SCRN_DATE <= "2024-01-17") | ## end date confirmed added 
       (SITE == "India-CMC" & ENROLL_SCRN_DATE >= "2023-06-20" & ENROLL_SCRN_DATE <= "2025-07-01") | ## end date confirmed added 
       (SITE == "India-SAS" & ENROLL_SCRN_DATE >= "2023-12-12" & ENROLL_SCRN_DATE <= "2025-03-06")) ~ 1, ## end date confirmed added 
    TRUE ~ 0
  )) %>% 
  relocate(REMAPP_ENROLL, REMAPP_AIM3_ENROLL, REMAPP_AIM3_TRI, .after = ENROLL)


table(MAT_ENROLL_FULL$REMAPP_AIM3_ENROLL, MAT_ENROLL_FULL$SITE, useNA = "ifany")
table(MAT_ENROLL_FULL$REMAPP_AIM3_TRI, MAT_ENROLL_FULL$SITE, useNA = "ifany")
table(MAT_ENROLL_FULL$REMAPP_ENROLL, MAT_ENROLL_FULL$SITE, useNA = "ifany")

table(remapp_ids_final$SITE, useNA = "ifany")
table(remapp_ids_final$REMAPP_AIM3_TRI, remapp_ids_final$SITE,useNA = "ifany")
table(MAT_ENROLL_FULL$DROPPED, MAT_ENROLL_FULL$SITE)

dropped_ids <- MAT_ENROLL_FULL %>% filter(DROPPED==1)
table(dropped_ids$SITE)
...
MAT_ENROLL <- MAT_ENROLL_FULL %>% 
  filter(DROPPED != 1) %>%  ## drop participants who are enrolled but missing mnh01 
  select(-DROPPED) # %>% 
# table(sub$SITE)
table(MAT_ENROLL_FULL$ENROLL)
table(MAT_ENROLL_FULL$ENROLL, MAT_ENROLL_FULL$SITE)
table(MAT_ENROLL_FULL$REMAPP_AIM3_ENROLL, MAT_ENROLL_FULL$SITE)
