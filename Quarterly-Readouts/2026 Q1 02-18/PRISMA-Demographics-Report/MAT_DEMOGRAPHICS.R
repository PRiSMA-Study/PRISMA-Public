...
#educated
educated = case_when(
  ## if M00_SCHOOL_SCORRES== 0 or 1, educated = M00_SCHOOL_SCORRES
  M00_SCHOOL_SCORRES %in% c(0,1) ~ M00_SCHOOL_SCORRES, 
  ## if M00_SCHOOL_SCORRES is missing but a valid years of education is reported, replace educated = 1
  M00_SCHOOL_SCORRES %in% c(55,77, NA) & (M00_SCHOOL_YRS_SCORRES >=0 & M00_SCHOOL_YRS_SCORRES < mat_age) ~ 1, 
  ## if M00_SCHOOL_SCORRES and years of education are missing, educated = 55
  M00_SCHOOL_SCORRES %in% c(55,77, NA) ~ 55, 
  TRUE ~ NA_real_
),
#Years of formal education 
school_yrs = case_when(
  ## if no education reported, years of school = 0 
  M00_SCHOOL_SCORRES == 0 ~ 0,    
  ## if education reported but missing years OR education status missing OR years of school >= maternal age, years of school = -5 
  (M00_SCHOOL_SCORRES %in% c(55,77, NA) & is.na(M00_SCHOOL_YRS_SCORRES)) | 
     (is.na(M00_SCHOOL_YRS_SCORRES) & educated==1) |
     educated ==1 & M00_SCHOOL_YRS_SCORRES >= mat_age ~ -5, 
  ## if education reported & years of education is LESS than maternal age, years of school =  M00_SCHOOL_SCORRES
  M00_SCHOOL_YRS_SCORRES >= 0 & (M00_SCHOOL_YRS_SCORRES < mat_age | is.na(mat_age)) ~ as.numeric(M00_SCHOOL_YRS_SCORRES),
  TRUE ~ NA_real_
)
) %>% 
  select(SITE, PREGID,mat_age, M00_SCHOOL_SCORRES, educated, M00_SCHOOL_YRS_SCORRES, school_yrs)


subset <- test %>% 
  select(SITE, PREGID, mat_age, M00_SCHOOL_SCORRES, educated, M00_SCHOOL_YRS_SCORRES, school_yrs) %>% 
  filter(is.na(school_yrs))

table(test$school_yrs, useNA = "ifany")

table(test$school_yrs, test$educated, useNA = "ifany")
...
# save(df_maternal, file= paste(path_to_save,"/df_maternal-updated", ".RData",sep = ""))
# save(df_demo, file= paste(path_to_save,"/df_maternal-updated", ".RData",sep = ""))
# write.csv(df_demo, file = paste0(path_to_tnt, "MAT_DEMOGRAPHIC-updated.csv"), row.names = FALSE)
# write.xlsx(df_demo, file = paste0(path_to_tnt, "MAT_DEMOGRAPHIC-updated.xlsx"), rownames = FALSE) 
# write_dta(df_demo, path = paste0(path_to_tnt, "MAT_DEMOGRAPHIC-updated.dta"))
