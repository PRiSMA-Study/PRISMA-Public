...
import delimited "$outcomes/GWG_OUTCOME_LONG_$datadate.csv", case(upper) clear
assert PREG_END == 1
/*
Notes on this data set:
- all pregnancies with an endpoint (all PREG_END==1)
- imputed early pregnancy values (BMI)
- includes non-singleton pregnancies -- check this?
- all visit types 
*/
drop V1 SCRNID
...
/*OLD denominator
**indicator if pregnancy ended between 238 days (34 weeks) and 294 days (42 weeks)

merge 1:1 MOMID PREGID using "$outcomes/MAT_ENDPOINTS", keepusing(PREG_END_GA)

gen PREG_END=1 if PREG_END_GA>=238 & PREG_END_GA<=294
*/
...
replace GWG_VALID = . if BMI4CAT_MISS == 1
replace GWG_ADEQUACY = . if BMI4CAT_MISS == 1
replace GWG_ADEQUACY = . if BMI4CAT_MISS == 2 //implausible BMI --> implausible GWG


/*OLD CODE: check for multiples
preserve
	
	
	*is woman carrying multiples? maximum across visits
	import delimited "$da/mnh01_merged.csv", bindquote(strict) case(upper) clear
	
	keep if M01_MAT_VISIT_MNH01<=2
	bysort SCRNID : egen FETUS_CT_PERES_US = max(M01_FETUS_CT_PERES_US)
	
	keep SITE SCRNID FETUS_CT_PERES_US
	duplicates drop
	
	merge 1:1 SCRNID using "$outcomes/MAT_ENROLL.dta"
	keep if ENROLL == 1
	drop _merge
	
	
	
	save "$wrk/MULTIPLES", replace
	
restore

merge 1:1 MOMID PREGID using "$wrk/MULTIPLES.dta", keepusing(FETUS_CT_PERES_US) 
keep if _merge == 3
drop _merge
...
merge m:1 SITE MOMID PREGID using "$outcomes/MAT_DEMOGRAPHIC",keepusing(SINGLETON)
keep if _merge==3

	count  if SINGLETON ==0 
	return list 
	global nonsingleton=`r(N)'
	**record number of non-singleton pregnancies
	
	count if SINGLETON==1
	return list
	global singleton=`r(N)'
	*record number of singleton pregnancies
