...
*Update: August 4, 2025 by E Oakley (add dates to outcomes)
*Update: November 19, 2025 (add non-severe PE variable)
*Update: December 17, 2025 (add date for severe high BP; revise timing variable for early vs. late onset)
*Update: February 17, 2026 (Update to protienuria rules for CHTN but NOT GHTN cases)
...
global da "Z:/Stacked Data/2026-01-30" // change date here as needed

	// Working Files Folder (TNT-Drive)
global wrk "D:\Users\emoakley\Documents\Outcome Data\2026-01-30" // set pathway here for where you want to save output data files (i.e., constructed analysis variables)

global OUT "Z:/Outcome Data/2026-01-30" // UPDATE DATE AS NEEDED

global date "260217" // today's date
...
/*************************************************************************
	...
	*Finalized outcome variables: 
	...
	HIGH_BP_SEVERE_DATE Among those who ever had severe high BP: first date of severe high BP reading or dx 
...
///////////////////////////////////////////////////////
	...
	* Code severe features: 
	...
	label var SEVERE_SEIZURES "Severe features: seizures or eclampsia (L&D)"
	label var SEVERE_HELLP "Severe features: HELLP syndrome (L&D)"
	label var SEVERE_SEVHYP "Severe features: severe hypertension (L&D)"
	label var SEVERE_PE "Severe features: pulmonary edema (L&D)"
	label var SEVERE_VISUAL "Severe features: visual symptoms (L&D)"
	label var SEVERE_ANY "Any severe feature(s) of preeclampsia (L&D)"
	...
	*Alternative date for those with missingness: 
	replace VISIT_DATE = date(m09_mat_ld_ohostdat, "YMD") if ///
		m09_mat_ld_ohostdat != "1907-07-07" & VISIT_DATE == . & ///
		m09_mat_ld_ohostdat != "1624-09-11"
...
*** Create a long BP dataset:
	...
	*** merge in date for pregnancy ended:
	...
	/////////////////////////////////
	*fix if PREG_END_DATE is missing: 
	replace VISIT_GA = VISIT_DATE - PREG_START_DATE if ///
		(PREG_END==1 & PREG_END_DATE ==.)
	replace VISIT_GA = . if PREG_END==1 & PREG_END_DATE == . & VISIT_GA >=320 & ///
		VISIT_GA != . 
	////////////////////////////////
	...
	*For main outcomes, we'll restrict to ANC measures & L&D form (MNH09) & ///
	...
	keep if (VISIT_DATE >= PREG_START_DATE & VISIT_DATE <= PREG_END_DATE & PREG_END==1) | /// for completed pregnancies, measures BEFORE end date
		...
		////////////////////////////////////////////////
		*Added on 8-5: review BP measures with no date: 
		gen BP_NO_DATE = 0 
		replace BP_NO_DATE = 1 if VISIT_DATE == . 
		
		*review timing: 
		tab TIMING if BP_NO_DATE == 1 
		
		*review if there are any valid measures: 
		list SITE BP_SYS_VSORRES_1 BP_DIA_VSORRES_1 BP_SYS_VSORRES_2 ///
			BP_DIA_VSORRES_2 BP_SYS_VSORRES_3 BP_DIA_VSORRES_3 BP_SYS_VSORRES ///
			BP_DIA_VSORRES TIMING if BP_NO_DATE==1
			
		*get rid of empty entries: 
		drop if BP_SYS_VSORRES==. & BP_DIA_VSORRES==. & VISIT_DATE==.
		drop if BP_SYS_VSORRES==77 & BP_DIA_VSORRES==. & VISIT_DATE == . 
		
		*Note: several observations in Zambia are from L&D but have no date; 
		*can this be resolved?
		list MOMID PREGID SITE BP_SYS_VSORRES BP_DIA_VSORRES TIMING ///
			PREG_END_DATE if BP_NO_DATE==1
			
		list MOMID PREGID SITE BP_SYS_VSORRES BP_DIA_VSORRES TIMING VISIT_DATE ///
			VISIT_GA if ///
			MOMID == "Z3-025-1684"
	...
	////////////////////////////////////////////////////////////////////	
		...
		keep if (VISIT_GA >= (20*7) & VISIT_GA!=.) | /// Keep all observations of BP >= 20 weeks GA
			...
			(VISIT_GA==. & TIMING==1 & PREG_END_DATE==. & PREG_END==1) // AND (added 8-5-25) keep L&D observations with NO PREG_END_DATE
			...
			*hospitalization that encompasses IPC. 
			...
			list SITE VISIT_DATE VISIT_GA BP_SYS_VSORRES BP_DIA_VSORRES TIMING if ///
				VISIT_GA==. 
	...
	*COUNTS: any BP measure:
	...
	gen FIRST_HIGH_BP_DATE = .
	format FIRST_HIGH_BP_DATE %td 
	gen FIRST_HIGH_BP_GA = . 
	
	gen HIGH_BP_ANC_NODATE = 0
	...
	replace FIRST_HIGH_BP_DATE = VISIT_DATE`num' if ///
		(HIGH_BP_DIA`num' == 1 | HIGH_BP_SYS`num' == 1) & ///
		(FIRST_HIGH_BP_DATE == . | ///
		 (VISIT_DATE`num' < FIRST_HIGH_BP_DATE & VISIT_DATE`num'!=. & ///
		  FIRST_HIGH_BP_DATE!= .))
		  
	replace FIRST_HIGH_BP_GA = VISIT_GA`num' if ///
		(HIGH_BP_DIA`num' == 1 | HIGH_BP_SYS`num' == 1) & ///
		(FIRST_HIGH_BP_GA == . | ///
		 (VISIT_DATE`num' < FIRST_HIGH_BP_DATE & VISIT_DATE`num'!=. & ///
		  FIRST_HIGH_BP_GA!= .))	
		  
	replace HIGH_BP_ANC_NODATE = 1 if VISIT_DATE`num'==. & ///
		VISIT_GA`num'==. & (HIGH_BP_DIA`num' == 1 | HIGH_BP_SYS`num' == 1)
	
	}
	
	tab HIGH_BP_COUNT, m 
	tab HIGH_BP_ANC_NODATE, m 
	
	sum FIRST_HIGH_BP_GA
	histogram FIRST_HIGH_BP_GA
	
	
	gen HIGH_BP_SEV_COUNT = 0 
	label var HIGH_BP_SEV_COUNT "Number of severe high BP readings"
	
	gen FIRST_SEV_HIGH_BP_DATE = .
	format FIRST_SEV_HIGH_BP_DATE %td 
	gen FIRST_SEV_HIGH_BP_GA = . 
	
	gen SEVHIGH_BP_ANC_NODATE = 0
	...
	replace FIRST_SEV_HIGH_BP_DATE = VISIT_DATE`num' if ///
		(SEVHIGH_BP_DIA`num' == 1 | SEVHIGH_BP_SYS`num' == 1) & ///
		(FIRST_SEV_HIGH_BP_DATE == . | ///
		 (VISIT_DATE`num' < FIRST_SEV_HIGH_BP_DATE & VISIT_DATE`num'!=. & ///
		  FIRST_SEV_HIGH_BP_DATE!= .))
		
	replace FIRST_SEV_HIGH_BP_GA = VISIT_GA`num' if ///
		(SEVHIGH_BP_DIA`num' == 1 | SEVHIGH_BP_SYS`num' == 1) & ///
		(FIRST_SEV_HIGH_BP_GA == . | ///
		 (VISIT_DATE`num' < FIRST_SEV_HIGH_BP_DATE & VISIT_DATE`num'!=. & ///
		  FIRST_SEV_HIGH_BP_GA!= .))	
		  
	replace SEVHIGH_BP_ANC_NODATE = 1 if VISIT_DATE`num'==. & ///
		VISIT_GA`num'==. & (SEVHIGH_BP_DIA`num' == 1 | SEVHIGH_BP_SYS`num' == 1)
	
	}	

	tab HIGH_BP_SEV_COUNT, m 
	
	tab SEVHIGH_BP_ANC_NODATE, m 
	
	sum FIRST_SEV_HIGH_BP_GA
	histogram FIRST_SEV_HIGH_BP_GA
	...
	* save a copy of high BP tabulations: 
	...
	BP_COUNT_PRISMA BP_COUNT_IPC BP_COUNT_HOSP FIRST_HIGH_BP_DATE ///
	FIRST_HIGH_BP_GA FIRST_SEV_HIGH_BP_DATE FIRST_SEV_HIGH_BP_GA ///
	HIGH_BP_ANC_NODATE SEVHIGH_BP_ANC_NODATE 
...
//////////////////////////////////////////////////////////////
	...
	*COUNTS: any BP measure:
	...
	gen FIRST_HIGH_BP_L20_DATE = . 
	format FIRST_HIGH_BP_L20_DATE %td 
	
	gen FIRST_HIGH_BP_L20_GA = .
	...
	replace FIRST_HIGH_BP_L20_DATE = VISIT_DATE`num' if ///
		(HIGH_BP_DIA`num' == 1 | HIGH_BP_SYS`num' == 1) & ///
		(FIRST_HIGH_BP_L20_DATE == . | ///
		(FIRST_HIGH_BP_L20_DATE > VISIT_DATE`num' & VISIT_DATE`num' != .))
		
	replace FIRST_HIGH_BP_L20_GA = VISIT_GA`num' if ///
		(HIGH_BP_DIA`num' == 1 | HIGH_BP_SYS`num' == 1) & ///
		(FIRST_HIGH_BP_L20_GA == . | ///
		(FIRST_HIGH_BP_L20_GA > VISIT_GA`num' & VISIT_GA`num' != .))
	
	}
	
	tab HIGH_BP_COUNT, m 
	
	histogram FIRST_HIGH_BP_L20_GA
	
	*review entries: 
	list if HIGH_BP_COUNT >1
	
	keep MOMID PREGID ENTRY_TOTAL HIGH_BP_COUNT HIGH_BP_SEV_COUNT ///
		FIRST_HIGH_BP_L20_DATE FIRST_HIGH_BP_L20_GA
...
////////////////////////////////////////////////////////////////////////////////
	...
	*Then, we'll look at dx of chronic htn in early pregnancy: 
	...
	replace HDP_MHOCCUR = -55 if HDP_MHOCCUR == 77 | HDP_MHOCCUR == 99 | HDP_MHOCCUR == 55
	...
	*** Updated on 8-5-2025: This can't be collapsed because we need 
	*** the dates; we need to instead reshape & review. 
	
	reshape wide HDP_TYPE_VISIT HDP_VISIT_DATE HDP_MHOCCUR HDP_DX_GHYP ///
		HDP_DX_PREEC HDP_DX_HTN HDP_DX_ECL HDP_DX_DK HDP_TREAT HDP_VISIT_GA ENTRY_TOTAL, ///
		i(MOMID PREGID) j(ENTRY_NUM)
	
	gen HDP_ANY_L20_DX = 0 
	gen HDP_ANY_L20_DX_DATE = . 
	format HDP_ANY_L20_DX_DATE %td 
	gen HDP_ANY_L20_DX_GA = .
	...
	foreach let in HDP_MHOCCUR HDP_DX_GHYP ///
		HDP_DX_PREEC HDP_DX_HTN HDP_DX_ECL HDP_DX_DK {
	
	gen `let' = .
	
	foreach num of numlist 1/$i{
	replace `let' = `let'`num' if `let' == . | `let'`num' > `let' & ///
		`let'`num'!=.
	}
	
		}
		
	foreach var of varlist HDP_MHOCCUR HDP_DX_GHYP ///
		HDP_DX_PREEC HDP_DX_HTN HDP_DX_ECL HDP_DX_DK {
	...
	replace HDP_ANY_L20_DX = 1 if HDP_MHOCCUR`num' == 1 | ///
		HDP_DX_GHYP`num' == 1 | HDP_DX_PREEC`num' == 1 | ///
		HDP_DX_HTN`num' == 1 |  HDP_DX_ECL`num' == 1 |  HDP_DX_DK`num' == 1
		
	replace HDP_ANY_L20_DX_DATE = HDP_VISIT_DATE`num' if ///
		(HDP_MHOCCUR`num' == 1 | ///
		HDP_DX_GHYP`num' == 1 | HDP_DX_PREEC`num' == 1 | ///
		HDP_DX_HTN`num' == 1 |  HDP_DX_ECL`num' == 1 |  HDP_DX_DK`num' == 1) & ///
		(HDP_ANY_L20_DX_DATE == . | ///
		 (HDP_VISIT_DATE`num'!=. & HDP_VISIT_DATE`num'< HDP_ANY_L20_DX_DATE))
		 
	replace HDP_ANY_L20_DX_GA = HDP_VISIT_GA`num' if ///
		(HDP_MHOCCUR`num' == 1 | ///
		HDP_DX_GHYP`num' == 1 | HDP_DX_PREEC`num' == 1 | ///
		HDP_DX_HTN`num' == 1 |  HDP_DX_ECL`num' == 1 |  HDP_DX_DK`num' == 1) & ///
		(HDP_ANY_L20_DX_GA == . | ///
		 (HDP_VISIT_GA`num'!=. & HDP_VISIT_GA`num'< HDP_ANY_L20_DX_GA))
	...
	replace HDP_TREAT_DATE = HDP_VISIT_DATE`num' if ///
		(HDP_TREAT`num'==1) & ///
		(HDP_TREAT_DATE == . | ///
		 (HDP_VISIT_DATE`num'!=. & HDP_VISIT_DATE`num'< HDP_TREAT_DATE))
		 
	replace HDP_TREAT_GA = HDP_VISIT_GA`num' if ///
		(HDP_TREAT`num'==1) & ///
		(HDP_TREAT_GA == . | ///
		(HDP_VISIT_DATE`num'!=. & HDP_VISIT_GA`num'< HDP_TREAT_GA))
	...
	keep MOMID PREGID HDP_MHOCCUR HDP_DX_GHYP ///
		HDP_DX_PREEC HDP_DX_HTN HDP_DX_ECL HDP_DX_DK ///
		HDP_ANY_L20_DX HDP_ANY_L20_DX_DATE HDP_ANY_L20_DX_GA ///
		HDP_TREAT HDP_TREAT_DATE HDP_TREAT_GA 
...
////////////////////////////////////////////////////////////////////////////////
	...
	* CREATE A SUMMARY VARIABLE FOR HOW SOMEONE IS CATEGORIZED INTO CHTN: 
	...
	gen HTN_ANY_DATE = .
	label var HTN_ANY_DATE "Date of CHTN identification (first high bp OR date of dx OR date of treatment)"
	
	replace HTN_ANY_DATE = min(FIRST_HIGH_BP_L20_DATE, HDP_ANY_L20_DX_DATE, ///
		HDP_TREAT_DATE, HTN_ANY_DATE)
	format HTN_ANY_DATE %td 
	
	tab HTN_ANY_DATE HTN_ANY, m 
	
	* review inconsistencies: 
	list if HTN_ANY_DATE!=. & (HTN_ANY==0 | HTN_ANY==55)
...
* * * * * * Review proteinuria measures
	...
	*** merge in for pregnancy end point: 
	merge m:1 MOMID PREGID using "$OUT/MAT_ENDPOINTS", keepusing(PREG_END ///
		PREG_END_DATE PREG_END_GA)
	drop if _merge == 2 
	drop _merge 
	...
	* DROP IF NOT DURING PREGNANCY!
	drop if VISIT_DATE > PREG_END_DATE & PREG_END_DATE != . & VISIT_DATE!=. & ///
		PREG_END==1 
		
	* DROP PROBLEM DATES (before pregnancy start)
	drop if VISIT_DATE < PREG_START_DATE & VISIT_DATE!=. & PREG_START_DATE!=. 
	...
	* New variables added on Feb-03-2026: Find result closest to delivery: 
	gen UA_PROT_LBORRES_FINAL = -99
	label var UA_PROT_LBORRES_FINAL "Final Protein Reading in Pregnancy"
	
	gen UA_PROT_TESTTYPE_FINAL = .
	label var UA_PROT_TESTTYPE_FINAL "Form reported final protein reading"
	gen UA_PROT_DATE_FINAL = . 
		format UA_PROT_DATE_FINAL %td 
		label var UA_PROT_DATE_FINAL "Date of final protein reading"
	gen UA_PROT_GA_FINAL = . 
	label var UA_PROT_GA_FINAL "GA at final protein reading"	
	...
	*CLOSEST TO DELIVERY: 
	...
	replace UA_PROT_TESTTYPE_FINAL = UA_TIMING`num' if ///
		(UA_PROT_GA_FINAL==. | (VISIT_GA`num'> UA_PROT_GA_FINAL) | ///
		(VISIT_GA`num'== UA_PROT_GA_FINAL & ///
		 UA_PROT_LBORRES`num'> UA_PROT_LBORRES_FINAL & ///
		 UA_PROT_LBORRES`num'!=. & UA_PROT_LBORRES_FINAL!=.)) & ///
		 VISIT_GA`num'!=. & UA_PROT_LBORRES`num'!=-55
		
	replace UA_PROT_DATE_FINAL = VISIT_DATE`num' if ///
		(UA_PROT_GA_FINAL==. | (VISIT_GA`num'> UA_PROT_GA_FINAL) | ///
		(VISIT_GA`num'== UA_PROT_GA_FINAL & ///
		 UA_PROT_LBORRES`num'> UA_PROT_LBORRES_FINAL & ///
		 UA_PROT_LBORRES`num'!=. & UA_PROT_LBORRES_FINAL!=.)) & ///
	...
	replace UA_PROT_LBORRES_FINAL = UA_PROT_LBORRES`num' if ///
		(UA_PROT_GA_FINAL==. | (VISIT_GA`num'> UA_PROT_GA_FINAL) | ///
		(VISIT_GA`num'== UA_PROT_GA_FINAL & ///
		 UA_PROT_LBORRES`num'> UA_PROT_LBORRES_FINAL & ///
		 UA_PROT_LBORRES`num'!=. & UA_PROT_LBORRES_FINAL!=.)) & ///
	...
	replace UA_PROT_GA_FINAL = VISIT_GA`num' if ///
		(UA_PROT_GA_FINAL==. | (VISIT_GA`num'> UA_PROT_GA_FINAL) | ///
		(VISIT_GA`num'== UA_PROT_GA_FINAL & ///
		 UA_PROT_LBORRES`num'> UA_PROT_LBORRES_FINAL & ///
		 UA_PROT_LBORRES`num'!=. & UA_PROT_LBORRES_FINAL!=.)) & ///
	...
	* resolve the missing entries (pregnancies with no urinalysis test results)
	replace UA_PROT_LBORRES_FINAL = -55 if UA_PROT_LBORRES == -55 & ///
		UA_PROT_LBORRES_FINAL==-99 
	
	*check for issues: 
	gen problem = 1 if UA_PROT_GA_FINAL > PREG_END_GA & PREG_END==1 & ///
		UA_PROT_GA_FINAL!=. 
	tab problem 
	
	label values UA_PROT_LBORRES UA_PROT_LBORRES_FINAL prot
	tab UA_PROT_LBORRES, m 
	
	tab UA_PROT_LBORRES UA_PROT_LBORRES_FINAL, m 
	...
	keep MOMID PREGID ENTRY_TOTAL UA_PROT_LBORRES UA_PROT_DATE UA_PROT_GA ///
		UA_PROT_TESTTYPE ///
		UA_PROT_LBORRES_FINAL UA_PROT_TESTTYPE_FINAL UA_PROT_DATE_FINAL ///
		UA_PROT_GA_FINAL
...
///////////////////////////////////////////////////////		
	...
	* merge in chronic hypertension file: 
	merge 1:1 MOMID PREGID using "$wrk/HTN_final", keepusing(HTN_ANY ///
		HIGH_BP_SEV_COUNT_LESS20 HTN_ANY_IND HTN_ANY_DATE ///
		FIRST_HIGH_BP_L20_DATE FIRST_HIGH_BP_L20_GA)
	...
	****** Merge in L&D form: 
	merge 1:1 MOMID PREGID using "$wrk/HDP_mnh09", keepusing(MAT_* ///
		SYMPTOM_* SEVERE_* HDP_* INDUCED_HDP CES_PREEC VISIT_DATE)
	...
	* set to L&D dx first: 
	gen `let'_EVER = HDP_DX_`let' if PREG_END_GA >= (20*7)
	...
	**** Create Groups of Interest: 
	...
	replace GHYP_IND = 5 if (DK_EVER == 1 | HTN_EVER == 1) & ///
		HTN_ANY != 1 & GHYP_IND == .		
			...
			// Diagnosis of Severe Hypertension at >= 20 weeks GA 
	replace HDP_GROUP = 2 if (HDP_SEVHYP_ANC==1 | ///
		(SEVERE_SEVHYP==1 & PREG_END_GA>=(20*7))) ///
		& HTN_ANY != 1
	
	replace GHYP_IND = 6 if  (HDP_SEVHYP_ANC==1 | ///
		(SEVERE_SEVHYP==1 & PREG_END_GA>=(20*7))) ///
		& HTN_ANY != 1 & GHYP_IND == . 
	...
	tab GHYP_IND HDP_GROUP, m 
		...
		label define ghypin 1 "1-2+ high bp measures" 2 "2-1+ severe bp" ///
			3 "3-1+ high BP & meds" 4 "4-Dx of GHYP" ///
			5 "5-Dx of other HTN" 6 "6-Dx of severe HTN"
		label values GHYP_IND ghypin
	...
	* Review variables that are potential dates for GHTN: 
		* First date of high BP: 
		sum FIRST_HIGH_BP_DATE
		
		* First date of clinical dx of gestational hypertension: 
		tab HDP_TREAT_DATE_ANC if (HDP_TREAT_ANC==1 | HDP_TREAT_LD==1 | ///
			HDP_TREAT2_LD==1) & GHTN==1, m 
			
		gen HDP_TREAT_DATE_LD = PREG_END_DATE if (HDP_TREAT_LD==1 | ///
			HDP_TREAT2_LD==1) & GHTN==1
		format HDP_TREAT_DATE_LD %td
			
		tab HDP_TREAT_DATE_LD if HDP_TREAT_ANC == . & ///
			(HDP_TREAT_ANC==1 | HDP_TREAT_LD==1 | ///
			HDP_TREAT2_LD==1) & GHTN==1, m 
			
		gen TREAT_NO_DATE = 1 if GHTN==1 & (HDP_TREAT_ANC == 1 | ///
			HDP_TREAT_LD==1 | HDP_TREAT2_LD==1) & ///
			HDP_TREAT_DATE_ANC==. & HDP_TREAT_DATE_LD==. 
			
		tab TREAT_NO_DATE, m 
			*As of 8/7/2025: no cases of treatment without date (good news!)
		
		* First date of clinical dx of hypertension >=20 weeks: 
		tab HDP_DX_GHYP_DATE_ANC if HDP_DX_GHYP_ANC==1, m 
		
		tab HDP_DX_GHYP_DATE_ANC if HDP_DX_GHYP ==1, m 
		
			// create a date for clinical dx of gestational hypertension AT L&D: 
		gen HDP_DX_GHYP_DATE_LD = PREG_END_DATE if HDP_DX_GHYP==1 & GHTN==1 & ///
			 PREG_END_GA >=(20*7)
		format HDP_DX_GHYP_DATE_LD %td 
		
		tab HDP_DX_GHYP_DATE_LD if HDP_DX_GHYP==1 & GHTN==1, m 
		
		* First date of clinical dx of severe hypertension >20 weeks 
		tab HDP_SEVHYP_DATE_ANC if GHTN==1, m 
		tab HDP_SEVHYP_DATE_ANC HDP_SEVHYP_ANC if GHTN==1, m 
		
			// create a date for clinical dx of severe hypertension AT L&D:
		gen SEVERE_SEVHYP_DATE_LD = PREG_END_DATE if SEVERE_SEVHYP == 1 & ///
			GHTN==1 & PREG_END_GA >=(20*7)
		format SEVERE_SEVHYP_DATE_LD %td 
		
		
		* First date of clinical dx of an unknown hypertensive disorder OR 
		* chronic hypertension at >= 20 weeks 
		tab HDP_DX_HTN_DATE_ANC if GHTN==1, m 
		tab HDP_DX_DK_DATE_ANC if GHTN==1, m 
		
			// create a date for clinical dx of hypertension AT L&D: 
		gen HDP_DX_DK_DATE_LD = PREG_END_DATE if HDP_DX_DK==1 & ///
			GHTN==1 & PREG_END_GA >=(20*7)
		format HDP_DX_DK_DATE_LD %td 
		
		gen HDP_DX_HTN_DATE_LD = PREG_END_DATE if HDP_DX_HTN==1 & ///
			GHTN==1 & PREG_END_GA >=(20*7)
		format HDP_DX_HTN_DATE_LD %td 
		
	/* NOTE: 8/8/2025: 
	
	   Confirmed with CRS by email that we will select the FIRST indicator 
	   of GHTN as the date of onset, regardless of the criteria that ultimately 
	   places these participants into the GHTN category. For example, if a 
	   participant has a clinical dx of GHTN, followed by 2 high BP readings 
	   later on, we take the earlier date of the clinical dx as the onset 
	   date. 
	   
	   The code below takes the MINIMUM date of any components of GHTN reported. 
	   
	*/
		
		
		
		* Select the MINIMUM of all of these components: 
		gen GHTN_DATE = min(FIRST_HIGH_BP_DATE, ///
							HDP_TREAT_DATE_ANC, HDP_TREAT_DATE_LD, ///
							HDP_DX_GHYP_DATE_ANC, HDP_DX_GHYP_DATE_LD, ///
							HDP_SEVHYP_DATE_ANC, SEVERE_SEVHYP_DATE_LD, ///
							HDP_DX_HTN_DATE_ANC, HDP_DX_DK_DATE_ANC, ///
							HDP_DX_HTN_DATE_LD, HDP_DX_DK_DATE_LD) ///
							if GHTN==1 
		format GHTN_DATE %td
		label var GHTN_DATE "Date of GHTN onset (first sign)"
		tab GHTN_DATE GHTN, m 
		
	gen GHTN_GA_WK = int((GHTN_DATE - PREG_START_DATE) / 7) if GHTN==1
	gen GHTN_GA = int(GHTN_DATE - PREG_START_DATE) if GHTN==1
	
	label var GHTN_GA_WK "Gestational Age at GHTN onset (first sign) in weeks"
	label var GHTN_GA "Gestational Age at GHTN onset (first sign) in days"
	
	sort GHYP_IND 
	
	/*Review GA at onset/dates by indication: 
	
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		PREG_END_DATE if GHTN==1 & GHYP_IND==1 & SITE=="Zambia"
	...
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		...
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		PREG_END_DATE if GHTN==1 & GHYP_IND==2

	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		PREG_END_DATE if GHTN==1 & GHYP_IND==3
		
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		HDP_DX_HTN_DATE_LD HDP_DX_DK_DATE_LD PREG_END_DATE if GHTN==1 & GHYP_IND==4
		
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		PREG_END_DATE if GHTN==1 & GHYP_IND==5
		
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		HDP_DX_HTN_DATE_LD HDP_DX_DK_DATE_LD PREG_END_DATE if GHTN==1 & GHYP_IND==6

		/*
	   twoway (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==1, mcolor(black)) ///
       (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==2, mcolor(blue))  ///
       (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==3, mcolor(red))  ///
       (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==4, mcolor(green)) ///
	   (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==5, mcolor(purple)) ///
	   (scatter  PREG_END_GA GHTN_GA if GHYP_IND ==6, mcolor(yellow)), ///
	   legend(label( 1 "2+ High BP" 2 "1+ Severe High BP" ///
	   3 "1+ High BP & Treatment" 4 "Dx of GHTN" 5 "Dx of HTN >=20 weeks" ///
	   6 "Dx of Severe HTN"))
	   */
	*/
	
	*review cases with missing dates: 
	list SITE GHYP_IND GHTN_DATE GHTN_GA_WK FIRST_HIGH_BP_DATE ///
		HDP_TREAT_DATE_ANC HDP_TREAT_DATE_LD HDP_DX_GHYP_DATE_ANC ///
		HDP_DX_GHYP_DATE_LD HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD ///
		HDP_DX_HTN_DATE_ANC HDP_DX_DK_DATE_ANC ///
		HDP_DX_HTN_DATE_LD HDP_DX_DK_DATE_LD ///
		FIRST_DX_HTN FIRST_DX_GHYP FIRST_DX_PREEC ///
		FIRST_HDP_TREAT FIRST_DX_MHOCCUR FIRST_DX_DK FIRST_DX_DX_ECL ///
		PREG_END_DATE if GHTN==1 & GHTN_DATE==.
		
	list if GHTN==1 & GHTN_DATE==.
	
	///////////////////////////////////////////////////////////////////////////
	
		* DATA CLEANING ACTION: 
		* MOMID Z3-025-1684 PREGID Z3-025-1684 is missing a pregnancy end date 
		* (currently under query but not yet resolved). Based on MNH04, we see 
		* that the final completed ANC visit occurred on 8-28-2024, so we can 
		* assume that participant was still pregnant up until that point. We 
		* will calculate the GA at this most recent ANC visit to determine if 
		* the pregnancy endpoint could have occurred at <34 weeks. If the ANC 
		* visit occurred at >=34 weeks, we can place this participant in the 
		* >=34 weeks indicator (even if we cannot give the exact onset date).
		
	list PREG_END_DATE PREG_END_GA if MOMID== "Z3-025-1684"
		
	gen LAST_SEEN = date("20240828", "YMD") if MOMID == "Z3-025-1684" & ///
		PREGID == "Z3-025-1684" & PREG_END_DATE == . 
	format LAST_SEEN %td 
	
	gen GA_LAST_SEEN = LAST_SEEN - PREG_START_DATE if MOMID == "Z3-025-1684" & ///
		PREGID == "Z3-025-1684" & PREG_END_DATE == .
	tab GA_LAST_SEEN, m 
	
		*Based on the 4-18 data, we do CANNOT determine if pregnancy endpoint 
		*occurred before or after 34 weeks, because last ANC visit occurred at 
		*31 weeks GA. Therefore, this participant will be missing data for this 
		*timing indicator. 
	
	drop LAST_SEEN GA_LAST_SEEN
	
	///////////////////////////////////////////////////////////////////////////
	
	
	* Create the 34-week indicator 
	gen GHTN_GTE34 = 0 if GHTN==1 & GHTN_GA < 238 & GHTN_GA >= 140 
	replace GHTN_GTE34 = 1 if GHTN==1 & GHTN_GA >= 238 & GHTN_GA != . 
	replace GHTN_GTE34 = 55 if GHTN == 1 & GHTN_GA == . 
	
	label var GHTN_GTE34 "GHTN onset was >=34 weeks GA"
	
	tab GHTN_GA_WK GHTN_GTE34, m 
	...
	*Check: 
	list SITE HDP_GROUP SYMPTOM_PROTEIN_LD PROT_BL ///
		UA_PROT_LBORRES UA_PROT_GA UA_PROT_LBORRES_FINAL UA_PROT_GA_FINAL ///
		PREG_END_GA if PREEC_IND==5 
	...
	replace HDP_GROUP = 4 if HTN_ANY== 1 & ///	
		...
		UA_PROT_LBORRES_FINAL !=0 & UA_PROT_DATE_FINAL!=1
...
////////////////////////////////////////////////////////////////////
	...
	*restrict to any cases of organ failure: 
	...
	duplicates tag MOMID PREGID, gen(multiple)
	
	list if multiple>0
	
	sort MOMID PREGID ORG_FAIL_DATE_M12 
	
	by MOMID PREGID: gen first_instance = _n
	
	keep if first_instance==1 
	
	drop multiple first_instance
	...
	*within 42 days postpartum (i.e., MAT_DYS_TIMING is 0 or 1 or 2)
	...
	replace HDP_GROUP = 5 if MAT_DYS == 1 & MAT_DYS == 1 & ///
		...
		* NOTE: NEED TO move Preeclampsia indicator & date HERE (instead of 
		* below) to determine timing issues: 
	...
	* SET DATE FOR PREECLAMPSIA OUTCOME: 
		* Review potential dates for PE dx: 
		* Here we will take the minimum of the following: 
			*Component 1: GHTN & Proteinuria dates:
				*Date of proteinuria ONLY IF it occurs after GHTN date 
				*Date of GHTN date IF it occurs after proteinuria 
			*And/or minimum of the following: 
				*Proteinuria reported at L&D 
				*Preeclampsia clinical diagnosis date
				*Date of severe high BP reading 
				*Date of severe high BP diagnosis
				*Appearance of a severe feature
			
	gen PREECLAMPSIA_DATE = . 
	format PREECLAMPSIA_DATE %td 
	label var PREECLAMPSIA_DATE "Date of Preeclampsia Onset"
	
	tab PREECLAMPSIA, m 
	
	tab GHTN_DATE if PREECLAMPSIA==1, m 
	
	*find first date with GHTN onset AND proteinuria is confirmed: 
		// baseline negative or missing: 
	gen BP_PROT_DATE = max(GHTN_DATE, UA_PROT_DATE) if PREECLAMPSIA==1 & ///
		(GHTN==1 & UA_PROT_LBORRES >=2 & UA_PROT_LBORRES != .) & ///
		(UA_PROT_PRIOR20_LBORRES==0 | UA_PROT_PRIOR20_LBORRES==-55 | ///
		 UA_PROT_PRIOR20_LBORRES==1 | UA_PROT_PRIOR20_LBORRES == .) 
		 
		format BP_PROT_DATE %td
		 
		// baseline +1: 
	replace BP_PROT_DATE = max(GHTN_DATE, UA_PROT_DATE) if PREECLAMPSIA==1 & ///
		(GHTN==1 & UA_PROT_LBORRES >=3 & UA_PROT_LBORRES != .) & ///
		(UA_PROT_PRIOR20_LBORRES==2) & BP_PROT_DATE==.
		
		// baseline +2: 
	replace BP_PROT_DATE = max(GHTN_DATE, UA_PROT_DATE) if PREECLAMPSIA==1 & ///
		(GHTN==1 & UA_PROT_LBORRES >=4 & UA_PROT_LBORRES != .) & ///
		(UA_PROT_PRIOR20_LBORRES==3) & BP_PROT_DATE==.
		
	*for those with Chronic HTN underlying, find date of proteinuria: 
		// baseline negative or missing:  
	replace BP_PROT_DATE = UA_PROT_DATE if PREECLAMPSIA==1 & ///
		HTN_ANY==1 & ///
		(UA_PROT_LBORRES >=2 & UA_PROT_LBORRES != .) & ///
		(UA_PROT_PRIOR20_LBORRES==0 | UA_PROT_PRIOR20_LBORRES==-55 | ///
		 UA_PROT_PRIOR20_LBORRES==1 | UA_PROT_PRIOR20_LBORRES == .) & ///
		 BP_PROT_DATE == .  
	...
	replace BP_PROT_DATE = UA_PROT_DATE if PREECLAMPSIA==1 & ///
		HTN_ANY==1 & ///
		(UA_PROT_LBORRES >=3 & UA_PROT_LBORRES != .) & ///
		(UA_PROT_PRIOR20_LBORRES==2) & BP_PROT_DATE == .  
	...
	tab BP_PROT_DATE PREEC_SUP_IND if PREECLAMPSIA==1 & HTN_ANY==1, m 
	
	
	* From here, we will take the FIRST criteria that qualified the woman as 
	* having preeclampsia: 
		* Relevant dates include: 
			// second of either High BP or Proteinuria 
		sum BP_PROT_DATE, format 
		
			// Proteinuria at L&D (symptom)
		gen SYMPTOM_PROTEIN_LD_DATE = PREG_END_DATE if SYMPTOM_PROTEIN_LD==1 & ///
			PREECLAMPSIA == 1 & ///
			(PREG_END_GA >=140)
		format SYMPTOM_PROTEIN_LD_DATE %td 
		
		sum SYMPTOM_PROTEIN_LD_DATE, format 
	
			// Preeclampsia clinical diagnosis date
		sum HDP_DX_PREEC_DATE_ANC, format 	
		
		gen HDP_DX_PREEC_DATE_LD = PREG_END_DATE if PREECLAMPSIA==1 & ///
			HDP_DX_PREEC == 1 & (PREG_END_GA >=140)
		format HDP_DX_PREEC_DATE_LD %td 
		
		sum HDP_DX_PREEC_DATE_LD, format 
			
			// Date of severe high BP reading 
		sum FIRST_SEV_HIGH_BP_DATE, format 
			
			// Date of severe high BP diagnosis
		sum HDP_SEVHYP_DATE_ANC, format 
		sum SEVERE_SEVHYP_DATE_LD, format 
		tab SEVERE_SEVHYP_DATE_LD SEVERE_SEVHYP, m 
			* NEEDS TO BE UPDATED TO INCLUDE THOSE WITH UNDERLYING HTN: 
		replace SEVERE_SEVHYP_DATE_LD = PREG_END_DATE if SEVERE_SEVHYP==1 & ///
			PREG_END_GA >=(20*7) & PREECLAMPSIA == 1 
			
			// Appearance of a severe feature
				// Eclampsia/seizures
		tab HDP_DX_ECL_DATE_ANC HDP_DX_ECL_ANC, m
		tab HDP_PPECL_DATE_ANC HDP_PPECL_GA_ANC, m 
		
		gen SEVERE_SEIZURES_DATE_LD = PREG_END_DATE if SEVERE_SEIZURES==1 & ///
			PREECLAMPSIA==1 & (PREG_END_GA >=140)
		format SEVERE_SEIZURES_DATE_LD %td 
		
		tab SEVERE_SEIZURES_DATE_LD SEVERE_SEIZURES, m
		
		replace SEVERE_SEIZURES_DATE_LD = PREG_END_DATE if SYMPTOM_SEIZURE_LD==1 & ///
			PREECLAMPSIA==1 & (PREG_END_GA >=140) & SEVERE_SEIZURES !=1
		format SEVERE_SEIZURES_DATE_LD %td 
		
		tab SEVERE_SEIZURES_DATE_LD if SEVERE_SEIZURES==1 | SYMPTOM_SEIZURE_LD ==1, m 
				
				// HELLP diagnosis 
		tab HDP_HELLP_DATE_ANC HDP_HELLP_ANC, m 
		
		gen SEVERE_HELLP_DATE_LD = PREG_END_DATE if SEVERE_HELLP==1 & ///
			PREECLAMPSIA==1 & (PREG_END_GA >=140)
		format SEVERE_HELLP_DATE_LD %td 
		
		tab SEVERE_HELLP_DATE_LD SEVERE_HELLP, m 
		
		
				// Organ dysfunction - if PE / GHTN only 
		tab MAT_DYS_M09, m 
		gen MAT_DYS_M09_DATE = PREG_END_DATE if MAT_DYS_M09==1 & (PREG_END_GA >=140)
		format MAT_DYS_M09_DATE %td
		tab ORG_FAIL_DATE_M12 MAT_DYS_M12, m 
		
		gen ORG_FAIL_DATE = min(MAT_DYS_M09_DATE, ORG_FAIL_DATE_M12) if ///
			(MAT_DYS_M09==1 | MAT_DYS_M12==1) & PREECLAMPSIA==1  
		format ORG_FAIL_DATE %td 
			
		tab ORG_FAIL_DATE if PREECLAMPSIA==1 & ///
			(MAT_DYS_M09==1 | MAT_DYS_M12==1) 
			
		tab ORG_FAIL_DATE, m 
		
		list GHTN PREECLAMPSIA GHTN_DATE ORG_FAIL_DATE if MAT_DYS_M09==1 | ///
			MAT_DYS_M12==1 | MAT_DYS_M19==1
		
		
	* Select from dates: 
	
	replace PREECLAMPSIA_DATE = min(BP_PROT_DATE, SYMPTOM_PROTEIN_LD_DATE, ///
		HDP_DX_PREEC_DATE_ANC, HDP_DX_PREEC_DATE_LD, FIRST_SEV_HIGH_BP_DATE, ///
		HDP_SEVHYP_DATE_ANC, SEVERE_SEVHYP_DATE_LD, HDP_DX_ECL_DATE_ANC, ///
		HDP_PPECL_DATE_ANC, SEVERE_SEIZURES_DATE_LD, HDP_HELLP_DATE_ANC, ///
		SEVERE_HELLP_DATE_LD, ORG_FAIL_DATE, ///
		SEVERE_SEIZURES_DATE_LD, HDP_SEIZURES_DATE_ANC) if PREECLAMPSIA==1 	
		
		*REMOVED DATES: HDP_VIS_DATE_ANC, SEVERE_VISUAL_LD_DATE,
			*SEVERE_PE_LD_DATE, HDP_PE_DATE_ANC, SEVERE_FEAT_EPIPAIN_DATE, 
			* SYMPTOM_HEADACHE_LD_DATE, SYMPTOM_HEADACHE_DATE_HOSP
			
		list GHTN PREECLAMPSIA GHTN_DATE ORG_FAIL_DATE PREECLAMPSIA_DATE ///
			if (MAT_DYS_M09==1 | MAT_DYS_M12==1 | MAT_DYS_M19==1) & ///
			PREECLAMPSIA==1
	...
	* create non-mutually exclusive indicators for severe features:
	...
	gen SEVERE_FEAT_`let' = 0 if HDP_GROUP == 5 
		...
		* Edit: organ dysfunction must be within pregnancy or 42 days pp: 
		...
		///////////////////////////////////////////////////
		* UPDATE DATES & INDICATORS FOR ADDITIONAL SEVERE FEATURES 
		...
		*Update on Feb 4 2026: these symptoms must occur within SEVEN DAY BEFORE
		*PE diagnosis (or, after) to be considered as symptoms of severe PE:  
		
	// Pulmonary edema if PE only
		*Identify incidence & dates: 
		tab HDP_GROUP SEVERE_PE, m 
		tab HDP_GROUP HDP_PE_ANC, m 
				
		tab HDP_PE_DATE_ANC HDP_PE_ANC, m 	
		
		gen SEVERE_PE_LD_DATE = PREG_END_DATE if SEVERE_PE == 1 & PREG_END_GA >=140
		format SEVERE_PE_LD_DATE %td 
		
		* Determine those that meet criteria + timing: 
		replace PREEC_SEV_IND = 6 if (HDP_GROUP == 4 | HDP_GROUP==3) & ///
			((HDP_PE_ANC == 1 & HDP_PE_DATE_ANC >= (PREECLAMPSIA_DATE -7)) | ///
			(SEVERE_PE == 1 & SEVERE_PE_LD_DATE >= (PREECLAMPSIA_DATE -7)))
			
		list PREEC_SEV_IND HDP_GROUP HDP_PE_ANC HDP_PE_DATE_ANC ///
			SEVERE_PE SEVERE_PE_LD_DATE PREECLAMPSIA PREECLAMPSIA_DATE if ///
			PREEC_SEV_IND==6	
		...
		// fix dates:
		replace PREECLAMPSIA_DATE = SEVERE_PE_LD_DATE if ///
			HDP_GROUP==5 & ///
			(SEVERE_PE == 1 & SEVERE_PE_LD_DATE < (PREECLAMPSIA_DATE))
			
		replace PREECLAMPSIA_DATE = HDP_PE_DATE_ANC if ///
			HDP_GROUP==5 & ///
			(HDP_PE_ANC == 1 & HDP_PE_DATE_ANC < (PREECLAMPSIA_DATE))
	...
	// Visual symptoms if PE only 
		tab HDP_VIS_DATE_ANC HDP_VIS_ANC, m 		
				
		gen SEVERE_VISUAL_LD_DATE = PREG_END_DATE if ///
			(SEVERE_VISUAL==1 | SYMPTOM_VISION_LD==1) & PREG_END_GA >=140 
		format SEVERE_VISUAL_LD_DATE %td 
		
		tab SEVERE_VISUAL_LD_DATE if (SEVERE_VISUAL==1 | SYMPTOM_VISION_LD==1) 

		* Determine those that meet criteria + timing: 
		replace PREEC_SEV_IND = 7 if (HDP_GROUP == 4 | HDP_GROUP==3) & ///
			((HDP_VIS_ANC == 1 & HDP_VIS_DATE_ANC >= (PREECLAMPSIA_DATE -7)) | ///
			((SEVERE_VISUAL == 1 | SYMPTOM_VISION_LD == 1) & ///
			  SEVERE_VISUAL_LD_DATE >= (PREECLAMPSIA_DATE -7 )))
	...
	list HDP_GROUP HDP_VIS_ANC HDP_VIS_DATE_ANC SEVERE_VISUAL /// 
		SYMPTOM_VISION_LD SEVERE_VISUAL_LD_DATE PREECLAMPSIA PREECLAMPSIA_DATE ///
		if HDP_VIS_ANC==1 | (SEVERE_VISUAL==1 | SYMPTOM_VISION_LD==1)
		
		
		// fix dates:
		replace PREECLAMPSIA_DATE = SEVERE_VISUAL_LD_DATE if ///
			HDP_GROUP==5 & (SEVERE_VISUAL==1 | SYMPTOM_VISION_LD==1) & ///
			SEVERE_VISUAL_LD_DATE < (PREECLAMPSIA_DATE)
			
		replace PREECLAMPSIA_DATE = HDP_VIS_DATE_ANC if ///
			HDP_GROUP==5 & HDP_VIS_ANC == 1 & ///
			HDP_VIS_DATE_ANC < (PREECLAMPSIA_DATE)		
	...
	// Epigastric pain if PE only 
		tab HDP_GROUP SYMPTOM_EPIPAIN_HOSP, m 
		tab SYMPTOM_EPIPAIN_DATE_HOSP SYMPTOM_EPIPAIN_HOSP, m // NO EVENTS
		
		gen SEVERE_FEAT_EPIPAIN_DATE = SYMPTOM_EPIPAIN_DATE_HOSP if ///
			SYMPTOM_EPIPAIN_HOSP==1 
		format SEVERE_FEAT_EPIPAIN_DATE %td 
		
		tab SEVERE_FEAT_EPIPAIN_DATE SYMPTOM_EPIPAIN_HOSP, m 
		
		* Determine those that meet criteria + timing: 
		replace PREEC_SEV_IND = 8 if (HDP_GROUP == 4 | HDP_GROUP==3) & ///
			(SYMPTOM_EPIPAIN_HOSP==1) & ///
			SYMPTOM_EPIPAIN_DATE_HOSP > (PREECLAMPSIA_DATE - 7)
			
		replace HDP_GROUP = 5 if  (HDP_GROUP == 4 | HDP_GROUP==3) & ///
			(SYMPTOM_EPIPAIN_HOSP==1) & ///
			SYMPTOM_EPIPAIN_DATE_HOSP > (PREECLAMPSIA_DATE - 7)	
			
		// fix dates - NOTE: EPIGASTRIC PAIN CANNOT BE USED AS THE FIRST SYMPTOM 
		// OF PREECLAMPSIA: 
		/*
		replace PREECLAMPSIA_DATE = SYMPTOM_EPIPAIN_DATE_HOSP if  ///
			HDP_GROUP==5 & SYMPTOM_EPIPAIN_HOSP==1 & ///
			SYMPTOM_EPIPAIN_DATE_HOSP < PREECLAMPSIA_DATE	
		*/
	...
	// Severe headache if PE only 
		gen SYMPTOM_HEADACHE_LD_DATE = PREG_END_DATE if  ///
			SYMPTOM_HEADACHE_LD == 1
		format SYMPTOM_HEADACHE_LD_DATE %td 
		
		tab SYMPTOM_HEADACHE_LD_DATE SYMPTOM_HEADACHE_LD, m 
		
		tab SYMPTOM_HEADACHE_DATE_HOSP SYMPTOM_HEADACHE_HOSP, m 
	
		* Determine those that meet criteria + timing: 
		replace PREEC_SEV_IND = 9 if (HDP_GROUP == 4 | HDP_GROUP==3) & ///
		((SYMPTOM_HEADACHE_HOSP==1 & SYMPTOM_HEADACHE_DATE_HOSP > (PREECLAMPSIA_DATE-7)) ///
		| (SYMPTOM_HEADACHE_LD==1 & SYMPTOM_HEADACHE_LD_DATE > (PREECLAMPSIA_DATE-7)))
			
			
		list HDP_GROUP PREEC_SEV_IND PREECLAMPSIA_DATE /// 
			SYMPTOM_HEADACHE_HOSP SYMPTOM_HEADACHE_DATE_HOSP ///
			SYMPTOM_HEADACHE_LD SYMPTOM_HEADACHE_LD_DATE if ///
	...
	*PREECLAMPSIA_SEV indicator: 
	...
	sort HDP_GROUP PREEC_IND 
	list GHTN HDP_GROUP PREEC_IND PREEC_SUP_IND PREEC_SEV_IND ///
		GHTN_DATE PREECLAMPSIA_DATE PREG_END_DATE if PREECLAMPSIA==1
		
	foreach var of varlist SEVERE_FEAT_SEIZURES SEVERE_FEAT_HELLP ///
		SEVERE_FEAT_ORGAN SEVERE_FEAT_SEVHIGH SEVERE_FEAT_SEVHYP /// 
		SEVERE_FEAT_PULMED SEVERE_FEAT_VISUAL SEVERE_FEAT_EPIPAIN ///
		SEVERE_FEAT_HEAD {
	
	replace `var' = 0 if `var'==. & PREECLAMPSIA_SEV==1
	
		}
		
		
	* Review: a small number of cases have no PE date; review below: 
	list if PREECLAMPSIA == 1 & PREECLAMPSIA_DATE == . 

		
		*Corrections made; the only remaining missing date is 
		* Zambia observation Z3-025-1684, where onset occurred at L&D and 
		* pregnancy end date is missing. 
		
		*As of Feb 4 (1-30-2026 data) is is resolved! ^^
		
	* Review: a small number of cases have PE date BEFORE GHTN date. Review 
	* of these cases here: 
	
	list SITE GHYP_IND PREEC_IND PREEC_SUP_IND PREEC_SEV_IND PREECLAMPSIA_DATE ///
		GHTN_DATE BP_PROT_DATE SYMPTOM_PROTEIN_LD_DATE ///
		HDP_DX_PREEC_DATE_ANC HDP_DX_PREEC_DATE_LD FIRST_SEV_HIGH_BP_DATE ///
		HDP_SEVHYP_DATE_ANC SEVERE_SEVHYP_DATE_LD HDP_DX_ECL_DATE_ANC ///
		HDP_PPECL_DATE_ANC SEVERE_SEIZURES_DATE_LD HDP_HELLP_DATE_ANC ///
		SEVERE_HELLP_DATE_LD ORG_FAIL_DATE HDP_PE_DATE_ANC ///
		SEVERE_SEIZURES_DATE_LD HDP_SEIZURES_DATE_ANC ///
		SEVERE_PE_LD_DATE HDP_VIS_DATE_ANC SEVERE_VISUAL_LD_DATE ///
		SEVERE_FEAT_EPIPAIN_DATE SYMPTOM_HEADACHE_LD_DATE ///
		SYMPTOM_HEADACHE_DATE_HOSP if PREECLAMPSIA==1 & GHTN==1 & ///
		PREECLAMPSIA_DATE < GHTN_DATE 
		
		* HERE, TWO OBSERVATIONS WITH CONFLICT BETWEEN GHTN & PE DATES AS OF 
		* 8-9-2025. 
			*For these two observations, all outcomes are defined by clinical 
			*dx only, with limited other signs of GHTN OR PE. Propose for now to 
			*default to the earlier DX date for both levels; will flag these two
			*cases for review in upcoming PE case review. 
			
		*As of Feb 4 (1-30-2026 data) is is resolved! ^^
			
		gen PREEC_CASE_REVIEW = 1 if GHTN==1 & PREECLAMPSIA==1 & ///
			GHTN_DATE > PREECLAMPSIA_DATE & GHTN_DATE!= . & ///
			PREECLAMPSIA_DATE != . 
			
		gen PREEC_CASE_REVIEW_REASON = 1 if GHTN==1 & PREECLAMPSIA==1 & ///
			GHTN_DATE > PREECLAMPSIA_DATE & GHTN_DATE!= . & ///
			PREECLAMPSIA_DATE != .  
			
		* Data cleaning: select the earlier of the two dates: 
		replace GHTN_DATE = PREECLAMPSIA_DATE if ///
			GHTN==1 & PREECLAMPSIA==1 & GHTN_DATE > PREECLAMPSIA_DATE 
		
		
		* Create before/after 34 weeks variable: 
		gen PREECLAMPSIA_GA = PREECLAMPSIA_DATE - PREG_START_DATE if ///
			PREECLAMPSIA == 1 
		label var PREECLAMPSIA_GA "Gestational age at Preeclampsia onset in days"
		sum PREECLAMPSIA_GA 
		
		gen PREECLAMPSIA_GA_WK = int(PREECLAMPSIA_GA / 7)
		label var PREECLAMPSIA_GA_WK "Gestational age at Preeclampsia onset in weeks"
		
		gen PREECLAMPSIA_GTE34 = 0 if PREECLAMPSIA==1 & ///
			PREECLAMPSIA_GA_WK < 34 & PREECLAMPSIA_GA_WK > 20 
		replace PREECLAMPSIA_GTE34 = 1 if PREECLAMPSIA==1 & ///
			PREECLAMPSIA_GA_WK >=34 & PREECLAMPSIA_GA_WK != . 
		replace PREECLAMPSIA_GTE34 = 55 if PREECLAMPSIA==1 & ///
			PREECLAMPSIA_DATE==. 
			
		label var PREECLAMPSIA_GTE34 "Preeclamspia onset was >=34 weeks GA"
		
		
		tab GHTN_GTE34 PREECLAMPSIA_GTE34, m 
		
		
		* SEVERE FEATURES DATE: 
			* Select from dates: 
			
		*NOTE: HERE, WE COMBINE DATES FOR SEVERE HEADACHE & EPIGASTRIC PAIN INTO  
		*A SINGLE COMPOSITE DATE; THESE SYMTPOMS MUST OCCUR AFTER PE ONSET AND 
		*SO MUST BE AFTER PE DATE TO BE INCLUDED IN THE FORMULA BELOW.
		* COMBINED VARIABLES INCLUDE: SEVERE_FEAT_EPIPAIN_DATE, 
			* SYMPTOM_HEADACHE_LD_DATE, SYMPTOM_HEADACHE_DATE_HOSP	
			
		gen sev_symp_temp_date = .
		format sev_symp_temp_date %td 
		
		foreach var of varlist SEVERE_FEAT_EPIPAIN_DATE  ///
			SYMPTOM_HEADACHE_LD_DATE SYMPTOM_HEADACHE_DATE_HOSP	 {
			
		replace sev_symp_temp_date = `var' if PREECLAMPSIA_SEV == 1 & ///
			`var' >= PREECLAMPSIA_DATE & PREECLAMPSIA_DATE != . & ///
			(sev_symp_temp_date==. | ///
			(`var' < sev_symp_temp_date)) & `var' != . 
		
		}
	
	gen PREECLAMPSIA_SEV_DATE = min(sev_symp_temp_date, FIRST_SEV_HIGH_BP_DATE, ///
		HDP_SEVHYP_DATE_ANC, SEVERE_SEVHYP_DATE_LD, HDP_DX_ECL_DATE_ANC, ///
		HDP_PPECL_DATE_ANC, SEVERE_SEIZURES_DATE_LD, HDP_HELLP_DATE_ANC, ///
		SEVERE_HELLP_DATE_LD, ORG_FAIL_DATE, HDP_PE_DATE_ANC, ///
		SEVERE_SEIZURES_DATE_LD, HDP_SEIZURES_DATE_ANC, ///
		SEVERE_PE_LD_DATE, HDP_VIS_DATE_ANC, SEVERE_VISUAL_LD_DATE) if ///
		PREECLAMPSIA_SEV==1 
	label var PREECLAMPSIA_SEV_DATE "Date of severe preeclampsia onset (first date of severe feature)"
	format PREECLAMPSIA_SEV_DATE %td 
		
		*review for missing: 
		tab PREECLAMPSIA_SEV_DATE PREECLAMPSIA_SEV, m 
		
		*review observations for severe PE with missing date: 
		list if PREECLAMPSIA_SEV==1 & PREECLAMPSIA_SEV_DATE==.
		
		*Note: 3 observations (as of 8-9-2025) are missing an onset date. One is 
		*the ongoing problem observation from Zambia with missing PREG_END_DATE.
		*The other 2 of these cases should be escalated for review to confirm if
		*these are in fact severe PE cases. Both are situations where the only 
		*severe symptom is headache, but headache occurs BEFORE the official 
		*PE onset date. Setting these for review below: 
		
		*As of Feb 4 (1-30-2026 data) is is resolved! ^^
		
		replace PREEC_CASE_REVIEW = 1 if ///
			PREECLAMPSIA_SEV_DATE==. & PREEC_SEV_IND==9 
		
		replace PREEC_CASE_REVIEW_REASON = 2 if PREECLAMPSIA_SEV_DATE==. & ///
			PREEC_SEV_IND==9 
			
		*Create severe PE GA at onset: 
		gen PREECLAMPSIA_SEV_GA = PREECLAMPSIA_SEV_DATE - PREG_START_DATE 
		label var PREECLAMPSIA_SEV_GA "Gestational age at Severe Preeclampsia onset in days"
		gen PREECLAMPSIA_SEV_GA_WK = (int(PREECLAMPSIA_SEV_GA/7)) 
		label var PREECLAMPSIA_SEV_GA_WK "Gestational age at Severe Preeclampsia onset in weeks"
	...
	* Added on December 17, 2025: Add date for severe high BP : 
	
	* date - measured
	gen HIGH_BP_SEVERE_DATE = FIRST_HIGH_BP_L20_DATE if HIGH_BP_SEV_COUNT_LESS20 >=1 & ///
		HIGH_BP_SEV_COUNT_LESS20 !=. 
	
	replace HIGH_BP_SEVERE_DATE = FIRST_SEV_HIGH_BP_DATE if HIGH_BP_SEV_COUNT >=1 & ///
		HIGH_BP_SEV_COUNT !=. & HIGH_BP_SEVERE_DATE==. 
		
	format HIGH_BP_SEVERE_DATE %td 
	label var HIGH_BP_SEVERE_DATE "Date of first severe high BP measure OR dx"
	
	tab HIGH_BP_SEVERE_DATE HIGH_BP_SEVERE_ANY, m 
	
	tab HIGH_BP_SEVERE_DATE HIGH_BP_SEVERE_DX, m 
	
	* date -- diagnosis 
	replace	HIGH_BP_SEVERE_DATE = min(HDP_SEVHYP_DATE_ANC, HIGH_BP_SEVERE_DATE) if ///
		HDP_SEVHYP_ANC==1 & HDP_SEVHYP_DATE_ANC!=. & HIGH_BP_SEVERE_DATE!=.
		
	replace	HIGH_BP_SEVERE_DATE = HDP_SEVHYP_DATE_ANC if ///
		HDP_SEVHYP_ANC==1 & HDP_SEVHYP_DATE_ANC!=. & HIGH_BP_SEVERE_DATE==.
		
	replace	HIGH_BP_SEVERE_DATE = min(SEVERE_SEVHYP_DATE_LD, HIGH_BP_SEVERE_DATE) if ///
		SEVERE_SEVHYP==1 & SEVERE_SEVHYP_DATE_LD!=. & HIGH_BP_SEVERE_DATE!=.
		
	replace	HIGH_BP_SEVERE_DATE = SEVERE_SEVHYP_DATE_LD if ///
		SEVERE_SEVHYP==1 & SEVERE_SEVHYP_DATE_LD!=. & HIGH_BP_SEVERE_DATE==.
	
	* ga - measured
	gen HIGH_BP_SEVERE_GA = FIRST_HIGH_BP_L20_GA if HIGH_BP_SEV_COUNT_LESS20 >=1 & ///
		HIGH_BP_SEV_COUNT_LESS20 !=. 
	
	replace HIGH_BP_SEVERE_GA = FIRST_SEV_HIGH_BP_GA if HIGH_BP_SEV_COUNT >=1 & ///
		HIGH_BP_SEV_COUNT !=. & HIGH_BP_SEVERE_GA==. 
	
	tab HIGH_BP_SEVERE_GA HIGH_BP_SEVERE_ANY, m 
	
	* ga -- diagnosis 
	replace	HIGH_BP_SEVERE_GA = min(HDP_SEVHYP_GA_ANC, HIGH_BP_SEVERE_GA) if ///
		HDP_SEVHYP_ANC==1 & HDP_SEVHYP_GA_ANC!=. & HIGH_BP_SEVERE_GA!=.
		
	replace	HIGH_BP_SEVERE_GA = HDP_SEVHYP_GA_ANC if ///
		HDP_SEVHYP_ANC==1 & HDP_SEVHYP_GA_ANC!=. & HIGH_BP_SEVERE_GA==.
		
	replace	HIGH_BP_SEVERE_GA = min(PREG_END_GA, HIGH_BP_SEVERE_DATE) if ///
		SEVERE_SEVHYP==1 & SEVERE_SEVHYP_DATE_LD!=. & HIGH_BP_SEVERE_DATE!=.
		
	replace	HIGH_BP_SEVERE_GA = PREG_END_GA if ///
		SEVERE_SEVHYP==1 & SEVERE_SEVHYP_DATE_LD!=. & HIGH_BP_SEVERE_DATE==.
		
	tab HIGH_BP_SEVERE_GA HIGH_BP_SEVERE_ANY, m 
	
	label var HIGH_BP_SEVERE_GA "GA at first severe high BP measure OR dx (days)"	
...
////////////////////////////////////////////////////////////
	...
	replace HDP_GROUP_MISS = . if HDP_GROUP==5 & POSTP_DX_PPEC ==1 & ///
		...
		* FIX DATES: Those with POSTP_DX_PPEC must have a date for:
			*PREECLAMPSIA_DATE & PREECLAMPSIA_SEV_DATE 
		list PREECLAMPSIA_DATE PREECLAMPSIA_SEV_DATE PREG_END_DATE ///
			POSTP_DX_PPEC_DT if HDP_GROUP==5 & POSTP_DX_PPEC==1 
		
		replace PREECLAMPSIA_DATE = min(PREECLAMPSIA_DATE, POSTP_DX_PPEC_DT) if ///
			HDP_GROUP==5 & POSTP_DX_PPEC==1
			
		replace PREECLAMPSIA_SEV_DATE = min(PREECLAMPSIA_SEV_DATE, POSTP_DX_PPEC_DT) if ///
			HDP_GROUP==5 & POSTP_DX_PPEC==1
	...
	tab POSTP_SEVERE_FEAT_SEIZURE SITE, m 
		...
		* FIX DATES: Those with POSTP_DX_PPEC must have a date for:
			*PREECLAMPSIA_DATE & PREECLAMPSIA_SEV_DATE 
		list PREECLAMPSIA_DATE PREECLAMPSIA_SEV_DATE PREG_END_DATE ///
			POSTP_SEVERE_FEAT_SEIZURE_DT if HDP_GROUP==5 & ///
			POSTP_SEVERE_FEAT_SEIZURE==1 
		
		replace PREECLAMPSIA_DATE = min(PREECLAMPSIA_DATE, ///
			POSTP_SEVERE_FEAT_SEIZURE_DT) if ///
			HDP_GROUP==5 & POSTP_SEVERE_FEAT_SEIZURE==1
			
		replace PREECLAMPSIA_SEV_DATE = min(PREECLAMPSIA_SEV_DATE, ///
			POSTP_SEVERE_FEAT_SEIZURE_DT) if ///
			HDP_GROUP==5 & POSTP_SEVERE_FEAT_SEIZURE==1
	...
	* Added on 8-9-2025: Add an indicator for postpartum onset of PREECLAMPSIA: 
	gen PREECLAMPSIA_POSTPARTUM = 0 if PREECLAMPSIA == 1 & ///
		(PREECLAMPSIA_DATE== . | PREECLAMPSIA_DATE <= PREG_END_DATE)
	
	replace PREECLAMPSIA_POSTPARTUM = 1 if PREECLAMPSIA == 1 & ///
		PREECLAMPSIA_DATE > PREG_END_DATE & PREECLAMPSIA_DATE!= . & ///
		PREG_END_DATE!= .
		
	gen PREECLAMPSIA_SEV_POSTPARTUM = 0 if PREECLAMPSIA_SEV == 1 & ///
		(PREECLAMPSIA_SEV_DATE== . | PREECLAMPSIA_SEV_DATE <= PREG_END_DATE)
	
	replace PREECLAMPSIA_SEV_POSTPARTUM = 1 if PREECLAMPSIA_SEV == 1 & ///
		PREECLAMPSIA_SEV_DATE > PREG_END_DATE & PREECLAMPSIA_SEV_DATE!= . & ///
		PREG_END_DATE!= .	
		
	sort PREECLAMPSIA_SEV_GA	
	list GHTN PREECLAMPSIA PREG_END_GA PREEC_IND PREEC_SEV_IND ///
		PREECLAMPSIA_DATE PREECLAMPSIA_SEV_DATE PREG_END_DATE ///
		PREECLAMPSIA_POSTPARTUM PREECLAMPSIA_SEV_POSTPARTUM if ///
		PREECLAMPSIA_SEV==1
		
	* Set to 77 if onset is postpartum for variable PREECLAMPSIA_GTE34: 
	replace PREECLAMPSIA_GTE34 = 77 if PREECLAMPSIA_GTE34 == . & ///
		PREECLAMPSIA==1 & PREECLAMPSIA_POSTPARTUM==1 
	...
	* IDENTIFY PRELIMINARY LIST OF CASES FOR REVIEW: 
	tab PREEC_CASE_REVIEW_REASON PREEC_CASE_REVIEW, m 
	
	* 1= GHTN_DATE is > PREECLAMPSIA_DATE
	* 2= Severe headache/epigastric pain is BEFORE preeclampsia confirmation 
	* 3= Date of non-severe preeclampsia is > 1 month BEFORE PREG_END_DATE 
	* 4= Date of severe preeclampsia is > 1 week from PREG_END_DATE 
	* 5= Postpartum eclampsia/seizures with no signs in pregnancy 
	* 6= CHTN with proteinuria (requested by CRS)
	* 7=Dx of preeclampsia with no high BP readings
	
	* 3= Date of non-severe preeclampsia is > 1 month BEFORE PREG_END_DATE 
	replace PREEC_CASE_REVIEW=1 if PREECLAMPSIA_DATE < (PREG_END_DATE -30) & ///
		PREECLAMPSIA==1 & PREECLAMPSIA_DATE !=. 
	replace PREEC_CASE_REVIEW_REASON=3 if PREEC_CASE_REVIEW_REASON==. & ///
		PREECLAMPSIA_DATE < (PREG_END_DATE -30) & PREECLAMPSIA==1 & PREECLAMPSIA_DATE !=. 
	
	* 4= Date of severe preeclampsia is > 1 week from PREG_END_DATE 
	replace PREEC_CASE_REVIEW=1 if PREECLAMPSIA_SEV_DATE < (PREG_END_DATE -7) & ///
		PREECLAMPSIA_SEV==1 & PREECLAMPSIA_SEV_DATE !=.
	replace PREEC_CASE_REVIEW_REASON=4 if PREEC_CASE_REVIEW_REASON==. & ///
		PREECLAMPSIA_SEV_DATE < (PREG_END_DATE -7) & PREECLAMPSIA_SEV==1 & ///
		PREECLAMPSIA_SEV_DATE != .
		
	* 5= Postpartum eclampsia/seizures with no signs in pregnancy 
	replace PREEC_CASE_REVIEW=1 if PREECLAMPSIA_SEV==1 & ///
		PREEC_IND==. & PREEC_SUP_IND==. & GHYP_IND==. & ///
		(PREEC_SEV_IND == 10 | PREEC_SEV_IND == 11)
	
	replace PREEC_CASE_REVIEW_REASON=5 if PREEC_CASE_REVIEW_REASON==. & ///
		 PREECLAMPSIA_SEV==1 & PREEC_IND==. & PREEC_SUP_IND==. & GHYP_IND==. & ///
		(PREEC_SEV_IND == 10 | PREEC_SEV_IND == 11)
		
	* 6= CHTN with proteinuria (requested by CRS)
	replace PREEC_CASE_REVIEW=1 if (HDP_GROUP == 4 | HDP_GROUP == 5) & ///
		(PREEC_SUP_IND==1 | PREEC_SUP_IND==2 | PREEC_SUP_IND==3 | ///
		PREEC_SUP_IND==4 | PREEC_SUP_IND==5)
	
	replace PREEC_CASE_REVIEW_REASON=6 if PREEC_CASE_REVIEW_REASON==. & ///
		(HDP_GROUP == 4 | HDP_GROUP == 5) & ///
		(PREEC_SUP_IND==1 | PREEC_SUP_IND==2 | PREEC_SUP_IND==3 | ///
		PREEC_SUP_IND==4 | PREEC_SUP_IND==5)
		
	* 7=Dx of preeclampsia with no high BP readings: 
	replace PREEC_CASE_REVIEW=1 if PREEC_IND==6 & HIGH_BP_COUNT==0
	
	replace PREEC_CASE_REVIEW_REASON=7 if PREEC_CASE_REVIEW_REASON==. & ///
		PREEC_IND==6 & HIGH_BP_COUNT==0	
		
	replace PREEC_CASE_REVIEW=1 if PREEC_SUP_IND==6 & HIGH_BP_COUNT==0
	
	replace PREEC_CASE_REVIEW_REASON=7 if PREEC_CASE_REVIEW_REASON==. & ///
		PREEC_SUP_IND==6 & HIGH_BP_COUNT==0	
	...
	* 7=Dx of preeclampsia with no high BP readings
	...
	label define rev 1 "GHTN_DATE > PE date" ///
		2 "Severe head/epi pain < PE date" ///
		3 "PE date >30 days before delivery" ///
		4 "Severe PE date >7 days before delivery" ///
		5 "PP ecla/seizues with no signs in pregnancy" ///
		6 "CHTN + proteinuria" ///
		7 "DX of PE with no high BP readings"
		
	label values PREEC_CASE_REVIEW_REASON rev 
	
	tab PREEC_CASE_REVIEW_REASON PREEC_CASE_REVIEW, m 
	
	*Fix PREECLAMPSIA_GTE34 VARIABLE: 
	replace PREECLAMPSIA_GTE34 = 0 if PREECLAMPSIA_GTE34 ==. & ///
		PREECLAMPSIA == 1 & PREECLAMPSIA_GA >=140 & PREECLAMPSIA_GA < 238 
	replace PREECLAMPSIA_GTE34 = 1 if PREECLAMPSIA_GTE34 == . & ///
		PREECLAMPSIA == 1 & PREECLAMPSIA_GA >= 238 & PREECLAMPSIA_GA != .
		
		
	*Fix severe features indicators: 
	foreach var of varlist PREEC_SEV_FEAT_SEIZURES PREEC_SEV_FEAT_HELLP ///
		PREEC_SEV_FEAT_ORGAN PREEC_SEV_FEAT_SEVHIGH PREEC_SEV_FEAT_SEVHYP ///
		PREEC_SEV_FEAT_PULMED PREEC_SEV_FEAT_VISUAL PREEC_SEV_FEAT_EPIPAIN ///
		PREEC_SEV_FEAT_HEAD PREEC_SEV_FEAT_POSTP_ECLAMPSIA ///
		PREEC_SEV_FEAT_POSTP_SEIZURES {
	...
	gen PREECLAMPSIA_NONSEV = PREECLAMPSIA 
	replace PREECLAMPSIA_NONSEV = 0 if PREECLAMPSIA_SEV==1 
	
	label var PREECLAMPSIA_NONSEV "Preeclampsia WITHOUT severe features"
	
	tab HDP_GROUP PREECLAMPSIA_NONSEV, m 
	
	* Create an indicator for timing of HDP onset: 
	gen HDP_ANY_GTE34 = GHTN_GTE34
	replace HDP_ANY_GTE34 = PREECLAMPSIA_GTE34 if GHTN_GTE34 ==. | ///
		(PREECLAMPSIA_GTE34 < GHTN_GTE34 & PREECLAMPSIA_GTE34!=.)
	replace HDP_ANY_GTE34 = 99 if HDP_GROUP == 1 
	replace HDP_ANY_GTE34 = 77 if PREECLAMPSIA_POSTPARTUM==1 & GHTN_GTE34==. & ///
		PREECLAMPSIA_GTE34==77
	
		
	tab HDP_ANY_GTE34, m 
	label var HDP_ANY_GTE34 "HDP (GHTN or PE) onset is greater than or equal to 34 weeks GA"
	
	tab HDP_ANY_GTE34 HDP_GROUP, m 
	
	list HDP_ANY_GTE34 GHTN_GA_WK PREECLAMPSIA_GA_WK HDP_GROUP PREECLAMPSIA_POSTPARTUM if ///
		HDP_GROUP >=2 & HDP_GROUP<=5
	...
	* save for report outcomes:
	keep SITE MOMID PREGID HTN_ANY HTN_ANY_IND ///
		...
		PREECLAMPSIA_SEV_DENOM PREECLAMPSIA_NONSEV ///
		GHTN_DATE GHTN_GA GHTN_GA_WK GHTN_GTE34 ///
		PREECLAMPSIA_DATE PREECLAMPSIA_GA PREECLAMPSIA_GA_WK PREECLAMPSIA_GTE34 ///
		PREECLAMPSIA_POSTPARTUM ///
		PREECLAMPSIA_SEV_DATE PREECLAMPSIA_SEV_GA PREECLAMPSIA_SEV_GA_WK ///
		HDP_ANY_GTE34 ///
		PREECLAMPSIA_SEV_POSTPARTUM PREEC_CASE_REVIEW PREEC_CASE_REVIEW_REASON
		
	tab PREECLAMPSIA_GA PREECLAMPSIA_GTE34 if PREECLAMPSIA==1, m 
	tab GHTN_GA GHTN_GTE34 if GHTN==1, m 
