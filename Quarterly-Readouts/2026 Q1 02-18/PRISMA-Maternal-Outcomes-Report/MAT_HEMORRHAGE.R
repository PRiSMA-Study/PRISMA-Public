...
#* Last updated: 20 November 2026 (Added dates of events)
...
# hemorrhage <- read.csv(paste0(path_to_tnt, "MAT_HEMORRHAGE" ,".csv"))
...
hem_export_merged <- hem_export_wide_rsn %>% 
  rename(HEM_APH_DATE = HEM_APH_DATE_MIDPOINT) %>% 
...
query_pph_pnc_blood_loss <- hem_export_merged %>% 
  ...
  # select(SITE, MOMID, PREGID, HEM_PPH, PREG_END_DATE, HEM_PPH_DATE, HEM_PPH_SEV_DATE,
  #        HEM_PPH_SEV, HEM_PPH_RSN_SOURCE, HEM_PPH_SEV_RSN_SOURCE) %>% 
  left_join(hem_ld %>% select(-PREG_END_DATE, -DOB), by = c("SITE", "MOMID", "PREGID")) %>% 
...
table(query_pph_pnc_blood_loss$exclusion_rsn, query_pph_pnc_blood_loss$HEM_PPH_AGE_PP_DAYS)
